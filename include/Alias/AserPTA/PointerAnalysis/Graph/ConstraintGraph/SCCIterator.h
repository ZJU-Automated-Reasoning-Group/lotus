...
// reverse -> whether we should DFS the graph in reverse order (so that the scc iterator is in topo order)
...
private:
    ...
    explicit SCCIterator(const GraphT &G) : visitNum(0), visitedNode(G.getNodeNum()), trueVisitedNode(G.getNodeNum()),
                                            lastRoot(0), consG(&G) {
        NodeRef entryN = G[0];
        DFSVisitOne(entryN);
        GetNextSCC();
    ...
    explicit SCCIterator(const GraphT &G, llvm::BitVector workList) : visitNum(0), visitedNode(std::move(workList)),
    ...
    ChildItTy child_begin(NodeRef node) {
        return child_begin_helper<cons, reverse>(node);
    }

    ChildItTy child_end(NodeRef node) {
        return child_end_helper<cons, reverse>(node);
    }

private:
    // Helper for child_begin - copy, forward
    template<Constraints C, bool R>
    typename std::enable_if<C == Constraints::copy && !R, ChildItTy>::type
    child_begin_helper(NodeRef node) { return node->succ_copy_begin(); }
    
    // Helper for child_begin - copy, reverse
    template<Constraints C, bool R>
    typename std::enable_if<C == Constraints::copy && R, ChildItTy>::type
    child_begin_helper(NodeRef node) { return node->pred_copy_begin(); }
    
    // Helper for child_begin - offset, forward
    template<Constraints C, bool R>
    typename std::enable_if<C == Constraints::offset && !R, ChildItTy>::type
    child_begin_helper(NodeRef node) { return node->succ_offset_begin(); }
    
    // Helper for child_begin - offset, reverse
    template<Constraints C, bool R>
    typename std::enable_if<C == Constraints::offset && R, ChildItTy>::type
    child_begin_helper(NodeRef node) { return node->pred_offset_begin(); }

    // Helper for child_end - copy, forward
    template<Constraints C, bool R>
    typename std::enable_if<C == Constraints::copy && !R, ChildItTy>::type
    child_end_helper(NodeRef node) { return node->succ_copy_end(); }
    
    // Helper for child_end - copy, reverse
    template<Constraints C, bool R>
    typename std::enable_if<C == Constraints::copy && R, ChildItTy>::type
    child_end_helper(NodeRef node) { return node->pred_copy_end(); }
    
    // Helper for child_end - offset, forward
    template<Constraints C, bool R>
    typename std::enable_if<C == Constraints::offset && !R, ChildItTy>::type
    child_end_helper(NodeRef node) { return node->succ_offset_end(); }
    
    // Helper for child_end - offset, reverse
    template<Constraints C, bool R>
    typename std::enable_if<C == Constraints::offset && R, ChildItTy>::type
    child_end_helper(NodeRef node) { return node->pred_offset_end(); }
