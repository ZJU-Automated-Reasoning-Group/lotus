...
#include "Alias/AserPTA/PointerAnalysis/Models/LanguageModel/ConsGraphBuilder.h"
...
protected:
    ...
    inline InterceptResult overrideFunction(const llvm::Function *F, const llvm::Instruction *callsite) {
        if (static_cast<SubClass *>(this)->isExtHeapAllocAPI(F, callsite)) {
        ...
        return static_cast<SubClass *>(this)->interceptFunction(F, callsite);
    }

    // return true if the call site is handled by the language model.
    inline bool overrideCallSite(const CtxFunction<ctx> *caller, const CtxFunction<ctx> *callee,
                                  const llvm::Instruction *callsite) {
        // context should match
        assert(CT::contextEvolve(caller->getContext(), callsite) == callee->getContext());

        if (static_cast<SubClass *>(this)->isExtHeapAllocAPI(callee->getFunction(), callsite)) {
            // handle different heap allocation APIs.
            static_cast<SubClass *>(this)->interceptHeapAllocSite(caller, callee, callsite);
            return true;
        }

        return static_cast<SubClass *>(this)->interceptCallSite(caller, callee, callsite);
    }

    LangModelBase(llvm::Module *M, llvm::StringRef entry)
        : Super(M, entry) {}
...
template <typename ctx, typename MemModel, typename PtsTy, typename SubClass>
class LangModelTrait<LangModelBase<ctx, MemModel, PtsTy, SubClass>> {
private:
    ...
    static inline LangModelTy *buildInitModel(llvm::Module *M, llvm::StringRef entry) {
    ...
    static inline CGNodeTy *indexObject(LangModelTy *model, ObjNodeTy *objNode, const llvm::GetElementPtrInst *gep) {
    ...
    // get corresponding pointer nodes that represent v (in all different contexts)
    static inline void getNodesForValue(LangModelTy *model,
                                        const llvm::Value *v, std::set<CGNodeTy *> &result) {
        // langModel->getNodesForValue(v, result);
    }

    // get corresponding pointer nodes that represent v
    static inline void getPointedBy(LangModelTy *model,
                                    const ctx *context, const llvm::Value *v, std::set<Pointer<ctx>> &result) {}
    ...
    [[nodiscard]]
    static inline const CallGraphNode<ctx> *getDirectNode(LangModelTy *model,
                                                          const ctx *C, const llvm::Function *F) {
        return model->getDirectNode(C, F);
    }

    [[nodiscard]]
    static inline const CallGraphNode<ctx> *getDirectNodeOrNull(LangModelTy *model,
                                                          const ctx *C, const llvm::Function *F) {
        return model->getDirectNodeOrNull(C, F);
    }

    [[nodiscard]]
    [[deprecated("use getInDirectCallSite(ctx, instruction) instead")]]
    static inline const CallGraphNode<ctx> *getInDirectNode(LangModelTy *model,
                                                            const ctx *C, const llvm::Instruction *I) {
        return model->getInDirectNode(C, I);
    }

    [[nodiscard]]
    static inline const InDirectCallSite<ctx> *getInDirectCallSite(LangModelTy *model,
                                                                   const ctx *C, const llvm::Instruction *I) {
        return model->getInDirectNode(C, I)->getTargetFunPtr();
    }

    static inline NodeID getSuperNodeIDForValue(LangModelTy *model,
                                                const ctx *C, const llvm::Value *V) {
