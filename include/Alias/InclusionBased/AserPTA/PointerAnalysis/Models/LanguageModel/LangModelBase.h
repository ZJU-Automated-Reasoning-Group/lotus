...
public:
  ...
  inline void interceptEntryFun(const llvm::Function *entry) {
    ...
    //                auto objNode = MMT::template
    //                allocateAnonObj<PT>(this->getMemModel(), // memory model
    //                                                                 CT::getInitialCtx(),
    //                                                                 //
    //                                                                 initial
    //                                                                 context
    ...
    //                                                                 recursively
    //
    //                auto ptrNode = this->getPtrNode(CT::getInitialCtx(),
    //                &(*it)); this->consGraph->addConstraints(objNode, ptrNode,
    //                Constraints::addr_of);
    //            }
    //        }
  }
...
protected:
  ...
  inline InterceptResult overrideFunction(const llvm::Function *F,
                                          const llvm::Instruction *callsite) {
  ...
  inline bool overrideCallSite(const CtxFunction<ctx> *caller,
    ...
    if (static_cast<SubClass *>(this)->isExtHeapAllocAPI(callee->getFunction(),
                                                         callsite)) {
    ...
    return static_cast<SubClass *>(this)->interceptCallSite(caller, callee,
                                                            callsite);
  }
...
public:
  ...
  static inline CGNodeTy *
  indexObjectForClients(LangModelTy *model, ObjNodeTy *objNode,
                        const llvm::GetElementPtrInst *gep) {
    return model->indexObject(objNode, gep);
  }
