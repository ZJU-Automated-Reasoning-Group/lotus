...
struct InterceptResult {
  enum class Option {
    ...
    ONLY_CALLSITE, // do not analyze into the function body, but keep the
                   // callsite
    IGNORE, // ignore the function completely (no callnode in the callgraph).
  };
  ...
  getOrCreateCallNode(const ctx *curCtx, const ctx *preCtx,
    ...
    */
    ...
    if (isEntryNode) {
      ...
      beforeNewNode(F, nullptr);

      return std::make_pair(callNode, true);
    }

    InterceptResult interceptResult = beforeNewNode(F, I);
    if (interceptResult.option == InterceptResult::Option::IGNORE) {
    ...
    if (llvm::isa<llvm::Function>(interceptResult.redirectTo)) {
      ...
      assert(llvm::isa<llvm::FunctionType>(
          indirect->getType()->getPointerElementType()));
      ...
      for (const auto &inst : BB) {
        if (llvm::isa<llvm::CallInst>(inst) ||
          ...
          if (cs.isIndirectCall()) {
            ...
            createEdge(node, targetNode, &inst, onNewEdge);
    ...
    */
    ...
    for (; it != et; it++) {
      ...
      if (result.first == nullptr) {
        ...
        assert(!result.first->isIndirectCall());

        resolvedNode->getTargetFunPtr()->resolvedToNode(result.first);
        createEdge(callerNode, result.first, callInst, onNewEdge);
