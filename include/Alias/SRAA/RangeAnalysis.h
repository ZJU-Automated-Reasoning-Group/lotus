...
public:
  ...
  public:
    ...
    TimeValue() : duration(std::chrono::steady_clock::duration::zero()) {}
    explicit TimeValue(std::chrono::steady_clock::duration d) : duration(d) {}
    ...
    TimeValue operator+(const TimeValue &op) const {
      return TimeValue(duration + op.duration);
    }
