...
namespace llvm {
template <> struct GraphTraits<pdg::Node *> {
...
template <> struct GraphTraits<pdg::ProgramDependencyGraph *> {
  ...
  static NodeRef getEntryNode(pdg::ProgramDependencyGraph *G) {
    return *G->getPDG()->begin();
  }
  static nodes_iterator nodes_begin(pdg::ProgramDependencyGraph *G) {
    return G->getPDG()->begin();
  }
  static nodes_iterator nodes_end(pdg::ProgramDependencyGraph *G) {
    return G->getPDG()->end();
  }
  static ChildIteratorType child_begin(NodeRef N) {
    return ChildIteratorType(N);
