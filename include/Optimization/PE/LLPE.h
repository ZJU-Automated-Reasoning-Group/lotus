...
// Include structures and functions for working with instruction and argument
// shadows.
...
struct PathCondition {
  ...
  PathCondition(uint32_t isi, BasicBlock *ibi, uint32_t ii, uint32_t fsi,
                BasicBlock *fbi, Constant *v, uint64_t off)
      : instStackIdx(isi), instBB(ibi), instIdx(ii), fromStackIdx(fsi),
        fromBB(fbi), offset(off) {
...
struct PathConditions {
  ...
  void addForType(PathCondition newCond, PathConditionTypes Ty) {
    ...
    case PathConditionTypeGlobalInit:
      IntmemPathConditions.push_back(newCond);
      break;
    case PathConditionTypeString:
      StringPathConditions.push_back(newCond);
      break;
    case PathConditionTypeStream:
      StreamPathConditions.push_back(newCond);
      break;
    }
  }
};
...
struct AllocatorFn {
  ...
  static AllocatorFn getVariableSize(uint32_t arg) { return AllocatorFn(arg); }
};
...
struct ReadFile {
  ...
  ReadFile(std::string n, uint64_t IO, uint32_t RS, bool _isFifo)
      : name(n), incomingOffset(IO), readSize(RS), needsSeek(true),
        isFifo(_isFifo) {}
...
struct GlobalStats {
  ...
  GlobalStats()
      : dynamicFunctions(0), dynamicContexts(0), dynamicBlocks(0),
        dynamicInsts(0), disabledContexts(0), resolvedBranches(0),
        constantInstructions(0), pointerInstructions(0), setInstructions(0),
        unknownInstructions(0), deadInstructions(0), residualBlocks(0),
        residualInstructions(0), mallocChecks(0), fileChecks(0),
        threadChecks(0), condChecks(0) {}
...
public:
  ...
  DenseMap<Function *, SmallSet<std::pair<BasicBlock *, BasicBlock *>, 1>>
      assumeEdges;
  ...
  DenseMap<const Function *, DenseMap<const Value *, std::string> *>
      functionTextCache;
  DenseMap<const Function *, DenseMap<const Value *, std::string> *>
      briefFunctionTextCache;
  ...
  // Of an allocation or FD, record instructions that may use it in the emitted
  // program.
  DenseMap<ShadowValue, std::vector<std::pair<ShadowValue, uint32_t>>>
      indirectDIEUsers;
  ...
  DenseSet<std::pair<IntegrationAttempt *, const ShadowLoopInvar *>>
      latchStoresRetained;
  ...
  DenseMap<const Value *, std::string> &getFunctionCache(const Function *F,
                                                         bool brief);
  ...
  void parsePathConditions(cl::list<std::string> &L, PathConditionTypes Ty,
                           InlineAttempt *IA);
  ...
  bool shouldAlwaysInline(Function *F) { return alwaysInline.count(F); }

  bool shouldAlwaysExplore(Function *F) { return alwaysExplore.count(F); }
  ...
  bool shouldAlwaysIterate(Function *F, BasicBlock *HBB) {
    DenseMap<Function *, SmallSet<BasicBlock *, 1>>::iterator it =
        alwaysIterLoops.find(F);
    return it != alwaysIterLoops.end() && it->second.count(HBB);
  }

  bool shouldAssumeEdge(Function *F, BasicBlock *BB1, BasicBlock *BB2) {
    DenseMap<Function *,
             SmallSet<std::pair<BasicBlock *, BasicBlock *>, 1>>::iterator it =
        assumeEdges.find(F);
  ...
  bool shouldIgnoreLoopChildren(Function *F, BasicBlock *HBB) {
    DenseMap<Function *, SmallSet<BasicBlock *, 1>>::iterator it =
        ignoreLoopsWithChildren.find(F);
    return it != ignoreLoopsWithChildren.end() && it->second.count(HBB);
  }

  bool assumeEndsAfter(Function *F, BasicBlock *HBB, uint64_t C) {
    DenseMap<std::pair<Function *, BasicBlock *>, uint64_t>::iterator it =
        maxLoopIters.find(std::make_pair(F, HBB));
  ...
  IntegratorTag *newTag() { return new IntegratorTag(); }

  uint32_t countPathConditionsAtBlockStart(ShadowBBInvar *BB,
  ...
  const ShadowLoopInvar *applyIgnoreLoops(const ShadowLoopInvar *, Function *,
                                          ShadowFunctionInvar *);
};

// Define a wrapper class for using the IHP's instruction text cache when
...
public:
  PrintCacheWrapper(LLPEAnalysisPass &_IHP, T _Val, bool _brief)
      : IHP(_IHP), Val(_Val), brief(_brief) {}
  void printTo(raw_ostream &ROS) { IHP.printValue(ROS, Val, brief); }
};

// Caching instruction text for debug and DOT export:
inline PrintCacheWrapper<const Value *> itcache(const Value &V,
                                                bool brief = false) {
  return PrintCacheWrapper<const Value *>(*GlobalIHP, &V, brief);
}
inline PrintCacheWrapper<ShadowValue> itcache(ShadowValue V,
                                              bool brief = false) {
  return PrintCacheWrapper<ShadowValue>(*GlobalIHP, V, brief);
}
inline PrintCacheWrapper<ShadowValue> itcache(ShadowInstruction *SI,
                                              bool brief = false) {
  return itcache(ShadowValue(SI), brief);
}
inline PrintCacheWrapper<ShadowValue> itcache(ShadowArg *SA,
                                              bool brief = false) {
  return itcache(ShadowValue(SA), brief);
}

template <class T>
raw_ostream &operator<<(raw_ostream &ROS, PrintCacheWrapper<T> Wrapper) {
  Wrapper.printTo(ROS);
  return ROS;
}

inline void printSV(raw_ostream &RSO, ShadowValue V) { RSO << itcache(V); }
...
inline ImprovedValSetSingle *newOverdefIVS() {
  ...
  return new (GlobalIHP->IVSAllocator.Allocate())
...
// PartialVal: a byte array with support functions for turning it into a
// Constant.
struct PartialVal {
  ...
  bool combineWith(Constant *C, uint64_t ConstOffset, uint64_t FirstDef,
                   uint64_t FirstNotDef, std::string *error);
...
inline bool operator==(const ImprovedValSetSingle &PB1,
                       const ImprovedValSetSingle &PB2) {
...
inline bool IVsEqualShallow(ImprovedValSet *IV1, ImprovedValSet *IV2) {
  ...
  if (!IV1->isMulti)
    return (*(cast<ImprovedValSetSingle>(IV1))) ==
           (*(cast<ImprovedValSetSingle>(IV2)));
  else
    return (*(cast<ImprovedValSetMulti>(IV1))) ==
           (*(cast<ImprovedValSetMulti>(IV2)));
}
...
protected:
  ...
  virtual WalkInstructionResult walkInstruction(ShadowInstruction *,
                                                void *Context) = 0;
  ...
  virtual void *copyContext(void *x) { return 0; }
...
public:
  ...
  IAWalker(void *IC = 0, bool ign = false)
      : PList(&Worklist1), CList(&Worklist2), initialContext(IC),
        doIgnoreEdges(ign) {
...
class BackwardIAWalker : public IAWalker {
  ...
  WalkInstructionResult walkFromInst(uint32_t, ShadowBB *, void *Ctx,
                                     ShadowInstruction *&StoppedCI);
  virtual void walkInternal();

public:
  virtual WalkInstructionResult reachedTop() { return WIRStopThisPath; }
  virtual WalkInstructionResult mayAscendFromContext(IntegrationAttempt *,
                                                     void *Ctx) {
    return WIRContinue;
  }

  BackwardIAWalker(uint32_t idx, ShadowBB *BB, bool skipFirst, void *IC = 0,
                   DenseSet<WLItem> *AlreadyVisited = 0, bool ign = false);
};
...
public:
  ...
  // For inlineAttempts this is 0; for loop iterations it is the index of the
  // loop's header within the invar info.
  uint32_t BBsOffset;
  ...
  IntegrationAttempt(LLPEAnalysisPass *Pass, Function &_F,
      ...
      : improvableInstructions(0), improvedInstructions(0),
        residualInstructions(-1), nesting_depth(depth), stack_depth(sdepth),
        pass(Pass), F(_F), L(_L), totalIntegrationGoodness(0),
        integrationGoodnessValid(false), peelChildren(1), pendingEdges(0),
        barrierState(BARRIER_NONE), tentativeLoadsRun(false),
        readsTentativeData(false), containsCheckedReads(false),
        checkedInstructionsHere(0), checkedInstructionsChildren(0),
        yieldState(BARRIER_NONE), commitState(COMMIT_NOT_STARTED),
        mayUnwind(false) {}
  ...
  virtual ShadowInstruction *getInstFalling(ShadowBBInvar *BB,
                                            uint32_t instIdx) = 0;
  ...
  bool analyseBlock(uint32_t &BBIdx, bool inLoopAnalyser, bool inAnyLoop,
                    bool skipStoreMerge, const ShadowLoopInvar *MyL);
  bool analyseBlockInstructions(ShadowBB *BB, bool inLoopAnalyser,
                                bool inAnyLoop);
  bool analyseInstruction(ShadowInstruction *SI, bool inLoopAnalyser,
                          bool inAnyLoop, bool &loadedVarargsHere, bool &bail);
  ...
  virtual bool tryEvaluateHeaderPHI(ShadowInstruction *SI, bool &resultValid,
                                    ImprovedValSet *&result);
  bool tryEvaluate(ShadowValue V, bool inLoopAnalyser, bool &loadedVararg);
  bool getNewResult(ShadowInstruction *SI, ImprovedValSet *&NewResult,
                    bool &loadedVararg);
  bool tryEvaluateOrdinaryInst(ShadowInstruction *SI, ImprovedValSet *&NewPB);
  bool tryEvaluateOrdinaryInst(ShadowInstruction *SI,
  ...
  bool tryFoldBitwiseOp(ShadowInstruction *SI,
                        std::pair<ValSetType, ImprovedVal> *Ops,
                        ValSetType &ImpType, ImprovedVal &Improved);
  unsigned getAlignment(ShadowValue);
  bool tryFoldPtrAsIntOp(ShadowInstruction *SI,
                         std::pair<ValSetType, ImprovedVal> *Ops,
                         ValSetType &ImpType, ImprovedVal &Improved);
  bool tryFoldPointerCmp(ShadowInstruction *SI,
                         std::pair<ValSetType, ImprovedVal> *Ops,
                         ValSetType &ImpType, ImprovedVal &Improved,
                         unsigned char *needsRuntimeCheck);
  bool tryFoldNonConstCmp(ShadowInstruction *SI,
                          std::pair<ValSetType, ImprovedVal> *Ops,
                          ValSetType &ImpType, ImprovedVal &Improved);
  bool tryFoldOpenCmp(ShadowInstruction *SI,
                      std::pair<ValSetType, ImprovedVal> *Ops,
                      ValSetType &ImpType, ImprovedVal &Improved);
  void getExitPHIOperands(ShadowInstruction *SI, uint32_t valOpIdx,
                          SmallVector<ShadowValue, 1> &ops,
                          SmallVector<ShadowBB *, 1> *BBs = 0);
  void getOperandRising(ShadowInstruction *SI, uint32_t valOpIdx,
                        ShadowBBInvar *ExitingBB, ShadowBBInvar *ExitedBB,
                        SmallVector<ShadowValue, 1> &ops,
                        SmallVector<ShadowBB *, 1> *BBs);
  void getCommittedExitPHIOperands(ShadowInstruction *SI, uint32_t valOpIdx,
                                   SmallVector<ShadowValue, 1> &ops,
                                   SmallVector<ShadowBB *, 1> *BBs);
  ...
  bool tryGetPathValue(ShadowValue V, ShadowBB *UserBlock,
                       std::pair<ValSetType, ImprovedVal> &Result);
  bool tryGetAsDefPathValue(ShadowValue V, ShadowBB *UserBlock,
                            std::pair<ValSetType, ImprovedVal> &Result);
  virtual bool tryGetPathValue2(ShadowValue V, ShadowBB *UserBlock,
                                std::pair<ValSetType, ImprovedVal> &Result,
                                bool asDef);
  bool tryGetPathValueFrom(PathConditions &PC, uint32_t stackDepth,
  ...
  virtual IntegrationAttempt *
  getIAForScopeFalling(const ShadowLoopInvar *Scope) = 0;
  ...
  InlineAttempt *getOrCreateInlineAttempt(ShadowInstruction *CI, bool &created,
                                          bool &needsAnalyse);
  bool callCanExpand(ShadowInstruction *Call, InlineAttempt *&Result);
  bool analyseExpandableCall(ShadowInstruction *SI, bool &changed,
  ...
  bool tryResolveLoadFromConstant(ShadowInstruction *, ImprovedVal Ptr,
                                  ImprovedValSetSingle &Result);
  bool tryResolveLoadFromVararg(ShadowInstruction *LoadI,
                                ImprovedValSet *&Result);
  bool tryForwardLoadPB(ShadowInstruction *LI, ImprovedValSet *&NewPB,
                        bool &loadedVararg);
  bool getConstantString(ShadowValue Ptr, ShadowInstruction *SearchFrom,
                         std::string &Result);
  virtual void applyMemoryPathConditions(ShadowBB *, bool inLoopAnalyser,
                                         bool inAnyLoop);
  ...
  void visitLoopExitingBlocksBW(ShadowBBInvar *ExitedBB,
                                ShadowBBInvar *ExitingBB, ShadowBBVisitor *,
                                void *Ctx, bool &firstPred);
  virtual WalkInstructionResult queuePredecessorsBW(ShadowBB *FromBB,
                                                    BackwardIAWalker *Walker,
                                                    void *Ctx) = 0;
  void visitNormalPredecessorsBW(ShadowBB *FromBB, ShadowBBVisitor *,
                                 void *Ctx);
  void queueReturnBlocks(BackwardIAWalker *Walker, void *);
  virtual void queueSuccessorsFWFalling(ShadowBBInvar *BB,
                                        ForwardIAWalker *Walker, void *Ctx,
                                        bool &firstSucc) = 0;
  virtual void queueSuccessorsFW(ShadowBB *BB, ForwardIAWalker *Walker,
                                 void *ctx);
  virtual bool queueNextLoopIterationFW(ShadowBB *PresentBlock,
                                        ShadowBBInvar *NextBlock,
                                        ForwardIAWalker *Walker, void *Ctx,
                                        bool &firstSucc) = 0;
  ...
  bool executeStatCall(ShadowInstruction *SI, Function *F,
                       std::string &Filename);
  WalkInstructionResult isVfsCallUsingFD(ShadowInstruction *VFSCall,
  ...
  void DSEHandleWrite(ShadowValue PtrOp, uint64_t Size,
                      ShadowInstruction *Writer, ShadowBB *BB);
  void tryKillStoresInLoop(const ShadowLoopInvar *L, bool commitDisabledHere,
                           bool disableWrites, bool latchToHeader = false);
  void tryKillStoresInUnboundedLoop(const ShadowLoopInvar *UL,
                                    bool commitDisabledHere,
                                    bool disableWrites);
  void DSEAnalyseInstruction(ShadowInstruction *I, bool commitDisabledHere,
                             bool disableWrites, bool enterCalls, bool &bail);

  // User visitors:

  virtual bool visitNextIterationPHI(ShadowInstructionInvar *I,
                                     DIVisitor &Visitor);
  virtual void visitExitPHI(ShadowInstructionInvar *UserI,
                            DIVisitor &Visitor) = 0;
  ...
  virtual void findResidualFunctions(DenseSet<Function *> &,
                                     DenseMap<Function *, unsigned> &);
  ...
  void printOutgoingEdge(ShadowBBInvar *BBI, ShadowBB *BB, ShadowBBInvar *SBI,
                         ShadowBB *SB, uint32_t i, bool useLabels,
                         const ShadowLoopInvar *deferEdgesOutside,
                         SmallVector<std::string, 4> *deferredEdges,
                         raw_ostream &Out, bool brief);
  void describeBlockAsDOT(ShadowBBInvar *BBI, ShadowBB *BB,
                          const ShadowLoopInvar *deferEdgesOutside,
                          SmallVector<std::string, 4> *deferredEdges,
                          raw_ostream &Out,
                          SmallVector<ShadowBBInvar *, 4> *forceSuccessors,
                          bool brief, bool plain = false);
  void describeScopeAsDOT(const ShadowLoopInvar *DescribeL, uint32_t headerIdx,
                          raw_ostream &Out, bool brief,
                          SmallVector<std::string, 4> *deferredEdges);
  void describeLoopAsDOT(const ShadowLoopInvar *L, uint32_t headerIdx,
                         raw_ostream &Out, bool brief);
  ...
  virtual bool getSpecialEdgeDescription(ShadowBBInvar *FromBB,
                                         ShadowBBInvar *ToBB,
                                         raw_ostream &Out) = 0;
  bool blockLiveInAnyScope(ShadowBBInvar *BB);
  virtual void printPathConditions(raw_ostream &Out, ShadowBBInvar *BBI,
                                   ShadowBB *BB);
  ...
  virtual BasicBlock *getSuccessorBB(ShadowBB *BB, uint32_t succIdx,
  ...
  virtual void emitPHINode(ShadowBB *BB, ShadowInstruction *I,
                           BasicBlock *emitBB);
  void fixupHeaderPHIs(ShadowBB *BB);
  void emitTerminator(ShadowBB *BB, ShadowInstruction *I, BasicBlock *emitBB);
  bool emitVFSCall(ShadowBB *BB, ShadowInstruction *I,
                   SmallVector<CommittedBlock, 1>::iterator &emitBB);
  void emitCall(ShadowBB *BB, ShadowInstruction *I,
                SmallVector<CommittedBlock, 1>::iterator &emitBB);
  Instruction *emitInst(ShadowBB *BB, ShadowInstruction *I, BasicBlock *emitBB);
  bool synthCommittedPointer(ShadowValue I,
                             SmallVector<CommittedBlock, 1>::iterator emitBB);
  bool synthCommittedPointer(ShadowValue *, Type *, ImprovedVal,
                             BasicBlock *emitBB, Value *&);
  ...
  void emitChunk(ShadowInstruction *I, BasicBlock *emitBB,
  ...
  Value *trySynthVal(ShadowValue *I, Type *targetType, ValSetType Ty,
                     const ImprovedVal &IV, BasicBlock *emitBB);
  ...
  void emitOrSynthInst(ShadowInstruction *I, ShadowBB *BB,
                       SmallVector<CommittedBlock, 1>::iterator &emitBB);
  ...
  bool commitStarted() { return commitState != COMMIT_NOT_STARTED; }
  Value *getCommittedValue(ShadowValue SV);
  Value *getCommittedValueOrBlock(ShadowInstruction *I, uint32_t idx,
  ...
  BasicBlock *createBasicBlock(LLVMContext &Ctx, const Twine &Name,
                               Function *AddF, bool isEntryBlock,
                               bool isFailedBlock);
  BasicBlock *CloneBasicBlockFrom(const BasicBlock *BB, ValueToValueMapTy &VMap,
                                  const Twine &NameSuffix, Function *F,
                                  uint32_t startIdx);
  void addPatchRequest(ShadowValue Needed, Instruction *PatchI,
                       uint32_t PatchOp);
  virtual void
  inheritCommitBlocksAndFunctions(std::vector<BasicBlock *> &NewCBs,
                                  std::vector<BasicBlock *> &NewFCBs,
                                  std::vector<Function *> &NewFs) = 0;
  ...
  uint32_t collectSpecIncomingEdges(
  ...
  void
  collectSpecPreds(ShadowBBInvar *instBlock, uint32_t instIdx,
                   SmallVector<std::pair<Value *, BasicBlock *>, 4> &preds);
  void collectCallFailingEdges(
  ...
  Value *emitCompareCheck(Value *realInst, const ImprovedValSetSingle *IVS,
                          BasicBlock *emitBB);
  ...
  emitExitPHIChecks(SmallVector<CommittedBlock, 1>::iterator emitIt,
                    ShadowBB *BB);
  Value *emitMemcpyCheck(ShadowInstruction *SI, BasicBlock *emitBB);
  SmallVector<CommittedBlock, 1>::iterator
  emitOrdinaryInstCheck(SmallVector<CommittedBlock, 1>::iterator emitIt,
                        ShadowInstruction *SI);
  SmallVector<CommittedBlock, 1>::iterator
  emitPathConditionChecks(ShadowBB *BB);
  ShadowValue getPathConditionSV(uint32_t instStackIdx, BasicBlock *instBB,
                                 uint32_t instIdx);
  ShadowValue getPathConditionSV(PathCondition &Cond);
  void
  emitPathConditionCheck(PathCondition &Cond, PathConditionTypes Ty,
                         ShadowBB *BB, uint32_t stackIdx,
                         SmallVector<CommittedBlock, 1>::iterator &emitBlockIt);
  void emitPathConditionChecksIn(
  ...
  void emitPathConditionChecks2(ShadowBB *BB, PathConditions &PC,
                                uint32_t stackIdx,
                                SmallVector<CommittedBlock, 1>::iterator &it);
  bool hasSpecialisedCompanion(ShadowBBInvar *BBI);
  void gatherPathConditionEdges(
  ...
  void noteAsExpectedChecksFrom(ShadowBB *BB, std::vector<PathCondition> &PCs,
                                uint32_t stackIdx);
  bool requiresBreakCode(ShadowInstruction *);
  bool subblockEndsWithSpecialTest(
      uint32_t idx,
  ...
  bool gatherTopOfBlockVFSChecks(
  ...
  ThreadLocalState shouldCheckCopy(ShadowInstruction &SI, ShadowValue PtrOp,
                                   ShadowValue LenSV);
  ThreadLocalState shouldCheckLoadFrom(ShadowInstruction &SI, ImprovedVal &Ptr,
                                       uint64_t LoadSize);
  ThreadLocalState shouldCheckLoad(ShadowInstruction &SI);
  void findTentativeLoadsInLoop(const ShadowLoopInvar *L,
                                bool commitDisabledHere, bool secondPass,
                                bool latchToHeader = false);
  void findTentativeLoadsInUnboundedLoop(const ShadowLoopInvar *L,
                                         bool commitDisabledHere,
                                         bool secondPass);
  void TLAnalyseInstruction(ShadowInstruction &, bool commitDisabledHere,
                            bool secondPass, bool inLoopAnalyser);
  ...
  void squashUnavailableObjects(ShadowInstruction &SI, ImprovedValSet *,
                                bool inLoopAnalyser);
  void squashUnavailableObjects(ShadowInstruction &SI, bool inLoopAnalyser);
  bool squashUnavailableObject(ShadowInstruction &SI,
                               const ImprovedValSetSingle &IVS,
                               bool inLoopAnalyser, ShadowValue ReadPtr,
                               int64_t ReadOffset, uint64_t ReadSize);
  ...
  void printDebugHeader(raw_ostream &Str) { printHeader(Str); }
...
public:
  PeelIteration(LLPEAnalysisPass *Pass, IntegrationAttempt *P, PeelAttempt *PP,
  ...
  virtual InlineAttempt *getFunctionRoot() { return parent->getFunctionRoot(); }
  ...
  virtual bool getSpecialEdgeDescription(ShadowBBInvar *FromBB,
                                         ShadowBBInvar *ToBB, raw_ostream &Out);
  ...
  virtual WalkInstructionResult
  queuePredecessorsBW(ShadowBB *FromBB, BackwardIAWalker *Walker, void *ctx);
  virtual bool queueNextLoopIterationFW(ShadowBB *PresentBlock,
                                        ShadowBBInvar *NextBlock,
                                        ForwardIAWalker *Walker, void *Ctx,
                                        bool &firstSucc);
  ...
  virtual IntegrationAttempt *
  getIAForScopeFalling(const ShadowLoopInvar *Scope);
  virtual void queueSuccessorsFWFalling(ShadowBBInvar *BB,
                                        ForwardIAWalker *Walker, void *Ctx,
                                        bool &firstSucc);
  ...
  void setExitingStore(void *, ShadowBBInvar *, const ShadowLoopInvar *,
                       StoreKind);

  virtual void
  inheritCommitBlocksAndFunctions(std::vector<BasicBlock *> &NewCBs,
                                  std::vector<BasicBlock *> &NewCFBs,
                                  std::vector<Function *> &NewFs);

  ShadowBB *getUniqueExitingBlock2(ShadowBBInvar *BBI,
                                   const ShadowLoopInvar *exitLoop, bool &bail);
...
public:
  ...
  PeelAttempt(LLPEAnalysisPass *Pass, IntegrationAttempt *P, Function &_F,
  ...
  void printDebugHeader(raw_ostream &OS) const { printHeader(OS); }
  void print(raw_ostream &OS) const;
...
struct IATargetInfo {
  ...
  IATargetInfo(uint32_t tCB, uint32_t tCI, uint32_t tSD)
      : targetCallBlock(tCB), targetCallInst(tCI), targetStackDepth(tSD) {}
};
...
public:
  InlineAttempt(LLPEAnalysisPass *Pass, Function &F, ShadowInstruction *_CI,
                int depth, bool isPathCond = false);
  ...
  AllocData &getAllocaWithIdx(uint32_t i) { return localAllocas[i]; }
  ...
  // Indexes from CLONED instruction/block to replacement PHI node to use in
  // that block.
  ...
  bool isUnsharable() {
    return hasVFSOps || isModel ||
           (sharing && !sharing->escapingMallocs.empty()) || Callers.empty();
  }

  bool isShared() { return Callers.size() > 1; }

  virtual BasicBlock *getEntryBlock();

  virtual InlineAttempt *getFunctionRoot() { return this; }
  ...
  bool analyseWithArgs(ShadowInstruction *Caller, bool withinUnboundedLoop,
                       bool inAnyLoop, uint32_t parent_stack_depth);
  bool analyseNoArgs(bool withinUnboundedLoop, bool inAnyLoop,
                     uint32_t parent_stack_depth);
  ...
  Value *getArgCommittedValue2(ShadowArg *SA, BasicBlock *emitBB,
                               Instruction *insertBefore);
  ...
  virtual void finishFailedBlockCommit();
  void
  markBBAndPreds(ShadowBBInvar *UseBBI, uint32_t instIdx,
  ...
  Value *getUnspecValue(uint32_t blockIdx, uint32_t instIdx, Value *V,
                        BasicBlock *BB, Instruction *InsertBefore);
  BasicBlock::iterator commitFailedPHIs(BasicBlock *BB, BasicBlock::iterator BI,
                                        uint32_t BBIdx);
  void remapFailedBlock(BasicBlock::iterator BI, BasicBlock *BB,
                        uint32_t blockIdx, uint32_t instIdx,
...
struct IAIterator {
  ...
  IAIterator(const IAIterator &other)
      : parent(other.parent), blockIdx(other.blockIdx), instIdx(other.instIdx) {
  }
  ...
  bool operator==(const IAIterator &other) {
    ...
    return parent == other.parent && blockIdx == other.blockIdx &&
           instIdx == other.instIdx;
  }

  bool operator!=(const IAIterator &other) { return !((*this) == other); }
...
inline bool hasNoCallers(InlineAttempt *IA) { return IA->Callers.empty(); }
...
Constant *extractAggregateMemberAt(Constant *From, int64_t Offset, Type *Target,
                                   uint64_t TargetSize, const DataLayout *);
...
const ShadowLoopInvar *immediateChildLoop(const ShadowLoopInvar *Parent,
                                          const ShadowLoopInvar *Child);
Constant *getConstReplacement(Value *, IntegrationAttempt *);
Constant *intFromBytes(const uint64_t *, unsigned, unsigned,
                       llvm::LLVMContext &);

// Implemented in Transforms/Integrator/SimpleVFSEval.cpp, so only usable with
// -integrator
bool getFileBytes(std::string &strFileName, uint64_t realFilePos,
                  uint64_t realBytes, std::vector<Constant *> &arrayBytes,
                  LLVMContext &Context, std::string &errors);

// Implemented in VMCore/AsmWriter.cpp, since that file contains a bunch of
// useful private classes if LLVM has been patched appropriately; otherwise
// stubbed out with simple implementations in Print.cpp.
PersistPrinter *getPersistPrinter(Module *);
void getInstructionsText(PersistPrinter *, const Function *IF,
                         DenseMap<const Value *, std::string> &IMap,
                         DenseMap<const Value *, std::string> &BriefMap);
void getGVText(PersistPrinter *, const Module *M,
               DenseMap<const GlobalVariable *, std::string> &GVMap,
               DenseMap<const GlobalVariable *, std::string> &BriefGVMap);
...
bool GetDefinedRange(ShadowValue DefinedBase, int64_t DefinedOffset,
                     uint64_t DefinedSize, ShadowValue DefinerBase,
                     int64_t DefinerOffset, uint64_t DefinerSize,
...
Function *cloneEmptyFunction(Function *F, GlobalValue::LinkageTypes LT,
                             const Twine &Name, bool addFailedReturnFlag);

Constant *getGVOffset(Constant *GV, int64_t Offset, Type *targetType);

// Load forwarding v3 functions:
bool addIVToPartialVal(const ImprovedVal &IV, ValSetType SetType,
                       uint64_t IVSOffset, uint64_t PVOffset, uint64_t Size,
                       PartialVal *PV, std::string *error);
bool addIVSToPartialVal(const ImprovedValSetSingle &IVS, uint64_t IVSOffset,
                        uint64_t PVOffset, uint64_t Size, PartialVal *PV,
                        std::string *error);
void readValRangeFrom(ShadowValue &V, uint64_t Offset, uint64_t Size,
                      ShadowBB *ReadBB, ImprovedValSet *store,
                      ImprovedValSetSingle &Result, PartialVal *&ResultPV,
                      bool &shouldTryMulti, std::string *error);
void readValRange(ShadowValue &V, int64_t Offset, uint64_t Size,
...
bool executeAtomicRMW(ShadowInstruction *SI, ImprovedValSet *&OldPB,
                      bool &loadedVararg);
bool executeCmpXchg(ShadowInstruction *SI, ImprovedValSet *&OldPB,
                    bool &loadedVararg);
void propagateStoreFlags(ImprovedValSetSingle &WrittenPtr,
                         ImprovedValSetSingle &WrittenVal, ShadowBB *StoreBB);

void getIVSSubVals(const ImprovedValSetSingle &Src, uint64_t Offset,
                   uint64_t Size, int64_t OffsetAbove,
                   SmallVector<IVSRange, 4> &Dest);
void getIVSSubVal(const ImprovedValSetSingle &Src, uint64_t Offset,
                  uint64_t Size, ImprovedValSetSingle &Dest);
void getConstSubVals(ShadowValue FromSV, uint64_t Offset, uint64_t TargetSize,
                     int64_t OffsetAbove, SmallVector<IVSRange, 4> &Dest);
Constant *valsToConst(SmallVector<IVSRange, 4> &subVals, uint64_t TargetSize,
                      Type *targetType);
void getConstSubVal(ShadowValue FromSV, uint64_t Offset, uint64_t TargetSize,
                    Type *TargetType, ImprovedValSetSingle &Result);
Constant *getSubConst(Constant *FromC, uint64_t Offset, uint64_t TargetSize,
                      Type *targetType = 0);
void clearRange(ImprovedValSetMulti *M, uint64_t Offset, uint64_t Size);
void replaceRangeWithPB(ImprovedValSet *Target,
                        const ImprovedValSetSingle &NewVal, int64_t Offset,
                        uint64_t Size);
void replaceRangeWithPBs(ImprovedValSet *Target,
                         const SmallVector<IVSRange, 4> &NewVals,
                         uint64_t Offset, uint64_t Size);
void truncateConstVal(ImprovedValSetMulti::MapIt &it, uint64_t off,
                      uint64_t size,
...
void readValRangeMultiFrom(uint64_t Offset, uint64_t Size,
                           ImprovedValSet *store,
                           SmallVector<IVSRange, 4> &Results,
                           ImprovedValSet *ignoreBelowStore, uint64_t ASize);
void readValRangeMulti(ShadowValue &V, uint64_t Offset, uint64_t Size,
                       ShadowBB *ReadBB, SmallVector<IVSRange, 4> &Results);
void executeMemcpyInst(ShadowInstruction *MemcpySI);
void executeVaCopyInst(ShadowInstruction *SI);
void executeAllocInst(ShadowInstruction *SI, AllocData &, Type *AllocType,
                      uint64_t AllocSize, int32_t frame, uint32_t idx);
...
void executeCopyInst(ShadowValue *Ptr, ImprovedValSetSingle &PtrSet,
                     ImprovedValSetSingle &SrcPtrSet, uint64_t Size,
                     ShadowInstruction *);
void executeVaStartInst(ShadowInstruction *SI);
void executeReadInst(ShadowInstruction *ReadSI, std::string &Filename,
...
void executeWriteInst(ShadowValue *Ptr, ImprovedValSetSingle &PtrSet,
                      ImprovedValSetSingle &ValPB, uint64_t PtrSize,
                      ShadowInstruction *);
void writeExtents(SmallVector<IVSRange, 4> &copyValues, ShadowValue &Ptr,
...
IntegratorTag *searchFunctions(IntegratorTag *thisTag, std::string &,
                               IntegratorTag *&startAt);
...
void intersectSets(DenseSet<ShadowValue> *Target,
...
void printPathCondition(PathCondition &PC, PathConditionTypes t, ShadowBB *BB,
                        raw_ostream &Out, bool HTMLEscaped);
...
bool IHPFoldIntOp(ShadowInstruction *SI,
                  std::pair<ValSetType, ImprovedVal> *Ops,
                  SmallVector<uint64_t, 4> &OpInts, ValSetType &ImpType,
                  ImprovedVal &Improved);
void DeleteDeadInstruction(Instruction *I);
void createTopOrderingFrom(BasicBlock *BB, std::vector<BasicBlock *> &Result,
