...
template <class ChildType, class ExtraState> struct SharedTreeNode {
  ...
  bool dropReference(uint32_t idx, uint32_t height,
                     std::vector<ShadowValue> *simplified);
  ChildType *getReadableStoreFor(uint32_t idx, uint32_t height);
  ChildType *getOrCreateStoreFor(uint32_t idx, uint32_t height,
                                 bool *isNewStore);
...
bool SharedTreeNode<ChildType, ExtraState>::dropReference(
    uint32_t idx, uint32_t height, std::vector<ShadowValue> *simplified) {
  ...
  if (!--refCount) {
    ...
    if (height == 0) {
      ...
      for (uint32_t i = 0; i < HEAPTREEORDER; ++i) {
        if (children[i])
          ((SharedTreeNode *)children[i])
              ->dropReference(idx + (i << (height * HEAPTREEORDERLOG2)),
                              height - 1, simplified);
      }
    }
...
template <class ChildType, class ExtraState>
ChildType *
SharedTreeNode<ChildType, ExtraState>::getReadableStoreFor(uint32_t idx,
                                                           uint32_t height) {

  uint32_t nextChild =
      (idx >> (height * HEAPTREEORDERLOG2)) & (HEAPTREEORDER - 1);
  ...
  if (height == 0) {
    ...
    return ((SharedTreeNode *)children[nextChild])
        ->getReadableStoreFor(idx, height - 1);
  }
}

template <class ChildType, class ExtraState>
ChildType *SharedTreeNode<ChildType, ExtraState>::getOrCreateStoreFor(
    uint32_t idx, uint32_t height, bool *isNewStore) {
  ...
  if (height == 0) {
    ...
    if (!children[nextChild])
      ...
      child =
          ((SharedTreeNode *)children[nextChild])->getWritableNode(height - 1);
...
template <class ChildType, class ExtraState>
SharedTreeNode<ChildType, ExtraState> *
SharedTreeNode<ChildType, ExtraState>::getWritableNode(uint32_t height) {
  ...
  if (height == 0) {
    ...
    for (uint32_t i = 0; i < HEAPTREEORDER; ++i) {
      ...
      if (children[i])
        newNode->children[i] =
            new ChildType(((ChildType *)children[i])->getReadableCopy());
    }
...
static inline bool derefLT(void **a, void **b) {
  if (!a)
    return !!b;
  else if (!b)
    return false;
...
static inline bool derefEQ(void **a, void **b) {
...
template <class ChildType, class ExtraState>
void SharedTreeNode<ChildType, ExtraState>::mergeHeaps(
    SmallVector<SharedTreeNode<ChildType, ExtraState> *, 4> &others,
    bool allOthersClobbered, uint32_t height, uint32_t idx,
    MergeBlockVisitor<ChildType, ExtraState> *visitor) {

  // All members of others are known to differ from this node. This node is
  // writable already. Like the frames case, merge in base objects when objects
  // are missing from this or the other tree if !allOthersClobbered; otherwise
  // intersect the trees. Note the special case that others might contain a null
  // pointer, which describes the empty tree.

  if (allOthersClobbered) {

    for (uint32_t i = 0; i < HEAPTREEORDER; ++i) {

      for (typename SmallVector<SharedTreeNode<ChildType, ExtraState> *,
                                4>::iterator it = others.begin(),
                                             itend = others.end();
        ...
        if ((!*it) || !((*it)->children[i])) {
          ...
          if (height == 0)
            ...
            ((SharedTreeNode *)children[i])
                ->dropReference(idx + i, height - 1, 0);
          children[i] = 0;
        }
      }
    }
    ...
    // Populate this node with base versions of nodes that are missing but
    // present in any other tree. Just add blank nodes for now and then the
    // recursion will catch the rest.
    for (uint32_t i = 0; i < HEAPTREEORDER; ++i) {
      ...
      for (typename SmallVector<SharedTreeNode<ChildType, ExtraState> *,
        ...
        if ((*it) && (*it)->children[i]) {
          ...
          if (height == 0)
            children[i] =
                new ChildType(ChildType::getEmptyStore().getReadableCopy());
          else
  ...
  for (uint32_t i = 0; i < HEAPTREEORDER; ++i) {
    ...
    // or ChildTypes. In the former case this avoids merges of identical
    // subtrees in the latter it skips merging ChildTypes that are shared (as
    // determined by ChildType::EQ) In either case refcounting is caught up when
    // unused maps are released at the top level.
    ...
    if (height == 0) {
      ...
      std::sort(incomingPtrs.begin(), incomingPtrs.end(),
                IndirectComp<ChildType>::LT);
      SmallVector<void **, 4>::iterator uniqend =
          std::unique(incomingPtrs.begin(), incomingPtrs.end(),
                      IndirectComp<ChildType>::EQ);
      ...
      for (SmallVector<void **, 4>::iterator it = incomingPtrs.begin();
           it != uniqend; ++it) {
        ...
        ChildType::mergeStores(mergeFromStore, (ChildType *)children[i], ASize,
                               visitor);
      ...
      SmallVector<void **, 4>::iterator uniqend =
          std::unique(incomingPtrs.begin(), incomingPtrs.end(), derefEQ);
      ...
      ((SharedTreeNode *)children[i])
          ->mergeHeaps(otherChildren, allOthersClobbered, height - 1, newIdx,
                       visitor);
    }
  }
}
...
template <class ChildType, class ExtraState>
void SharedTreeNode<ChildType, ExtraState>::print(raw_ostream &RSO, bool brief,
                                                  uint32_t height,
                                                  uint32_t idx) {
...
static inline uint32_t getRequiredHeight(uint32_t idx) {
...
template <class ChildType, class ExtraState>
ChildType *SharedTreeRoot<ChildType, ExtraState>::getReadableStoreFor(
    const ShadowValue &V) {
...
template <class ChildType, class ExtraState>
void SharedTreeRoot<ChildType, ExtraState>::growToHeight(uint32_t newHeight) {

  for (uint32_t i = 0, ilim = (newHeight - height); i != ilim; ++i) {

    SharedTreeNode<ChildType, ExtraState> *newNode =
        new SharedTreeNode<ChildType, ExtraState>();
...
template <class ChildType, class ExtraState>
void SharedTreeRoot<ChildType, ExtraState>::grow(uint32_t idx) {

  // Need to make the tree taller first (hopefully the above test is cheaper
  // than getReqdHeight)
...
template <class ChildType, class ExtraState>
bool SharedTreeRoot<ChildType, ExtraState>::mustGrowFor(uint32_t idx) {

  // Can't accommodate more than 2^32 heap items! Change this if we use a
  // uint64_t for heap indices.
...
template <class ChildType, class ExtraState>
ChildType *
SharedTreeRoot<ChildType, ExtraState>::getOrCreateStoreFor(ShadowValue &V,
                                                           bool *isNewStore) {
...
template <class ChildType, class ExtraState>
void SharedTreeRoot<ChildType, ExtraState>::clear(
    std::vector<ShadowValue> *simplified) {
...
template <class ChildType, class ExtraState>
bool SharedTreeRoot<ChildType, ExtraState>::dropReference(
    std::vector<ShadowValue> *simplified) {
...
template <class ChildType, class ExtraState> struct SharedStoreMap {
  ...
  SharedStoreMap(InlineAttempt *_IA, uint32_t initSize)
      : store(initSize), refCount(1), IA(_IA), empty(true) {}

  SharedStoreMap *getWritableStoreMap();
  bool dropReference(uint32_t stack_depth,
                     std::vector<ShadowValue> *simplified);
...
template <class ChildType, class ExtraState>
SharedStoreMap<ChildType, ExtraState> *
SharedStoreMap<ChildType, ExtraState>::getWritableStoreMap() {
  ...
  LFV3(errs() << "COW break local map " << this << " with " << store.size()
              << " entries\n");
  SharedStoreMap *newMap = new SharedStoreMap(IA, store.size());

  uint32_t i = 0;
  for (typename std::vector<ChildType>::iterator it = store.begin(),
                                                 it2 = store.end();
       it != it2; ++it, ++i) {
    if (!it->isValid())
      continue;
...
template <class ChildType, class ExtraState>
void SharedStoreMap<ChildType, ExtraState>::clear(
    uint32_t stack_depth, std::vector<ShadowValue> *simplified) {
...
template <class ChildType, class ExtraState>
SharedStoreMap<ChildType, ExtraState> *
SharedStoreMap<ChildType, ExtraState>::getEmptyMap(uint32_t stack_depth) {
...
template <class ChildType, class ExtraState>
bool SharedStoreMap<ChildType, ExtraState>::dropReference(
    uint32_t stack_depth, std::vector<ShadowValue> *simplified) {
...
template <class ChildType, class ExtraState>
void SharedStoreMap<ChildType, ExtraState>::print(raw_ostream &RSO,
                                                  bool brief) {
...
template <class ChildType, class ExtraState> struct LocalStoreMap {
  ...
  LocalStoreMap(uint32_t s)
      : frames(s), heap(), allOthersClobbered(false), refCount(1) {}
  ...
  bool empty();
  void
  copyEmptyFrames(SmallVector<SharedStoreMap<ChildType, ExtraState> *, 4> &);
...
template <class ChildType, class ExtraState>
ChildType *
LocalStoreMap<ChildType, ExtraState>::getOrCreateStoreFor(ShadowValue &V,
                                                          bool *isNewStore) {
...
template <class ChildType, class ExtraState>
ChildType *LocalStoreMap<ChildType, ExtraState>::getReadableStoreFor(
    const ShadowValue &V) {
...
template <class ChildType, class ExtraState>
LocalStoreMap<ChildType, ExtraState> *
LocalStoreMap<ChildType, ExtraState>::getWritableFrameList() {

  if (refCount == 1)
    return this;

  LocalStoreMap<ChildType, ExtraState> *newMap =
      new LocalStoreMap<ChildType, ExtraState>(frames.size());
...
template <class ChildType, class ExtraState>
std::vector<ChildType> &
LocalStoreMap<ChildType, ExtraState>::getWritableFrame(int32_t frameNo) {
...
template <class ChildType, class ExtraState>
void LocalStoreMap<ChildType, ExtraState>::clear() {
...
template <class ChildType, class ExtraState>
bool LocalStoreMap<ChildType, ExtraState>::empty() {
...
template <class ChildType, class ExtraState>
LocalStoreMap<ChildType, ExtraState> *
LocalStoreMap<ChildType, ExtraState>::getEmptyMap() {
...
template <class ChildType, class ExtraState>
void LocalStoreMap<ChildType, ExtraState>::copyEmptyFrames(
    SmallVector<SharedStoreMap<ChildType, ExtraState> *, 4> &copyFrom) {
  ...
  for (uint32_t i = 0; i < frames.size(); ++i) {
    ...
    frames[i] = new SharedStoreMap<ChildType, ExtraState>(copyFrom[i]->IA,
                                                          newFrameSize);
  }
}

template <class ChildType, class ExtraState>
void LocalStoreMap<ChildType, ExtraState>::copyFramesFrom(
    const LocalStoreMap<ChildType, ExtraState> &other) {
...
template <class ChildType, class ExtraState>
bool LocalStoreMap<ChildType, ExtraState>::dropReference(
    std::vector<ShadowValue> *Simplified) {
...
template <class ChildType, class ExtraState>
void LocalStoreMap<ChildType, ExtraState>::popStackFrame() {
...
template <class ChildType, class ExtraState>
void LocalStoreMap<ChildType, ExtraState>::pushStackFrame(InlineAttempt *IA) {

  frames.push_back(
      new SharedStoreMap<ChildType, ExtraState>(IA, allocFrameSize(IA)));
}

template <class ChildType, class ExtraState>
void LocalStoreMap<ChildType, ExtraState>::print(raw_ostream &RSO, bool brief) {
...
struct MergeBlockVisitor : public ShadowBBVisitor {
  ...
  MergeBlockVisitor(IntegrationAttempt *origin, bool uvm = false,
                    bool v = false)
      : ShadowBBVisitor(true), originContext(origin), newMap(0),
        useVarargMerge(uvm), verbose(v) {}

  void mergeFrames(MapType *toMap,
                   typename SmallVector<MapType *, 4>::iterator fromBegin,
                   typename SmallVector<MapType *, 4>::iterator fromEnd,
                   uint32_t idx);
  void mergeHeaps(MapType *toMap,
                  typename SmallVector<MapType *, 4>::iterator fromBegin,
                  typename SmallVector<MapType *, 4>::iterator fromEnd);
  void mergeFlags(MapType *toMap,
                  typename SmallVector<MapType *, 4>::iterator fromBegin,
                  typename SmallVector<MapType *, 4>::iterator fromEnd);
...
// Comparator for finding the best target heap: we want the tallest heap, and of
// those, we favour a writable one. Finally compare pointers.
template <class ChildType, class ExtraState>
bool rootTallerThan(const LocalStoreMap<ChildType, ExtraState> *r1,
                    const LocalStoreMap<ChildType, ExtraState> *r2) {
...
template <class ChildType, class ExtraState>
bool rootsEqual(const LocalStoreMap<ChildType, ExtraState> *r1,
                const LocalStoreMap<ChildType, ExtraState> *r2) {
...
template <class ChildType, class ExtraState>
void MergeBlockVisitor<ChildType, ExtraState>::mergeHeaps(
    MapType *toMap, typename SmallVector<MapType *, 4>::iterator fromBegin,
    typename SmallVector<MapType *, 4>::iterator fromEnd) {
  ...
  for (typename SmallVector<MapType *, 4>::iterator it = fromBegin;
       it != fromEnd; ++it)
  ...
  for(SmallVector<LocalStoreMap*, 4>::iterator it = fromBegin; it != fromEnd;
  ++it) { errs() << "Merging in:\n"; if((*it)->heap.root)
      (*it)->heap.root->print(errs(), false, (*it)->heap.height - 1, 0);
  }
  */

  // This sorts first by heap height, then by pointer address, so it also finds
  // the tallest heap.
  std::sort(incomingRoots.begin(), incomingRoots.end(),
            rootTallerThan<ChildType, ExtraState>);
  typename SmallVector<MapType *, 4>::iterator uniqend =
      std::unique(incomingRoots.begin(), incomingRoots.end(),
                  rootsEqual<ChildType, ExtraState>);

  // Heaps never differ?
  if (std::distance(incomingRoots.begin(), uniqend) == 1)
    return;

  release_assert(incomingRoots[0]->heap.height != 0 &&
                 "If heaps differ at least one must be initialised!");
  ...
  if (!toMap->heap.root) {
    ...
    toMap->heap.root =
        toMap->heap.root->getWritableNode(toMap->heap.height - 1);
  }
  ...
  for (typename SmallVector<MapType *, 4>::iterator it = incomingRoots.begin();
       it != uniqend; ++it) {
    ...
    // Temporarily grow heaps that are shorter than the target to make the merge
    // easier to code. Leave their height attribute unchanged as an indicator we
    // need to undo this shortly. These maps might be shared so it's important
    // they are seen unmodified ouside this function.
    if (thisMap->heap.height != 0 &&
        thisMap->heap.height < toMap->heap.height) {
  ...
  toMap->heap.root->mergeHeaps(roots, toMap->allOthersClobbered,
                               toMap->heap.height - 1, 0, this);
  ...
  for (typename SmallVector<MapType *, 4>::iterator it = incomingRoots.begin();
    ...
    LocalStoreMap<ChildType, ExtraState> *thisMap = *it;
    uint32_t tempFramesToRemove =
        incomingRoots[0]->heap.height - thisMap->heap.height;
    ...
    for (uint32_t i = 0; i < tempFramesToRemove; ++i) {
      ...
      release_assert(removeNode->refCount == 1 &&
                     "Removing shared node in post-treemerge cleanup?");
      delete removeNode;
    }
  }
}
...
template <class ChildType, class ExtraState>
void MergeBlockVisitor<ChildType, ExtraState>::mergeFrames(
    MapType *toMap, typename SmallVector<MapType *, 4>::iterator fromBegin,
    typename SmallVector<MapType *, 4>::iterator fromEnd, uint32_t idx) {
  ...
  typename SmallVector<FrameType *, 4>::iterator uniqend =
      std::unique(incomingFrames.begin(), incomingFrames.end());
  ...
  FrameType *mergeToFrame = toMap->frames[idx] =
      toMap->frames[idx]->getWritableStoreMap();
  std::vector<ChildType> &mergeToStore = mergeToFrame->store;

  InlineAttempt *thisFrameIA = mergeToFrame->IA;

  // Merge in each other frame. Note toMap->allOthersClobbered has been set to
  // big-or over all maps already.
  for (typename SmallVector<FrameType *, 4>::iterator it =
           incomingFrames.begin();
       it != uniqend; ++it) {
    ...
    if (toMap->allOthersClobbered) {
      ...
      // Remove any existing mappings in mergeToFrame that do not occur in
      // mergeFromFrame:

      for (uint32_t i = 0, ilim = mergeToStore.size(); i != ilim; ++i) {

        if (mergeToStore[i].isValid() &&
            (i >= mergeFromStore.size() || !mergeFromStore[i].isValid())) {
      ...
      LFV3(errs() << "Both maps don't have allOthersClobbered; reading through "
                     "allowed\n");
  ...
  for (uint32_t i = 0, ilim = mergeToStore.size(); i != ilim; ++i) {
    ...
    for (typename SmallVector<SharedStoreMap<ChildType, ExtraState> *,
                              4>::iterator incit = incomingFrames.begin();
         incit != uniqend; ++incit) {
      ...
      if (mergeFromFrame->store.size() > i &&
          mergeFromFrame->store[i].isValid())
    ...
    /*   std::unique(incomingStores.begin(), incomingStores.end(),
     * ChildType::EQ); */

    for (typename SmallVector<ChildType *, 4>::iterator
             incit = incomingStores.begin(),
             incitend = incomingStores.end();
...
void MergeBlockVisitor<ChildType, ExtraState>::doMerge() {
  ...
  incomingStores.reserve(
      std::distance(incomingBlocks.begin(), incomingBlocks.end()));

  for (SmallVector<ShadowBB *, 4>::iterator it = incomingBlocks.begin(),
                                            itend = incomingBlocks.end();
       it != itend; ++it) {
  ...
  if (std::distance(incomingStores.begin(), uniqend) > 1) {
    ...
    // See if we can avoid a CoW break by using a writable incoming store as the
    // target.
    for (typename SmallVector<MapType *, 4>::iterator it =
             incomingStores.begin();
         it != uniqend; ++it) {
    ...
    // Position 0 is the target; the rest should be merged in. CoW break if
    // still necessary: Note retainMap is set to the original map rather than
    // the new one as the CoW break drops a reference to it so it should not be
    // unref'd again below.
    retainMap = incomingStores[0];
    MapType *mergeMap = incomingStores[0] =
        incomingStores[0]->getWritableFrameList();
    LFV3(errs() << "Merge target will be " << mergeMap << "\n");

    typename SmallVector<MapType *, 4>::iterator firstMergeFrom =
        incomingStores.begin();
    ++firstMergeFrom;
    ...
    // Merge each frame, passing the InlineAttempt corresponding to each frame
    // in turn.
  ...
  // Drop refs against each incoming store apart from the store that was either
  // used or implicitly unref'd as part of the CoW break at getWritableFrameMap.
