...
public:
  ...
  const z3::expr &getExpr() const { return e; }

  /// Get z3 solver, singleton design here to make sure we only have one context
  static z3::solver &getSolver();

  /// Get z3 context, singleton design here to make sure we only have one
  /// context
  ...
  /// null expression
  static z3::expr nullExpr() { return getContext().int_const("null"); }

  /// get id
  inline unsigned id() const { return e.id(); }

  /// get hash id
  inline unsigned hash() const { return e.hash(); }

  inline const std::string to_string() const { return e.to_string(); }

  inline bool is_numeral() const { return e.is_numeral(); }

  inline bool is_bool() const { return e.is_bool(); }

  inline Z3Expr simplify() const { return e.simplify(); }

  inline int64_t get_numeral_int64() const { return e.get_numeral_int64(); }

  inline int get_numeral_int() const { return e.get_numeral_int(); }
  ...
  friend Z3Expr int2bv(unsigned n, const Z3Expr &e) {
  ...
  friend Z3Expr operator!(const Z3Expr &lhs) { return !lhs.getExpr(); }
  ...
  z3::sort get_sort() const { return getExpr().get_sort(); }
  //%}
  ...
  static unsigned getExprSize(const Z3Expr &z3Expr);

  /// Return the unique true condition
  static inline Z3Expr getTrueCond() { return getContext().bool_val(true); }

  /// Return the unique false condition
  static inline Z3Expr getFalseCond() { return getContext().bool_val(false); }
