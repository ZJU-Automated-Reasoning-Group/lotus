...
public:
  ...
  inline bool IsVariable() {
    return contents.is_const() && variables.count(contents.to_string());
  }
  ...
  SMTNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
          IRBuilder<> &t_builder, unsigned t_integer_width,
          const std::map<std::string, Value *> &t_variables, expr t_contents);
  virtual ~SMTNode() {}
  virtual Value *ToLLVM() = 0;
  virtual APInt LargestIntegerConstant() = 0;
  virtual APInt AbstractSingle(APInt assumption) = 0;
  virtual expr ToSMT(unsigned width, std::map<std::string, expr> svariables,
                     solver *sol) = 0;
};
...
public:
  ...
  expr ToSMT(unsigned width, std::map<std::string, expr> svariables,
             solver *sol) override;
  RealNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
           IRBuilder<> &t_builder, unsigned t_integer_width,
           const std::map<std::string, Value *> &t_variables, expr t_contents);
};
...
public:
  ...
  expr ToSMT(unsigned width, std::map<std::string, expr> svariables,
             solver *sol) override;
  IntegerNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
              IRBuilder<> &t_builder, unsigned t_integer_width,
...
public:
  ...
  expr ToSMT(unsigned width, std::map<std::string, expr> svariables,
             solver *sol) override;
  FloatingNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
               IRBuilder<> &t_builder, unsigned t_integer_width,
               const std::map<std::string, Value *> &t_variables,
               expr t_contents);
};
...
public:
  ...
  expr ToSMT(unsigned width, std::map<std::string, expr> svariables,
             solver *sol) override;
  BitvectorNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
                IRBuilder<> &t_builder, unsigned t_integer_width,
                const std::map<std::string, Value *> &t_variables,
                expr t_contents);
};
...
public:
  ...
  BooleanNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
              IRBuilder<> &t_builder, unsigned t_integer_width,
              const std::map<std::string, Value *> &t_variables,
              expr t_contents);
};
