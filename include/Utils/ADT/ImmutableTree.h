...
public:
  iterator(Node *_root, bool atBeginning)
      : root(_root->incref()), stack(root->height) {
    if (atBeginning) {
...
template <class K, class V, class KOV, class CMP>
ImmutableTree<K, V, KOV, CMP>::Node::Node(Node *_left, Node *_right,
...
template <class K, class V, class KOV, class CMP>
inline typename ImmutableTree<K, V, KOV, CMP>::Node *
ImmutableTree<K, V, KOV, CMP>::Node::incref() {
  ++references;
  return this;
}
...
template <class K, class V, class KOV, class CMP>
typename ImmutableTree<K, V, KOV, CMP>::Node *
ImmutableTree<K, V, KOV, CMP>::Node::balance(Node *left,
...
ImmutableTree<K, V, KOV, CMP>::operator=(const ImmutableTree &s) {
...
ImmutableTree<K, V, KOV, CMP>::upper_bound(const key_type &key) const {
  ...
  if (it != end &&
      !key_compare()(key,
                     key_of_value()(*it))) // no need to loop, no duplicates
    ++it;
  return it;
}
