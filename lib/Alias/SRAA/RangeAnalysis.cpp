...
void InterProceduralRA<CGT>::MatchParametersAndReturnValues(
  ...
  for (Value::use_iterator UI = F.use_begin(), E = F.use_end(); UI != E; ++UI) {
    ...
    CallBase *CB = dyn_cast<CallBase>(caller);
    if (!CB)
      continue;

    if (U->get() != CB->getCalledOperand())
      continue;

    // Iterate over the real parameters and put them in the data structure
    unsigned argIndex = 0;
    for (unsigned a = 0, ae = CB->arg_size(); a != ae; ++a, ++argIndex) {
      Parameters[argIndex].second = CB->getArgOperand(a);
    }
