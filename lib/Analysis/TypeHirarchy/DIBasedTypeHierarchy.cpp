...
DIBasedTypeHierarchy::DIBasedTypeHierarchy(const llvm::Module &M) {
  // -- Find all types
  {
    ...
    auto VT = buildVTables(DIF, VertexTypes, TypeToVertex, M);
  ...
  for (const auto &Curr : SerializedData.VTables) {
    ...
    for (const auto &FuncName : Curr) {
      if (FuncName == LLVMVFTable::NullFunName) {
        ...
        CurrVTable.push_back(M->getFunction(FuncName));
      }
    }
...
auto DIBasedTypeHierarchy::subTypesOf(size_t TypeIdx) const noexcept
  ...
  auto Pair = TransitiveDerivedIndex[TypeIdx];
...
void DIBasedTypeHierarchy::printAsDot(llvm::raw_ostream &OS) const {
  ...
  // add nodes
  for (const auto &KV : TypeToVertex) {
    const auto *Type = KV.first;
    size_t Vtx = KV.second;
    OS << Vtx << "[label=\"";
