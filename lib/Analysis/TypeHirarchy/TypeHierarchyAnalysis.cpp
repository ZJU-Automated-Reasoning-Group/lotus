...
/*
   ...
   i8*), i8* bitcast (i32 (%class.D*)* @_ZThn4_N1D1fEv to i8*)] }
...
// Helper function to convert LLVM values/types to string for logging
static std::string llvmToString(const Value *V) {
  std::string str;
...
static std::string llvmToString(const Type *T) {
  std::string str;
  raw_string_ostream rso(str);
  rso << *T;
  return rso.str();
...
public:
  using function_vector_t = TypeHierarchyAnalysis::function_vector_t;

  TypeHierarchyAnalysis_Impl(Module &Module)
  ...
  ~TypeHierarchyAnalysis_Impl() = default;
...
bool TypeHierarchyAnalysis_Impl::hasVtable(const StructType *ty) const {
  return m_vtables.find(ty) != m_vtables.end();
}

TypeHierarchyAnalysis_Impl::vtable_t &
TypeHierarchyAnalysis_Impl::getVtable(const StructType *ty) {
  assert(hasVtable(ty));
  auto it = m_vtables.find(ty);
  return it->second;
}

const TypeHierarchyAnalysis_Impl::vtable_t &
TypeHierarchyAnalysis_Impl::getVtable(const StructType *ty) const {
  assert(hasVtable(ty));
  auto it = m_vtables.find(ty);
  return it->second;
}
...
int TypeHierarchyAnalysis_Impl::getVtableIndex(const CallBase &CB) {
  /*
...
void TypeHierarchyAnalysis_Impl::closureCHG(void) {
  ...
  SPDLOG_DEBUG("cha-closure: Boolean adjacency matrix before closure");
  for (unsigned i = 0; i < num_nodes; ++i) {
    std::stringstream ss;
    for (unsigned j = 0; j < num_nodes; ++j) {
      ss << "(" << i << "," << j << ")=" << m[i][j] << " ";
    }
    SPDLOG_DEBUG("cha-closure: row {}: {}", i, ss.str());
  }
  ...
  SPDLOG_DEBUG("cha-closure: Boolean adjacency matrix after closure");
  for (unsigned i = 0; i < num_nodes; ++i) {
    std::stringstream ss;
    for (unsigned j = 0; j < num_nodes; ++j) {
      ss << "(" << i << "," << j << ")=" << m[i][j] << " ";
...
void TypeHierarchyAnalysis_Impl::buildCHG(void) {
...
// argument.
bool TypeHierarchyAnalysis_Impl::matchVirtualSignature(
...
void TypeHierarchyAnalysis_Impl::addCandidateFunction(
  ...
  if (this_->getType()->isPointerTy()) {
    if (const StructType *this_type =
      ...
      if (vtable_index < 0) {
        ...
        SPDLOG_WARN("Cannot find vtable index for indirect call:\n {}", cb_str);
        return false;
      }
...
void TypeHierarchyAnalysis_Impl::printVtables(raw_ostream &o) const {
...
void TypeHierarchyAnalysis_Impl::printClassHierarchy(raw_ostream &o) const {
  for (auto &kv : m_graph) {
...
bool TypeHierarchyAnalysis_Impl::isVCallResolved(
...
TypeHierarchyAnalysis::TypeHierarchyAnalysis(Module &M)
    : m_cha_impl(std::make_unique<TypeHierarchyAnalysis_Impl>(M)) {}

TypeHierarchyAnalysis::~TypeHierarchyAnalysis(void) {}

void TypeHierarchyAnalysis::calculate(void) { m_cha_impl->calculate(); }

bool TypeHierarchyAnalysis::isVCallResolved(const llvm::CallBase &CS) const {
...
void TypeHierarchyAnalysis::printClassHierarchy(raw_ostream &o) const {
  m_cha_impl->printClassHierarchy(o);
}

void TypeHierarchyAnalysis::printVtables(raw_ostream &o) const {
  m_cha_impl->printVtables(o);
}

void TypeHierarchyAnalysis::printStats(raw_ostream &o) const {
  m_cha_impl->printStats(o);
}

/** LLVM pass **/
class TypeHierarchyAnalysisPass : public ModulePass {
public:
  static char ID;

  TypeHierarchyAnalysisPass() : ModulePass(ID), m_cha(nullptr) {}

  ~TypeHierarchyAnalysisPass() = default;

  bool runOnModule(Module &M) override {
    m_cha.reset(new TypeHierarchyAnalysis(M));
    m_cha->calculate();
  ...
  const TypeHierarchyAnalysis &getCHA() const { return *m_cha; }

private:
  std::unique_ptr<TypeHierarchyAnalysis> m_cha;
};

char TypeHierarchyAnalysisPass::ID = 0;

llvm::Pass *createTHAPass() { return new TypeHierarchyAnalysisPass(); }

} // end namespace lotus

static llvm::RegisterPass<lotus::TypeHierarchyAnalysisPass>
    X("tha", "Type Hierarchy Analysis", false, false);
