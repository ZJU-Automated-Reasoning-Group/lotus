...
void GraphAlgorithms::dfs(Graph &g, int vid, vector<int> &preorder,
...
void GraphAlgorithms::topological_sort(Graph &g, vector<int> &ts) {
...
// from Klaus Simon's paper "An improved algorithm for transitive closure on
// acyclic digraphs"
void GraphAlgorithms::transitive_closure(Graph g, Graph &tc) {
  vector<int> ts;
...
void GraphAlgorithms::compute_pred(Graph g, vector<set<int>> &predMap) {
  vector<int> ts;
  topological_sort(g, ts);
  ...
  for (rvit = ts.rbegin(); rvit != ts.rend(); rvit++) {
    el = g.in_edges(*rvit);
    for (eit = el.begin(); eit != el.end(); eit++) {
      predMap[*rvit].insert(predMap[*eit].begin(), predMap[*eit].end());
    }
    predMap[*rvit].insert(*rvit);
  }
}

void GraphAlgorithms::findTreeCover(Graph g, Graph &tree, vector<set<int>> &pred) {
  vector<int> ts;
  topological_sort(g, ts);
...
void GraphAlgorithms::findTreeCover(Graph &g, Graph &tree, vector<set<int>> &pred,
  ...
  for (rvit = ts.rbegin(); rvit != ts.rend(); rvit++) {
    ...
    for (eit = el.begin(); eit != el.end(); eit++) {
      ...
      pred[*rvit].insert(pred[*eit].begin(), pred[*eit].end());
    }
    ...
    exit(0); } sum += pred[*rvit].size(); cout << sum << "\t";
    */
  }
  //	cout << '\n';
}
...
void GraphAlgorithms::findTreeCover(Graph g, Graph &tree) {
  vector<int> ts;
  topological_sort(g, ts);
...
// find longest path
void GraphAlgorithms::findTreeCoverL(Graph g, Graph &tree) {
  vector<int> ts;
  topological_sort(g, ts);
...
// given start node
void GraphAlgorithms::tarjan(Graph &g, int vid, int &index,
...
void GraphAlgorithms::mergeSCC(Graph &g, int *on, vector<int> &reverse_topo_sort) {
  vector<int> sn;
  unordered_map<int, pair<int, int>> order;
  int ind = 0;
...
void GraphAlgorithms::topo_leveler(Graph &g) {
...
int GraphAlgorithms::topo_level(Graph &g, int vid) {
...
void GraphAlgorithms::traverse(Graph &tree, int vid, int &pre_post,
                         vector<bool> &visited) {
  visited[vid] = true;
...
void GraphAlgorithms::pre_post_labeling(Graph &tree) {
  ...
  /*
          ...
          for (vit = vl.begin(),i=0; vit != vl.end(); vit++,i++)
                  cout << i << ":\t" << tree[i].pre_order << "," <<
     tree[i].post_order << '\n';
  */
}

// void GraphAlgorithms::treePathDecomposition(Graph tree, Graph& g,
// unordered_map<int, list<int> >& pathMap) {
void GraphAlgorithms::treePathDecomposition(Graph tree, Graph &g,
  ...
  /*
     ...
     { path1 = *mlit; cout << "path [" << i << "]: "; for (lit = path1.begin();
     lit != path1.end(); lit++) cout << *lit << " "; cout << '\n';
          }
  */
}

// path decomposition
void GraphAlgorithms::pathDecomposition(Graph &g, vector<vector<int>> &pathMap,
  ...
  for (rit = reverse_topo_sort.rbegin(), i = 0; rit != reverse_topo_sort.rend();
       rit++) {
    k = *rit;
    if (visited[k])
      continue;
...
// for test
void GraphAlgorithms::genRandomGraph(int n, double c, char *filename) {
...
bool GraphAlgorithms::DFSCheck(Graph &graph, int vid, BitVector *visited, int trg) {
#ifdef DFSDEBUG
...
bool GraphAlgorithms::DFSReach(Graph &graph, int src, int trg) {
#ifdef DFSDEBUG
  cout << "DFS path: " << "(" << src << ", " << trg << "[" << graph[trg].o_vid
       << "])" << '\n';
#endif
  int gsize = graph.num_vertices();
  BitVector *visited = new BitVector(gsize);
...
void GraphAlgorithms::buildGateGraphByNodeFast(Graph &g, BitVector *isgates, int vid,
                                         int radius, map<int, int> &gateindex,
                                         Graph &gategraph, vector<int> &que,
                                         vector<int> &dist, int &ref) {
  int u;
  EdgeList el;
  EdgeList::iterator eit;
...
int GraphAlgorithms::buildGateGraphByNodeFastWrite(Graph &g, BitVector *isgates,
                                             int vid, int radius,
                                             map<int, int> &gateindex,
                                             ostream &out, vector<int> &que,
                                             vector<int> &dist, int &ref) {
  int u;
  EdgeList el;
  EdgeList::iterator eit;
...
int GraphAlgorithms::buildGateGraphWrite(Graph &graph, BitVector *isgate, int radius,
                                   ostream &out) {
  int gsize = graph.num_vertices();
  map<int, int> gateindex;
  int index = 0;
  ...
  for (mit = gateindex.begin(); mit != gateindex.end(); mit++) {
    int esize = buildGateGraphByNodeFastWrite(graph, isgate, mit->first, radius,
                                              gateindex, out, que, dist, ref);
    edgesize += esize;
  }
  return edgesize;
}

bool GraphAlgorithms::DFSCheckCnt(Graph &graph, int vid, vector<int> &visited,
                            int trg, int qcnt) {
  if (vid == trg)
    return true;
  visited[vid] = qcnt;
...
bool GraphAlgorithms::DFSReachCnt(Graph &graph, int src, int trg,
                            vector<int> &visited, int &qcnt) {
  qcnt++;
  return DFSCheckCnt(graph, src, visited, trg, qcnt);
}

void GraphAlgorithms::collectInLocalGates(Graph &g, const BitVector *isgates,
                                    int vid, int radius, vector<int> &ingates) {
  int u;
  EdgeList el;
  EdgeList::iterator eit;
  deque<int> que;
...
void GraphAlgorithms::collectOutLocalGates(Graph &g, const BitVector *isgates,
                                     int vid, int radius,
                                     vector<int> &outgates) {
  int u;
  EdgeList el;
...
bool GraphAlgorithms::BFSOutLocalReach(Graph &g, const BitVector *isgates,
                                 int radius, int src, int trg,
                                 vector<int> &outgates) {
  int u;
  EdgeList el;
  EdgeList::iterator eit;
...
int GraphAlgorithms::visit(Graph &tree, int vid, int &pre_post, vector<bool> &visited,
  ...
  for (eit = el.begin(); eit != el.end(); eit++) {
    if (!visited[*eit]) {
      pre_order =
          min(pre_order, visit(tree, *eit, pre_post, visited, dfslabels));
    } else
...
void GraphAlgorithms::grail_labeling(Graph &tree, vector<pair<int, int>> &dfslabels) {
  vector<int> roots = tree.getRoots();
  vector<int>::iterator sit;
  int pre_post = 0;
  vector<bool> visited(tree.num_vertices(), false);
  dfslabels.clear();
  dfslabels =
      vector<pair<int, int>>(tree.num_vertices(), pair<int, int>(-1, -1));
  std::random_device rd;
  std::mt19937 g(rd());
