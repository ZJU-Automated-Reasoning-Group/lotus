 ...
 *
 * This module provides the Index class, which implements various indexing
 ...
 *
 * Index performance is optimized through a combination of:
 ...
 *
 * Initializes a Index object for performing reachability queries. The
 * constructor:
 ...
 */
Index::Index(const char *filestem, Graph &ig, int _r, double _ps, bool mat)
    : g(ig) {
  initFlags();
  epsilon = _r;
  preselectratio = _ps;
...
// Index::Index(const char* filestem, const char* grafile, int _r) {
//	epsilon = _r;
//	initFlags();
//	string filestr(filestem);
//	filestemstr = filestr;
...
// Index::Index(const char* gatefile, const char* ggfile,
...
int Index::getGateSize() const { return gatesize; }

string Index::getFilestem() const { return filestemstr; }

void Index::setMethodName(string _method_name) { method_name = _method_name; }

string Index::getMethodName() const { return method_name; }

long Index::getIndexSize() const {
  if (method_name == "DFS")
    return 0;
  long size = 0;
  for (int i = 0; i < gsize; i++) {
...
vector<long> Index::indexSize() const {
...
void Index::setRadius(int _r) { epsilon = _r; }

int Index::getGateEdgeSize() {
  return gateedgesize;
  //	return gategraph.num_edges();
}

vector<string> Index::makeggfilename(const char *filestem) {
...
void Index::initGateGraph(const char *ggfilename) {
...
void Index::initGates(const char *gatefile) {
...
void Index::initIndex(const char *indexfile) {
...
// void Index::outIndex(const char* out_index_file){
//	ofstream outer(out_index_file);
//	for
// }

void Index::computeLocalGates(bool ispartial) {
  vector<int> nodes;
  if (ispartial) {
  ...
  for (int i = 0; i < nodes.size(); i++) {
    GraphAlgorithms::collectInLocalGates(g, gates, nodes[i], radius,
                                   localgates[nodes[i]][0]);
    GraphAlgorithms::collectOutLocalGates(g, gates, nodes[i], radius,
                                    localgates[nodes[i]][0]);
  }
}

void Index::computeMultiLabelsQuickRej(int num_labels) {
  mutipleLabeling(num_labels);
}

// default strategy: randomly select #num vertices
void Index::selectPartialNodes(vector<int> &nodes, int num) {
  assert(num <= gsize);
  nodes.clear();
  if (num == gsize) {
    for (int i = 0; i < gsize; i++)
...
void Index::mutipleLabeling(int num_labels) {
  int index = 0;
  vector<pair<int, int>> dfslabels;
  graillabels = vector<vector<pair<int, int>>>(gsize, vector<pair<int, int>>());
  for (int i = 0; i < num_labels; i++) {
    dfslabels.clear();
    //	GraphAlgorithms::randomDFSLabeling(gategraph,dfslabels);
    GraphAlgorithms::grail_labeling(g, dfslabels);
    index = 0;
...
void Index::computeTopoOrder() {
  useTopoOrder = true;
  GraphAlgorithms::topological_sort(g, topoid);
}
 ...
 */
bool Index::reach(int src, int trg) {
  return GraphAlgorithms::DFSReachCnt(g, src, trg, visited, QueryCnt);
  //	return GraphAlgorithms::DFSReachVisitedNum(g,src,trg,visitnum);
}

bool Index::reachWithoutMat(int src, int trg) {
  return GraphAlgorithms::DFSReachCnt(g, src, trg, visited, QueryCnt);
  //	return GraphAlgorithms::DFSReachVisitedNum(g,src,trg,visitnum);
}

bool Index::test_reach(int src, int trg) {
  bool r = reach(src, trg);
  bool ans = GraphAlgorithms::DFSReach(g, src, trg);
  if (r != ans) {
...
bool Index::test_nomreach(int src, int trg) {
 ...
 */
void Index::initMaterialization() {
  inoutgates =
      vector<vector<vector<int>>>(gsize, vector<vector<int>>(2, vector<int>()));
  inneigs = vector<BitVector *>(gsize, NULL);
...
void Index::materialization(int num) {
...
void Index::selectMaterialized(int num) {
  num = min(gsize, num);
...
void Index::materializeInNeighbors(int vid) {
  inneigs[vid] = new BitVector(gsize);
  QueryCnt++;
 ...
 */
void Index::precomputeGates(int vid, bool out) {
  if (gates->get(vid)) {
    return;
  }
  int u, val, index = 0, endindex = 0, nid;
...
void Index::displayInfor(ostream &out) {
  for (int i = 0; i < gsize; i++)
    displayInfor(i, out);
}

void Index::displayInfor(int vid, ostream &out) {
...
void Index::displayIndex(ostream &out) {
  if (indextype == 3)
    return;
...
void Index::displayLabelsByNode(int vid, ostream &out) {
...
void Index::displayLabels(ostream &out) {
...
void Index::displayGates(ostream &out) {
  out << "Gates: ";
  for (int i = 0; i < gsize; i++) {
    if (gates->get(i)) {
      out << i << " ";
...
void Index::displayGrailLabels(ostream &out) {
  if (!useMultiLabels)
    return;
  out << "GRAIL Labels" << '\n';
  for (int i = 0; i < gsize; i++) {
...
void Index::displayLocalGatesByNode(int vid, ostream &out) {
  if (!useLocalGates && !usePartialLocalGates)
    return;
  cout << vid << " ";
  if (gates->get(vid))
...
void Index::displayLocalGates(ostream &out) {
  if (!useLocalGates && !usePartialLocalGates)
    return;
  out << "Local Gates" << '\n';
  for (int i = 0; i < gsize; i++) {
