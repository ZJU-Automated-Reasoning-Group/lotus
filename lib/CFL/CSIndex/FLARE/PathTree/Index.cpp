 ...
 *
 * Initializes a path tree with the given graph and allocates necessary data
 * structures.
 */
Index::Index(Graph &graph) : g(graph) {
 ...
 *
 * Initializes a path tree with the given graph and topological sort
 * information.
 */
Index::Index(Graph &graph, vector<int> ts) : g(graph) {
  grts = ts;
  int maxid = g.num_vertices();
...
void Index::compute_tcm() {
  double tcsize = 0;
  Graph tc(g.num_vertices());
  GraphAlgorithms::transitive_closure(g, tc);
  EdgeList el;
...
void Index::index_size(int *ind_size) {
  int isize = 0;
  int uncover_size = 0;
  //	displayLabels();
  ...
  /*
     ...
     '\n'; cout << "path tree cover set size percentage: " <<
     pathMap.size()*2.00/(isize*1.00) << '\n';
  */
...
void Index::transform(WeightedGraph branch, Graph &graph) {
...
void Index::buildWeightPathGraph_Pred() {
  int gs = g.num_vertices();
  // perform path decomposition
  //	GraphAlgorithms::pathDecomposition(g, pathMap);
  Graph tree(gs);
  for (int i = 0; i < gs; i++)
    tree.addVertex(i);
  ...
  GraphAlgorithms::findTreeCover(g, tree, predMap, grts);
  ...
  GraphAlgorithms::treePathDecomposition(tree, g, pathMap);
...
// build weighted path graph
void Index::buildWeightPathGraph(int type) {
  // perform path decomposition
  if (type == 4)
    GraphAlgorithms::pathDecomposition(g, pathMap, grts);
...
void Index::buildEquEdgeset(map<int, set<int>> &pathtopo, Graph &equgraph) {
...
void Index::pathDFS(int vid, int &order, int &first_order,
                       vector<bool> &visited) {
  visited[vid] = true;
...
void Index::readPathMap(ifstream &cfile) {
  int n;
  string buf;
  getline(cfile, buf);
  istringstream(buf) >> n;
...
void Index::createLabels(int type, ifstream &cfile, bool compress) {
...
#ifdef DEBUG
    pg.printGraph();
    WeightedGraphAlgorithms::checkBranching(pg, branch);
    cout << "***********************************************" << '\n';
    branch.printGraph();
//	exit(0);
#else
    WeightedGraphAlgorithms::findMaxBranching(pg, branch);
#endif
  } else {
#ifdef DEBUG
    WeightedGraphAlgorithms::checkBranching1(pg, branch);
#else
    WeightedGraphAlgorithms::findMaxBranching1(pg, branch);
#endif
  }
  ...
  */
  ...
  gettimeofday(&before_time, NULL);
  GraphAlgorithms::pre_post_labeling(newbranch);
  ...
  */
  ...
  GraphAlgorithms::topological_sort(newbranch, reverse_topo_sort);
  gettimeofday(&before_time, NULL);
  vector<int> path;
  vector<int>::iterator lit;
  ...
  //	GraphAlgorithms::topological_sort(g, reverse_topo_sort);
  EdgeList el;
  EdgeList::iterator eit;
  ...
  if (compress) {
    ...
    int num_cluster =
        (int)(g.num_vertices() * size_ratio > 10 ? g.num_vertices() * size_ratio
                                                 : 10);
    DataCompression dc(out_uncover, grts, num_cluster, g.num_vertices());
    dc.comp_kmeans();
...
void Index::mergeVector(vector<int> &v1, vector<int> &v2) {
  if (v2.empty())
    return;
...
void Index::insertSet(set<int> &s1, set<int> &s2) {
...
double Index::cover_ratio() {
  ...
  //	cout << "# TC is covered by ChainTree(Index) = " << counter << '\n';

  return (counter * 1.0) / (tcsize * 1.0);
}

double Index::compress_ratio() {
  int gsize = g.num_vertices();
...
bool Index::reach_dc(int src, int trg) {
  if (!effective) {
...
bool Index::test_reach_dc(int src, int trg) {
...
void Index::save_labels(ofstream &label_file) {
