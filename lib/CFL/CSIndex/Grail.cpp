 ...
 * labeling.
 */
Grail::Grail(Graph &graph, int Dim, int labelingType, bool pool, int poolsize)
  ...
  for (i = 0; i < POOLSIZE; i++) {
    switch (labelingType) {
    case 0:
      Grail::randomlabeling(graph);
      break;
    case 1:
      Grail::setIndex(graph, i);
...
void Grail::set_level_filter(bool lf) { LEVEL_FILTER = lf; }
...
int Grail::customvisit(Graph &tree, int vid, int &pre_post,
...
// traverse tree to label node with pre and post order by giving a start node
int Grail::fixedreversevisit(Graph &tree, int vid, int &pre_post,
                             vector<bool> &visited, int traversal) {
  //	cout << "entering " << vid << '\n';
  visited[vid] = true;
  EdgeList el = tree.out_edges(vid);
