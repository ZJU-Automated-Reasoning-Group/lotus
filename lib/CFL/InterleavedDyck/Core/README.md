...
The public API in `include/CFL/InterleavedDyck/Core/Graph.h` defines:
...
|---|---|
| `StagedBounds` | Direct typed lower/upper pipeline input |
| `InterleavedDyckUnary` | Unary projection, bidirected validation, and shared exact-solver backends |
...
Bidirecting is an algorithm policy: both InterleavedDyckUnary algorithms can
reject or explicitly symmetrize; StagedBounds and MCFL analyze the directed
graph as supplied.
