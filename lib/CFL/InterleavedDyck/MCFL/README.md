...
`CanaryInterleavedDyckMCFL` is a native C++17 implementation of the algorithms in
...
named **Interleaved-Dyck Staged Bounds** module currently stored under the
directory `CFL/InterleavedDyck/StagedBounds`. For exact bidirected unary
reachability, use the independent `CFL/InterleavedDyck/Unary` module.

## Generic solver

The public API is split across `include/CFL/InterleavedDyck/MCFL/Graph.h`, `Grammar.h`, and
...
`include/CFL/InterleavedDyck/MCFL/InterleavedDyck.h` generates both grammar families for any
positive dimension:
...
- `InterleavedDyckGraphReduction` specializes in graph simplification for two Dyck
...
- `InterleavedDyckUnary` computes the exact answer for the bidirected unary
...
adapts it to the generic string-labeled MCFL graph. This lets StagedBounds and
