...
void vSSA::createSigmasIfNeeded(BasicBlock *BB) {
  ...
  comparison->op_end(); it != e; ++it) { Value *operand = *it; if
  (isa<Instruction>(operand) || isa<Argument>(operand)) { insertSigmas(ti,
  operand);
        }
      }
    }
...
SmallVector<PHINode *, 25> vSSA::insertPhisForSigma(Value *V, PHINode *sigma) {
  ...
  for (std::set<BasicBlock *>::iterator DFit = DF_BB.begin(),
    ...
    // vSSA_PHI is needed
    if (condition) {
      //      // Need to discover from which BasicBlock the sigma value comes
      //      from
      //      // To do that, we need to find which predecessor of BB_infrontier
      //      is dominated by BB_next BasicBlock *predBB = NULL;
      //
      //      for (pred_iterator PI = pred_begin(BB_infrontier), PE =
      //      pred_end(BB_infrontier); PI != PE; ++PI) {
      //        predBB = *PI;
      //
...
bool vSSA::dominateAny(BasicBlock *BB, Value *value) {
  ...
  // do it Otherwise, if the use is dominated by the BasicBlock in-frontier, we
  // create the vSSA_PHI
