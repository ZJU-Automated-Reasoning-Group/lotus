...
// Implement instruction/block analysis concerning control flow, i.e.
// determining a block's status and relatedly analysing terminator instructions.

// Return true if, when LLPE's specialisation assumptions hold, this function is
// certain to be entered.
...
// Return true if, when LLPE's specialisation assumptions hold, this loop
// iteration is certain to be entered.
bool PeelIteration::entryBlockIsCertain() {

  if (iterationCount == 0)
    return parent->getBB(parentPA->L->preheaderIdx)->isMarkedCertain();

  // Otherwise it's certain if we're certain to iterate and at least the
  // previous header was certain.
  PeelIteration *prevIter = parentPA->Iterations[iterationCount - 1];
  return prevIter->getBB(parentPA->L->latchIdx)->isMarkedCertain() &&
         prevIter->allExitEdgesDead();
}

// Return true if the user has instructed we should explore this function
// aggressively, using the same rules as for code that is certainly reachable.
...
// Return true if the user has instructed we should explore this loop iteration
// aggressively, using the same rules as for code that is certainly reachable.
bool PeelIteration::entryBlockAssumed() {

  // A PeelIteration object being created at all indicates at least the
  // assumption that we will run.
  return true;
}

// Create the entry block (function entry point or loop header) and set it
// certain or assumed-reachable if appropriate. Return true if the block was
// created or its status changed.
...
// Set edge from block-instance BB with terminator TI to Target alive. Return
// true if the edge status changed (i.e. it was not already marked alive)
static bool setEdgeAlive(Instruction *TI, ShadowBB *BB, BasicBlock *Target) {
  ...
  for (unsigned I = 0; I != NumSucc; ++I) {
    ...
    if (thisTarget == Target) {
      ...
      // Mark this edge alive. In some cases there may be multiple copies of the
      // edge e.g. for several switch cases with the same destination.
      if (!BB->succsAlive[I])
        changed = true;
...
// Is BB1I -> BB2I known to be infeasible in this context and all child loop
// contexts?
bool IntegrationAttempt::edgeIsDeadRising(ShadowBBInvar &BB1I,
                                          ShadowBBInvar &BB2I,
                                          bool ignoreThisScope) {
  ...
  if (PeelAttempt *LPA =
          getPeelAttempt(immediateChildLoop(L, BB1I.naturalScope))) {
...
bool IntegrationAttempt::blockIsDeadRising(ShadowBBInvar &BBI) {
  ...
  if (PeelAttempt *LPA =
          getPeelAttempt(immediateChildLoop(L, BBI.naturalScope))) {
...
bool IntegrationAttempt::checkBlockOutgoingEdges(ShadowInstruction *SI) {
  ...
  // TOCHECK: I think this only returns false if the block ends with an
  // Unreachable inst?
  ...
  if (inst_is<InvokeInst>(SI)) {
    ...
    // I mark the exceptional edge reachable here if the call is disabled, even
    // though we might have proved it isn't feasible. This could be improved by
    // converting the invoke into a call in the final program.
  ...
  ConstantInt *ConstCondition =
      dyn_cast_or_null<ConstantInt>(getConstReplacement(Condition));
  if (!ConstCondition) {

    if (Condition.t == SHADOWVAL_INST || Condition.t == SHADOWVAL_ARG) {

      // Switch statements can operate on a ptrtoint operand, of which only
      // ptrtoint(null) is useful:
      if (ImprovedValSetSingle *IVS =
              dyn_cast_or_null<ImprovedValSetSingle>(getIVSRef(Condition))) {

        if (IVS->onlyContainsNulls()) {

          ConstCondition = cast<ConstantInt>(
              Constant::getNullValue(SI->invar->I->getOperand(0)->getType()));
        }
      }
    }
  }
  ...
  if (ConstCondition) {
    ...
    if (takenTarget) {
      // We know where the instruction is going -- remove this block
      // as a predecessor for its other targets.
      LPDEBUG("Branch or switch instruction given known target: "
              << takenTarget->getName() << "\n");
  ...
  if ((Switch = dyn_cast_inst<SwitchInst>(SI)) &&
      ...
      IVS->SetType == ValSetTypeScalar && !IVS->Values.empty()) {
    ...
    for (unsigned i = 0, ilim = IVS->Values.size(); i != ilim; ++i) {
      ...
      SwitchInst::CaseIt targetit = Switch->findCaseValue(
          cast<ConstantInt>(getConstReplacement(IVS->Values[i].V)));
...
// Evaluate the block terminator instruction SI to determine feasible
// successors, and juggle store reference counts to give a reference to each
// live successor and drop this block's reference. If the terminator is an
// Invoke the call has already been explored if appropriate. Returns true if
// anything changed.
bool IntegrationAttempt::tryEvaluateTerminator(ShadowInstruction *SI,
                                               bool thisBlockLoadedVararg) {
  ...
  // errs() << "Leaving block " << SI->parent->invar->BB->getParent()->getName()
  // << "/" << SI->parent->invar->BB->getName() << " with store " <<
  // SI->parent->localStore << " refcount " << SI->parent->localStore->refCount
  // << "\n";

  // This block relinquishes its reference. Might free the store in e.g. an
  // unreachable block.
  ...
  for (uint32_t i = 0; i < BBI->succIdxs.size(); ++i) {
    ...
    if (!IA->getBB(BB->invar->succIdxs[i])) {
      ...
      // Can grant the new block some status if either (a) I have status and
      // this is my only live successor, or (b) this edge should be assumed. In
      // practice this is mainly useful conferring ASSUMED status; certainty is
      // detected mainly using checkBlockStatus, below.
    ...
    IA->getBB(BB->invar->succIdxs[i])->useSpecialVarargMerge =
        thisBlockLoadedVararg;
  }
...
// context is entered, this block will definitely be reached). If it does, and
// the entry block is certainly- or assumed-reachable, inherit its status. Note
// this is distinct from LLVM's post-dominator analysis because it takes edges
// proved dead during partial evaluation into account. It also depends on
