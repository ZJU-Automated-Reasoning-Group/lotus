...
// List of functions that should never be expanded, even if bitcode
// implementations appear to be available. Most are syscalls or thin wrappers
// around syscalls, but some are allocators or synchronisation functions which
// we handle specially.
...
// We always do if this path is a certain consequence of our specialisation
// assumptions or the user explicitly directs us to; we never do if this would
// be a (transitive) recurison on a path that only may be reached due to the
// possibility of infinite regress.

bool IntegrationAttempt::shouldInlineFunction(ShadowInstruction *SI,
                                              Function *FCalled) {
...
// Check all the possible reasons why call instruction SI shouldn't make a
// specialisation context.

bool IntegrationAttempt::callCanExpand(ShadowInstruction *SI,
  ...
  if (!FCalled) {
    LPDEBUG("Ignored " << itcache(SI)
                       << " because it's an uncertain indirect call\n");
    return false;
  }

  if (FCalled->isDeclaration()) {
    LPDEBUG("Ignored " << itcache(SI)
                       << " because we don't know the function body\n");
    return false;
  }

  if (!shouldInlineFunction(SI, FCalled)) {
    LPDEBUG("Ignored " << itcache(SI)
                       << " because it shouldn't be inlined (not on certain "
                          "path, and would cause recursion)\n");
    return false;
  }
  ...
  if (functionIsBlacklisted(FCalled)) {
    LPDEBUG(
        "Ignored "
        << itcache(SI)
        << " because it is a special function we are not allowed to inline\n");
    return false;
  }
...
// Get an existing specialisation context representing the call made by SI, or
// try to create one. 'created' is set if the context is new; 'needsAnalyse' is
// set to false if we have reused a context that already existed and had the
// same preconditions as this call-site (only possible when function sharing is
// on)

InlineAttempt *IntegrationAttempt::getOrCreateInlineAttempt(
    ShadowInstruction *SI, bool &created, bool &needsAnalyse) {
  ...
  if (InlineAttempt *Share = pass->findIAMatching(SI)) {
    ...
    if (pass->verboseSharing)
      errs() << "SHARE: " << itcache(SI) << " #" << Share->SeqNumber
             << " (refs: " << Share->Callers.size() << ")\n";
  ...
  if (Result) {
    if (!Result->isShared())
      ...
      if (pass->verboseSharing)
        errs() << "BREAK: " << itcache(SI) << " #" << Result->SeqNumber
               << " -> #" << Unshared->SeqNumber << "\n";
  ...
  Function *FCalled = getCalledFunction(SI);
  SmallDenseMap<Function *, Function *>::iterator it =
      pass->modelFunctions.find(FCalled);
  bool isModel = it != pass->modelFunctions.end();
  if (isModel)
    FCalled = it->second;

  InlineAttempt *IA =
      new InlineAttempt(pass, *FCalled, SI, this->nesting_depth + 1);
  IA->isModel = isModel;
...
void PeelIteration::checkFinalIteration() {
  ...
  if (edgeIsDead(LatchBB, HeaderBB) ||
      pass->assumeEndsAfter(&F, getBBInvar(L->headerIdx)->BB, iterationCount)) {
...
PeelIteration *PeelAttempt::getOrCreateIteration(unsigned iter) {
  ...
  PeelIteration *NewIter =
      new PeelIteration(pass, parent, this, F, iter, nesting_depth);
...
bool PeelIteration::allExitEdgesDead() {
  ...
  for (std::vector<std::pair<uint32_t, uint32_t>>::const_iterator
           EI = parentPA->L->exitEdges.begin(),
           EE = parentPA->L->exitEdges.end();
       EI != EE; ++EI) {
...
PeelIteration *PeelIteration::getOrCreateNextIteration() {
  ...
  // Usually this is case due to optimistic edges and such, but could also
  // result from executing unreachable within the loop.
  if (willIterate) {
    ShadowBBInvar *latchBB = getBBInvar(parentPA->L->latchIdx);
    ShadowBBInvar *headerBB = getBBInvar(parentPA->L->headerIdx);
    if (edgeIsDead(latchBB, headerBB))
  ...
  if (!willIterate) {
    ...
    LPDEBUG("Won't peel loop "
            << getLName()
            << " yet because at least one exit edge is still alive\n");
    return 0;
  }
...
// Find an existing specialisation context for loop L (an immediate child of
// this context).
PeelAttempt *IntegrationAttempt::getPeelAttempt(const ShadowLoopInvar *L) {

  DenseMap<const ShadowLoopInvar *, PeelAttempt *>::const_iterator it =
      peelChildren.find(L);
...
// Find an existing loop specialisation, or else check if it's appropriate to
// specialise this loop per-iteration as opposed to analysing the general case
// of the loop body.
PeelAttempt *
IntegrationAttempt::getOrCreatePeelAttempt(const ShadowLoopInvar *NewL) {
  ...
  if (!preheaderBB->isMarkedCertainOrAssumed()) {
    ...
    LPDEBUG("Will not expand loop "
            << getBBInvar(NewL->headerIdx)->BB->getName()
            << " because the preheader is not certain/assumed to execute\n");
    return 0;
  }

  LPDEBUG("Inlining loop with header "
          << getBBInvar(NewL->headerIdx)->BB->getName() << "\n");
...
// Functions that drop store references that were retained so that loop fixed
// points could be found. Exiting edges keep references in case this is the last
// iteration; the latch edge keeps a reference in case we have to iterate again;
// when it becomes clear which is the case one or other class of backup
// reference must be dropped.
...
void PeelIteration::dropExitingStoreRef(uint32_t fromIdx, uint32_t toIdx) {
  ...
  if (BB && (!edgeIsDead(BB->invar, toBBI)) &&
      !edgeBranchesToUnspecialisedCode(BB->invar, toBBI)) {

    if (BB->invar->naturalScope != L) {

      const ShadowLoopInvar *ChildL =
      ...
      if (PeelAttempt *ChildPA = getPeelAttempt(ChildL)) {
        ...
        if (ChildPA->isTerminated()) {
          ...
          for (std::vector<PeelIteration *>::iterator
                   it = ChildPA->Iterations.begin(),
                   itend = ChildPA->Iterations.end();
               it != itend; ++it) {
...
void PeelIteration::dropExitingStoreRefs() {
  ...
  for (std::vector<std::pair<uint32_t, uint32_t>>::const_iterator
           it = LInfo->exitEdges.begin(),
           itend = LInfo->exitEdges.end();
       it != itend; ++it) {
...
void PeelIteration::dropLatchStoreRef() {
  ...
  // If the last latch block was holding a store ref for the next iteration,
  // drop it.
