...
// Functions relating to dead instruction elimination. This used to be a
// seperate phase, but is now run for each context as soon as possible after PE
// has completed. It should detect as many instructions as possible that aren't
// going to have users in the specialised program and save us synthesising them
// (motivation: creating then deleting an instruction in LLVM is quite
// expensive, and can 'leak' memory due to uniqued constants)
...
// This visitor will have its visit(...) method called for every user of a
// particular value V that we're trying to show is dead. It sets maybeLive
// whenever we conclude V might be necessary in the final program.
...
public:
  ...
  virtual void visit(ShadowInstruction *UserI, IntegrationAttempt *UserIA,
    ...
    // The instruction must remain around if it might be used by failed code
    // paths. Ideally we would only keep things whose live ranges extend across
    // tests that branch to failed code, but here's a cheap approximation: If
    // the user block can be reached on failed paths, and the user is not
    // certainly committed to the same block as the original, then such a live
    // range may exist and we'll keep the value.

    if (UserInA->blocksReachableOnFailure &&
        UserInA->blocksReachableOnFailure->count(UserI->parent->invar->idx)) {

      if ((!V.isInst()) || UserI->parent != V.u.I->parent ||
    ...
    if ((inst_is<CallInst>(UserI) || inst_is<InvokeInst>(UserI)) &&
        !inst_is<MemIntrinsic>(UserI)) {

      if (UserI->parent->IA->isResolvedVFSCall(UserI)) {

        if (V == UserI->getCallArgOperand(0) &&
            !UserI->parent->IA->VFSCallWillUseFD(UserI))
          return;
      ...
      if ((!IA) || (!IA->isEnabled()) || IA->commitsOutOfLine()) {
        LLVM_DEBUG(
            dbgs()
            << "Must assume instruction alive due to use in unexpanded call "
            << itcache(UserI) << "\n");
        maybeLive = true;
        return;
      }
      ...
      if (V == UserI->getOperandFromEnd(1)) {
        ...
        if (CalledF) {
          ...
          const CallBase *CB = cast<CallBase>(UserI->invar->I);
          ...
          for (unsigned i = 0; i < CB->arg_size(); ++i) {
            ...
            if (UserI->getCallArgOperand(i) == V) {
              ...
              } else if (!IA->willBeReplacedOrDeleted(
                             ShadowValue(&(IA->argShadows[i])))) {
    ...
    } else if (inst_is<MemTransferInst>(UserI) &&
               UserI->parent->IA->canSynthMTI(UserI)) {
  ...
  virtual void notifyUsersMissed() { maybeLive = true; }

  virtual bool shouldContinue() { return !maybeLive; }
};

// Does V allocate memory, either on the stack or by calling a known allocation
// function?
...
// Eliminate some easy cases of instructions that might appear dead, but need to
// be kept for their side-effects. Some, such as branches, might get eliminated
// through other means, but shouldn't go away just because they aren't directly
// used.
...
// Implement logic passing the visitor around. Might genericise this if more
// situations emerge requiring us to visit users across contexts.

// PeelIteration overrides this; if not overridden this is a function instance
// and there is no next iteration to worry about.
bool IntegrationAttempt::visitNextIterationPHI(ShadowInstructionInvar *I,
                                               DIVisitor &Visitor) {

  return false;
}

// See if the next iteration's header phis use I. We'll assume they need it if
// we didn't establish a unique iteration count for this loop, or if there are
// branches out of specialised code, e.g. due to guard check failures, leading
// to an unspecialised copy of the loop.
bool PeelIteration::visitNextIterationPHI(ShadowInstructionInvar *I,
                                          DIVisitor &Visitor) {
  ...
  if (inst_is<PHINode>(I)) {
    ...
    if (I->parent->idx == L->headerIdx) {
      ...
      if (PeelIteration *PI = getNextIteration()) {
        ...
        // but there is no such iteration, so it is unused unless there may be
        // an out-of-loop user.
...
// VI is an in-loop instruction that uses Visitor.V, a loop invariant. Check
// whether any iteration will require the value. Assume it is used if we're
// synthesising an unspecialised copy of the loop for any reason.
void PeelIteration::visitVariant(ShadowInstructionInvar *VI,
                                 DIVisitor &Visitor) {

  const ShadowLoopInvar *immediateChild =
      immediateChildLoop(L, VI->parent->outerScope);
...
// Visit each iteration of this loop, looking for VI using Visitor.V. This might
// involve exploring nested loops recursively.
void PeelAttempt::visitVariant(ShadowInstructionInvar *VI, DIVisitor &Visitor) {

  if (!isTerminated())
    Visitor.notifyUsersMissed();

  // Is this a header PHI? If so, this definition-from-outside can only matter
  // for the preheader edge.
  if (VI->parent->naturalScope == L && VI->parent->idx == L->headerIdx &&
      isa<PHINode>(VI->I)) {

    Visitor.visit(Iterations[0]->getInst(VI), Iterations[0], VI->parent->idx,
                  VI->idx);
    return;
  }

  for (std::vector<PeelIteration *>::iterator it = Iterations.begin(),
                                              itend = Iterations.end();
       it != itend; ++it) {
...
// Visitor.V is defined in-loop, used out-of-loop by UserI (but we require LCSSA
// form, so certainly an exit phi node). Check if UserI is alive.
void InlineAttempt::visitExitPHI(ShadowInstructionInvar *UserI,
                                 DIVisitor &Visitor) {

  release_assert(UserI->parent->naturalScope == 0 &&
                 "Reached bottom visiting exit PHI");
  Visitor.visit(getInst(UserI), this, UserI->parent->idx, UserI->idx);
}

// Walk up the tree of loops to find Visitor.V's user UserI. Note if we discover
// any loop without a known iteration count, the loops will be emitted unchanged
// and Visitor.V should be marked alive.
void PeelIteration::visitExitPHI(ShadowInstructionInvar *UserI,
                                 DIVisitor &Visitor) {
...
void IntegrationAttempt::visitUser(ShadowInstIdx &User, DIVisitor &Visitor) {
  ...
  // Figure out what context cares about this value. The only possibilities are:
  // this loop iteration, the next iteration of this loop (latch edge of header
  // phi), a child loop (defer to it to decide what to do), or a parent loop
  // (again defer). Note that nested cases (e.g. this is an invariant two
  // children deep) are taken care of in the immediate child or parent's logic.

  if (User.blockIdx == INVALID_BLOCK_IDX ||
      User.instIdx == INVALID_INSTRUCTION_IDX)
    return;
  ...
  if (UserL == L) {
    ...
    if (!visitNextIterationPHI(SII, Visitor)) {
      ...
      Visitor.visit(getInst(User.blockIdx, User.instIdx), this, User.blockIdx,
                    User.instIdx);
    }
    ...
    if ((!L) || L->contains(UserL)) {
      ...
      // Used in a child loop. Check if that child exists at all and defer to
      // it.
      ...
      if (LPA && LPA->isEnabled())
        ...
        Visitor.visit(
            0, this, User.blockIdx,
            User.instIdx); // Loop not explored, but a failed version may exist
      else
        Visitor.notifyUsersMissed();
...
// Visit each user for V (== Visitor.V), looking for live users that will
// require us to synthesise V.
...
// Returns true if V is *already* tagged dead, and/or will be replaced in the
// specialised program
...
// Returns true if V is *already* tagged dead, and/or will be replaced with a
// constant in the specialised program
...
bool IntegrationAttempt::valueIsDead(ShadowValue V) {
  ...
  if (val_is<ReturnInst>(V)) {
    ...
    DenseMap<ShadowValue,
             std::vector<std::pair<ShadowValue, uint32_t>>>::iterator findit =
        GlobalIHP->indirectDIEUsers.find(V);
    if (findit != GlobalIHP->indirectDIEUsers.end()) {

      // TODO: come up with a more memory-efficient way to determine when an
      // allocation, FD or similar can be removed.
      ...
      AllocData* AD =
      getAllocData(cast<ImprovedValSetSingle>(I->i.PB)->Values[0].V);
      ...
      FDGlobalState& GFDS =
      GlobalIHP->fds[cast<ImprovedValSetSingle>(I->i.PB)->Values[0].V.getFd()];
      ...
      release_assert(0 && "Not an allocation, not an FD, but has indirect
      users?");
      ...
      // If the IA has been deallocated then the user must have been committed
      already
...
void IntegrationAttempt::runDIE() {
  ...
  for (uint32_t i = nBBs; i > 0; --i) {
    ...
    if (BB->invar->naturalScope != L) {
      ...
      const ShadowLoopInvar *EnterL =
          immediateChildLoop(L, BB->invar->naturalScope);
      ...
      while (i > 0 && ((!BBs[i - 1]) ||
                       EnterL->contains(BBs[i - 1]->invar->naturalScope)))
        --i;
      ++i;
      continue;
    }
...
// Called only for unbounded loops, which cannot contain expanded subloops, so
// no need to check.
void IntegrationAttempt::gatherIndirectUsersInLoop(const ShadowLoopInvar *L) {
  ...
  for (uint32_t bblim = BBsOffset + nBBs;
       bbi != bblim && ((!L) || L->contains(getBBInvar(bbi)->naturalScope));
       ++bbi) {
