...
// Functions to describe the hierarchy of peel and inline attempts in
// DOT format for easy review.  Used to provide the UI's basic block
// graph view.
...
// Helper: shorten str to include at most maxlen characters from the original
// string, plus some suffix.
...
// Replace special characters with HTML escapes, and double backslashes (DOT
// recognises both forms of escape sequence)
...
// Print any derived result concerning SV (e.g. a constant, or an allocation
// identifier, or a VFS specialisation report. Print nothing if we don't have a
// concrete result.
void IntegrationAttempt::printRHS(ShadowValue SV, raw_ostream &Out) {
  ...
  */
  ...
  DenseMap<ShadowInstruction *, std::string>::iterator optit =
      pass->optimisticForwardStatus.find(SI);
  if (!PBPrinted) {
  ...
  if (inst_is<CallInst>(SI)) {
    DenseMap<ShadowInstruction *, OpenStatus *>::iterator it =
        pass->forwardableOpenCalls.find(SI);
    if (it != pass->forwardableOpenCalls.end()) {
      Out << it->second->Name << "("
          << (it->second->success ? "success" : "not found") << ")";
    } else {
      DenseMap<ShadowInstruction *, ReadFile>::iterator it =
          pass->resolvedReadCalls.find(SI);
      if (it != pass->resolvedReadCalls.end())
        Out << it->second.name << " (" << it->second.incomingOffset << "-"
            << it->second.incomingOffset + (it->second.readSize - 1) << ")";
    }
  }
}

// When we're viewing a loop iteration, we draw target blocks outside this
// iteration as an opaque label, rather than drawing all the instructions as
// usual. Get such a special label if applicable. No edges leave this content
// for a function...
bool InlineAttempt::getSpecialEdgeDescription(ShadowBBInvar *FromBB,
                                              ShadowBBInvar *ToBB,
                                              raw_ostream &Out) {
...
bool PeelIteration::getSpecialEdgeDescription(ShadowBBInvar *FromBB,
                                              ShadowBBInvar *ToBB,
                                              raw_ostream &Out) {
  ...
  if (FromBB->idx == L->latchIdx && ToBB->idx == L->headerIdx) {
    ...
    Out << "\"Exit block " << escapeHTML(ToBB->BB->getName().str()) << "\"";
    return true;
  }
...
// Print an edge from BB -> SB. In brief mode, unreachable blocks / edges and
// branches to unspecialised code are omitted.
void IntegrationAttempt::printOutgoingEdge(
    ShadowBBInvar *BBI, ShadowBB *BB, ShadowBBInvar *SBI, ShadowBB *SB,
    uint32_t i, bool useLabels, const ShadowLoopInvar *deferEdgesOutside,
    SmallVector<std::string, 4> *deferredEdges, raw_ostream &Out, bool brief) {
...
// Print path conditions, appearing instruction-like at the top of the relevant
// block.
void llvm::printPathCondition(PathCondition &PC, PathConditionTypes t,
                              ShadowBB *BB, raw_ostream &Out,
                              bool HTMLEscaped) {
  ...
  if (!PC.instBB) {
    ...
    IntegrationAttempt *ArgIA =
        getIAWithTargetStackDepth(BB->IA->getFunctionRoot(), PC.instStackIdx);
...
static void printPathConditionList(std::vector<PathCondition> &conds,
                                   PathConditionTypes t, raw_ostream &Out,
                                   ShadowBBInvar *BBI, ShadowBB *BB) {
...
static void printPathConditionsFrom(PathConditions &PC, raw_ostream &Out,
                                    ShadowBBInvar *BBI, ShadowBB *BB) {

  // Mention if there are symbolic path conditions or functions here:
  printPathConditionList(PC.IntPathConditions, PathConditionTypeInt, Out, BBI,
                         BB);
  printPathConditionList(PC.AsDefIntPathConditions, PathConditionTypeInt, Out,
                         BBI, BB);
  printPathConditionList(PC.IntmemPathConditions, PathConditionTypeIntmem, Out,
                         BBI, BB);
  printPathConditionList(PC.StringPathConditions, PathConditionTypeString, Out,
                         BBI, BB);
...
void IntegrationAttempt::printPathConditions(raw_ostream &Out,
                                             ShadowBBInvar *BBI, ShadowBB *BB) {
...
void InlineAttempt::printPathConditions(raw_ostream &Out, ShadowBBInvar *BBI,
                                        ShadowBB *BB) {
...
// Helpers for below:
BasicBlock *InlineAttempt::getEntryBlock() { return &F.getEntryBlock(); }

BasicBlock *PeelIteration::getEntryBlock() {
  return getBBInvar(L->headerIdx)->BB;
}

// Draw BB (in brief mode dead blocks are omitted). Yellow highlights indicate
// blocks that will be reached for sure if the specialisation is entered and no
// implicit checks (e.g. for thread interference) fail; green means the same and
// additionally the block is not in any loop context; orange means that explicit
// checks (e.g. user-specified specialisation conditions) must pass too. Light
// blue means that Clang's vararg code has been special-cased here (a temporary
// measure until the vararg intrinsic work properly).
void IntegrationAttempt::describeBlockAsDOT(
    ShadowBBInvar *BBI, ShadowBB *BB, const ShadowLoopInvar *deferEdgesOutside,
    SmallVector<std::string, 4> *deferredEdges, raw_ostream &Out,
    SmallVector<ShadowBBInvar *, 4> *forceSuccessors, bool brief, bool plain) {
  ...
  Out << "Node" << BBI->BB
      << " [shape=plaintext,fontsize=10,label=<<table cellspacing=\"0\" "
         "border=\"0\"><tr><td colspan=\""
      << numSuccessors << "\" border=\"1\"><table border=\"0\">\n";
  ...
  if (BBI->BB == getEntryBlock())
    Out << "Entry block: ";
  Out << escapeHTML(BBI->BB->getName().str()) << " </font></td></tr>\n";
  ...
  for (std::vector<ShadowValue>::iterator VI = Vals.begin(), VE = Vals.end();
       VI != VE; ++VI) {

    std::string textColour;
    Out << "<tr><td border=\"0\" align=\"left\" bgcolor=\""
        << getValueColour(*VI, textColour, plain) << "\">";
    if (!textColour.empty())
  ...
  if (useLabels) {
    ...
    Out << "<tr>\n";
    unsigned i = 0;
    for (const BasicBlock *SI : successors(BBI->BB)) {
      Out << "<td port=\"s" << i << "\" border=\"1\">"
          << DOTGraphTraits<const Function *>::getEdgeSourceLabel(BBI->BB, SI)
          << "</td>\n";
      ++i;
    }
    Out << "</tr>\n";
  ...
  if (forceSuccessors) {
    ...
    for (SmallVector<ShadowBBInvar *, 4>::iterator
             it = forceSuccessors->begin(),
             it2 = forceSuccessors->end();
         it != it2; ++it) {
      ...
      printOutgoingEdge(BBI, BB, SuccBBI, SuccBB, 0, false, deferEdgesOutside,
                        deferredEdges, Out, brief);
    }

  } else {

    // Print the successor edges *except* any loop exit edges, since those must
    // occur in parent context.
    for (uint32_t i = 0; i < BBI->succIdxs.size(); ++i) {
      ...
      printOutgoingEdge(BBI, BB, SuccBBI, SuccBB, i, useLabels,
                        deferEdgesOutside, deferredEdges, Out, brief);
    }
  }
}
...
// In normal mode, draw its blocks as per usual but with a grouping frame
// indicating loop structure.
void IntegrationAttempt::describeLoopAsDOT(const ShadowLoopInvar *DescribeL,
                                           uint32_t headerIdx, raw_ostream &Out,
                                           bool brief) {
  ...
  Out << "subgraph \"cluster_"
      << DOT::EscapeString(HeaderBBI->BB->getName().str()) << "\" {";

  bool loopIsIgnored =
      pass->shouldIgnoreLoop(&F, getBBInvar(DescribeL->headerIdx)->BB);
  bool loopExplored = false;
  bool loopTerminated = false;

  if (!loopIsIgnored) {

    DenseMap<const ShadowLoopInvar *, PeelAttempt *>::iterator InlIt =
        peelChildren.find(DescribeL);
  ...
  if (loopIsIgnored) {
    ...
    describeBlockAsDOT(getBBInvar(headerIdx + BBsOffset),
                       getBB(headerIdx + BBsOffset), 0, 0, Out,
                       &liveExitingBlocks, brief, loopTerminated);

    const std::vector<std::pair<uint32_t, uint32_t>> &exitEdges =
        DescribeL->exitEdges;

    for (SmallVector<ShadowBBInvar *, 4>::iterator
             it = liveExitingBlocks.begin(),
             it2 = liveExitingBlocks.end();
         it != it2; ++it) {
      ...
      describeBlockAsDOT(BBI, getBB(*BBI), DescribeL, &deferredEdges, Out,
                         &Targets, brief, loopTerminated);
    }
    ...
    for (idx = headerIdx, BBInvar = getBBInvar(headerIdx + BBsOffset);
         DescribeL->contains(BBInvar->naturalScope);
         ++idx, BBInvar = getBBInvar(idx + BBsOffset)) {
  ...
  Out << "label = \"Loop " << DOT::EscapeString(HeaderBBI->BB->getName().str())
      << " (";
  ...
  for (SmallVector<std::string, 4>::iterator it = deferredEdges.begin(),
                                             it2 = deferredEdges.end();
       it != it2; ++it) {
...
// Draw the whole scope DescribeL, which starts at block #headerIdx, and may be
// null to indicate the outermost (function) scope (in which case headerIdx ==
// 0).
void IntegrationAttempt::describeScopeAsDOT(
    const ShadowLoopInvar *DescribeL, uint32_t headerIdx, raw_ostream &Out,
    bool brief, SmallVector<std::string, 4> *deferredEdges) {
  ...
  for (i = headerIdx, BBI = getBBInvar(headerIdx + BBsOffset);
    ...
    describeBlockAsDOT(BBI, BB, deferredEdges ? DescribeL : 0, deferredEdges,
                       Out, 0, brief);
  }
}

// If we'll need DOT files for the user interface they are saved as
// specialisation proceeds.
...
void IntegrationAttempt::saveDOT2(bool brief) {
  ...
  raw_fd_ostream RFO(filename.c_str(), error, sys::fs::OF_None);

  if (error) {

    errs() << "Failed to open " << filename << ": " << error.message() << " at "
           << __FILE__ << "::" << __LINE__ << "\n";
    return;
  }
...
void IntegrationAttempt::describeAsDOT(raw_ostream &Out, std::string &othername,
                                       bool brief) {
...
void IntegrationAttempt::describeTreeAsDOT(std::string path) {
  ...
  std::error_code error;
  raw_fd_ostream os(graphPath.c_str(), error, sys::fs::OF_None);
