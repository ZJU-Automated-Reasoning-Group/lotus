...
// Helpers for the driver pass to check whether it's appropriate to show a
// disable-this-context checkbox:

bool InlineAttempt::canDisable() { return Callers.size() == 1; }

bool PeelIteration::canDisable() { return false; }

// Implement data export functions, for the GUI to get a description of the
// context tree:

bool IntegrationAttempt::hasChildren() {

  return (child_calls_begin(this) != child_calls_end(this)) ||
         peelChildren.size();
}

bool PeelAttempt::hasChildren() { return Iterations.size() != 0; }
...
void InlineAttempt::addExtraTagsFrom(PathConditions &PC, IntegratorTag *myTag) {
  ...
  for (std::vector<PathFunc>::iterator it = PC.FuncPathConditions.begin(),
    ...
    if (it->stackIdx == UINT_MAX ||
        it->stackIdx == targetCallInfo->targetStackDepth) {
...
static uint32_t getTagBlockIdx(const IntegratorTag *t,
...
IntegratorTag *llvm::searchFunctions(IntegratorTag *thisTag,
                                     std::string &search,
