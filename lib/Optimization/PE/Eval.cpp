...
// Evaluate a merge instruction (phi or select). These produce set-typed values
// if more than one outcome is possible, and mark symbolic pointers and file
// descriptors as 'escaped' if this leads to their being members of an
// overdefined ("could be anything") set.
bool IntegrationAttempt::tryEvaluateMerge(ShadowInstruction *I,
  ...
  if (NewPB && (NewIVS = dyn_cast<ImprovedValSetSingle>(NewPB)) &&
      NewIVS->isWhollyUnknown()) {

    for (SmallVector<ShadowValue, 1>::iterator it = Vals.begin(),
                                               itend = Vals.end();
         it != itend; ++it)
...
// Merge a vector of values into an ImprovedValSet, if possible. Any
// fundamentally incompatible values will set NewPB overdefined.
bool IntegrationAttempt::getMergeValue(SmallVector<ShadowValue, 4> &Vals,
                                       ImprovedValSet *&NewPB) {

  bool anyInfo = false;
  bool verbose = false;

  // For now, only support straight copying of multis; otherwise just return
  // overdef.

  bool anyMultis = false;
  for (SmallVector<ShadowValue, 4>::iterator it = Vals.begin(),
                                             it2 = Vals.end();
       it != it2; ++it) {
  ...
  if (anyMultis) {
    ...
    if (Vals.size() > 1)
      ...
      NewPB = new ImprovedValSetMulti(
          *cast<ImprovedValSetMulti>(getIVSRef(Vals[0])));
    return true;
...
// as appropriate.
ShadowValue
PeelIteration::getLoopHeaderForwardedOperand(ShadowInstruction *SI) {

  PHINode *PN = cast_inst<PHINode>(SI);

  // Careful here: this function is used during commit when loop structure can
  // be temporarily disrupted by cloning blocks (e.g. one might branch to the
  // header pending remapping, knocking out the preheader).
...
bool IntegrationAttempt::tryEvaluateHeaderPHI(ShadowInstruction *SI,
                                              bool &resultValid,
...
bool PeelIteration::tryEvaluateHeaderPHI(ShadowInstruction *SI,
                                         bool &resultValid,
                                         ImprovedValSet *&result) {
...
// Fetch a list of live exit edges ExitingBB -> ExitedBB, and the corresponding
// operand to exit phi SI, operand valOpIdx. Note that the edge might exit more
// than one loop. LLPE requires LCSSA-form input, so we know SI is certainly a
// phi instruction.
void IntegrationAttempt::getOperandRising(ShadowInstruction *SI,
                                          uint32_t valOpIdx,
                                          ShadowBBInvar *ExitingBB,
                                          ShadowBBInvar *ExitedBB,
  ...
  if (ExitingBB->naturalScope != L) {
    ...
    if (PeelAttempt *PA =
            getPeelAttempt(immediateChildLoop(L, ExitingBB->naturalScope))) {
  ...
  ShadowValue NewOp;
  if (valOp.instIdx != INVALID_INSTRUCTION_IDX &&
      valOp.blockIdx != INVALID_BLOCK_IDX)
...
// Fetch the incoming values for phi node SI, operand valOpIdx, which might be a
// loop exit phi. If BBs is supplied, note the predecessor ShadowBBs for each
// operand too.
void IntegrationAttempt::getExitPHIOperands(ShadowInstruction *SI,
                                            uint32_t valOpIdx,
                                            SmallVector<ShadowValue, 1> &ops,
                                            SmallVector<ShadowBB *, 1> *BBs) {
...
// Evaluate comparing an assumed-successful file descriptor (i.e. a non-negative
// value) against constant int CmpVal. For example, this can take care of the
// typical if(fd != -1) or if(fd < 0) test. As and when LLPE gains support for
// generic range values, this should be folded into that feature.
static ShadowValue getOpenCmpResult(CmpInst *CmpI, int64_t CmpVal, bool flip) {
  ...
  case CmpInst::ICMP_SLE:
    ...
    break;
  default:
    LLVM_DEBUG(dbgs() << "Failed to fold " << itcache(*CmpI)
                      << " because it compares a symbolic FD using an "
...
bool IntegrationAttempt::tryFoldOpenCmp(ShadowInstruction *SI,
  ...
  if (CmpIntValid) {
    ...
    if (!Improved.V.isInval()) {
      LPDEBUG("Comparison against file descriptor resolves to "
              << itcache(Improved.V) << "\n");
    ...
    ImpType =
        CmpIntType == ValSetTypeUnknown ? ValSetTypeUnknown : ValSetTypeOverdef;
  }
...
// Try to resolve a compare involving non-constant operands. In future this
// could diagnose inputs with disjoint ranges; for now it just takes care of
// tests against minimum and maximum values for the comparison's input type.
bool IntegrationAttempt::tryFoldNonConstCmp(
    ShadowInstruction *SI, std::pair<ValSetType, ImprovedVal> *Ops,
  ...
  if ((Op0CIValid = tryGetConstantInt(Ops[0].second.V, Op0CI)))
    Op0AP = APInt(
        cast<IntegerType>(Ops[0].second.V.getNonPointerType())->getBitWidth(),
        Op0CI);
  if ((Op1CIValid = tryGetConstantInt(Ops[1].second.V, Op1CI)))
    Op1AP = APInt(
        cast<IntegerType>(Ops[1].second.V.getNonPointerType())->getBitWidth(),
        Op1CI);

  // Only handle constant vs. nonconstant here; 2 constants is handled
  // elsewhere.
...
// Helper: do we know op points to a particular symbolic object or is a
// constant? Similar to AliasAnalysis' isIdentifiedObject test.
  ...
  if (op.isVal()) {
    ...
    if (ConstantExpr *CE = dyn_cast_val<ConstantExpr>(op))
      return CE->getOpcode() == Instruction::IntToPtr &&
             isa<Constant>(CE->getOperand(0));
  }

  return false;
}

// Helper: can we find a null-test for the given allocation instruction? We will
// try to keep only one null test along any particular path. Ideally one day we
// will support generic driven conditionals (i.e. noting that b is true or false
// in the two branches of an if(b) ... else ... block), but for now the
// well-known if(!(x = malloc(...))) pattern is special-cased.
class FindTestWalker : public ForwardIAWalker {
  ...
  virtual WalkInstructionResult walkInstruction(ShadowInstruction *ThisI,
                                                void *Context) {
  ...
  virtual bool blockedByUnexpandedCall(ShadowInstruction *CI, void *C) {
    Result = AllocEscaped;
    return true;
  }
...
public:
  ...
  FindTestWalker(ShadowInstruction *_StartI, ShadowInstruction *_TestI)
      : ForwardIAWalker(_StartI->invar->idx, _StartI->parent, true),
        AllocI(_StartI), TestI(_TestI) {}
};

// Check whether we can assume allocation instruction V is non-null because it
// has already been null-tested. This should be generalised to drive test
// results into dominated blocks.
static bool heapPointerAlreadyTested(ShadowValue &V, ShadowInstruction *TestI) {
  ...
  if (AD.isCommitted) {
    ...
    errs() << "Unable to perform all-paths search for " << itcache(V)
           << ": allocating context already committed\n";
    return false;
  }

  // Determine if this test dominates all other tests. Instructions are visited
  // in topological order, so this must be the first test. Walk forwards
  // starting at the allocation, and determine whether we reach TestI or a
  // control flow split first.
...
// Return value: true for "we've handled it" and false for "try ordinary
// constant folding". Here we try to resolve comparisons of two pointers, or a
// pointer against a constant. This can take care of null tests, arbitrary
// comparison of known-offset pointers with a common base, and equality of
// pointers across different allocations.
bool IntegrationAttempt::tryFoldPointerCmp(
    ShadowInstruction *SI, std::pair<ValSetType, ImprovedVal> *Ops,
    ValSetType &ImpType, ImprovedVal &Improved,
  ...
  if ((Ops[0].first != ValSetTypeScalar && Ops[0].first != ValSetTypePB) ||
      (Ops[1].first != ValSetTypeScalar && Ops[1].first != ValSetTypePB))
    return false;
  ...
  // Don't check the types here because we need to accept cases like comparing a
  // ptrtoint'd pointer against an integer null. The code for case 1 works for
  // these; all other cases require that both values resolved to pointers.

  Type *I64 = Type::getInt64Ty(CmpI->getContext());
  Constant *zero = ConstantInt::get(I64, 0);
  Constant *one = ConstantInt::get(I64, 1);

  // 1. Comparison between two null pointers, or a null pointer and a resolved
  // pointer:
  ...
  if (op0Arg && op1Arg && (op0Arg == zero || op1Arg == zero)) {
    ...
    ImpType = ValSetTypeScalar;
    Improved = ShadowValue(ConstantFoldCompareInstOperands(
        CmpI->getPredicate(), op0Arg, op1Arg, *GlobalTD));
  ...
  if (op0 == op1) {
    ...
    // Always do a signed test here, assuming that negative indexing off a
    // pointer won't wrap the address space and end up with something large and
    // positive.

    op0Arg = ConstantInt::get(I64, Ops[0].second.Offset);
    op1Arg = ConstantInt::get(I64, Ops[1].second.Offset);
    ImpType = ValSetTypeScalar;
    Improved.V = ShadowValue(ConstantFoldCompareInstOperands(
        getSignedPred(CmpI->getPredicate()), op0Arg, op1Arg, *GlobalTD));
    return true;
  }

  // 3. Restricted comparison of pointers with a differing base: we can compare
  // for equality only as we don't know memory layout at this stage. It's
  // possible that in the machine representation pointers with differing bases
  // could have equal representations, but e.g. LLVM basic alias analysis would
  // already say these don't alias, so any surprises from LLPE could also have
  // come from standard LLVM optimisation.
...
// Get a special value to indicate a negated pointer, useful because some other
// optimisations result in the pointer being temporarily negated during pointer
// arithmetic.
static bool tryGetNegatedPointer(ShadowValue checkOp, uint64_t &SubOp0,
                                 ShadowValue &SubOp1Base,
                                 int64_t &SubOp1Offset) {

  if ((!checkOp.isInst()) ||
      checkOp.u.I->invar->I->getOpcode() != Instruction::Sub)
    return false;
...
unsigned LLPEAnalysisPass::getMallocAlignment() { return mallocAlignment; }

// Fetch alignment if V is a known allocation, and its allocation instruction /
// global gives an alignment or the user has informed us about the heap
  ...
  if (V.isPtrIdx()) {
    ...
    if (V.getFrameNo() == -1) {
      ...
      if (AD->allocValue.isInst())
        ...
        GlobalObject *GO = cast<GlobalObject>(AD->allocValue.getGV()->G);
...
// Evaluate pointer arithmetic, other than that using getelementptr. We support
// addition, subtraction, discovering alignment if we know it, and
// setting/getting the least significant bits, again if we know the pointer's
// alignment so this is predictable.
bool IntegrationAttempt::tryFoldPtrAsIntOp(
    ShadowInstruction *SI, std::pair<ValSetType, ImprovedVal> *Ops,
    ValSetType &ImpType, ImprovedVal &Improved) {
  ...
  if (BOp->getOpcode() == Instruction::Sub) {
    ...
    if (!Op0Ptr) {
      ...
      // This is a constant minus a pointer, which some compilers e.g.
      // DragonEgg-3.2 + gcc-4.6 can produce. Mark it as pointer with unknown
      // offset, to hopefully catch it later in the special case under ::Add
    ...
    if (!Op1Ptr) {
      ...
      if (Ops[0].second.Offset != LLONG_MAX &&
          Ops[1].second.Offset != LLONG_MAX) {
        ImpType = ValSetTypeScalar;
        Improved = ShadowValue::getInt(
            BOp->getType(),
            (uint64_t)(Ops[0].second.Offset - Ops[1].second.Offset));
        return true;
      }
    }
    ...
    if (Op0Ptr && Op1Ptr) {
      ...
      // Check for the special case mentioned above: some compilers generate
      // constructions like: (const - ptr) + ptr, where the former operand is
      // noted as (ptr + ?) since ImprovedVal has no way to indicate the
      // negation of a pointer at the moment. Try to find that case if it has
      // happened here.
      ...
      ImpType = ValSetTypeScalar;
      int64_t NewVal =
          (Ops[otherOp].second.Offset + ((int64_t)SubOp0)) - SubOp1Offset;
    ...
    uint64_t NumC;
    bool NumCValid =
        tryGetConstantInt(Op0Ptr ? Ops[1].second.V : Ops[0].second.V, NumC);
    ...
    // Common technique to discover a pointer's alignment -- and it with a small
    // integer. Answer if we can.
    ...
    do {
      ...
      if (Align > MaskC) {
        ...
        // In this case the instruction is masking off bits that are known zero
        // in the original allocation; thus it is masking only the Offset.
        ImpType = ValSetTypePB;
        Improved.V = Ops[0].second.V;
    ...
    // Otherwise, the usual rule: the and op cannot take a pointer into a
    // different allocated object.
    ...
    // Or may be used to set offset bits, similar to the And case above
    // subtracting from it.
...
// Execute a special "is-same-object" instruction. These are user-specified
// tests that check whether two pointers are known to refer to the same object,
// and can be used to replace ptr >= base && ptr < (base + alloc_size), which
// LLPE can't model (yet).
void llvm::executeSameObject(ShadowInstruction *SI) {
  ...
  if (SI->i.PB) {
    ...
    if (ConstantInt *CI =
            dyn_cast_or_null<ConstantInt>(getConstReplacement(SI))) {
...
bool IntegrationAttempt::tryFoldBitwiseOp(
    ShadowInstruction *SI, std::pair<ValSetType, ImprovedVal> *Ops,
    ValSetType &ImpType, ImprovedVal &Improved) {
  ...
  if (BOp->getOpcode() == Instruction::And) {
    ...
    if (!allOnes) {
      ...
      if (tryGetConstantInt(Ops[1].second.V, Op1CI)) {
        ...
        APInt Op1AP(cast<IntegerType>(Ops[1].second.V.getNonPointerType())
                        ->getBitWidth(),
                    Op1CI);
...
static Constant *tryCastTo(Constant *C, Type *Ty) {
  ...
  if (!CastInst::isBitCastable(C->getType(), Ty))
    return 0;
...
// Analyse instruction SI, whose operands have already been extracted to array
// Ops, and if we find anything write it to ImpType / Improved.
// needsRuntimeCheck may be set if we find we're analysing a pointer comparison
// that needs to be verified at runtime.
void IntegrationAttempt::tryEvaluateResult(
    ShadowInstruction *SI, std::pair<ValSetType, ImprovedVal> *Ops,
    ValSetType &ImpType, ImprovedVal &Improved,
    unsigned char *needsRuntimeCheck) {
  ...
  // Try a special case for forwarding FDs: they can be passed through any cast
  // preserving 32 bits. We optimistically pass vararg cookies through all
  // casts.
  if (inst_is<CastInst>(SI)) {
    ...
    if (Ops[0].first == ValSetTypeFD) {
      if (!((SrcTy->isIntegerTy(32) || SrcTy->isIntegerTy(64) ||
             SrcTy->isPointerTy()) &&
            (DestTy->isIntegerTy(32) || DestTy->isIntegerTy(64) ||
             DestTy->isPointerTy()))) {
    ...
    if (Ops[0].first != ValSetTypeScalar) {
      ...
      // Pass FDs, pointers, vararg cookies through. This includes ptrtoint and
      // inttoptr.
      ImpType = Ops[0].first;
  ...
  if (inst_is<CmpInst>(SI)) {
    ...
    if (tryFoldOpenCmp(SI, Ops, ImpType, Improved))
      return;
    if (inst_is<ICmpInst>(SI) &&
        tryFoldPointerCmp(SI, Ops, ImpType, Improved, needsRuntimeCheck))
      return;
    ...
    if (Ops[0].first == ValSetTypePB) {
      ...
      if (Improved.Offset != LLONG_MAX) {
        ...
        for (uint32_t i = 1, ilim = SI->getNumOperands(); i != ilim;
             ++i, ++GTI) {

          uint64_t OpC;

          if (Ops[i].first != ValSetTypeScalar ||
              !tryGetConstantInt(Ops[i].second.V, OpC)) {
            // Uncertain
            Improved.Offset = LLONG_MAX;
            break;
          } else {
            ...
            if (StructType *STy = GTI.getStructTypeOrNull()) {
              Improved.Offset +=
                  GlobalTD->getStructLayout(STy)->getElementOffset(OpC);
            } else {
        ...
        if (Improved.Offset == LLONG_MAX && SI->getNumOperands() == 2) {
          ...
          // Special case: gep (baseptr + c1), negatedptr, where negatedptr = c2
          // - (baseptr + c3) Resolves to integer c1 + c2 - c3
      ...
      if (Ops[1].first == ValSetTypeVarArg) {
        ...
        InlineAttempt *calledIA =
            Ops[0].second.V.getInst()->parent->IA->getFunctionRoot();
    ...
    ImpType = (Ops[0].first == ValSetTypeUnknown ? ValSetTypeUnknown
                                                 : ValSetTypeOverdef);
    return;
    ...
    Constant *Ext = ConstantFoldExtractValueInstruction(
        Agg, cast<ExtractValueInst>(SI->invar->I)->getIndices());
    if (Ext) {
      ImpType = ValSetTypeScalar;
  ...
  // These are uniqued and stay alive forever, which can consume a *lot* of
  // memory, so this path handles the common case of integer unary and binary
  // operations.

  SmallVector<uint64_t, 4> intOperands;
  bool allOpsInts = true;

  for (unsigned i = 0, ilim = I->getNumOperands(); i != ilim && allOpsInts;
       i++) {

    uint64_t opInt;
    if (Ops[i].first != ValSetTypeScalar ||
        !tryGetConstantInt(Ops[i].second.V, opInt))
      allOpsInts = false;
    else
  ...
  for (unsigned i = 0, ilim = I->getNumOperands(); i != ilim; i++) {
    ...
    if (Ops[i].first == ValSetTypePB) {
      ...
      if (Constant *OpBase =
              dyn_cast_or_null<Constant>(Ops[i].second.V.getVal())) {

        if (OpBase->isNullValue()) {

          instOperands.push_back(
              getGVOffset(OpBase, Ops[i].second.Offset, OpBase->getType()));
          continue;
        }
      }
    }
  ...
  if (const CmpInst *CI = dyn_cast<CmpInst>(I)) {
    ...
    // Rare corner case: we get here but the compare args are not of the same
    // type. Example: comparing a constant against ptrtoint(null). Coerece to
    // the real instruction's type if possible.
    if (instOperands[0]->getType() != CI->getOperand(0)->getType()) {
      instOperands[0] =
          tryCastTo(instOperands[0], CI->getOperand(0)->getType());
      if (!instOperands[0]) {
        ImpType = ValSetTypeUnknown;
        return;
      }
    ...
    if (instOperands[1]->getType() != CI->getOperand(1)->getType()) {
      instOperands[1] =
          tryCastTo(instOperands[1], CI->getOperand(1)->getType());
      if (!instOperands[1]) {
        ImpType = ValSetTypeUnknown;
        return;
      }
    ...
    newConst = ConstantFoldCompareInstOperands(
        CI->getPredicate(), instOperands[0], instOperands[1], *GlobalTD);

  } else if (LoadInst *LI = dyn_cast<LoadInst>(I)) {
    newConst =
        ConstantFoldLoadFromConstPtr(instOperands[0], LI->getType(), *GlobalTD);
  } else
    newConst = ConstantFoldInstOperands(I, instOperands, *GlobalTD, GlobalTLI);

  if (newConst) {

    // Filter out cases that have just wrapped a ConstantExpr around the
    // operands. Acceptable cases here: inttoptr(const)
...
// All Ops are known not to have multi values. Extract each Op in turn until
// we've discovered all SI's operands, then call tryEvaluateResult.
bool IntegrationAttempt::tryEvaluateOrdinaryInst(
    ShadowInstruction *SI, ImprovedValSetSingle &NewPB,
    std::pair<ValSetType, ImprovedVal> *Ops, uint32_t OpIdx) {
...

  {
    ...
    if (ArgPB.isWhollyUnknown()) {
      ...
      if (!tryGetPathValue(OpV, SI->parent, Ops[OpIdx])) {
        ...
        Ops[OpIdx].first =
            ArgPB.Overdef ? ValSetTypeOverdef : ValSetTypeUnknown;
...
// Evaluate a comparison involving one or more composite values -- typically
// this is a constant array or struct, or a file-descriptor+flags int64, but
// compilers for non-C languages could make more use of composite-typed
// instructions.
bool IntegrationAttempt::tryEvaluateMultiCmp(ShadowInstruction *SI,
                                             ImprovedValSet *&NewIV) {
  ...
  case MCRFALSE: {
    ...
    ShadowValue Result =
        ShadowValue(MCR == MCRTRUE ? ConstantInt::getTrue(LLC)
                                   : ConstantInt::getFalse(LLC));
...
MultiCmpResult IntegrationAttempt::tryEvaluateMultiEq(ShadowInstruction *SI) {
  ...
  // EQ is true if true of all fields, false if false anywhere, or unknown
  // otherwise.
  ...
  for (ImprovedValSetMulti::MapIt it = IVM->Map.begin(), endit = IVM->Map.end();
       it != endit; ++it) {

    std::pair<ValSetType, ImprovedVal> Ops[2];
    Type *SubValType =
        Type::getIntNTy(SI->invar->I->getContext(), it.stop() - it.start());
    Constant *SubVal =
        constFromBytes(&(CmpBytes[it.start()]), SubValType, GlobalTD);
...
// Try to flatten elements from the list-of-extents InIVM into a single value
// Result. ShiftInt and resSize can be used to select a subrange. This will
// succeed if e.g. we manage to flatten a constant struct into a constant int,
// but fail if we try to mash incompatible types together, like an integer and a
// symbolic file descriptor.
static void flattenIVM(ImprovedValSetMulti *InIVM, uint64_t resSize,
                       int64_t ShiftInt, uint32_t elemsLimit, uint32_t nextElem,
                       uint32_t *elems,
  ...
  if (nextElem == elemsLimit) {
    ...
    uint32_t thisElem = 0;
    for (ImprovedValSetMulti::MapIt it = InIVM->Map.begin(),
                                    endit = InIVM->Map.end();
         it != endit; ++it) {
      ...
      if (!addIVToPartialVal(IVS.Values[elems[thisElem++]], IVS.SetType,
                             std::max(-ShiftedStart, (int64_t)0),
                             std::max(ShiftedStart, (int64_t)0),
                             std::min(ShiftedStop, (int64_t)resSize) -
                                 std::max(ShiftedStart, (int64_t)0),
                             &PV, 0)) {
    ...
    Constant *PVConst =
        PVToConst(PV, resSize, GlobalIHP->RootIA->F.getContext());
    ...
    for (uint32_t i = 0, ilim = setSizesInRange[nextElem]; i != ilim; ++i) {
      ...
      elems[nextElem] = i;
      flattenIVM(InIVM, resSize, ShiftInt, elemsLimit, nextElem + 1, elems,
                 setSizesInRange, Result);
    }
  }
}

bool IntegrationAttempt::tryEvaluateMultiInst(ShadowInstruction *SI,
                                              ImprovedValSet *&NewIV) {
  ...
  // * Truncate
  // * PHI, select, and other copies (but these are implemented in the
  // merge-instruction path)
  ...
  case Instruction::Trunc: {
    ...
    if (opcode != Instruction::Trunc) {
      ...
      ConstantInt *ShiftAmt =
          cast_or_null<ConstantInt>(getConstReplacement(SI->getOperand(1)));
      if (!ShiftAmt) {
        NewIV = newOverdefIVS();
        return true;
      }
    ...
    ImprovedValSetMulti *InIVM =
        cast<ImprovedValSetMulti>(getIVSRef(SI->getOperand(0)));

    // Discounting items that will be out of range, will the new value be a
    // simple integer? How many items will remain visible post shift?
    ...
    for (ImprovedValSetMulti::MapIt it = InIVM->Map.begin(),
      ...
      if (!(IVS.SetType == ValSetTypeScalar || IVS.Overdef ||
            !IVS.isInitialised()))
    ...
    if (uniqueVal) {
      ...
      for (ImprovedValSetMulti::MapIt it = NewIVM->Map.begin(),
                                      endit = NewIVM->Map.end();
           it != endit; ++it) {
  ...
  case Instruction::And: {
    ...
    // Evaluate against each subcomponent. Any mask against a non-scalar except
    // 0x00 and 0xff -> overdef.
    ImprovedValSetMulti *IVM;
    Constant *MaskC;
    if (!(IVM = dyn_cast<ImprovedValSetMulti>(getIVSRef(SI->getOperand(0))))) {
      IVM = cast<ImprovedValSetMulti>(getIVSRef(SI->getOperand(1)));
    ...
    if (!anyNonScalarVisible) {
      ...
      for (ImprovedValSetMulti::MapIt it = IVM->Map.begin(),
        ...
        if (it.value().SetType == ValSetTypePB ||
          ...
          Type *valType = Type::getIntNTy(SI->invar->I->getContext(),
                                          it.stop() - it.start());
          ...
          if (!addIVSToPartialVal(it.value(), 0, it.start(),
                                  it.stop() - it.start(), &PV, 0)) {
            NewIV = newOverdefIVS();
            return true;
          }
        }
      ...
      // XXX: ConstantFoldConstantExpression doesn't exist in llvm 5.0
      // if(ConstantExpr* MaskedCE = dyn_cast<ConstantExpr>(MaskedConst))
      // MaskedConst = ConstantFoldConstantExpression(MaskedCE);
      MaskedConst = ConstantFoldConstant(MaskedConst, *GlobalTD, GlobalTLI);
      ...
      // FD or pointer components retained, but necessarily aren't the whole
      // thing. Create a new multi, with each individual part masked
      // appropriately.
      ...
      for (ImprovedValSetMulti::MapIt it = IVM->Map.begin(),
        ...
        if (it.value().SetType == ValSetTypePB ||
          ...
          if (thisPreserved)
            ...
            NewIVM->Map.insert(it.start(), it.stop(),
          ...
          std::pair<ValSetType, ImprovedVal> Ops[2];
          Type *SubMaskType = Type::getIntNTy(SI->invar->I->getContext(),
                                              it.stop() - it.start());
          Constant *SubMask =
              constFromBytes(&(Mask[it.start()]), SubMaskType, GlobalTD);
...
// Try to evaluate instruction SI: dispatch to either the "normal" or
// composite-value paths. Instructions with side-effects have already been
// filtered out.
bool IntegrationAttempt::tryEvaluateOrdinaryInst(ShadowInstruction *SI,
                                                 ImprovedValSet *&NewPB) {

  bool anyMultis = false;

  for (uint32_t i = 0, ilim = SI->getNumOperands(); i != ilim && !anyMultis;
       ++i) {
...
// Evaluate general instruction SI. Dispatch to the load-forwarding code or use
// the ordinary evaluation code in this file as appropriate. Return true if we
// set NewResult. Set loadedVararg if this context becomes 'tainted' by reading
// varargs.
bool IntegrationAttempt::getNewResult(ShadowInstruction *SI,
                                      ImprovedValSet *&NewResult,
                                      bool &loadedVararg) {
  ...
  if (tryMerge) {
    ...
    if (ImprovedValSetSingle *IVS = dyn_cast<ImprovedValSetSingle>(NewResult)) {
      ...
      if (IVS->isWhollyUnknown()) {
        ...
        for (uint32_t i = 0, ilim = SI->invar->operandIdxs.size(); i != ilim;
             ++i)
...
// The following two functions used to track values that can be indirectly used
// in the specialised program: allocations and file descriptors. However this
// method was too memory-inefficient, so for the time being they are never
// eliminated unless they are trivially unused.
static bool willUseIndirectly(ImprovedValSet *IV) {

  if (ImprovedValSetSingle *IVS = dyn_cast_or_null<ImprovedValSetSingle>(IV)) {

    if ((IVS->SetType == ValSetTypeFD || IVS->SetType == ValSetTypePB) &&
        IVS->Values.size() == 1)
      return true;
  }
  ...
  if (willUseIndirectly(NewPB)) {
    ...
    if (NewIVS->Values[0].V.isPtrIdx()) {
      ...
      if (AD->allocValue.isInst() && !AD->isCommitted) {
        ...
        // GlobalIHP->indirectDIEUsers[AD->allocValue];
      ...
      if (!FDS.isCommitted) {
        // std::vector<std::pair<ShadowValue, uint32_t> >& Users =
        // GlobalIHP->indirectDIEUsers[ShadowValue(FDS.SI)];
        // Users.push_back(std::make_pair(V, V.getCtx()->SeqNumber));
      }
    }
  }
...
// Main entry point for this file: (re-)evaluate general instruction or argument
// V. If we're analysing an unbounded loop (inLoopAnalyser) we can short-cut
// this process when analysing V for the second time, because values only get
// less informative on repeated analysis.
bool IntegrationAttempt::tryEvaluate(ShadowValue V, bool inLoopAnalyser,
                                     bool &loadedVararg) {

  ImprovedValSet *OldPB = getIVSRef(V);
  ImprovedValSetSingle *OldPBSingle =
      dyn_cast_or_null<ImprovedValSetSingle>(OldPB);
  // The latter case here implies a multi which is always defined.
  bool OldPBValid =
      (OldPBSingle && OldPBSingle->isInitialised()) || (OldPB && !OldPBSingle);
  ...
  // as opposed to one defining an /argument/, which is checked elsewhere.
  {
    ...
    ImprovedValSetSingle *NewIVS;
    if ((NewIVS = dyn_cast<ImprovedValSetSingle>(NewPB)) &&
        (NewIVS->isWhollyUnknown() || NewIVS->Values.size() > 1)) {
