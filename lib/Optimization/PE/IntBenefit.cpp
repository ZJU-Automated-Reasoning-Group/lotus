...
// getResidualInstructions: return a best-case residual instruction count, where
// we assume that any code size increase will cause us to opt not to unroll a
// loop.
...
int64_t PeelAttempt::getResidualInstructions() {
  ...
  int64_t maxSize =
      (*(Iterations.begin()))->getTotalInstructionsIncludingLoops();
...
int64_t IntegrationAttempt::getResidualInstructions() {
  ...
  int64_t totalInstructions =
      ((int)getTotalInstructions()) - ((int)getElimdInstructions());
...
static void findResidualFunctionsInConst(DenseSet<Function *> &ElimFunctions,
                                         Constant *C) {
  ...
  if (Function *F = dyn_cast<Function>(C)) {
    ...
    for (Use &U : CE->operands()) {
      ...
      findResidualFunctionsInConst(ElimFunctions, cast<Constant>(U.get()));
    }
  }
}

// Count instructions likely to be residualised (retained in the specialised
// program) in this context.
void InlineAttempt::findResidualFunctions(
...
void IntegrationAttempt::findResidualFunctions(
    DenseSet<Function *> &ElimFunctions,
    DenseMap<Function *, unsigned> &TotalResidualInsts) {
  ...
  for (uint32_t i = 0; i < nBBs; ++i) {
    ...
    for (uint32_t j = 0; j < BB->insts.size(); ++j) {
      ...
      if (inst_is<CallInst>(I) || inst_is<InvokeInst>(I)) {
        ...
        // Else the caller is unresolved: we'll eliminate all possibly-called
        // functions because they're used in a live instruction somewhere.

      } else {

        for (Use &OpUse : I->invar->I->operands()) {

          if (Constant *C = dyn_cast<Constant>(OpUse.get())) {
  ...
  for (DenseMap<const ShadowLoopInvar *, PeelAttempt *>::iterator
    ...
    for (unsigned i = 0; i < iterCount; ++i) {
      ...
      it->second->Iterations[i]->findResidualFunctions(ElimFunctions,
                                                       TotalResidualInsts);
    }
  }
}
...
void IntegrationAttempt::findProfitableIntegration() {
  ...
  // This differs from the elimdInstructions value in that dead blocks are not
  // counted since they wouldn't get run at all.
  ...
  for (uint32_t i = 0; i < nBBs; ++i) {
    ...
    if (L != BBL && ((!L) || L->contains(BBL))) {
      ...
      DenseMap<const ShadowLoopInvar *, PeelAttempt *>::iterator findit =
          peelChildren.find(immediateChildLoop(L, BBL));

      // Count unexpanded loops as ours:
      if (findit == peelChildren.end() || (!findit->second->isEnabled()) ||
          (!findit->second->isTerminated()))
        BBL = L;
    }
  ...
  // errs() << getShortHeader() << ": int-goodness " << totalIntegrationGoodness
  // << " (child: " << childIntegrationGoodness << ", codesize: " <<
  // newInstPenalty << ", timebonus: " << timeBonus << ")\n";
}
...
// Does this instruction count for accounting / performance measurement?
// Essentially: can this possibly be improved?
bool llvm::instructionCounts(Instruction *I) {

  if (isa<DbgInfoIntrinsic>(I))
    return false;
  if (BranchInst *BI = dyn_cast<BranchInst>(I))
    if (BI->isUnconditional()) // Don't count unconditional branches as they're
                               // already as specified as they're getting
      return false;
  return true;
}

// Count instructions that have been improved (resolved to a constant or
// similar) in this block.
void IntegrationAttempt::collectBlockStats(ShadowBBInvar *BBI, ShadowBB *BB) {

  uint32_t i = 0;

  for (BasicBlock::iterator BI = BBI->BB->begin(), BE = BBI->BB->end();
       BI != BE; ++BI, ++i) {
...
void InlineAttempt::collectAllLoopStats() {
  ...
  for (SmallVector<ShadowLoopInvar *, 4>::const_iterator
           LoopI = invarInfo->TopLevelLoops.begin(),
           LoopE = invarInfo->TopLevelLoops.end();
       LoopI != LoopE; ++LoopI)
...
void PeelIteration::collectAllLoopStats() {
  ...
  for (SmallVector<ShadowLoopInvar *, 1>::const_iterator
           LoopI = L->childLoops.begin(),
           LoopE = L->childLoops.end();
       LoopI != LoopE; ++LoopI)
    collectLoopStats(*LoopI);
}
