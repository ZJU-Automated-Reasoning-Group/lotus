...
void InlineAttempt::commitArgsAndInstructions() {
  ...
  if (isCommitted()) {
    ...
    // 1, 						       pass->fakeDebugType, false, 						       true, 1); 	DILexicalBlock fakeBlock =
    // DIB.createLexicalBlock(fakeFunction, fakeFile, 1, 0); 	DebugLoc newFakeLoc
    // = DebugLoc::getFromDILexicalBlock(fakeBlock);
