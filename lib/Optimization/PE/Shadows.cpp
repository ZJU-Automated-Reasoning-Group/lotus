...
// edge.
void llvm::createTopOrderingFrom(BasicBlock *BB,
                                 std::vector<BasicBlock *> &Result,
                                 SmallSet<BasicBlock *, 8> &Visited,
                                 LoopInfo *LI, const Loop *MyL) {
  ...
  // Ordering here: first the loop's successors, then the loop body (by walking
  // to the header with succ_iterator below) and then this block, the preheader.
  if (MyL != BBL) {

    SmallVector<BasicBlock *, 4> ExitBlocks;
    BBL->getExitBlocks(ExitBlocks);
    for (SmallVector<BasicBlock *, 4>::iterator it = ExitBlocks.begin(),
                                                it2 = ExitBlocks.end();
         it != it2; ++it) {
  ...
  // Explore all successors within this loop. This will enter a child loop if
  // this BB is a preheader; note BBL may not equal MyL, but is certainly its
  // child.
...
static void ignoreChildLoops(SmallSet<BasicBlock *, 1> &headers,
                             const Loop *L) {
...
// Return the tightest enclosing loop that isn't specified ignored by a user
// directive. Loops that are ignored in this way are discounted for
// per-iteration exploration.
const ShadowLoopInvar *
LLPEAnalysisPass::applyIgnoreLoops(const ShadowLoopInvar *L, Function *F,
                                   ShadowFunctionInvar *FInfo) {
...
// Translate the information in LoopInfo descriptor L into a ShadowLoopInvar
// object, which uses block-indices instead of BasicBlock* pointers.
ShadowLoopInvar *LLPEAnalysisPass::getLoopInfo(
    ShadowFunctionInvar *FInfo, DenseMap<BasicBlock *, uint32_t> &BBIndices,
    const Loop *L, DominatorTree *DT, ShadowLoopInvar *ParentLoop) {

  release_assert(L->isLoopSimplifyForm() && L->isLCSSAForm(*DT) &&
                 "Don't forget to run loopsimplify and lcssa first!");
  ...
  // If we're supposed to ignore this loop and all children, register them now
  // so that applyIgnoreLoops does the right thing.
  ...
  // This is an edge which, if killed, means we may assume that the loop will
  // iterate and continue investigating. In other words, sooner or later the
  // loop *must* terminate using this exit edge, even if the CFG makes it appear
  // otherwise.
  LInfo->optimisticEdge = std::make_pair(0xffffffff, 0xffffffff);

  for (uint32_t i = LInfo->headerIdx,
                ilim = LInfo->headerIdx + L->getNumBlocks();
       i != ilim; ++i) {

    // TODO: Fix or discard outerScope. It should assign blocks to a loop scope
    // when certain user-specified loops are ignored (so their blocks inherit
    // the parent loop's scope). However I think the support for this edge case
    // has rotted since it was first implemented. Note these will be overwritten
    // if the block is also within a child loop.
    ...
    if (OptEdgeSink) {
      release_assert(LInfo->optimisticEdge.first == 0xffffffff &&
                     "Only one optimistic edge allowed per loop");
  ...
  // for certain that it exits, rather than while we can show it doesn't, as
  // usual?
...

  {
    ...
    for (unsigned i = 0; i < exitEdges.size(); ++i)
      LInfo->exitEdges.push_back(std::make_pair(
          BBIndices[exitEdges[i].first], BBIndices[exitEdges[i].second]));
  }
...
void LLPEAnalysisPass::initShadowGlobals(Module &M, uint32_t extraSlots) {
  ...
  // extraSlots are reserved for new globals we know will be introduced between
  // now and specialisation start.
  nGlobals += extraSlots;
  shadowGlobals = new ShadowGV[nGlobals];

  // Assign them all numbers before computing initialisers, because the
  // initialiser can reference another global, and getValPB will then lookup in
  // shadowGlobalsIdx.

  for (Module::global_iterator it = M.global_begin(), itend = M.global_end();
       it != itend; ++it, ++i) {
  ...
  for (Module::global_iterator it = M.global_begin(), itend = M.global_end();
    ...
    if (it->isConstant()) {
      shadowGlobals[i].storeSize =
          GlobalTD->getTypeStoreSize(shadowGlobals[i].G->getType());
      continue;
    }
    ...
    AD.storeSize = GlobalTD->getTypeStoreSize(
        cast<PointerType>(it->getType())->getPointerElementType());
    AD.isCommitted = true;
...
const GlobalValue *llvm::getUnderlyingGlobal(const GlobalValue *V) {
  ...
  if (const GlobalAlias *GA = dyn_cast<GlobalAlias>(V)) {
    const GlobalValue *Aliasee =
        dyn_cast_or_null<GlobalValue>(GA->getAliasee());
    if (!Aliasee)
      return 0;
    else
...
// Create shadow information for function F, including top-sorting its blocks to
// give them indices and thus a sensible order for specialisation.
ShadowFunctionInvar *LLPEAnalysisPass::getFunctionInvarInfo(Function &F) {

  // Already described?
  DenseMap<Function *, ShadowFunctionInvar *>::iterator findit =
      functionInfo.find(&F);
  if (findit != functionInfo.end())
    return findit->second;

  // Beware! This LoopInfo instance and whatever Loop objects come from it are
  // only alive until the next call to getAnalysis. Therefore the
  // ShadowLoopInvar objects we make here must mirror all information we're
  // interested in from the Loops.
  ...
  // Top-sort all blocks, including child loop. Thanks to trickery in
  // createTopOrderingFrom, instead of giving all loop blocks an equal topsort
  // value due to the latch edge cycle, we order the header first, then the loop
  // body in topological order ignoring the latch, then its exit blocks.
  std::vector<BasicBlock *> TopOrderedBlocks;
  SmallSet<BasicBlock *, 8> VisitedBlocks;

  createTopOrderingFrom(&F.getEntryBlock(), TopOrderedBlocks, VisitedBlocks, LI,
                        /* loop = */ 0);

  // Since topsort gives a bottom-up ordering.
  std::reverse(TopOrderedBlocks.begin(), TopOrderedBlocks.end());

  // Assign indices to each BB and instruction (IIndices is useful since
  // otherwise we have to walk the instruction list to get from an instruction
  // to its index)
  ...
  for (uint32_t i = 0; i < TopOrderedBlocks.size(); ++i) {
    ...
    for (uint32_t j = 0; PI != PE; ++PI, ++j) {
      ...
      if (SBB.predIdxs[j] > i) {
        ...
        if ((!BBScope) || SBB.BB != BBScope->getHeader()) {
          ...
          errs() << "Warning: block " << SBB.BB->getName() << " in "
                 << F.getName() << " has predecessor " << (*PI)->getName()
                 << " that comes after it topologically, but this is not a "
                    "loop header. The program is not in well-nested natural "
                    "loop form.\n";
        }
      }
    }
    ...
    for (uint32_t j = 0; BI != BE; ++BI, ++j) {
      ...
      if (PHINode *PN = dyn_cast<PHINode>(I)) {
        ...
        for (unsigned k = 0, kend = PN->getNumIncomingValues(); k != kend;
             ++k) {

          if (Instruction *OpI = dyn_cast<Instruction>(PN->getIncomingValue(k)))
            operandIdxs[k] =
                ShadowInstIdx(BBIndices[OpI->getParent()], IIndices[OpI]);
          else if (GlobalVariable *OpGV = const_cast<GlobalVariable *>(
                       getGlobalVar(PN->getIncomingValue(k))))
            operandIdxs[k] =
                ShadowInstIdx(INVALID_BLOCK_IDX, getShadowGlobalIndex(OpGV));
          else
          ...
          if (Instruction *OpI = dyn_cast<Instruction>(I->getOperand(k)))
            operandIdxs[k] =
                ShadowInstIdx(BBIndices[OpI->getParent()], IIndices[OpI]);
                       ...
                       getGlobalVar(I->getOperand(k))))
            operandIdxs[k] =
                ShadowInstIdx(INVALID_BLOCK_IDX, getShadowGlobalIndex(OpGV));
          else if (BasicBlock *OpBB = dyn_cast<BasicBlock>(I->getOperand(k)))
            operandIdxs[k] =
                ShadowInstIdx(BBIndices[OpBB], INVALID_INSTRUCTION_IDX);
          else
            operandIdxs[k] = ShadowInstIdx();
        }
      }
      ...
      for (k = 0, UI = I->use_begin(); k != nUsers; ++k, ++UI) {
        ...
        if (Instruction *UserI = dyn_cast<Instruction>(UI->getUser())) {
          ...
          userIdxs[k] =
              ShadowInstIdx(BBIndices[UserI->getParent()], IIndices[UserI]);
  ...
  RetInfo.BBs =
      ImmutableArray<ShadowBBInvar>(FShadowBlocks, TopOrderedBlocks.size());
  ...
  for (; i != F.arg_size(); ++i, ++AI) {
    ...
    for (; UI != UE; ++UI, ++j) {
      ...
      if (Instruction *UsedI = dyn_cast<Instruction>(UsedV)) {
        ...
        Users[j] =
            ShadowInstIdx(BBIndices[UsedI->getParent()], IIndices[UsedI]);
  ...
  // Populate map from loop headers to header index. Due to the topological
  // sort, all loops consist of that block + L->getBlocks().size() further,
  // contiguous blocks, making is-in-loop easy to compute.
  ...
  // Count alloca instructions at the start of the function; this will control
  // how large the std::vector that represents the frame will be initialised.
  RetInfo.frameSize = 0;
  for (BasicBlock::iterator it = F.getEntryBlock().begin(),
                            itend = F.getEntryBlock().end();
       it != itend && isa<AllocaInst>(it); ++it)
    ++RetInfo.frameSize;

  // "&& RootIA" checks whether we're inside the initial context creation, in
  // which case we should allocate a frame whether or not main can ever allocate
  // to avoid the frame index underflowing in some circumstances.
  if ((!RetInfo.frameSize) && RootIA) {

    // Magic value indicating the function will never alloca anything and we can
    // skip all frame processing.
    RetInfo.frameSize = -1;

    for (inst_iterator I = inst_begin(F), E = inst_end(F);
         I != E && RetInfo.frameSize == -1; ++I) {
...
// Prepare the context-specific data structures, tying them to known invariant
// information.
...
void InlineAttempt::prepareShadows() {
  ...
  nBBs = F.size();
  release_assert(
      nBBs == invarInfo->BBs.size() &&
      "Function contains unreachable blocks, run simplifycfg first!");
  // Create a basic-block array, initially all marked unreachable (null).
  BBs = new ShadowBB *[nBBs];
  for (uint32_t i = 0; i < nBBs; ++i)
    BBs[i] = 0;
  // Indicates where this loop scope begins -- this is a function root, so its
  // scope begins at block zero.
  BBsOffset = 0;
...
// Create a shadow basic block -- called the first time we can show blockIdx is
// reachable.
ShadowBB *IntegrationAttempt::createBB(uint32_t blockIdx) {

  release_assert((!BBs[blockIdx - BBsOffset]) &&
                 "Creating block for the second time");
  ...
  newBB->insts =
      ImmutableArray<ShadowInstruction>(insts, newBB->invar->insts.size());
...
ShadowInstructionInvar *IntegrationAttempt::getInstInvar(uint32_t blockidx,
                                                         uint32_t instidx) {

  return &(invarInfo->BBs[blockidx].insts[instidx]);
}

// Get invariant information about BBs[blockidx][instidx], in this or a parent
// loop scope.
ShadowInstruction *InlineAttempt::getInstFalling(ShadowBBInvar *BB,
                                                 uint32_t instIdx) {
...
// Get invariant information about BBs[blockidx][instidx], in this or a parent
// loop scope.
ShadowInstruction *PeelIteration::getInstFalling(ShadowBBInvar *BB,
                                                 uint32_t instIdx) {
...
// Get invariant information about BBs[blockidx][instidx], in this or a parent
// loop scope. Return zero if the instruction falls outside this loop scope.
ShadowInstruction *IntegrationAttempt::getInst(uint32_t blockIdx,
                                               uint32_t instIdx) {
...
// Create an integer shadow value, using a cheap representation for common
// types. This is needed because specialisation can generate many intermediate
// values and LLVM Constants are uniqued and live forever.
...
// Note that due to LCSSA form operands are always in the same context or a
// parent, except for exit PHI operands, which are special cased in HCF's
...
ShadowValue ShadowInstruction::getOperand(uint32_t i) {
  ...
  if (blockOpIdx == INVALID_BLOCK_IDX) {
    ...
    if (SII.instIdx != INVALID_INSTRUCTION_IDX) {
      ...
      return ShadowValue(
          &(parent->IA->getFunctionRoot()->argShadows[A->getArgNo()]));
    ...
    // BasicBlock operand, only encountered on this path with Invoke
    // instructions.
    return ShadowValue();
...
bool ShadowValue::objectAvailable() const {
  ...
  case SHADOWVAL_OTHER: {
    // Special locations (e.g. TLS) are purely symbolic; they can't be
    // represented in a specialised program.
...
// Find a unique exiting edge from this loop iteration if one exists. Set bail
// if there are multiple exiting edges; return null if there are multiple or
// none.
ShadowBB *PeelIteration::getUniqueExitingBlock2(ShadowBBInvar *BBI,
  ...
  for (uint32_t i = 0, ilim = BBI->succIdxs.size();
       i != ilim && exitingEdges < 2; ++i) {
...
bool ShadowInstruction::hasOrderingConstraint() {
  ...
  case Instruction::AtomicCmpXchg: {
    ...
    return (
        isStrongerThan(cmpx->getSuccessOrdering(), AtomicOrdering::Unordered) ||
        isStrongerThan(cmpx->getFailureOrdering(), AtomicOrdering::Unordered));
    // XXX: modified by JN
    // In llvm 5.0 the operator > is explicitly deleted
...
// For example, if this context refers to an inner loop and Scope refers to its
// (grand-)parent, return the enclosing context that matches Scope. If it talks
// about a child loop, just return this context.
IntegrationAttempt *
...
IntegrationAttempt *
PeelIteration::getIAForScopeFalling(const ShadowLoopInvar *Scope) {
...
IntegrationAttempt *
InlineAttempt::getIAForScopeFalling(const ShadowLoopInvar *Scope) {
