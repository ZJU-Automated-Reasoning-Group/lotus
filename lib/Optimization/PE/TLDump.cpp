...
// Debugging functions that dunp the state of the tentative-load or dead-store
// analysis code.
...
void TLDump(IntegrationAttempt *IA) {
  ...
  for (uint32_t i = IA->BBsOffset, ilim = IA->BBsOffset + IA->nBBs; i != ilim;
       ++i) {
    ...
    if (BB->invar->naturalScope &&
      ...
      while (i != ilim &&
             BB->invar->naturalScope->contains(IA->getBBInvar(i)->naturalScope))
        ++i;
      --i;
      continue;
    }
