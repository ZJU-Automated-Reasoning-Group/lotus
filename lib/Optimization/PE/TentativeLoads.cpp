...
// These are loads whose incoming dataflow (a) crosses a /yield point/, a point
// where we must assume that another thread got a chance to run and messed with
// our state, (b) is not dominated by other loads or stores that will check the
// incoming state / overwrite it with known state, and (c) is not known to be
// thread-local regardless.

// The main phase has already taken care of part (c) for us by setting
// ShadowInstruction::u.load.isThreadLocal when the load was known to be from a
// thread-private object. We will set the same flag wherever it's clear that
// checking this load would be redundant.
...
TLLocalStore *TLMapPointer::getMapForBlock(ShadowBB *BB) { return BB->tlStore; }
...
// Merge two is-known-good arrays. An offset is good if it's good in both
// parents.
void TLMapPointer::mergeStores(TLMapPointer *mergeFrom, TLMapPointer *mergeTo,
                               uint64_t ASize, TLMerger *Visitor) {

  // Intersect the sets per byte. The values are just booleans, so overwriting
  // without erasing is fine.

  SmallVector<std::pair<uint64_t, uint64_t>, 4> keepRanges;

  for (TLMapTy::iterator it = mergeFrom->M->begin(),
                         itend = mergeFrom->M->end();
       it != itend; ++it) {

    for (TLMapTy::iterator toit = mergeTo->M->find(it.start()),
                           toitend = mergeTo->M->end();
  ...
  mergeTo->M->clear();
  for (SmallVector<std::pair<uint64_t, uint64_t>, 4>::iterator
           it = keepRanges.begin(),
           itend = keepRanges.end();
       it != itend; ++it) {
...
// Get a writable array for value O, including breaking CoW data structures
// above it.
...
// Clear the store to mark everything tentative (needs checking against
// concurrent interference). Note that an instruction here forced us to repeat
// all checks.
static void markAllObjectsTentative(ShadowInstruction *SI, ShadowBB *BB) {
  ...
  if (inst_is<LoadInst>(SI) || inst_is<AtomicRMWInst>(SI))
    errs() << "Clobber all at " << SI->parent->IA->F.getName() << ","
           << SI->parent->invar->BB->getName() << ","
           << std::distance(SI->parent->invar->BB->begin(),
                            BasicBlock::iterator(SI->invar->I))
           << "\n";
}

// Note that GoodPtr[Offset:Offset+Len] has been checked and need not be
// rechecked until another memory-ordered instruction or yield point means we
// must assume possible inter-thread communication again. Mark the good offsets
// as of block BB.
static void markGoodBytes(ShadowValue GoodPtr, uint64_t Len,
                          bool contextEnabled, ShadowBB *BB,
                          uint64_t Offset = 0) {

  // If we're in a disabled context, the loads and stores here will
  // be committed unmodified, in particular without checks that their results
  // are as expected, and so they do not make any subsequent check redundant.
  ...
  if (!addRanges.empty()) {
    ...
    for (SmallVector<std::pair<uint64_t, uint64_t>, 1>::iterator
             it = addRanges.begin(),
             itend = addRanges.end();
         it != itend; ++it) {
...
// A path condition checks that a certain value is as expected at runtime. Mark
// it as checked as of block BB.
static void walkPathCondition(PathConditionTypes Ty, PathCondition &Cond,
                              bool contextEnabled, ShadowBB *BB) {
...
static void walkPathConditions(PathConditionTypes Ty,
...
static void walkPathConditionsIn(PathConditions &PC, uint32_t stackIdx,
                                 ShadowBB *BB, bool contextEnabled,
                                 bool secondPass) {
  ...
  for (std::vector<PathFunc>::iterator it = PC.FuncPathConditions.begin(),
    ...
    // Path conditions can be treated like committed code, as the user is
    // responsible for checking their applicability.
...
// Mark the targets of all path condition checks PC at the top of BB checked.
void llvm::TLWalkPathConditions(ShadowBB *BB, bool contextEnabled,
                                bool secondPass) {
  ...
  if (IA->targetCallInfo)
    walkPathConditionsIn(GlobalIHP->pathConditions,
                         IA->targetCallInfo->targetStackDepth, BB,
                         contextEnabled, secondPass);

  // Consider path conditions that apply to all calls to this function
  if (BB->IA->invarInfo->pathConditions)
    walkPathConditionsIn(*BB->IA->invarInfo->pathConditions, UINT_MAX, BB,
                         contextEnabled, secondPass);
}

// Mark bytes read or written by a memcpy-like instruction as checked.
static void walkCopyInst(ShadowValue CopyFrom, ShadowValue CopyTo,
                         ShadowValue LenSV, bool contextEnabled, ShadowBB *BB) {
...
static void updateTLStore(ShadowInstruction *SI, bool contextEnabled) {
  ...
  if (inst_is<AllocaInst>(SI)) {
    ...
    markGoodBytes(ShadowValue(SI),
                  SI->parent->IA->getFunctionRoot()
                      ->localAllocas[Base.u.PtrOrFd.idx]
                      .storeSize,
                  contextEnabled, SI->parent);

  } else if (LoadInst *LI = dyn_cast_inst<LoadInst>(SI)) {

    // Load: mark good (our caller will have emitted a check if necessary)
    // unless the load is itself a synchronisation point.
    if ((LI->isVolatile() || SI->hasOrderingConstraint()) &&
        !SI->parent->IA->pass->atomicOpIsSimple(LI))
      markAllObjectsTentative(SI, SI->parent);
    else
      markGoodBytes(SI->getOperand(0),
                    GlobalTD->getTypeStoreSize(LI->getType()), contextEnabled,
                    SI->parent);

  } else if (StoreInst *StoreI = dyn_cast_inst<StoreInst>(SI)) {

    // I don't think there's a need to regard a volatile /store/ as a yield
    // point, as this is *outgoing* interthread communication if it
    // communication at all. Compare pthread_unlock which is not a yield point
    // to pthread_lock, which is.
    // if(StoreI->isVolatile())
    // markAllObjectsTentative(SI, SI->parent);
    // else
    markGoodBytes(
        SI->getOperand(1),
        GlobalTD->getTypeStoreSize(StoreI->getValueOperand()->getType()),
        contextEnabled, SI->parent);
    ...
    if (SI->isThreadLocal == TLS_MUSTCHECK &&
        !SI->parent->IA->pass->atomicOpIsSimple(SI->invar->I))
      markAllObjectsTentative(SI, SI->parent);
    else
      markGoodBytes(SI->getOperand(0),
                    GlobalTD->getTypeStoreSize(SI->getType()), contextEnabled,
                    SI->parent);
    ...
    if (inst_is<MemSetInst>(SI)) {
      ...
      markGoodBytes(SI->getCallArgOperand(0), MemSize, contextEnabled,
                    SI->parent);

    } else if (inst_is<MemTransferInst>(SI)) {

      // Memcpy: mark both args good.
      walkCopyInst(SI->getCallArgOperand(0), SI->getCallArgOperand(1),
                   SI->getCallArgOperand(2), contextEnabled, SI->parent);
      ...
      if (ReadFile *RF = SI->parent->IA->tryGetReadFile(SI)) {
        ...
        markGoodBytes(SI->getCallArgOperand(1), RF->readSize, contextEnabled,
                      SI->parent);

      } else if ((findit = SpecialFunctionMap.find(F)) !=
                 SpecialFunctionMap.end()) {
        ...
        case SF_REALLOC:
          ...
          // Realloc == copy + malloc.
          walkCopyInst(SI, SI->getCallArgOperand(0), SI->getCallArgOperand(1),
                       contextEnabled, SI->parent);
          // Fall through to:
...

        {
          ...
          markGoodBytes(SV, GlobalIHP->heap[Base.u.PtrOrFd.idx].storeSize,
                        contextEnabled, SI->parent);
        }
      ...
      } else if (CallI && (((!F) && !GlobalIHP->programSingleThreaded) ||
                           GlobalIHP->yieldFunctions.count(F))) {

        // Unknown function or explicitly-annotated yield function -- may
        // overwrite anything; everything needs re-checking.

        if (GlobalIHP->pessimisticLocks.count(CallI)) {

          // Pessimistic locks clobber at specialisation time, so we already
          // assume everything was overwritten; no runtime checking required.
          return;
        }

        SmallDenseMap<CallInst *, std::vector<GlobalVariable *>, 4>::iterator
            findit = GlobalIHP->lockDomains.find(CallI);

        if (findit != GlobalIHP->lockDomains.end()) {

          // This a bit of a hack -- the user can annotate a particular yield
          // function as only clobbering some particular globals.
...
static bool shouldCheckRead(ImprovedVal &Ptr, uint64_t Size, ShadowBB *BB) {
  ...
  if (verbose) {
    ...
    for (TLMapTy::iterator it = Map->M->begin(), itend = Map->M->end();
         it != itend; ++it) {
  ...
  bool coveredByMap =
      (it != Map->M->end() && ((int64_t)it.start()) <= Ptr.Offset &&
       ((int64_t)it.stop()) >= Ptr.Offset + ((int64_t)Size));

  return !coveredByMap;
}

// Is any of PtrOp[0:LenSV] currently marked tentative, as of block SI.parent?
ThreadLocalState IntegrationAttempt::shouldCheckCopy(ShadowInstruction &SI,
                                                     ShadowValue PtrOp,
                                                     ShadowValue LenSV) {
  ...
  // memcpyValues is unpopulated if the copy didn't "work" during
  // specialisation, so there is nothing to check.
  DenseMap<ShadowInstruction *, SmallVector<IVSRange, 4>>::iterator findit =
      GlobalIHP->memcpyValues.find(&SI);
  if (findit == GlobalIHP->memcpyValues.end() || !findit->second.size())
    return TLS_NEVERCHECK;

  // Check each concrete value that was successfully read during information
  // prop
...
ThreadLocalState IntegrationAttempt::shouldCheckLoadFrom(ShadowInstruction &SI,
                                                         ImprovedVal &Ptr,
                                                         uint64_t LoadSize) {
  ...
  if (IV) {
    ...
    for (ImprovedValSetMulti::MapIt it = IV->Map.begin(), itend = IV->Map.end();
         it != itend; ++it) {
  ...
  return shouldCheckRead(Ptr, LoadSize, SI.parent) ? TLS_MUSTCHECK
                                                   : TLS_NOCHECK;
}

// Does memory-reading instruction SI require a runtime check?
// It does if we succesfully extracted some information during specialisation,
// but the object we read from is marked tentative.
ThreadLocalState IntegrationAttempt::shouldCheckLoad(ShadowInstruction &SI) {
  ...
  if (inst_is<LoadInst>(&SI)) {
    ...
    if (IV) {
      ...
      for (uint32_t i = 0, ilim = IVS->Values.size();
           i != ilim && result != TLS_MUSTCHECK; ++i)
        result =
            std::min(shouldCheckLoadFrom(SI, IVS->Values[i], LoadSize), result);
    ...
    // Realloc instruction
    return shouldCheckCopy(SI, SI.getCallArgOperand(0),
                           SI.getCallArgOperand(1));
  }
}
...
// Merge known-good-bytes maps at the start of block BB, by visiting its
// predecessor blocks.
...
void InlineAttempt::findTentativeLoads(bool commitDisabledHere,
                                       bool secondPass) {
...
// SI read IVS from ReadPtr[ReadOffset:ReadOffset+ReadSize]. If IVS refers to an
// unavailable pointer or FD, clobber ReadPtr as we cannot check the read was as
// expected.
bool IntegrationAttempt::squashUnavailableObject(
    ShadowInstruction &SI, const ImprovedValSetSingle &IVS, bool inLoopAnalyser,
    ShadowValue ReadPtr, int64_t ReadOffset, uint64_t ReadSize) {
  ...
  if (squash) {
    ...
    release_assert((!inLoopAnalyser) &&
                   "TODO: squashUnavailableObject implementation for loops");
    ...
    // Overwrite the pointer in the store to prevent future readers from
...
// SI's result is PB. Check whether it mentions any unavailable pointers or file
// descriptors, and if it does clobber the memory SI read it from.
void IntegrationAttempt::squashUnavailableObjects(ShadowInstruction &SI,
                                                  ImprovedValSet *PB,
                                                  bool inLoopAnalyser) {

  if (ImprovedValSetSingle *IVS = dyn_cast_or_null<ImprovedValSetSingle>(PB)) {
    if (squashUnavailableObject(SI, *IVS, inLoopAnalyser, SI.getOperand(0), 0,
                                GlobalTD->getTypeStoreSize(SI.getType())))
      IVS->setOverdef();
  } else {
    ...
    for (ImprovedValSetMulti::MapIt it = IVM->Map.begin(),
      ...
      if (squashUnavailableObject(SI, it.value(), inLoopAnalyser,
                                  SI.getOperand(0), it.start(),
                                  it.stop() - it.start())) {
...
// The result of this load (or data read by this copy instruction) may contain
// pointers or FDs which are not available because their defining context is not
// going to be committed, but it requires a check and the check cannot be
// synthesised. Therefore replace them with Unknown. The replacement is done
// using executeWriteInst to emulate a clobbering write over the memory that
// held the unverifiable pointer.
void IntegrationAttempt::squashUnavailableObjects(ShadowInstruction &SI,
                                                  bool inLoopAnalyser) {
  ...
  if (inst_is<LoadInst>(&SI) || inst_is<AtomicCmpXchgInst>(&SI)) {
    ...
    if (findit != pass->memcpyValues.end()) {
      ...
      for (SmallVector<IVSRange, 4>::iterator it = findit->second.begin(),
        ...
        if (squashUnavailableObject(SI, it->second, inLoopAnalyser,
                                    SI.getCopySource(), it->first.first,
          ...
          release_assert(WriteP.SetType == ValSetTypePB &&
                         WriteP.Values.size());
...
// If this load read a pointer or FD that is currently unrealisable (i.e. has
// been previously committed but currently has no committed value), volunteer to
// replace it, becoming the new definitive version, if SI is always executed and
// thus this version must be reachable from all (future) users.
void IntegrationAttempt::replaceUnavailableObjects(ShadowInstruction &SI,
                                                   bool inLoopAnalyser) {

  if (inLoopAnalyser)
    return;

  if (inst_is<LoadInst>(&SI) || inst_is<CallInst>(&SI) ||
      inst_is<InvokeInst>(&SI)) {
    ...
    if (getBaseAndConstantOffset(ShadowValue(&SI), Base, Offset, false)) {
      ...
      if (Base.isPtrIdx() && Base.getFrameNo() == -1) {
        ...
        if (AD->isCommitted && !AD->committedVal) {
          ...
          // This means that the save phase will record the new reference and
          // patch refs will be accrued in the meantime.
          errs() << itcache(&SI)
                 << " stepping up as new canonical reference for "
                 << itcache(Base) << "\n";
          AD->isCommitted = false;
          AD->allocValue = ShadowValue(&SI);
          // Should be safe to change the allocType, as until now the allocation
          // had no examplar pointer and thus could not be referenced.
      ...
      ImprovedValSetSingle *IVS =
          dyn_cast_or_null<ImprovedValSetSingle>(SI.i.PB);
      if (IVS && IVS->Values.size() == 1 && IVS->SetType == ValSetTypeFD &&
...
// Main entry point for updating the tentative loads map according to SI's
// effects.
void IntegrationAttempt::TLAnalyseInstruction(ShadowInstruction &SI,
                                              bool commitDisabledHere,
                                              bool secondPass,
                                              bool inLoopAnalyser) {

  // Note that TLS_NEVERCHECK may have been assigned already during the main
  // analysis phase, signifying a load from a known thread-local object.
  ...
  if (SI.readsMemoryDirectly()) {
    ...
    // Known that we must check when this block is reached from a loop
    // preheader? If so whether it is tentative from the latch is irrelevant.
    ...
    // This is a little awkwardly placed since expanded calls are not tentative
    // loads, but this way it's together with load instructions replacing an
    // unavailable object.
    replaceUnavailableObjects(SI, inLoopAnalyser);
...
// Entry point for discovering a fixed-point solution for a general loop
// iteration. For example, in a simple case we'll have to check a load in the
// loop header if it could be interfered with by concurrent thread action either
// in the preheader or in the latch block. commitDisabledHere notes that this
// context won't generate specialised code, so we can't emit checks, only note
// memory that needs checking in the future. secondPass indicates this is the
// second time walking this context so loop latch edges will already have an
// associated TLStore.
void IntegrationAttempt::findTentativeLoadsInUnboundedLoop(
    const ShadowLoopInvar *UL, bool commitDisabledHere, bool secondPass) {
  ...
  if (!edgeIsDead(getBBInvar(UL->latchIdx), getBBInvar(UL->headerIdx))) {
    ...
    if (!secondPass) {
      // Passing true for the last parameter causes the store to be given to the
      // header from the latch and not to any exit blocks.
    ...
    // Latch edge has been killed, despite this being a general iteration. One
    // pass will suffice.
    findTentativeLoadsInLoop(UL, commitDisabledHere, secondPass);
  }
}

// Find tentative loads within loop L. commitDisabledHere and secondPass have
// the same meaning as above; latchToHeader indicates we should retain TLStores
// at each loop latch edge for consumption during the second pass.
// (!secondPass) does not necessarily imply latchToHeader, e.g. in the case
// where a latch edge is dead. This will recurse into each child context as it
// is encountered.
void IntegrationAttempt::findTentativeLoadsInLoop(const ShadowLoopInvar *L,
                                                  bool commitDisabledHere,
                                                  bool secondPass,
                                                  bool latchToHeader) {
  ...
  for (uint32_t i = startIdx, ilim = nBBs + BBsOffset;
       i != ilim && ((!L) || L->contains(getBBInvar(i)->naturalScope)); ++i) {
    ...
    if (BB->invar->naturalScope != L) {
      ...
      PeelAttempt *LPA;
      // If we have a child context and have established the loop's iteration
      // count:
      if ((LPA = getPeelAttempt(BB->invar->naturalScope)) &&
          LPA->isTerminated()) {

        // Loop header gets its TLStore from the preheader:
        LPA->Iterations[0]->BBs[0]->tlStore =
            getBB(NewLInfo->preheaderIdx)->tlStore;
        ...
        for (uint32_t j = 0, jlim = LPA->Iterations.size(); j != jlim; ++j) {
          ...
          LPA->Iterations[j]->findTentativeLoadsInLoop(
              BB->invar->naturalScope, commitDisabled, secondPass);
          // Pass the TLStore over the latch edge:
          if (j + 1 != jlim)
            LPA->Iterations[j + 1]->BBs[0]->tlStore =
                LPA->Iterations[j]->getBB(latchIdx)->tlStore;
        }

      } else {

        // Loop is unbounded:
        findTentativeLoadsInUnboundedLoop(
            BB->invar->naturalScope,
            commitDisabledHere ||
                (LPA && !LPA->isEnabled()), // Is commit disabled for the child?
            secondPass);
      }

      // Advance 'i' past the loop:
      while (i != ilim &&
             BB->invar->naturalScope->contains(getBBInvar(i)->naturalScope))
        ++i;
      --i;
      continue;
    }
    ...
    for (uint32_t j = 0, jlim = BB->invar->insts.size(); j != jlim; ++j) {
      ...
      if (InlineAttempt *IA = getInlineAttempt(&SI)) {
        ...
        IA->findTentativeLoads(commitDisabledHere || !IA->isEnabled(),
                               secondPass);
    ...
    // Give a store copy to each successor block that needs it. If latchToHeader
    // is true, ignore branches to outside the current loop; otherwise ignore
    // any latch->header edge.
    ...
    for (uint32_t i = 0; i < BB->invar->succIdxs.size(); ++i) {
      ...
      if (L) {
        ...
        if (L != this->L && latchToHeader &&
            !L->contains(SuccBBI->naturalScope))
          continue;
        else if (L != this->L && (!latchToHeader) &&
                 SuccBBI->idx == L->headerIdx) {
    ...
    // Return blocks need to keep their references until they're consumed by
    // doTLCallMerge.
...
// Stats interface -- count instructions that need a runtime check. Recurse into
// uncommitted child contexts.
void IntegrationAttempt::countTentativeInstructions() {
  ...
  for (uint32_t i = BBsOffset, ilim = BBsOffset + nBBs; i != ilim; ++i) {
    ...
    for (uint32_t j = 0, jlim = BBI->insts.size(); j != jlim; ++j) {
      ...
      // This should count only instructions that are checked because their
      // result might be invalidated by the concurrent action of other threads
      // in the same address space. Instructions with SI->needsRuntimeCheck set
      // are checked to implement a path condition or other check and so should
      // not be included in the count.

      if (requiresRuntimeCheck2(ShadowValue(SI), false) &&
          SI->needsRuntimeCheck == RUNTIME_CHECK_NONE)
  ...
  for (DenseMap<const ShadowLoopInvar *, PeelAttempt *>::iterator
    ...
    for (uint32_t i = 0, ilim = it->second->Iterations.size(); i != ilim; ++i) {
      ...
      checkedInstructionsChildren +=
          it->second->Iterations[i]->checkedInstructionsChildren;
    }
  }
}
...
bool IntegrationAttempt::containsTentativeLoads() { return readsTentativeData; }
...
bool IntegrationAttempt::requiresRuntimeCheck2(ShadowValue V,
  ...
  // Check introduced by a user assertion, or by checking that malloc doesn't
  // fail at runtime?
  if (SI->needsRuntimeCheck == RUNTIME_CHECK_AS_EXPECTED)
    return true;

  // Check introduced by the possibility of concurrent file alteration, or a
  // read from a FIFO which might vary from our expectations?
  if (includeSpecialChecks &&
  ...
  if (inst_is<MemTransferInst>(SI) ||
      ((!inst_is<CallInst>(SI)) && SI->readsMemoryDirectly())) {

    // Check introduced by the TL pass finding that this memory-reading
    // instruction's result may be subject to interference by concurrent code?
    if (SI->isThreadLocal == TLS_MUSTCHECK)
      return true;
    ...
    // but we're not emitting a specialised version and so its return value may
    // be questionable?
    ...
    // Similarly, check that phi nodes escaping from a child loop context have
    // their expected results when we'd like to make checks during the loop but
    // can't because we're not emitting a specialised loop body?
    ...
    for (uint32_t i = 0, ilim = BB->invar->predIdxs.size(); i != ilim; ++i) {
      ...
      if (predBBI->naturalScope != L &&
        ...
        PeelAttempt *LPA =
            getPeelAttempt(immediateChildLoop(L, predBBI->naturalScope));
        if (LPA && LPA->isTerminated() && (!LPA->isEnabled()) &&
            LPA->containsTentativeLoads())
          return !SI->i.PB->isWhollyUnknown();
      }
    }
  }
  ...
  for (uint32_t i = BBsOffset, ilim = BBsOffset + nBBs; i != ilim; ++i) {
    ...
    if (BBI->naturalScope != L) {
      ...
      if ((LPA = getPeelAttempt(subL)) && LPA->isTerminated() &&
        ...
        // We'll emit specialised code per iteration. Check for spec-to-unspec
        // edges in each such iteration.
    ...
    for (uint32_t j = 0, jlim = BBI->insts.size(); j != jlim; ++j) {
      ...
      if (requiresRuntimeCheck2(ShadowValue(SI), false) ||
        ...
        // Loop exit PHIs that need testing will be tested after the PHIs are
        // all evaluated, rather than introduce a branch after each one.
        if (inst_is<PHINode>(SI) && (j + 1) != jlim &&
            inst_is<PHINode>(&BB->insts[j + 1]))
          continue;

        // Note that we need unspecialised block variants available from this
        // instruction's successor onwards. Invoke instruction? (Only an invoke
        // could be a terminator, and also produce a value that needs checking)
        if (j == jlim - 1)
          getFunctionRoot()->markBlockAndSuccsReachableUnspecialised(
        ...
        // Special checks *precede* the instruction, hence 'j' vs. 'j + 1'
        // above.
        ...
        // If the call may bail to unspecialised code internally we'll need to
        // bail ourselves after it returns.
        if (IA->hasFailedReturnPath()) {

          // If this is the block terminator then it must be an invoke
          // instruction, the only kind of terminator that produces a checkable
          // value. If it is an invoke, mark the normal continuation reachable
          // on failure.
          if (j == jlim - 1)
            getFunctionRoot()->markBlockAndSuccsReachableUnspecialised(
                BB->invar->succIdxs[0], 0);
          else
            getFunctionRoot()->markBlockAndSuccsReachableUnspecialised(i,
                                                                       j + 1);
        }
      }
    }
...
// We've decided that context IA, called from SI, will not have a specialised
// variant in the final program. Rerun tentative load analysis now that we know
// we won't be able to emit any inline checks in specialised code.
void llvm::rerunTentativeLoads(ShadowInstruction *SI, InlineAttempt *IA,
                               bool inLoopAnalyser) {

  // This indicates the call never returns, so there's nobody to consume a
  // TLStore.
  ...
  if (IA->readsTentativeData) {
    ...
    // There may have been thread interference during the function, and/or it
    // may have read data that needed checking from prior interference and may
    // have used it, unchecked, to calculate its return value or store values to
    // memory. Everything needs checking at this point.
    errs() << "Warning: disabled context " << IA->SeqNumber
           << " reads tentative information\n";
    SI->parent->tlStore = SI->parent->tlStore->getEmptyMap();
    SI->parent->tlStore->allOthersClobbered = true;
    IA->backupTlStore->dropReference();

    // Note that if IA returns a pointer we won't be able to directly refer to
    // its allocation site.
    if (IA->returnValue)
      SI->parent->IA->squashUnavailableObjects(*SI, IA->returnValue,
                                               inLoopAnalyser);
