...
// This file describes the modified and referenced parameters of
// various Linux syscalls.  If you want to generalise to another
// kernel or treat the libc interface like the syscall interface (for
// example) you'll want to factor this out.

// Functions to retrieve mod-ref locations that depend on other call
// parameters, e.g. the 'read' syscall gives the read extent as
// another parameter.
...
static void getErrno(ShadowValue CS, ShadowValue &V, uint64_t &Size) {
  ...
  if (GlobalVariable *GV = cast<CallInst>(CS.getBareVal())
                               ->getParent()
                               ->getParent()
                               ->getParent()
                               ->getGlobalVariable("errno", true)) {
    V = ShadowValue(GV);
    Size = MemoryLocation::UnknownSize;
  }
}
...
// Either:
// { getBufferFunc, _, _ }, where getBufferFunc retrieves the buffer and size
// during specialisation Or: { NULL, argument number, buffer size }
...
// Sized parameters, These read/modify a particular argument size. This could
// need fixing if there's a chance we're building against a different struct
// than the kernel or alignment, pointer-size differ etc.
...
// Begin function descriptions. These are null-terminated lists of
// IHPLocationMRInfos.

static IHPLocationMRInfo JustErrno[] = {{&locErrno}, {0}};

static IHPLocationMRInfo ReadMR[] = {{&locErrno}, {&locReadBuf}, {0}};

static IHPLocationMRInfo ReallocMR[] = {{&locArg0}, {&locReturnVal}, {0}};

static IHPLocationMRInfo MallocMR[] = {{&locReturnVal}, {0}};

static IHPLocationMRInfo TCGETSMR[] = {{&locErrno}, {&locTermios}, {0}};

static IHPLocationMRInfo Arg1AndErrnoMR[] = {{&locErrno}, {&locArg1}, {0}};

static IHPLocationMRInfo GettimeofdayMR[] = {
    {&locErrno}, {&locArg0}, {&locArg1}, {0}};

static IHPLocationMRInfo Arg0AndErrnoMR[] = {{&locErrno}, {&locArg0}, {0}};
...
static IHPLocationMRInfo AcceptMR[] = {
    ...
    {&locErrno}, {&locArg1}, {&locSocklenArg2}, {0}

};

static IHPLocationMRInfo PollMR[] = {

    {&locErrno}, {&locPollFds}, {0}

};

static IHPLocationMRInfo NanosleepMR[] = {

    {&locErrno}, {&locTimespecArg1}, {0}
...
// This isn't very general, since TCGETS etc can alias other ioctls with
...
// that depends on the current specialisation state, currently only used for
// 'ioctl'.
...
// Populate tables relating Function* to mod-ref info, instead of looking up by
// name every time.
...
IHPFunctionInfo *LLPEAnalysisPass::getMRInfo(Function *F) {
  ...
  DenseMap<Function *, IHPFunctionInfo>::iterator findit =
      functionMRInfo.find(F);
