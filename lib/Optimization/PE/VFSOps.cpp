...
// This file contains functions that propagate a symbolic FD table through the
// program and evaluate open, read and other operations with respect to it. The
// table is propagated from block to block, forked and joined with control flow
// that isn't straightened at specialisation time, and so on.

using namespace llvm;

// Should we eliminate checks against file modification which appear to be
// redundant as there are no intervening thread yield points? This is not
// totally safe as e.g. ptrace or polling /proc to look at the file position
// could enable an observer to tell the difference between a specialised program
// that only checks for file modifications and the original unspecialised
// program.
...
// Attempt to retrieve a constant string from Ptr, using the symbolic store as
// of SearchFrom if necessary. Used to get the filename argument for 'open' et
// al.
bool IntegrationAttempt::getConstantString(ShadowValue Ptr,
                                           ShadowInstruction *SearchFrom,
                                           std::string &Result) {

  StringRef RResult;
  if (Ptr.isVal() &&
      getConstantStringInfo(cast<Constant>(Ptr.getVal()), RResult)) {
    Result = RResult.str();
    return true;
  }
  ...
  for (; success; ++StrOffset) {
    ...
    if (CGInit)
      ...
      readValRange(StrBase, StrOffset, 1, SearchFrom->parent, byte, 0,
                   0 /* fwdError */);

    if (byte.Overdef || byte.SetType != ValSetTypeScalar ||
        byte.Values.size() != 1) {
...
// Merge mergeFrom into mergeTo, e.g. at top of a basic block with two
// predecessors.
void FDStoreMerger::merge2(FDStore *mergeTo, FDStore *mergeFrom) {

  // Simple merge rule: FDs only defined on one path or the other go away
  // entirely, FDs with conflicting positions go to pos -1 (unknown), all others
  // stay.
  ...
  for (uint32_t i = 0, ilim = mergeTo->fds.size(); i != ilim; ++i) {
    ...
    // 'clean' means we're confident that FD positions and files are as
    // expected; there's no need to check they're as expected e.g. due to
    // another thread using the FD in the meantime, or another thread or program
    // altering the file.
...
void FDStoreMerger::doMerge() {
  ...
  typename SmallVector<FDStore *, 4>::iterator uniqend =
  ...
  if (std::distance(incomingStores.begin(), uniqend) > 1) {
    ...
    // target.
    for (typename SmallVector<FDStore *, 4>::iterator it =
    ...
    for (SmallVector<FDStore *, 4>::iterator it = firstMergeFrom; it != uniqend;
         ++it) {
...
// Merge FD tables after a call instruction in CallerBB that is specialised as
// CallIA.
...
// Simple hack to avoid obviously-pointless specialisation. Of course these
// could mount elsewhere and this should be configurable.
static bool filenameIsForbidden(std::string &s) {

  return s.empty() || s.find("/proc/") == 0 || s.find("/sys/") == 0 ||
         s.find("/dev/") == 0;
}

// Constructors for an object describing a file or fifo which is at some point
// read during specialisation.
FDGlobalState::FDGlobalState(ShadowInstruction *_SI, bool _isFifo)
    : SI(_SI), isCommitted(false), CommittedVal(0), isFifo(_isFifo) {}

FDGlobalState::FDGlobalState(bool _isFifo)
    : SI(0), isCommitted(false), CommittedVal(0), isFifo(_isFifo) {}

// Try to discover an 'open' call made by SI, and extract the filename, mode
// etc. If we do, update the local FD table.
bool IntegrationAttempt::tryPromoteOpenCall(ShadowInstruction *SI) {
  ...
  if (Function *SysOpen = F.getParent()->getFunction("open")) {
    ...
    if (FT->getNumParams() == 2 && FT->getReturnType()->isIntegerTy(32) &&
        ...
        FT->getParamType(1)->isIntegerTy(32) && FT->isVarArg()) {
      ...
      if (Function *FCalled = getCalledFunction(SI)) {
        ...
        if (FCalled == SysOpen) {
          ...
          uint64_t RawMode64;
          if (tryGetConstantIntReplacement(SI->getCallArgOperand(1),
                                           RawMode64)) {
            int RawMode = (int)RawMode64;
            if (RawMode & O_WRONLY) {
              LPDEBUG("Can't promote open call "
                      << itcache(SI) << " because it is not O_RDONLY\n");
              return true;
            }
          } else {
            LPDEBUG("Can't promote open call "
                    << itcache(SI)
                    << " because its mode argument can't be resolved\n");
            return true;
          }
          ...
          if (!getConstantString(NameArg, SI, Filename)) {
            LPDEBUG("Can't promote open call "
                    << itcache(SI)
                    << " because its filename argument is unresolved\n");
            return true;
          }
          ...
          if (exists) {
            ...
            cast<ImprovedValSetSingle>(SI->i.PB)->set(
                ImprovedVal(ShadowValue::getFdIdx(newId)), ValSetTypeFD);

            LPDEBUG("Successfully promoted open of file "
                    << Filename << ": queueing initial forward attempt\n");

          } else {
            Constant *negOne =
                ConstantInt::get(SI->invar->I->getType(), (uint64_t)-1, true);
            cast<ImprovedValSetSingle>(SI->i.PB)->set(
                ImprovedVal(ShadowValue(negOne)), ValSetTypeScalar);
            LPDEBUG("Open of " << Filename << " returning ENOENT\n");
          }

          // Can't share functions that open() or we'll confuse the two open
          // points.
          noteVFSOp();
          ...
          LPDEBUG("Unable to identify "
                  << itcache(SI)
                  << " as an open call because it calls something else\n");
        }
        ...
        LPDEBUG("Unable to identify "
                << itcache(SI)
                << " as an open call because its target is unknown\n");
      }
      ...
      LPDEBUG("Unable to identify "
              << itcache(SI)
              << " as an open call because the symbol 'open' resolves to "
                 "something with inappropriate type!\n");
    }
    ...
    LPDEBUG("Unable to identify "
            << itcache(SI)
            << " as an open call because no symbol 'open' is in scope\n");
  }

  return false;
}

// Check if V is a well-known-constant FD (just stdin for now) or is known to
// point to a symbolic FD.
...
// Add 'Filename' to the list of files we've consumed from in generating the
// specialised program, and therefore which must be watched for concurrent
// alteration to ensure correctness.
void llvm::noteLLIODependency(std::string &Filename) {

  std::vector<std::string>::iterator findit =
      std::find(GlobalIHP->llioDependentFiles.begin(),
                GlobalIHP->llioDependentFiles.end(), Filename);

  if (findit == GlobalIHP->llioDependentFiles.end())
    GlobalIHP->llioDependentFiles.push_back(Filename);
}

// Try to run '[f]stat' call SI, which calls F, and investigates file
// 'Filename'.
bool IntegrationAttempt::executeStatCall(ShadowInstruction *SI, Function *F,
                                         std::string &Filename) {
...
// Try to find a VFS function call made by SI, and if we find one try to execute
// it at specialisation time. Return value: is this a VFS call (regardless of
// whether we resolved it successfully)
bool IntegrationAttempt::tryResolveVFSCall(ShadowInstruction *SI) {
  ...
  if (!(F->getName() == "read" || F->getName() == "llseek" ||
        F->getName() == "lseek" || F->getName() == "lseek64" ||
        F->getName() == "close" || F->getName() == "stat" ||
  ...
  if (F->getName() == "stat") {
    ...
    // TODO: Add LF resolution code notifying file size. All users so far have
    // just used stat as an existence test. Similarly set errno = ENOENT as
    // appropriate.

    // Return false in all cases so that we fall through to handleUnexpandedCall
    // and clobber the stat buffer.

    std::string Filename;
    if (!getConstantString(SI->getCallArgOperand(0), SI, Filename)) {
      LPDEBUG("Can't resolve stat call "
              << itcache(SI)
              << " because its filename argument is unresolved\n");
      return false;
    }
  ...
  bool perturbsFDs = F->getName() == "read" || F->getName() == "llseek" ||
                     F->getName() == "lseek" || F->getName() == "lseek64";
  ...
  if (F->getName() == "isatty") {
    ...
    // FD 0 is stdin which may or may not be terminal; no other symbolic FD can
    // currently be a tty.
  ...
  } else if (F->getName() == "llseek" || F->getName() == "lseek" ||
             F->getName() == "lseek64") {
    ...
    if ((!tryGetConstantIntReplacement(SI->getCallArgOperand(2),
                                       seekWhence64)) ||
    ...
    if (cBytes > bytesAvail) {
      LPDEBUG("Desired read of " << cBytes << " truncated to " << bytesAvail
                                 << " (EOF)\n");
      cBytes = bytesAvail;
    }

    LPDEBUG("Successfully resolved " << itcache(SI) << " which reads " << cBytes
                                     << " bytes\n");
    ...
    setReplacement(SI,
                   ConstantInt::get(Type::getInt64Ty(F->getContext()), cBytes));
...
ReadFile *IntegrationAttempt::tryGetReadFile(ShadowInstruction *CI) {
  ...
  DenseMap<ShadowInstruction *, ReadFile>::iterator it =
      pass->resolvedReadCalls.find(CI);
...
WalkInstructionResult
IntegrationAttempt::isVfsCallUsingFD(ShadowInstruction *VFSCall,
                                     ShadowInstruction *FD, bool ignoreClose) {
  ...
  if (CalleeName == "read") {
    ...
    case AliasResult::MayAlias:
      LPDEBUG("Can't resolve VFS call because FD argument of "
              << itcache(VFSCall) << " is unresolved\n");
      return WIRStopWholeWalk;
    case AliasResult::NoAlias:
      LPDEBUG("Ignoring " << itcache(VFSCall)
                          << " which references a different file\n");
      return WIRContinue;
    ...
    // Finding this indicates we could double-close if this path were followed
    // for real! Walk through it to find its predecessors. If we're walking
    // forwards this is a chain ender.
    return ignoreClose ? WIRContinue : WIRStopThisPath;

  } else if (CalleeName == "llseek" || CalleeName == "lseek" ||
             CalleeName == "lseek64") {
...
void IntegrationAttempt::resolveReadCall(ShadowInstruction *CI,
                                         struct ReadFile RF) {

  pass->resolvedReadCalls[CI] = RF;
}

void IntegrationAttempt::resolveSeekCall(ShadowInstruction *CI,
                                         struct SeekFile SF) {
...
bool IntegrationAttempt::isResolvedVFSCall(ShadowInstruction *I) {
  ...
  if (inst_is<CallInst>(I)) {
    ...
    return pass->forwardableOpenCalls.count(I) ||
           pass->resolvedReadCalls.count(I) || pass->resolvedSeekCalls.count(I);
  }
...
bool IntegrationAttempt::VFSCallWillUseFD(ShadowInstruction *CI) {
...

  {
    DenseMap<ShadowInstruction *, SeekFile>::iterator it =
        pass->resolvedSeekCalls.find(CI);
...
bool IntegrationAttempt::isUnusedReadCall(ShadowInstruction *CI) {
  ...
  // Return true if CI is a read instruction that won't be in the final
  // committed program: this is true if it's zero-length or if there are no live
  // users of the buffer it writes.
  DenseMap<ShadowInstruction *, ReadFile>::iterator it =
      pass->resolvedReadCalls.find(CI);
  if (it != pass->resolvedReadCalls.end()) {
...
//// Implement a CFG walker that determines whether a seek call can be elim'd or
///a read call's / implied SEEK_CUR can be omitted when residualising.

// This all ought to be implemented as part of the main walk, a la dead-store
// elimination, since the seek calls we're removing are akin to stores that are
// provably overwritten. However for now a separate walker does the trick most
// of the time.
...
public:
  ...
  SeekInstructionUnusedWalker(ShadowInstruction *_FD, ShadowInstruction *Start)
      : ForwardIAWalker(Start->invar->idx, Start->parent, true), FD(_FD),
        seekNeeded(false) {}
...
static bool callMayUseFD(ShadowInstruction *SI, ShadowInstruction *FD) {
  ...
  // This call cannot affect the FD we're pursuing unless (a) it uses the FD, or
  // (b) the FD escapes (is stored) and the function is non-pure.

  Function *calledF = getCalledFunction(SI);

  // None of the blacklisted syscalls not accounted for under vfsCallBlocksOpen
  // mess with FDs in a way that's important to us.
...
bool SeekInstructionUnusedWalker::blockedByUnexpandedCall(ShadowInstruction *SI,
                                                          void *) {
...
WalkInstructionResult
SeekInstructionUnusedWalker::walkInstruction(ShadowInstruction *I, void *) {
  ...
  if (WIR == WIRContinue)
    ...
    if (!IA->isResolvedVFSCall(I)) {
      ...
      // But apparently we couldn't understand it. Perhaps some of its arguments
      // are vague.
      seekNeeded = true;
      return WIRStopWholeWalk;
      ...
      // SuccessorInstructions is currently not used, as isResolvedVFSCall tells
      // us enough: that the Seek's successor on this path is executed at
      // specialisation time and will not be residualised.
...
// Identify VFS operations that can be eliminated from the final program because
// their effects would not be used. For example, if an 'lseek' call will
// definitely be followed by another SEEK_CUR on all paths it can be omitted.
void IntegrationAttempt::tryKillAllVFSOps() {
  ...
  for (uint32_t i = 0, ilim = nBBs; i != ilim; ++i) {
    ...
    for (uint32_t j = 0, jlim = BB->insts.size(); j != jlim; ++j) {
      ...
      if (inst_is<CallInst>(SI)) {
        ...
        {
          // Even if a read call is executed during specialisation, do we need
          // to seek the FD so that subsequent syscalls see the file position
          // where they expect it?
          DenseMap<ShadowInstruction *, ReadFile>::iterator it =
              pass->resolvedReadCalls.find(SI);
          if (it != pass->resolvedReadCalls.end()) {
        ...
        {
          // Does the seek call still need to exist in the final program when
          // its successor 'read' calls may have been executed already?
          DenseMap<ShadowInstruction *, SeekFile>::iterator it =
              pass->resolvedSeekCalls.find(SI);
          if (it != pass->resolvedSeekCalls.end()) {
            int32_t FD = getFD(SI->getCallArgOperand(0));
            if (FD == -1)
              continue;
  ...
  for (DenseMap<const ShadowLoopInvar *, PeelAttempt *>::const_iterator
...
// Read strFileName[realFilePos : realFilePos + realBytes] as an array of i8
// typed Constants. 'errors' will carry a verbose error report. Return true on
// success.
bool llvm::getFileBytes(std::string &strFileName, uint64_t realFilePos,
