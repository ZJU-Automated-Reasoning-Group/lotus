...
// File name is historical -- these functions used to be called from
// LLVM core code, back when we were hacking core passes rather than
// re-implement part of their logic internally. In due course these
...
// Convert a Value into an ImprovedVal. Simple constants are easy; the
// tricky it is looking through complex constant expressions involving
// pointers -- for example, turning &(G[4]) into an appropriate
// symbolic object and offset.
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(C)) {
    ...
    case Instruction::IntToPtr: {
      ...
      if (CastVal.first == ValSetTypePB)
        ...
        if (ConstantExpr *SubCE = dyn_cast_or_null<ConstantExpr>(
                getSingleConstant(CastVal.second.V))) {
          if (SubCE->getOpcode() == Instruction::PtrToInt)
            return std::make_pair(ValSetTypeScalar,
                                  ImprovedVal(SubCE->getOperand(0)));
        }
        return std::make_pair(ValSetTypeScalar,
                              ImprovedVal(ConstantExpr::getIntToPtr(
    ...
    case Instruction::PtrToInt: {
      ...
      if (CastVal.first == ValSetTypePB)
        ...
        return std::make_pair(ValSetTypeScalar,
                              ImprovedVal(ConstantExpr::getPtrToInt(
                                  CE->getOperand(0), CE->getType())));
      } else
        return std::make_pair(ValSetTypeUnknown, ImprovedVal());
    }
...

    {
      ...
      for (User::op_iterator I = GEP->idx_begin(), E = GEP->idx_end(); I != E;
        ...
        if (StructType *STy = GTI.getStructTypeOrNull()) {
          Offset += GlobalTD->getStructLayout(STy)->getElementOffset(
              OpC->getZExtValue());
        } else {
      ...
      return std::make_pair(
          ValSetTypePB,
          ImprovedVal(BasePB.second.V, BasePB.second.Offset + Offset));
    }
    ...
    case Instruction::Sub: {
      ...
      if (Op1.first == ValSetTypeScalar && Op2.first == ValSetTypeScalar) {
        ...
        if (CE->getOpcode() == Instruction::Add)
          return std::make_pair(ValSetTypeScalar,
                                ImprovedVal(ConstantExpr::getAdd(
                                    getSingleConstant(Op1.second.V),
                                    getSingleConstant(Op2.second.V))));
        else
          return std::make_pair(ValSetTypeScalar,
                                ImprovedVal(ConstantExpr::getSub(
                                    getSingleConstant(Op1.second.V),
                                    getSingleConstant(Op2.second.V))));
      }
      ...
      if (CE->getOpcode() == Instruction::Add) {
        ...
        if (tryGetConstantInt(Op2.second.V, Op2C))
          return std::make_pair(
              ValSetTypePB,
              ImprovedVal(Op1.second.V, Op1.second.Offset + Op2C));
        else
          return std::make_pair(ValSetTypePB,
                                ImprovedVal(Op1.second.V, LLONG_MAX));
          ...
          if (Op2.first == ValSetTypePB) {
            ...
            if (Op1.second.V == Op2.second.V) {
              ...
              if (Op1.second.Offset == LLONG_MAX ||
                  Op2.second.Offset == LLONG_MAX)
                return std::make_pair(ValSetTypeUnknown, ShadowValue());
              else
                return std::make_pair(
                    ValSetTypeScalar,
                    ImprovedVal(ShadowValue(ConstantInt::get(
                        Type::getInt64Ty(CE->getContext()),
    ...
    case Instruction::And: {
      ...
      if (AndConst < GlobalAlign) {
        ...
        return std::make_pair(ValSetTypeScalar,
                              ShadowValue::getInt(CE->getType(), Result));

      } else if (AndConstSigned < 0 &&
                 (-AndConstSigned) <= (int64_t)GlobalAlign) {

        // Masking offset, pointer bits unaffected
        return std::make_pair(
            ValSetTypePB,
            ImprovedVal(Op1.second.V, Op1.second.Offset & AndConst));
    ...
    if (isa<Function>(C))
      ...
      return std::make_pair(
          ValSetTypePB,
          ImprovedVal(
              &(GlobalIHP->shadowGlobals[GlobalIHP->getShadowGlobalIndex(GV)]),
              0));
    else
    ...
    return std::make_pair(
        ValSetTypeScalar,
        ShadowValue::getInt(CI->getType(), CI->getLimitedValue()));
...
// Find a Function corresponding to CCalledV. This might be a constant, or might
// be an indirect function call that has a known target in context.
    ...
    if (getImprovedValSetSingle(CalledV, PB)) {
      ...
      if (!PB.Overdef) {
        ...
        for (unsigned i = 0; i < PB.Values.size(); ++i) {
          ...
          Constant *ThisVal =
              dyn_cast_or_null<Constant>(PB.Values[i].V.getVal());
          if (!ThisVal) {
            OnlyVal = 0;
            break;
          }
...
Function *llvm::getCalledFunction(ShadowInstruction *SI) {
  ...
  if (inst_is<CallInst>(SI))
    ...
    release_assert(0 &&
                   "getCalledFunction called on non-call, non-invoke inst");

  // Shouldn't usually happen, but isCopyInst() called from the DOT printer
  // can run into this situation when drawing in-loop blocks from an outer
  // context.
  if (Op.isInval())
    return 0;
...
void llvm::printPB(raw_ostream &out, ImprovedValSetSingle PB, bool brief) {
  ...
  if (PB.Overdef)
    ...
    out << "{ ";
    for (SmallVector<ImprovedVal, 4>::const_iterator it = PB.Values.begin(),
                                                     it2 = PB.Values.end();
         it != it2; ++it) {
