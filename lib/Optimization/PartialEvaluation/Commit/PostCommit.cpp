...
// These optimisations fold a committed residual-code function into a neater
// form. We do this as we go because in certain cases it can dramatically reduce
// the amount of useless code emitted and thus improve the size of program we
// can deal with.
...
static BasicBlock *getUniqueSuccessor(BasicBlock *BB) {
  ...
  // We might run on blocks without a terminator when contexts are not yet
  // committed.
...
void postCommitOptimiseBlocks(T itstart, T itend, Callback &CB,
  ...
  for (std::vector<Instruction *>::iterator Delit = Del.begin(),
                                            Delend = Del.end();
       Delit != Delend; ++Delit)
  ...
  for (std::vector<std::vector<BasicBlock *>>::iterator
           chainit = Chains.begin(),
           itend = Chains.end();
       chainit != itend; ++chainit) {
...
struct PCOBBsCB {
  ...
  void willReplace(BasicBlock *Old) { isEntryBlock = Old == IA->entryBlock; }
...
struct DerefAdaptor {
  ...
  operator BasicBlock *() { return *inner; }
};

// Main post-commit optimisation entry point. I'm not totally certain, but I
// think optimising a basic-block list that hasn't been inserted into a residual
// function yet has been disabled because some LLVM core functions fail if
// BB->getParent() is null. Such blocks will be treated once they've been
// assigned a final function; trying to do them ahead of time was just to limit
// transient memory consumption anyhow.
...
void InlineAttempt::postCommitOptimise() {
  ...
  if (CommitF) {
    ...
    // Now top-sort the blocks, excluding the failed blocks by annotating them
    // 'visited' to start with.
    {

      SmallSet<BasicBlock *, 8> Visited;
      for (Function::iterator it = firstFailedBlock, itend = CommitF->end();
           it != itend; ++it)
        Visited.insert(&*it);
      ...
      for (std::vector<BasicBlock *>::iterator it = Ordered.begin(),
                                               itend = Ordered.end();
           it != itend; ++it) {

        // Splice this block before fit, fit moves forwards and continue,
        // or else fit is already in the right place, leave it and insert before
        // its successor.
    ...
    //    PCOBBsCB CB(this);
    //    postCommitOptimiseBlocks(DerefAdaptor(CommitBlocks.begin()),
    //    DerefAdaptor(CommitBlocks.end()), CB);
  }
}
