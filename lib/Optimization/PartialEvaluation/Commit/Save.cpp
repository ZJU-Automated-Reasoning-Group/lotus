...
// These functions relate to committing the specialised program as concrete
// bitcode. ConditionalSpec.cpp implements the aspects of commit which insert
// unspecialised function fragments for the case where specialisation
// assumptions are found to be false at runtime.
...
#include "Optimization/PartialEvaluation/LLPECopyPaste.h"
...
// Name the output basic blocks for easier debugging? Significantly increases
// output size.
...
// Prepare for the commit: remove instruction mappings that are (a) invalid to
// write to the final program and (b) difficult to reason about once the loop
// structures start to be modified by unrolling and so on.
...
void IntegrationAttempt::prepareCommit() {
  ...
  for (IAIterator it = child_calls_begin(this), it2 = child_calls_end(this);
       it != it2; ++it) {
  ...
  for (DenseMap<const ShadowLoopInvar *, PeelAttempt *>::iterator
           it = peelChildren.begin(),
           it2 = peelChildren.end();
       it != it2; ++it) {

    unsigned iterCount = it->second->Iterations.size();
    unsigned iterLimit =
        (it->second->Iterations.back()->iterStatus == IterationStatusFinal)
            ? iterCount
            : iterCount - 1;
    ...
    if (!it->second->isEnabled()) {
      ...
      if (it->second->isTerminated()) {
        ...
        // Loop hasn't been analysed for the general case -- do a rough and
        // ready approximation that emits any edge that is alive in any
        // iteration.

        const ShadowLoopInvar *LInfo = it->second->L;
        for (uint32_t i = LInfo->headerIdx;
             i < nBBs && it->first->contains(getBBInvar(i)->naturalScope);
             ++i) {
          ...
          for (uint32_t j = 0, jlim = BBI->succIdxs.size(); j != jlim; ++j) {
            ...
            BB->succsAlive[j] =
                !edgeIsDeadRising(*BBI, *getBBInvar(BBI->succIdxs[j]), true);
          }
        }
      }
...
// Assign basic blocks a name like f-N, where f is the source function name and
// N is a sequential identifier also visible in the GUI.
...
std::string PeelIteration::getCommittedBlockPrefix() {
  ...
  {
    raw_string_ostream RSO(ret);
    RSO << F.getName() << "-L" << getLName() << "-I" << iterationCount << "-"
        << SeqNumber << " ";
  }
  return ret;
}

// Create an empty function which has the same attributes, linkage etc as F. If
// addFailedReturnFlag is set, change the existing return type T into { T, i1 }
// (or just i1 if void), where the new boolean return will be used to indicate
// whether this function wants to bail out to unspecialised code.
Function *llvm::cloneEmptyFunction(Function *F, GlobalValue::LinkageTypes LT,
                                   const Twine &Name,
                                   bool addFailedReturnFlag) {
  ...
  if (addFailedReturnFlag) {
    ...
    if (OldReturn->isVoidTy()) {
      ...
      NewReturn =
          StructType::get(F->getContext(), ArrayRef<Type *>(NewReturnArgs, 2));
    }

    NewFType = FunctionType::get(
        NewReturn,
        ArrayRef<Type *>(&*NewFType->param_begin(), &*NewFType->param_end()),
        NewFType->isVarArg());
  }
  ...
  if (addFailedReturnFlag) {
    ...
    // The zeroext and signext attributes specify that codegen should expand
    // these to fill a register if it needs to satisfy the C ABI. Since cloned
    // functions (except for the root, whose type is never altered) are not
    // externally called, we don't care about the CC.

    // SRet is an ABI-compliance thing too, and is illegal when the return type
    // is not void.

    auto attrs = NewF->getAttributes();

    attrs = attrs.removeRetAttribute(F->getContext(), Attribute::ZExt);
    attrs = attrs.removeRetAttribute(F->getContext(), Attribute::SExt);
    if (NewFType->getNumParams() != 0)
      attrs =
          attrs.removeParamAttribute(F->getContext(), 0, Attribute::StructRet);
...
// Return true if it will be necessary to insert code on the path leading from
// specialised to unspecialised code following a failed check that SI's result
// was as expected.
...
// Create a new specialised basic block. If AddF is specified, put it in that
// function; otherwise note it as a BB that contains specialised code / is a
// clone of an unspecialised block in the original program (according to
// isFailedBlock); it will get added to a function later. If isEntryBlock is
// set, ensure it is at the front of the block list.
BasicBlock *IntegrationAttempt::createBasicBlock(LLVMContext &Ctx,
                                                 const Twine &Name,
                                                 Function *AddF,
                                                 bool isEntryBlock,
                                                 bool isFailedBlock) {
  ...
  BasicBlock *newBlock;
  if (AddTo && getFunctionRoot()->firstFailedBlock != AddTo->end() &&
      !isFailedBlock) {

    // Add block before any failed blocks
    newBlock = BasicBlock::Create(Ctx, Name, AddTo,
                                  &*(getFunctionRoot()->firstFailedBlock));

  } else {

    // Add block to end of function, or to no function if !AddTo
    newBlock = BasicBlock::Create(Ctx, Name, AddTo);
    if (AddTo && isFailedBlock &&
        getFunctionRoot()->firstFailedBlock == AddTo->end())
...
// If this function will be committed inline, create special blocks that
// correspond to the return path or the failed-specialisation-assumptions path
// to unspecialised code.
void InlineAttempt::commitCFG() {
  ...
  if (!commitsOutOfLine()) {
    ...
    returnBlock = createBasicBlock(
        F.getContext(), VerboseNames ? (StringRef(Pref) + "callexit") : "",
        CommitF, false, false);
    ...
    if (hasFailedReturnPath()) {
      ...
      failedReturnBlock =
          createBasicBlock(F.getContext(), PreName, CommitF, false, true);
...
void IntegrationAttempt::commitCFG() {
  ...
  for (uint32_t i = 0; i < nBBs; ++i) {
    ...
    if (currentLoop == L && BB->invar->naturalScope != L) {
      ...
      // Entering a loop. First write the blocks for each iteration that's being
      // unrolled:
      ...
      if (PA && PA->isEnabled() && PA->isTerminated()) {
        ...
        // Create failed blocks before the loop iterations, so they're available
        // as branch targets.
        for (unsigned j = i + 1;
             j != nBBs &&
             skipL->contains(getBBInvar(j + BBsOffset)->naturalScope);
             ++j)
        ...
        // If the loop has terminated, skip emitting specialised blocks in this
        // context.
        while (i < nBBs &&
               skipL->contains(getBBInvar(i + BBsOffset)->naturalScope))
          ++i;
        --i;
        continue;
      }
    ...
    BasicBlock *firstNewBlock = createBasicBlock(F.getContext(), Name, CF,
                                                 isCommittedEntryBlock, false);

    BB->committedBlocks.push_back(
        CommittedBlock(firstNewBlock, firstNewBlock, 0));
    if (isEntryBlock)
      getFunctionRoot()->entryBlock = firstNewBlock;

    // Create extra empty blocks for each path condition that's effective here:
    // If OmitChecks is specified, no tests are emitted and so no blocks are
    // needed.
    uint32_t nCondsHere =
        pass->omitChecks
            ? 0
            : pass->countPathConditionsAtBlockStart(BB->invar, BB->IA);

    for (uint32_t k = 0; k < nCondsHere; ++k) {

      if (pass->verbosePCs) {

        // The previous block will contain a path condition check: give it a
        // break block that will sit on the edge from specialised to
        // unspecialised code.
        ...
        BasicBlock *breakBlock =
            createBasicBlock(F.getContext(), BlockName, CF, false, true);
    ...
    if (BB->insts[0].needsRuntimeCheck == RUNTIME_CHECK_READ_LLIOWD &&
      ...
      if (pass->verbosePCs || requiresBreakCode(&BB->insts[0])) {
        ...
        BasicBlock *breakBlock =
            createBasicBlock(F.getContext(), BreakName, CF, false, true);
        BB->committedBlocks.back().breakBlock = breakBlock;
      }
    ...
    for (uint32_t j = 0; j < BB->insts.size(); ++j) {
      ...
      if (InlineAttempt *IA = getInlineAttempt(SI)) {
        ...
        if (IA->isEnabled()) {
          ...
          if (!IA->commitsOutOfLine()) {
            ...
            BB->committedBlocks.push_back(
                CommittedBlock(IA->returnBlock, IA->returnBlock, j + 1));
            ...
            if (IA->failedReturnBlock) {
              ...
              if (inst_is<CallInst>(SI))
                targetBlock = getFunctionRoot()->getSubBlockForInst(
                    BB->invar->idx, j + 1);
              else
                targetBlock = getFunctionRoot()->getSubBlockForInst(
                    BB->invar->succIdxs[1], 0);
              BranchInst::Create(targetBlock, IA->failedReturnBlock);
            }

          } else {

            // Requires a break afterwards if the target function might branch
            // onto a failed path. Invoke instructions are a bit special in this
            // respect, as the new block will only contain a failure check.
            if (IA->hasFailedReturnPath()) {

              BasicBlock *newBlock = createBasicBlock(
                  F.getContext(),
                  VerboseNames ? StringRef(Pref) + "OOL callexit" : "", CF,
                  false, false);
              BB->committedBlocks.push_back(
                  CommittedBlock(newBlock, newBlock, j + 1));
            }
          }
      ...
      // If we need a check *before* this instruction (at the moment only true
      // if it's a read or [f]stat call that will require an inline check) then
      // add a break.
      if (SI->needsRuntimeCheck == RUNTIME_CHECK_READ_LLIOWD &&
          !GlobalIHP->omitChecks) {

        if (j != 0) {

          if (pass->verbosePCs || requiresBreakCode(SI)) {

            BasicBlock *breakBlock = createBasicBlock(
                F.getContext(),
                VerboseNames ? StringRef(Name) + ".vfsbreak" : "", CF, false,
                true);
            BB->committedBlocks.back().breakBlock = breakBlock;
          }

          BasicBlock *newSpecBlock = createBasicBlock(
              F.getContext(), VerboseNames ? StringRef(Name) + ".vfspass" : "",
              CF, false, false);
          BB->committedBlocks.push_back(
              CommittedBlock(newSpecBlock, newSpecBlock, j + 1));
        }
      }

      // If we have a disabled call, exit phi for a disabled loop, or tentative
      // load then insert a break for a check. This path also handles path
      // conditions that are checked as they are defined, rather than at the top
      // of a block that may be remote from the definition site. Thankfully for
      // my sanity, having your return value checked and being committed as a
      // specialised call are mutually exclusive.
      if (requiresRuntimeCheck(SI, true) &&
          SI->needsRuntimeCheck != RUNTIME_CHECK_READ_LLIOWD) {

        if (j + 1 != BB->insts.size() && inst_is<PHINode>(SI) &&
            inst_is<PHINode>(&BB->insts[j + 1]))
          continue;
        ...
        if (pass->verbosePCs) {
          ...
          // The previous block will break due to a tentative load. Give it a
          // break block. For most kinds of break this should belong to the old
          // subblock; for invoke instructions it should belong to the new one
          // so that the old one's break edge doesn't have an intermediary block
          // and so can be used for the unwind edge.
          breakBlock = createBasicBlock(
              F.getContext(), VerboseNames ? StringRef(Name) + ".tlbreak" : "",
              CF, false, true);
        ...
        BasicBlock *newSpecBlock = createBasicBlock(
            F.getContext(), VerboseNames ? StringRef(Name) + ".checkpass" : "",
            CF, false, false);
        BB->committedBlocks.push_back(
            CommittedBlock(newSpecBlock, newSpecBlock, j + 1));
    ...
    // If the block has ignored edges outgoing, it will branch direct to
    // unspecialised code. Make a break block for that purpose.
    if (pass->verbosePCs && hasLiveIgnoredEdges(BB)) {

      BB->committedBlocks.back().breakBlock = createBasicBlock(
          F.getContext(), VerboseNames ? StringRef(Name) + ".directbreak" : "",
          CF, false, true);
    }
  }
}

// Convert a shadow-value to the value we should refer to in the committed
// program.
Value *IntegrationAttempt::getCommittedValue(ShadowValue SV) {
  ...
  case SHADOWVAL_GV:
    return SV.u.GV->G;
  case SHADOWVAL_INST: {
    release_assert(SV.u.I->committedVal &&
...
// Resolve a symbolic argument to a concrete value usable during commit. If
// we're committing out of line that just means an actual Argument; if we're
// committing inline it means the concrete value passed to the "call", or
// value-forwarding trickery if our parent hasn't been concretised yet.
...
Value *InlineAttempt::getArgCommittedValue2(ShadowArg *SA, BasicBlock *emitBB,
                                            Instruction *insertBefore) {
  ...
  if (commitsOutOfLine() || !isEnabled()) {
    ...
    if (!uniqueParent->commitStarted()) {
      ...
      // Inlined in place -- use the corresponding value of our call
      // instruction. For sharing to work all arg values must match, so just use
      // caller #0.
...
BasicBlock *InlineAttempt::getCommittedEntryBlock() { return entryBlock; }

// Get the succIdx'th committed successor of BB, which is complicated when edges
// branch out of a loop or into a region of a function that isn't considered for
// specialisation.
BasicBlock *PeelIteration::getSuccessorBB(ShadowBB *BB, uint32_t succIdx,
                                          bool &markUnreachable) {

  // Is this the latch->header edge?
  if (BB->invar->idx == parentPA->L->latchIdx &&
      succIdx == parentPA->L->headerIdx) {

    // Usually this will point to the next iteration's header. The second case
    // tries to cater for committing several specialised iterations followed by
    // a general iteration, but I think at the moment the specialised iterations
    // are discarded in this case (i.e. either the loop is unrolled either
    ...
    if (PeelIteration *PI = getNextIteration())
      ...
      if (iterStatus == IterationStatusFinal) {
        release_assert(pass->assumeEndsAfter(&F, getBBInvar(L->headerIdx)->BB,
                                             iterationCount) &&
...
// The tricky case here happens when the user has specified that particular
// paths are not interesting for specialisation and so we want to branch
// directly to an unspecialised variant.
BasicBlock *InlineAttempt::getSuccessorBB(ShadowBB *BB, uint32_t succIdx,
                                          bool &markUnreachable) {
...
// Fetch a successor committed-block for BB. This is tricky when we're entering
// or leaving loops, which means finding an appropriate block in a child or
// parent context.
BasicBlock *IntegrationAttempt::getSuccessorBB(ShadowBB *BB, uint32_t succIdx,
                                               bool &markUnreachable) {
  ...
  if ((!SuccBBI->naturalScope) || SuccBBI->naturalScope->contains(L))
    ...
    // Else, BBI is further in than this block: we must be entering exactly one
    // loop. Only enter if we're emitting the loop in its proper scope:
    // otherwise we're writing the residual version of a loop.
    if (BB->invar->outerScope == L) {
      PeelAttempt *PA;
      if ((PA = getPeelAttempt(SuccBBI->naturalScope)) && PA->isTerminated() &&
          PA->isEnabled())
        return PA->Iterations[0]
            ->getBB(*SuccBBI)
            ->committedBlocks.front()
            .specBlock;
    }
...
Constant *llvm::getConstAsType(Constant *C, Type *Ty) {
  ...
  (void)Op; // getCastOpcode validates; ConstantExpr::getCast will assert if
            // invalid
...
Value *llvm::getValAsType(Value *V, Type *Ty, BasicBlock *insertAtEnd) {
  ...
  Instruction::CastOps Op = CastInst::getCastOpcode(V, false, Ty, false);
  return CastInst::Create(Op, V, Ty, VerboseNames ? "speccast" : "",
                          insertAtEnd);
}
...
PHINode *llvm::makePHI(Type *Ty, const Twine &Name, BasicBlock *emitBB) {
  ...
  // Manually check for existing non-PHI instructions because BB->getFirstNonPHI
  // assumes a finished block
...
// Make a phi node corresponding to specialised instruction I, committing to
// emitBB, in the special loop-header case.
void PeelIteration::emitPHINode(ShadowBB *BB, ShadowInstruction *I,
                                BasicBlock *emitBB) {

  // Special case: emitting own header PHI. Emit a unary PHI drawing on either
  // the preheader argument or the latch one.
  ...
  if (BB->invar->idx == parentPA->L->headerIdx) {
    ...
    PHINode *NewPN;
    I->committedVal = NewPN =
        makePHI(I->invar->I->getType(), VerboseNames ? "header" : "", emitBB);
    ShadowBB *SourceBB;
    ...
    Value *PHIOp = getValAsType(
        getCommittedValue(SourceV), PN->getType(),
        SourceBB->committedBlocks.back().specBlock->getTerminator());
...
void IntegrationAttempt::getCommittedExitPHIOperands(
    ShadowInstruction *SI, uint32_t valOpIdx, SmallVector<ShadowValue, 1> &ops,
    SmallVector<ShadowBB *, 1> *BBs) {
  ...
  // SI->parent->invar->scope != L checks if we're emitting a PHI for a residual
  // loop body.
...
void IntegrationAttempt::populatePHINode(ShadowBB *BB, ShadowInstruction *I,
                                         PHINode *NewPN) {

  // There used to be a case here handling loops with one or more specialised
  // iterations but which were ultimately unbounded. I scrapped that because
  // it's too complicated handling the intermediate case between straightened
  // and fully general loops, but it's in git if you need it.
  ...
  for (uint32_t i = 0, ilim = I->invar->operandIdxs.size(); i != ilim; i++) {
    ...
    for (uint32_t j = 0; j < predValues.size(); ++j) {
      Value *PHIOp = getValAsType(
          getCommittedValue(predValues[j]), NewPN->getType(),
          predBBs[j]->committedBlocks.back().specBlock->getTerminator());
...
// Make a phi node corresponding to specialised instruction I, committing to
// emitBB.
void IntegrationAttempt::emitPHINode(ShadowBB *BB, ShadowInstruction *I,
                                     BasicBlock *emitBB) {
  ...
  // Make an empty node for the time being; this will be revisted once the loop
  // body is emitted
...
void IntegrationAttempt::fixupHeaderPHIs(ShadowBB *BB) {
  ...
  for (i = 0; i < BB->insts.size() && inst_is<PHINode>(&(BB->insts[i])); ++i) {
    if ((!BB->insts[i].committedVal) ||
        !isa<PHINode>(BB->insts[i].committedVal))
      continue;
    populatePHINode(BB, &(BB->insts[i]),
                    cast<PHINode>(BB->insts[i].committedVal));
  }
}

// Get the committed version of I's idx'th operand. If we're returning an
// unspecialised basic block because I is branching out of the user-directed
// specialisation region, and the user has requested verbose diagnostics in this
// case, failValue and failBlock are populated with the branch target and
// perhaps switch value by which we left specialised code.
Value *IntegrationAttempt::getCommittedValueOrBlock(ShadowInstruction *I,
                                                    uint32_t idx,
                                                    ConstantInt *&failValue,
  ...
  if (I->invar->operandIdxs[idx].instIdx == INVALID_INSTRUCTION_IDX &&
    ...
    release_assert((SBB || markUnreachable) &&
                   "Failed to get successor BB (2)");

    if (markUnreachable) {

      // Create an unreachable BB to branch to:
      BasicBlock *UBB = createBasicBlock(
          I->invar->I->getContext(), VerboseNames ? "synth-unreachable" : "",
          getFunctionRoot()->CommitF, false, true);
      // The following is only currently needed when running with
      // int-omit-checks: exceptions lead to immediate death, but we still need
      // a landingpad to produce a sane module.
      if (LandingPadInst *LPI = dyn_cast<LandingPadInst>(
              getBBInvar(succIdx)->BB->getFirstNonPHI())) {

        Instruction *NewLPI = LPI->clone();
        // No remapping necessary, as LP arg is just a constant pointer to a
      ...
      ShadowBBInvar *TargetBBI =
          getBBInvar(I->invar->operandIdxs[idx].blockIdx);

      if (pass->verbosePCs &&
          edgeBranchesToUnspecialisedCode(BB->invar, TargetBBI) &&
          !isExceptionEdge(BB->invar, TargetBBI)) {
        ...
        if (inst_is<SwitchInst>(I)) {
          ...
          if (idx == 1) {
            ...
            failValue =
                cast<ConstantInt>(getSingleConstant(I->getOperand(idx - 1)));
          }
...
// Synthesise I, a terminator instruction for BB, writing to emitBB. This is a
// little complicated when we're committing a return instruction out-of-line or
// we need to deal with branches to unspecialised code.
void IntegrationAttempt::emitTerminator(ShadowBB *BB, ShadowInstruction *I,
                                        BasicBlock *emitBB) {
  ...
  if (inst_is<ReturnInst>(I)) {
    ...
    if (!IA->returnBlock) {
      ...
      // If it's possible for specialisation assumptions to fail here or in a
      // child context, we need to return an extra flag indicating whether or
      // not this has happened. Here we always return 'true' indicating to
      // continue on the specialised path; in ConditionalSpec.cpp we can see the
      // opposite case being synthesised.
      if (IA->hasFailedReturnPath() && !IA->isRootMainCall()) {
        ...
        if (!retVal) {
          ...
          StructType *retType = cast<StructType>(
              getFunctionRoot()->CommitF->getFunctionType()->getReturnType());
          Type *normalRet = retType->getElementType(0);
          Constant *undefRet = UndefValue::get(normalRet);
          // Create { undef, true }
          Value *aggTemplate =
              ConstantStruct::get(retType, undefRet, cast<Constant>(retFlag));
          // Return { retval, true }
          retVal = InsertValueInst::Create(aggTemplate, retVal, 0,
                                           VerboseNames ? "success_ret" : "",
                                           emitBB);
        }
      }
      ...
      // This is a return instruction but we're committing inline. We should
      // have a target phi node to give the 'returned' value. No need for the
      // success flag dealt with above, as failed paths have a dedicated return
      // block separate from this one.
      ...
      if (IA->returnPHI && I->dieStatus == INSTSTATUS_ALIVE) {
        Value *PHIVal = getValAsType(getCommittedValue(I->getOperand(0)),
                                     F.getFunctionType()->getReturnType(), BI);
        IA->returnPHI->addIncoming(PHIVal,
                                   BB->committedBlocks.back().specBlock);
      }
    }
  ...
  for (uint32_t i = 0; i < BB->invar->succIdxs.size(); ++i) {
    ...
    if (BB->succsAlive[i]) {
      ...
      if (pass->omitChecks &&
          edgeBranchesToUnspecialisedCode(BB->invar,
                                          getBBInvar(BB->invar->succIdxs[i])))
        continue;
  ...
  if (knownSucc != 0xffffffff) {
    ...
    release_assert((inst_is<SwitchInst>(I) || inst_is<BranchInst>(I)) &&
    ...
    if (!breakSuccessors.empty()) {
      ...
      // If we're going to report departing specialised code at a switch
      // instruction, make a block that reports the failure and then switches
      // again to reach unspecialised code. This path should never handle invoke
      // instructions, and so this treatment of failed paths is acceptable.
      ...
      if (breakSuccessors.size() == 1) {
        ...
        unsigned nCases = defaultSwitchTarget == 0 ? breakSuccessors.size()
                                                   : breakSuccessors.size() - 1;

        SwitchInst *NewSI = SwitchInst::Create(
            newTerm->getOperand(0), defaultSwitchTarget, nCases, breakBlock);
...
static GlobalVariable *getFileBytesGlobal(ReadFile &RF) {
  ...
  if (!getFileBytes(RF.name, RF.incomingOffset, RF.readSize, constBytes,
                    Context, errors)) {
  ...
  ArrayType *ArrType =
      ArrayType::get(IntegerType::get(Context, 8), constBytes.size());
  Constant *ByteArray = ConstantArray::get(ArrType, constBytes);

  // Create a const global for the array:

  return new GlobalVariable(*getGlobalModule(), ArrType, true,
                            GlobalValue::InternalLinkage, ByteArray, "");
}

// Can't just use getOrInsertFunction as it ignores functions with local/private
// linkage.
...
static void emitSeekTo(Value *FD, uint64_t Offset, BasicBlock *emitBB) {
  ...
  FunctionType *FT = FunctionType::get(/* ret = */ Int64Ty,
                                       ArrayRef<Type *>(ArgTypes, 3), false);
  ...
  CallInst *SeekC =
      CallInst::Create(FT, SeekFn, ArrayRef<Value *>(CallArgs, 3), "", emitBB);
  if (Function *SeekF = dyn_cast<Function>(SeekFn))
    SeekC->setCallingConv(SeekF->getCallingConv());
}

// Emit a syscall instruction. It might be resolved down to a no-op, or might
// require repositioning with lseek64 before execution.
bool IntegrationAttempt::emitVFSCall(
    ShadowBB *BB, ShadowInstruction *I,
    SmallVector<CommittedBlock, 1>::iterator &emitBBIter) {
  ...
  {
    DenseMap<ShadowInstruction *, ReadFile>::iterator it =
        pass->resolvedReadCalls.find(I);
    if (it != pass->resolvedReadCalls.end()) {

      // This is a read() call that was successfully executed at specialisation
      // time.
      ...
      if ((I->needsRuntimeCheck == RUNTIME_CHECK_READ_LLIOWD ||
        ...
        // We need to check the read result is still valid, either with
        // straightforward memcmp or by checking with the watch daemon.
        BasicBlock *breakBlock = emitBBIter->breakBlock;
        Value *CheckTest;

        if (it->second.isFifo) {

          // As a fifo, we must (a) do the read as usual, then (b) use memcmp to
          // check that the results are the way we expect.

          // Emit the read as-is
          CallInst *readInst = cast<CallInst>(emitInst(BB, I, emitBB));

          // Build a memcmp check. Always tested this with images already linked
          // to a libc implementation, but hopefully this produces the right
          // memcmp signature to acquire a dynamically-linked version too.
          Value *readBuffer = readInst->getArgOperand(1);
          if (readBuffer->getType() != GInt8Ptr)
            readBuffer = new BitCastInst(
                readBuffer, GInt8Ptr, VerboseNames ? "readcast" : "", emitBB);

          Value *checkBuffer = getFileBytesGlobal(it->second);
          if (checkBuffer->getType() != GInt8Ptr)
            checkBuffer = new BitCastInst(
                checkBuffer, GInt8Ptr, VerboseNames ? "readcast" : "", emitBB);

          Constant *MemcmpSize = ConstantInt::get(GInt64, it->second.readSize);

          FunctionType *MemCmpFT =
              FunctionType::get(GInt32, {GInt8Ptr, GInt8Ptr, GInt64}, false);
          Value *MemCmpFn = F.getParent()
                                ->getOrInsertFunction("memcmp", MemCmpFT)
                                .getCallee();
          Value *CallArgs[] = {readBuffer, checkBuffer, MemcmpSize};
          Instruction *ReadMemcmp = CallInst::Create(
              MemCmpFT, MemCmpFn, ArrayRef<Value *>(CallArgs, 3), "", emitBB);
          // CheckTest must be true on failure, so we test memcmp(...) != 0

          Constant *Zero32 = Constant::getNullValue(GInt32);
          CheckTest =
              new ICmpInst(*emitBB, CmpInst::ICMP_NE, ReadMemcmp, Zero32);

          DenseMap<ShadowInstruction *, TrackedStore *>::iterator findit =
              pass->trackedStores.find(I);
          ...
          Type *Int32Ty = IntegerType::get(Context, 32);
          FunctionType *CheckFT = FunctionType::get(Int32Ty, false);
          Value *CheckFn = F.getParent()
                               ->getOrInsertFunction("lliowd_ok", CheckFT)
                               .getCallee();
          Value *CheckResult = CallInst::Create(
              CheckFT, CheckFn, ArrayRef<Value *>(), "readcheck", emitBB);

          Constant *Zero32 = Constant::getNullValue(Int32Ty);
          CheckTest =
              new ICmpInst(*emitBB, CmpInst::ICMP_EQ, CheckResult, Zero32);

          // Seek to the right position in the break block, so that
          // unspecialised code finds the file pointer where it expects to.
        ...
        if (pass->verbosePCs) {
          ...
          std::string message;
          {
            ...
            RSO << "Denied permission to use specialised files reading "
                << it->second.name << " in " << emitBB->getName() << "\n";
          }

          emitRuntimePrint(breakBlock, message, 0);
        }

        // A checked read instruction terminates this block. Emit on-success
        // code to the next BB:
        ++emitBBIter;

        // Branch to the real read instruction on failure for lliowd checks, or
        // the instruction after otherwise (in that case the read has already
        // been executed)
        uint32_t targetInst =
            it->second.isFifo ? I->invar->idx + 1 : I->invar->idx;
        BasicBlock *failTarget =
            getFunctionRoot()->getSubBlockForInst(BB->invar->idx, targetInst);
        ...
        // Emit the rest of the read implementation in the next specialised
        // block:
        emitBB = successTarget;
      }

      // Insert a seek call if that turns out to be necessary (i.e. if that FD
      // may be subsequently used without an intervening SEEK_SET)
      ...
      // If the read may be used, copy from the global containing the file's
      // bytes into the given buffer (i.e. a read() becomes a memcpy from a
      // constant global). If it's a read from a fifo then the copy was emitted
      // *before* the check.
      ...
      if (it->second.readSize > 0 &&
        ...
        Constant *CopySource = ConstantExpr::getGetElementPtr(
            cast<PointerType>(ArrayGlobal->getType())->getPointerElementType(),
            ArrayGlobal, Idxs, 2);
        ...
        Function *MemCpyFn = Intrinsic::getDeclaration(
            F.getParent(), Intrinsic::memcpy, ArrayRef<Type *>(Tys, 3));
        Value *ReadBuffer = getCommittedValue(I->getCallArgOperand(1));
        release_assert(ReadBuffer && "Committing read atop dead buffer?");
        Value *DestCast =
            new BitCastInst(getCommittedValue(I->getCallArgOperand(1)),
                            VoidPtrTy, VerboseNames ? "readcast" : "", emitBB);

        Value *CallArgs[] = {DestCast, CopySource, MemcpySize,
                             ConstantInt::get(Type::getInt32Ty(Context), 1),
                             ConstantInt::get(Type::getInt1Ty(Context), 0)};

        Instruction *ReadMemcpy =
            CallInst::Create(MemCpyFn->getFunctionType(), MemCpyFn,
                             ArrayRef<Value *>(CallArgs, 5), "", emitBB);
...

  {
    ...
    DenseMap<ShadowInstruction *, SeekFile>::iterator it =
        pass->resolvedSeekCalls.find(I);
    if (it != pass->resolvedSeekCalls.end()) {

      // MayDelete means this seek is redundant (every instruction that will
      // read the file pointer in the future has been folded away)
  ...
  if ((!pass->omitChecks) &&
      I->needsRuntimeCheck == RUNTIME_CHECK_READ_LLIOWD &&
      (CalledF->getName() == "stat" || CalledF->getName() == "fstat")) {

    LLVMContext &Context = emitBB->getContext();

    // Emit an lliowd_ok check, and if it fails branch to the real stat
    // instruction.
    Type *Int32Ty = IntegerType::get(Context, 32);
    FunctionType *CheckFT = FunctionType::get(Int32Ty, false);
    Value *CheckFn =
        F.getParent()->getOrInsertFunction("lliowd_ok", CheckFT).getCallee();
    Value *CheckResult =
        CallInst::Create(CheckFT, CheckFn, ArrayRef<Value *>(),
                         VerboseNames ? "readcheck" : "", emitBB);
    ...
    BasicBlock *failTarget =
        getFunctionRoot()->getSubBlockForInst(BB->invar->idx, I->invar->idx);
    ...
    if (pass->verbosePCs) {
      ...
      {
        raw_string_ostream RSO(message);
        RSO << "Denied permission to use specialised files on (f)stat in "
            << emitBB->getName() << "\n";
      }
  ...
  // Neither read nor seek nor stat (or any of those which couldn't be
  // specialised?)
  return false;
}

// Get the basic block that follows from an 'invoke' on the non-exceptional
// path.
BasicBlock *IntegrationAttempt::getInvokeNormalSuccessor(ShadowInstruction *I,
                                                         bool &toCheckBlock) {
  ...
  // An invoke's normal return path should go to special block added to the end
  // of its parent's committedBlock list to perform a test if necessary. This
  // might be because its return value should be checked, or because it has a
  // failed return path. If no check is needed, simply branch to its ordinary
  // successor. This is a little trickier than a normal call because invoke is
  // also a terminator.
  ...
  if (toCheckBlock)
    ...
    Value *opV = getCommittedValueOrBlock(I, I->getNumOperands() - 2,
                                          ignFailValue, failBlock);
    release_assert(opV);

    if (failBlock)
      release_assert(0 && "Case not covered yet: invoke instruction with "
                          "ignored normal return edge");

    return cast<BasicBlock>(opV);
  }
}

// Synthesise call instruction I into *emitBBIter. Adjust emitBBIter to point to
// wherever the call's successor instruction should be emitted (i.e. leave it
// alone if the call was inlined straightforwardly, or bump it otherwise).
void IntegrationAttempt::emitCall(
    ShadowBB *BB, ShadowInstruction *I,
    SmallVector<CommittedBlock, 1>::iterator &emitBBIter) {
  ...
  if (InlineAttempt *IA = getInlineAttempt(I)) {
    ...
    if (IA->isEnabled()) {
      ...
      if (!IA->commitsOutOfLine()) {
        ...
        // Commit will have written blocks in the same residual function as this
        // context.

        // Branch from the current write BB to the call's entry block:
        BranchInst::Create(IA->getCommittedEntryBlock(), emitBB);

        // Take the return PHI (or lack thereof) as this instruction's committed
        // value.
        ...
        // Build a call to IA->CommitF with some parameters stubbed out
        // (replaced with undef) if not required.

        const CallBase *OldCB = cast<CallBase>(I->invar->I);
        AttributeList attrs = OldCB->getAttributes();
        ...
        for (uint32_t i = 0; i != ilim; ++i) {
          ...
          if (i < FType->getNumParams()) {
            ...
            needTy = OldCB->getArgOperand(i)->getType();
          }
        ...
        if (inst_is<InvokeInst>(I)) {
          ...
          // Note that the iterator is bumped forwards, but we keep emitBB as-is
          // for now to emit the call or invoke into the old block.
        ...
        if (inst_is<CallInst>(I) ||
            (inst_is<InvokeInst>(I) && !BB->succsAlive[1])) {

          // A normal call or an invoke that cannot throw.

          NewI = CallInst::Create(IA->CommitF->getFunctionType(), IA->CommitF,
                                  Args, "", emitBB);
          ...
          BasicBlock *exnBlock =
              getFunctionRoot()->getSubBlockForInst(BB->invar->succIdxs[1], 0);
          NewI =
              InvokeInst::Create(IA->CommitF->getFunctionType(), IA->CommitF,
                                 invokeNormalDest, exnBlock, Args, "", emitBB);
        }

        // Bring emitBB back into sync with the iterator if we bumped it above
        // getting an invoke instruction's destination. This means that any
        // checks needed by that invoke will be emitted to this block.
        emitBB = emitBBIter->specBlock;

        CallBase *NewCB = cast<CallBase>(NewI);
        ...
        if (IA->hasFailedReturnPath()) {
          ...
          // This out-of-line call might fail. If it did, branch to
          // unspecialised code. Retrieve the boolean (or struct-with-boolean)
          // it returned to indicate whether we should break to unspec code now:
          ...
          if (IA->F.getFunctionType()->getReturnType()->isVoidTy()) {
            ...
            CallFailed = ExtractValueInst::Create(
                NewI, ArrayRef<unsigned>(1), VerboseNames ? "retfailflag" : "",
                emitBB);
            I->committedVal = ExtractValueInst::Create(
                NewI, ArrayRef<unsigned>(0), VerboseNames ? "ret" : "", emitBB);
          }
          ...
          if (inst_is<CallInst>(I)) {
            ...
            // If the call result isn't as expected, branch to the instruction
            // after in unspecialised code.
            ++emitBBIter;
            successTarget = emitBBIter->specBlock;
            failTarget = getFunctionRoot()->getSubBlockForInst(
                BB->invar->idx, I->invar->idx + 1);

          } else {

            // Invoke is already a terminator -- branch chooses between the
            // specialised or unspecialised successors. emititer already bumped
            // above.
            bool markUnreachable = false;
            successTarget =
                getSuccessorBB(BB, BB->invar->succIdxs[0], markUnreachable);
            if (markUnreachable) {
              successTarget =
                  createBasicBlock(emitBB->getContext(),
      ...
      if (!IA->commitsOutOfLine()) {
        ...
        if (IA->emittedAlloca) {
          ...
          // If the residual, inline function allocates stack memory, bound its
          // lifetime with stacksave/restore. Sticking its allocas at top of
          // function would create a huge stack frame.
          ...
          release_assert(FunctionPred &&
                         "No unique entry to inlined function?");
          CallInst *SavedPtr = CallInst::Create(
              StackSave->getFunctionType(), StackSave, ArrayRef<Value *>(),
              VerboseNames ? "savedstack" : "", FunctionPred->getTerminator());

          // Restore on successful return
          CallInst::Create(StackRestore->getFunctionType(), StackRestore,
                           ArrayRef<Value *>(SavedPtr), "", IA->returnBlock);

          // Restore on failed
          if (IA->failedReturnBlock) {
            CallInst::Create(StackRestore->getFunctionType(), StackRestore,
                             ArrayRef<Value *>(SavedPtr), "",
                             IA->failedReturnBlock->getFirstNonPHI());
        ...
        if (IA->returnPHI && IA->returnPHI->getNumIncomingValues() == 0) {
          ...
          IA->returnPHI = 0;
          I->committedVal =
              UndefValue::get(IA->F.getFunctionType()->getReturnType());
        }

        // If it's an invoke instruction then this is the terminator!
        // If it commits out of line, then it doesn't unwind but might fail
        // (deviate from specialisation assumptions). The successful and failed
        // return blocks should branch to the invoke's non-exception successor.
        ...
        if (inst_is<InvokeInst>(I)) {
          ...
          if (IA->failedReturnBlock) {
            ...
            BasicBlock *failTarget = getFunctionRoot()->getSubBlockForInst(
                BB->invar->succIdxs[0], 0);
  ...
  if (pass->verbosePCs && !getInlineAttempt(I)) {
    ...
    if (CalledF && CalledF->doesNotReturn() && !CalledF->doesNotThrow()) {
      ...
      {
        raw_string_ostream RSO(msg);
        RSO << "Leaving specialised code by entering noreturn throws function "
            << CalledF->getName() << "\n";
      }
  ...
  if (InvokeInst *NewInvoke = dyn_cast<InvokeInst>(NewI)) {
    ...
    // If an invoke with a disabled IA was emitted, its return value may need to
    // be checked; in this case it should branch to another subblock of the same
    // BB rather than its usual successor, and the check will be constructed by
    // our caller.
...
static void checkEmittedInst(Instruction *I) {
  ...
  if (Broken) {
    ...
    errs() << "WARNING: suspicious instruction emitted in block "
           << I->getParent() << "\n";
  }
}

// Emit generic instruction I into emitBB. We already know it isn't resolved to
// a constant, dead etc.
Instruction *IntegrationAttempt::emitInst(ShadowBB *BB, ShadowInstruction *I,
                                          BasicBlock *emitBB) {
  ...
  for (uint32_t i = 0; i < I->getNumOperands(); ++i) {
    ...
    release_assert((!failBlock) &&
                   "Case not handled yet: invoke with ignored normal return");
  }

  // Warn if specialised code has been left via an exception (but stayed within
  // the general domain of specialisation)
  if (pass->verbosePCs && isa<LandingPadInst>(newI)) {

    std::string msg;
    {
      raw_string_ostream RSO(msg);
      RSO << "Landed at landing pad inst in block " << BB->invar->BB->getName()
          << " / " << F.getName() << " / " << SeqNumber << "\n";
    }
    emitRuntimePrint(emitBB, msg, 0);
  }
  ...
  ShadowValue Base;
  AllocData *AD;
  if (getBaseObject(ShadowValue(I), Base) && Base.isPtrIdx() &&
      (AD = getAllocData(Base)) && AD->allocValue.u.I == I) {
  ...
  if (isa<StoreInst>(newI) || isa<MemSetInst>(newI)) {
    ...
    DenseMap<ShadowInstruction *, TrackedStore *>::iterator findit =
        GlobalIHP->trackedStores.find(I);
...

  {
    ...
    // Don't use forwardableOpenCalls here because surrogates for FDs need
    // recording too. The defining characteristic is always resolving to an FD
    // that points back to this instruction.

    ImprovedValSetSingle *IVS = dyn_cast_or_null<ImprovedValSetSingle>(I->i.PB);
    if (IVS && IVS->SetType == ValSetTypeFD && IVS->Values.size() == 1 &&
        IVS->Values[0].V.isFdIdx()) {
...
Constant *llvm::getGVOffset(Constant *GV, int64_t Offset, Type *targetType) {
  ...
  if (Offset == 0)
    ...
    Constant *OffC = ConstantInt::get(Type::getInt64Ty(GV->getContext()),
                                      (uint64_t)Offset, true);
    OffsetGV = ConstantExpr::getGetElementPtr(
        cast<PointerType>(CastGV->getType())->getPointerElementType(), CastGV,
        OffC);
  }
...
// May be impossible if I is not specific enough, or completely unknown. Return
// false in that case.
bool IntegrationAttempt::synthCommittedPointer(
    ShadowValue I, SmallVector<CommittedBlock, 1>::iterator emitBB) {

  Value *Result;
  ImprovedValSetSingle *IVS =
      dyn_cast_or_null<ImprovedValSetSingle>(getIVSRef(I));
  if ((!IVS) || IVS->SetType != ValSetTypePB || IVS->Values.size() != 1)
    return false;

  bool ret = synthCommittedPointer(&I, getValueType(I), IVS->Values[0],
                                   emitBB->specBlock, Result);
  if (ret)
...
// be directly referenced, e.g. because its specialisation context was not
// committed and so there is no unique instruction that describes this
// particular object.
...
// Get the InlineAttempt at a particular stack depth by walking our parent
// contexts.
InlineAttempt *InlineAttempt::getStackFrameCtx(int32_t frameIdx) {

  // frameSize == -1 means no stack frame and the allocation really belongs to
  // our caller.
  if (stack_depth == frameIdx && invarInfo->frameSize != -1)
    return this;
  else
    return activeCaller->parent->IA->getFunctionRoot()->getStackFrameCtx(
        frameIdx);
}

// Synthesise a pointer if we know its base object, have a unique instruction
// that allocates it, etc. If we can't do that, return false and we'll have to
// keep the pointer reference verbatim, meaning we need the operand alive and so
// on.
bool IntegrationAttempt::synthCommittedPointer(ShadowValue *I, Type *targetType,
                                               ImprovedVal IV,
                                               BasicBlock *emitBB,
                                               Value *&Result) {
  ...
  if (Base.isGV()) {
    ...
    if (!BaseI) {
      ...
      // Base has not been committed yet. Create a trivial select instruction
      // that will be populated with the allocation when it is committed. This
      // is rather wasteful, but it saves having every synthCommitedPointer do
      // their own check and register.
    ...
    if (Offset == 0) {
      ...
      if (BaseI->getType() == targetType)
        ...
        Result = CastInst::CreatePointerCast(
            BaseI, targetType, VerboseNames ? "synthcast" : "", emitBB);
      return true;
    }
    ...
    InTy = cast<PointerType>(InTy)->getPointerElementType();
    if (Type *ElTy = XXXFindElementAtOffset(InTy, Offset, GEPIdxs, GlobalTD)) {

      Result = GetElementPtrInst::Create(
          InTy, BaseI, GEPIdxs, VerboseNames ? "synthgep" : "", emitBB);
      if ((!isa<PointerType>(targetType)) ||
          ElTy != cast<PointerType>(targetType)->getPointerElementType())
        Result = CastInst::CreatePointerCast(
            Result, targetType, VerboseNames ? "synthcastback" : "", emitBB);
      return true;
    }
    ...
    if (BaseI->getType() != Int8Ptr)
      CastI = new BitCastInst(BaseI, Int8Ptr, VerboseNames ? "synthcast" : "",
                              emitBB);
    else
      CastI = BaseI;

    // Offset:
    Constant *OffsetC = ConstantInt::get(Type::getInt64Ty(emitBB->getContext()),
                                         (uint64_t)Offset, true);
    Value *OffsetI = GetElementPtrInst::Create(
        cast<PointerType>(CastI->getType())->getPointerElementType(), CastI,
        OffsetC, VerboseNames ? "synthgep" : "", emitBB);
    ...
    if (targetType == Int8Ptr) {
      ...
      Result = (CastInst::CreatePointerCast(
          OffsetI, targetType, VerboseNames ? "synthcastback" : "", emitBB));
    }
  }
...
// Check whether I can be synthesised as a constant, pointer to a known base,
// etc. If not it must be represented literally in the output program.
bool IntegrationAttempt::canSynthVal(ShadowValue *I, ValSetType Ty,
                                     const ImprovedVal &IV) {

  if (Ty == ValSetTypeScalar)
    return true;
  else if (Ty == ValSetTypeFD) {
    return ((!I) || (!I->isInst()) ||
            (I->u.I != pass->fds[IV.V.u.PtrOrFd.idx].SI)) &&
...
// Produce a Value describing I by any means other than literally cloning the
// instruction. If that's impossible return null and we'll do the plain clone
// elsewhere. Cast I to targetType when synthesising. I's unique ImprovedVal is
// already known to be IV with type Ty. Write any needed cast / GEP / load
// instructions to emitBB.
Value *IntegrationAttempt::trySynthVal(ShadowValue *I, Type *targetType,
                                       ValSetType Ty, const ImprovedVal &IV,
                                       BasicBlock *emitBB) {
  ...
  if (Ty == ValSetTypeScalar) {
    ...
    if (canSynthVal(I, Ty, IV)) {
      ...
      if (!FDS.CommittedVal) {
        ...
        // Open instruction not committed yet. Create a 'select' instruction
        // that will be patched out when the open call is eventually committed.
        Value *True = ConstantInt::getTrue(emitBB->getContext());
    ...
    // Pointer -- create a direct reference to the original allocation if
    // possible.
    Value *V;
...
static Instruction *emitMemcpyInst(Value *To, Value *From, uint64_t Size,
                                   BasicBlock *emitBB) {
  ...
  Function *MemCpyFn = Intrinsic::getDeclaration(
      getGlobalModule(), Intrinsic::memcpy, ArrayRef<Type *>(Tys, 3));
  ...
  return CallInst::Create(MemCpyFn->getFunctionType(), MemCpyFn,
                          ArrayRef<Value *>(CallArgs, 5), "", emitBB);
}

// Emit code to write (chunkBegin-chunkEnd] to I's first operand. This might be
// a simple store, or might need a memcpy call from a composite-typed object.
// newInstructions accumulates the stores, calls and casts required to do this,
// which are also written to emitBB.
// 'Chunks' are written in this way because ideally we'd use a big memcpy in
// preference to many small store instructions, but pointers, FDs and other
// non-constants can prevent us from doing so.
void IntegrationAttempt::emitChunk(
    ShadowInstruction *I, BasicBlock *emitBB,
    SmallVector<IVSRange, 4>::iterator chunkBegin,
    SmallVector<IVSRange, 4>::iterator chunkEnd,
  ...
  Type *targetType;
  if (chunkSize == 1 && GlobalTD->getTypeStoreSize(
                            getValueType(chunkBegin->second.Values[0].V)) <= 8)
    targetType =
        PointerType::getUnqual(getValueType(chunkBegin->second.Values[0].V));
  else
    targetType = BytePtr;
  ...
  if (chunkSize == 1) {
    ...
    ShadowValue IVal(I);
    Value *newVal = trySynthVal(&IVal, getValueType(IV.V),
                                chunkBegin->second.SetType, IV, emitBB);
    ...
    if (elSize > 8) {
      ...
      GlobalVariable *CopyFrom = new GlobalVariable(
          *getGlobalModule(), newVal->getType(), true,
          GlobalValue::InternalLinkage, cast<Constant>(newVal));
      ...
      release_assert(newVal->getType() ==
                     cast<PointerType>(targetPtrSynth->getType())
    ...
    for (SmallVector<IVSRange, 4>::iterator it = chunkBegin; it != chunkEnd;
         ++it) {

      ImprovedVal &IV = it->second.Values[0];
      ShadowValue IVal(I);
      Value *newVal = trySynthVal(&IVal, getValueType(IV.V), it->second.SetType,
                                  IV, emitBB);
    ...
    StructType *SType =
        StructType::get(emitBB->getContext(), Types, /*isPacked=*/true);
    Constant *CS = ConstantStruct::get(SType, Copy);
    GlobalVariable *GCS = new GlobalVariable(*getGlobalModule(), SType, true,
                                             GlobalValue::InternalLinkage, CS);
    Constant *GCSPtr = ConstantExpr::getBitCast(GCS, BytePtr);

    newInstructions.push_back(emitMemcpyInst(
        targetPtrSynth, GCSPtr, lastOffset - chunkBegin->first.first, emitBB));
  }
}

// Can a memcpy or memmove (a memory-transfer instruction, MTI) be synthesised
// (i.e. do we know the unique objects it writes and where it writes them?)
...
// Emit a sequence of stores and/or memcpy-from-constant-global instructions
// that have the effect of memcpy/memmove call I. If this is impossible (e.g.
// we're not certain of what is written or where it is written), return false.
bool IntegrationAttempt::trySynthMTI(ShadowInstruction *I, BasicBlock *emitBB) {
  ...
  // The method: for consecutive scalars or pointers-to-globals, synthesise a
  // packed struct and memcpy from it. For non-constant pointers and FDs,
  // produce stores.
  ...
  for (SmallVector<IVSRange, 4>::iterator it = Vals.begin(), itend = Vals.end();
    ...
    if (it->second.SetType == ValSetTypeScalar ||
        (it->second.SetType == ValSetTypePB &&
         (it->second.Values[0].V.isGV() || it->second.Values[0].V.isVal()))) {
...
// Try to synthesise instruction I, which means returning a constant if
// possible, or creating a pointer expressed as a simple gep-and-cast from a
// known base object (alloca, malloc/similar, global...) If we cannot do this
// (e.g. I doesn't have a known concrete result, or we can't synthesise an
// appropriate pointer) then return false and our caller will have to create a
// literal clone of the original instruction instead. For memcpy instructions,
// try to turn them into a series of memcpy or store actions drawing from
// constants and known pointers. Emit any necessary instructions (e.g. casts) to
// emitBB.
bool IntegrationAttempt::trySynthInst(ShadowInstruction *I, BasicBlock *emitBB,
                                      Value *&Result) {
  ...
  // AtomicRMW and AtomicCmpXchg can be assigned values and not require a
  // runtime check if their operand was known to be thread-local; however for
  // now emit them for their side-effects.
  ...
  ShadowValue IVal(I);
  Result =
      trySynthVal(&IVal, I->getType(), IVS->SetType, IVS->Values[0], emitBB);
  return !!Result;
}

// Similar to above, but try to synthesise an argument value.
bool IntegrationAttempt::trySynthArg(ShadowArg *A, BasicBlock *emitBB,
                                     Value *&Result) {
  ...
  ShadowValue AVal(A);
  Result =
      trySynthVal(&AVal, A->getType(), IVS->SetType, IVS->Values[0], emitBB);
  return !!Result;
}

// Identify functions like llvm.uadd.with.overflow which are essentially
...
// Create a synthetic version of I, or if that's impossible clone it. Emit
// synthesised / cloned instructions to *emitBB, and bump the emitBB iterator if
// subsequent instructions should target a different block because we emitted an
// inline call or an out-of-line call that needed us to possibly branch to
// unspecialised code.
void IntegrationAttempt::emitOrSynthInst(
    ShadowInstruction *I, ShadowBB *BB,
    SmallVector<CommittedBlock, 1>::iterator &emitBB) {

  bool useCallPath = (inst_is<CallInst>(I) || inst_is<InvokeInst>(I)) &&
                     (!inst_is<MemIntrinsic>(I)) && !isPureCall(I);
  ...
  // Return instruction "dead" status means it won't be used -- but we must
  // synthesise something if this is an out-of-line commit. Read instruction
  // "dead" status means its memory writes are useless, but its return value is
  // still perhaps used.

  if (willBeDeleted(ShadowValue(I)) && (!I->invar->I->isTerminator()) &&
      (!pass->resolvedReadCalls.count(I)))
    return;

  // If a runtime check is required we need the literal instruction available.
  // The second parameter specifies this doesn't catch instructions that
  // requires custom checks such as VFS operations.
  ...
  else if (I->invar->I->isTerminator())
    emitTerminator(BB, I, emitBB->specBlock);
  else
    emitInst(BB, I, emitBB->specBlock);
}

// Write all instructions that fall within loop ScopeL. i is the index of the
// first basic block that falls within it (i.e. the header), and as a
// side-effect this function should leave i pointing to the first block not in
// ScopeL. ScopeL might be null, in which case this is a function root scope.
void IntegrationAttempt::commitLoopInstructions(const ShadowLoopInvar *ScopeL,
                                                uint32_t &i) {
  ...
  for (; i < nBBs; ++i) {
    ...
    if (BBI->naturalScope != ScopeL) {
      ...
      // Entering a nested loop. First write the blocks for each iteration
      // that's being unrolled:
      ...
      if (PA && PA->isEnabled() && PA->isTerminated()) {
        ...
        // If the loop has terminated, skip populating the blocks in this
        // context, but do allow any unspecialised ('failed') version of this
        // loop and its children to populate their header PHI nodes by pulling
        // from the specialised iterations we just committed.
        const ShadowLoopInvar *skipL = BBI->naturalScope;
        while (i < nBBs &&
               skipL->contains(getBBInvar(i + BBsOffset)->naturalScope)) {

          const ShadowLoopInvar *ThisL =
              getBBInvar(i + BBsOffset)->naturalScope;
    ...
    uint32_t j;
    SmallVector<CommittedBlock, 1>::iterator emitPHIsTo =
        BB->committedBlocks.begin();

    // Even if there are path conditions, emit specialised PHIs into the first
    // committed block for BB. This means if we need to make a top-of-block
    // check, like checking that a specialisation assumption still holds, the
    // block's phi nodes precede it regardless.
    for (j = 0; j < BB->insts.size() && inst_is<PHINode>(&(BB->insts[j]));
         ++j) {
    ...
    release_assert(emitPHIsTo == BB->committedBlocks.begin() &&
                   "PHI emission should not move the emit pointer");
    ...
    // If the PHI nodes are loop exit PHIs that need their values checking, emit
    // the check.
    if (j != 0) {

      ShadowInstruction *prevSI = &BB->insts[j - 1];
      if (inst_is<PHINode>(prevSI) &&
          requiresRuntimeCheck(ShadowValue(prevSI), false))
    ...
    for (; j < BB->insts.size(); ++j) {
      ...
      // This only emits "check as expected" checks: simple comparisons that
      // ensure a value determined during specialisation matches the real value.
      // VFS ops (and perhaps others to come) produce special checks.
      if (requiresRuntimeCheck(ShadowValue(I), false))
        emitBlockIt = emitOrdinaryInstCheck(emitBlockIt, I);
    }

    // All instructions in this block are now available, so any companion
    // unspecialised block should now be able to populate phi nodes at
    // specialised-to-unspecialised junctions.
    populateFailedBlock(i + BBsOffset);
...
// // Apply the same debug tag to all of 'blocks'. Used to provide simple
// insight in GDB about the provenance
// // of specialised code that crashes.
// static void applyLocToBlocks(const DebugLoc& loc, const
// std::vector<BasicBlock*>& blocks) {
//     for(std::vector<BasicBlock*>::const_iterator it = blocks.begin(), itend =
//     blocks.end(); it != itend; ++it) {
// 	for(BasicBlock::iterator IIt = (*it)->begin(), IEnd = (*it)->end(); IIt
// != IEnd; ++IIt) { 	  if(!(IIt->getDebugLoc())) IIt->setDebugLoc(loc);
// 	}
//     }

// }

// Commit all arguments and instructions in this function context, using
// synthetic constants / pointers instead of the original instructions wherever
// possible. This recurses into loop contexts and child calls as they are found,
// and calls to populate phi nodes that unify values at
// specialised-to-unspecialised control-flow edges as their source operands
// become available. Child calls may or may not already have been committed,
// depending on whether we're in straight-line specialised code or not.
void InlineAttempt::commitArgsAndInstructions() {
  ...
  if (isCommitted()) {
    ...
    // before us, and needed to refer to one of our arguments. In this case a
    // dummy 'select' instruction will have been introduced which we can now
    // replace.
    for (uint32_t i = 0; i < F.arg_size(); ++i) {

      ShadowArg *SA = &(argShadows[i]);
      if (SA->patchInst)
        SA->patchInst->replaceAllUsesWith(
            getArgCommittedValue(SA, (BasicBlock *)0));
    }

  } else {

    // Commit this context now. The CFG will have already been created,
    // including multiple residual blocks per BB ('committedBlocks') when a BB
    // needs to have result-as-expected checks inserted, e.g. to check against
    // clobbering by concurrent threads.

    // Start emitting code to the first block:
    SmallVector<CommittedBlock, 1>::iterator emitBB =
        BBs[0]->committedBlocks.begin();
    ...
    // Create return PHI if needed. This is a phi node that collects return
    // values if we're committing inline.
    if (returnBlock && !isVoidTy)
      returnPHI = makePHI(Ret, VerboseNames ? "retval" : "", returnBlock);
    else
      returnPHI = 0;

    // ...and this is a phi node that collects return values if we branch to
    // unspecialised code e.g. due to an assumption check failing. If we commit
    // out-of-line then a separate boolean return is used instead of the two
    // different target blocks / phi nodes like this.
    if (failedReturnBlock && !isVoidTy)
      failedReturnPHI =
          makePHI(Ret, VerboseNames ? "failedretval" : "", failedReturnBlock);
    else
      failedReturnPHI = 0;

    // Introduce an lliowd_init call if ordered. This sets up a connection to
    // the file-watcher daemon which we will use to check that file-reading
    // calls may still be assumed to yield their specialisation-time results
    // (i.e. the file we're reading has not changed in the meantime)
    if ((pass->llioPreludeStackIdx != -1 && targetCallInfo &&
         targetCallInfo->targetStackDepth ==
             (uint32_t)pass->llioPreludeStackIdx) ||
        (((uint32_t)pass->llioPreludeStackIdx) ==
             pass->targetCallStack.size() &&
         isStackTop)) {

      Type *Void = Type::getVoidTy(emitBB->specBlock->getContext());
      FunctionCallee WDInitFC = getGlobalModule()->getOrInsertFunction(
          "lliowd_init", FunctionType::get(Void, false));
      CallInst::Create(WDInitFC, ArrayRef<Value *>(), "", emitBB->specBlock);
    }

    // Commit all local instructions.
    commitInstructions();

    // Patch any child contexts that wanted to refer to alloca instructions that
    // take place here, but which had to introduce a temporary select
    // instruction instead because we hadn't committed yet.
    fixNonLocalStackUses();

    // Should we emit debug tags to indicate which binary code derived
    // from which specialisation contexts?  It's 'fake' in that it
    // doesn't describe the original source code file / line.
    // if(pass->emitFakeDebug) {

    //   DenseMap<Function*, DebugLoc>::iterator findit =
    //   pass->fakeDebugLocs.find(&F); DebugLoc* pFakeLoc;
    ...
    // 1, pass->fakeDebugType, false,
    // true, 1); 	DILexicalBlock fakeBlock =
    // DIB.createLexicalBlock(fakeFunction, fakeFile, 1, 0); 	DebugLoc
    // newFakeLoc = DebugLoc::getFromDILexicalBlock(fakeBlock);
    ...
    // 	  for(BasicBlock::iterator IIt = it->begin(), IEnd = it->end(); IIt !=
    // IEnd; ++IIt) 	    if(IIt->getDebugLoc().isUnknown())
    ...
    // Do this here rather than in CommitCFG because blocks handling unreachable
    // branches can be created during emitTerminator.
    if (uniqueParent) {

      // If we already know the residual function we're writing to, committed
      // blocks should have been added to it instead of entering these arrays.
      if (CommitF)
        release_assert(CommitBlocks.empty() && CommitFailedBlocks.empty());

      uniqueParent->inheritCommitBlocksAndFunctions(
          CommitBlocks, CommitFailedBlocks, CommitFunctions);
    }
  }
}
...
void IntegrationAttempt::commitInstructions() {
  ...
  // If the user asked for a verbose specialisation, print to indicate we
  // entered specialised code.
  ...
  // Commit instructions local to this loop (or the whole function if this is an
  // InlineAttempt and so L is null)
  uint32_t i = 0;
  commitLoopInstructions(L, i);

  if ((!L) && getFunctionRoot()->isRootMainCall()) {

    // Patch references to pseudo-allocations based on the root function's
    // arguments.
  ...
  // This should be the last reference to the failed block maps here:
  // deallocate.
...
// Remove any references to BB's instructions as committed versions of heap
// objects or file descriptors.
static void unregisterCommittedAllocations(BasicBlock *BB) {
  ...
  for (BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI) {
    ...
    Instruction *I = &*BI;
    {
      DenseMap<Value *, uint32_t>::iterator findit =
          GlobalIHP->committedHeapAllocations.find(I);
...
// Remove any references to F's instructions as committed versions of heap
// objects or file descriptors.
...
// Delete all BBs and committed functions from all arguments. Used when
// releasing a whole tree of contexts whose specialised code is no longer
// needed.
static void releaseCC(std::vector<Function *> &CommitFunctions,
...
// Delete all BBs and committed functions from all arguments. Used when
// releasing a whole tree of contexts whose specialised code is no longer
// needed. Note there is no need to recurse into child contexts because they
// pass their committed code to their parents when their commit phase finishes
// and so they themselves can be deallocated.
...
void IntegrationAttempt::releaseMemoryPostCommit() {
  ...
  for (IAIterator it = child_calls_begin(this), itend = child_calls_end(this);
       it != itend; ++it) {
  ...
  for (uint32_t i = BBsOffset, ilim = BBsOffset + nBBs; i != ilim; ++i) {
    ...
    if (BB) {
      ...
      for (uint32_t j = 0, jlim = BB->insts.size(); j != jlim; ++j) {
        ...
        {
          DenseMap<ShadowInstruction *, TrackedAlloc *>::iterator findit =
              pass->trackedAllocs.find(SI);
...
void InlineAttempt::finaliseAndCommit(bool inLoopAnalyser) {
  ...
  if (isEnabled()) {
    ...
    if (activeCaller->parent->dseStore) {
      activeCaller->parent->dseStore =
          activeCaller->parent->dseStore->getEmptyMap();
...
void LLPEAnalysisPass::commit() {
  ...
  if (!(omitChecks || llioDependentFiles.empty())) {
    ...
    // Note files that were read by specialised code, and so which must be
    // checked for modification during specialisation.
    writeLliowdConfig();
    ...
    if (preludeBlock) {
      ...
      FunctionCallee WDInitFC = getGlobalModule()->getOrInsertFunction(
          "lliowd_init", FunctionType::get(Void, false));
      CallInst::Create(WDInitFC, ArrayRef<Value *>(), "", &*it);
    }
  }
  ...
  if (!statsFile.empty()) {
    ...
    raw_fd_ostream RFO(statsFile.c_str(), error, sys::fs::OF_Text);
    if (error)
