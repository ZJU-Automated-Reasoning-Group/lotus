...
// This file contains logic relating to use of PartialVal to do bytewise
// reinterpretation from one or more source values.
...
PartialVal::PartialVal(uint64_t nbytes)
    : partialBufBytes(nbytes), loadFinished(false) {
...
bool PartialVal::isComplete() { return loadFinished; }

// Copy bytes in from the given buffer, targeting the range
// (FirstDef-FirstNotDef], marking each valid.
void PartialVal::combineWith(uint8_t *Other, uint64_t FirstDef,
  ...
  loadFinished = true;
  // Meaning of the predicate: stop at the boundary, or bail out if there's no
  // more setting to do and there's no hope we've finished.
  for (uint64_t i = 0; i < partialBufBytes && (loadFinished || i < FirstNotDef);
       ++i) {
...
void PartialVal::combineWith(PartialVal &Other, uint64_t FirstDef,
                             uint64_t FirstNotDef) {

  combineWith((uint8_t *)Other.partialBuf, FirstDef, FirstNotDef);
}

bool PartialVal::combineWith(Constant *C, uint64_t ReadOffset,
  ...
  if (!XXXReadDataFromGlobal(C, ReadOffset, tempBuf, FirstNotDef - FirstDef,
                             *GlobalTD)) {
    LLVM_DEBUG(dbgs() << "XXXReadDataFromGlobal failed; perhaps the source "
                      << *(C) << " can't be bitcast?\n");
    if (error)
      *error = "RDFG";
    return false;
...
Constant *llvm::PVToConst(PartialVal &PV, uint64_t Size, LLVMContext &Ctx) {
  ...
  if (Size <= 8) {
    ...
    return ConstantDataArray::get(
        Ctx, ArrayRef<uint8_t>((uint8_t *)PV.partialBuf, Size));
}

bool llvm::addIVToPartialVal(const ImprovedVal &IV, ValSetType SetType,
                             uint64_t IVOffset, uint64_t PVOffset,
                             uint64_t Size, PartialVal *PV,
                             std::string *error) {
...
bool llvm::addIVSToPartialVal(const ImprovedValSetSingle &IVS,
                              uint64_t IVSOffset, uint64_t PVOffset,
                              uint64_t Size, PartialVal *PV,
                              std::string *error) {
  ...
  return addIVToPartialVal(IVS.Values[0], IVS.SetType, IVSOffset, PVOffset,
                           Size, PV, error);
}
