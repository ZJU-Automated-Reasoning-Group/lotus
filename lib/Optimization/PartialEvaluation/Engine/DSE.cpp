...
// Dead store elimination using essentially the same technique as
// Transforms/Scalar/DSE.cpp, only taking into account that we've been computing
// a probable flow through the program.
...
// Functions relating to dead store elimination: identifying stores all of whose
// users have been eliminated or wired direct to the stored value, so the store
// itself is no longer needed. This is also attempted for memory-transfer
// (memcpy, memmove) instructions and read(2) calls that are being specialised
// away.
...
// TrackedStore: a store, which may or may not have been synthesised as a
// specialised instruction yet, which is eligible for elimination.
TrackedStore::TrackedStore(ShadowInstruction *_I, uint64_t ob)
    : I(_I), isCommitted(false), committedInsts(0), nCommittedInsts(0),
      outstandingBytes(ob), isNeeded(false) {
...
// Eligible for killing? isNeeded will have been set if it is needed;
// committedInsts will have been cleared if it has already been killed.
    ...
    for (unsigned op = 0, e = DeadInst->getNumOperands(); op != e; ++op) {
      ...
      if (Instruction *OpI = dyn_cast<Instruction>(Op)) {
        ...
        if (GlobalVariable *GV =
                dyn_cast_or_null<GlobalVariable>(getUnderlyingObject(C))) {
...
// Tag an instruction dead, or if already synthesised, delete the emitted
// instructions.
...
// so that e.g. store %x, i32 0; %y = cast %x to i8*; load i8 %y is handled
// correctly.
...
// Implement TrackedAlloc, which tracks whether an allocation instruction has
// been used, perhaps permitting them to be deleted when they go out of scope.

TrackedAlloc::TrackedAlloc(ShadowInstruction *_SI)
    : SI(_SI), isCommitted(false), nRefs(1), isNeeded(false) {
...
// The DSEMaps used to track killable stores might contain stores that clearly
// cannot be eliminated because they have been proven maybe-used in the
// meantime. Remove useless records like this. Return true if the particular
// record *argit can go away.
...
DSEMapPointer DSEMapPointer::getReadableCopy() {
  ...
  // Get a copy of this DSEMap. Called when a DSE map is duplicated due to
  // control flow divergence. At present we always do a real copy and update the
  // byte counts of all referenced stores. Take the opportunity to exclude any
  // stores that turn out to have been needed, both here and at the target (the
  // information is of no further value).
  ...
  for (DSEMapTy::iterator it = M->begin(), itend = M->end(); it != itend;) {
    ...
    if (GCStores(it)) {
      ...
      for (DSEMapEntry::const_iterator entryit = oldEntry.begin();
           entryit != oldEntry.end(); ++entryit) {
...
static void derefBytes(DSEMapEntry *entry, uint64_t nBytes) {
  ...
  for (DSEMapEntry::iterator entryit = entry->begin(); entryit != entry->end();
       ++entryit)
...
void DSEMapPointer::release() {
  ...
  // The store entries themselves are not reference counted, so drop refs to all
  // mentioned TrackedStores and delete the map.
  ...
  for (DSEMapTy::iterator it = M->begin(), itend = M->end(); it != itend;
    ...
    // an equality comparison, but we do need to mutate the TrackedStores it
    // contains.
    const DSEMapEntry &oldEntry = it.value();
...
static void setAllNeeded(DSEMapEntry *entry) {
  ...
  for (DSEMapEntry::iterator it = entry->begin(), itend = entry->end();
       it != itend; ++it)
    (*it)->isNeeded = true;
}
...
static void setAllNeeded(DSELocalStore::FrameType &frame) {
  ...
  for (std::vector<DSEMapPointer>::iterator it = frame.store.begin(),
                                            itend = frame.store.end();
       it != itend; ++it) {
...
static void setAllNeeded(DSELocalStore::NodeType *node, uint32_t height) {
  ...
  if (height == 0) {
    ...
    for (uint32_t i = 0, ilim = HEAPTREEORDER; i != ilim; ++i) {
      ...
      DSELocalStore::NodeType *child =
          (DSELocalStore::NodeType *)node->children[i];
      if (child)
...
// Mark all stores alive. Called when an unknown address may be loaded, so all
// stores crossing this point must live.
void llvm::setAllNeededTop(DSELocalStore *store) {

  // No need for CoW breaks: if a location is needed, it is needed everywhere.
  for (SmallVector<DSELocalStore::FrameType *, 4>::iterator
           it = store->frames.begin(),
           itend = store->frames.end();
       it != itend; ++it) {
...
// Called at a control-flow merge: merge the store-tracking information from
// each incoming branch.
void DSEMapPointer::mergeStores(DSEMapPointer *mergeFrom,
                                DSEMapPointer *mergeTo, uint64_t ASize,
                                DSEMerger *Visitor) {

  // Just union the two stores together. They can't be the same store.
  release_assert(mergeFrom != mergeTo && mergeFrom->M != mergeTo->M);

  // Merge the allocation tracking: target map should have it if it doesn't
  // already.
  ...
  // The union should be per-byte, so insert a split in mergeTo whereever one
  // exists in mergeFrom. Take the opportunity to garbage collect: anything with
  // isNeeded set should be omitted.

  for (DSEMapTy::iterator fromit = mergeFrom->M->begin();
       fromit != mergeFrom->M->end();) {
  ...
  for (DSEMapTy::iterator toit = mergeTo->M->begin();
       toit != mergeTo->M->end();) {
  ...
  for (DSEMapTy::iterator fromit = mergeFrom->M->begin(),
    ...
    if (toit == mergeTo->M->end() || toit.start() > fromit.start()) {
      ...
      toit.setStopUnchecked(fromit.start());
    ...
    if (toit != mergeTo->M->end() && toit.start() < fromit.stop() &&
        toit.stop() > fromit.stop()) {

      uint64_t oldStop = toit.stop();
      toit.setStopUnchecked(fromit.stop());
      mergeTo->M->insert(fromit.stop(), oldStop, toit.value());
    }
  }

  // The to-sequence should now be defined everywhere the from-sequence is, and
  // has a break at least as often as the from-sequence. Add missing store refs
  // into the to-sequence and account for the new outstanding bytes that result.
  ...
  for (; fromit != mergeFrom->M->end(); ++fromit) {
    ...
    for (; toit != mergeTo->M->end() && toit.stop() <= fromit.stop(); ++toit) {
      ...
      for (DSEMapEntry::const_iterator it = fromEntry.begin(),
                                       itend = fromEntry.end();
           it != itend && !willAdd; ++it) {

        TrackedStore *fromStore = *it;
        if (std::find(toEntry.begin(), toEntry.end(), fromStore) ==
            toEntry.end())
          willAdd = true;
      }
      ...
      toit.insert(oldStart, oldStop,
                  newEntry); // Restores iterator to same position as before.
    }
  }
}
...
void DSEMapPointer::useWriters(int64_t Offset, uint64_t Size) {
  ...
  // Knock out whole map entries wherever they overlap, because records with
  // isNeeded = true are never any use and are only retained to save from having
  // to keep a reverse index from TrackedStores to maps they are stored in.

  for (DSEMapTy::iterator it = M->find(Offset), itend = M->end();
       it != itend && it.start() < End; it.erase()) {

    const DSEMapEntry &E = it.value();
    for (DSEMapEntry::const_iterator Eit = E.begin(), Eend = E.end();
         Eit != Eend; ++Eit)
...
// Insert a new store (or other mem-writing instruction). May kill an existing
// store (write-after-write) targeting the same location.
void DSEMapPointer::setWriter(int64_t Offset, uint64_t Size,
  ...
  for (DSEMapTy::iterator it = M->find(Offset), itend = M->end();
       it != itend && it.start() < End;) {

    if (((int64_t)it.start()) < Offset) {

      uint64_t oldStop = it.stop();
      it.setStopUnchecked(Offset);
      derefBytes(const_cast<DSEMapEntry *>(&it.value()),
                 std::min(oldStop, End) - Offset);
...
// Merge DSE stores on leaving a call, drawing a store from each live return
// block.
...
// PtrOp is the pointer operand of some memory-reading instruction in block BB,
// reading Size bytes. By now we know the reading instruction will be emitted in
// the specialised program. Mark stores needed as appropriate.
void IntegrationAttempt::DSEHandleRead(ShadowValue PtrOp, uint64_t Size,
                                       ShadowBB *BB) {
...
// Writer (from BB) writes through PtrOp, writing Size bytes. If possible,
// figure out where it writes and record the writer in the block's DSEMap.
void IntegrationAttempt::DSEHandleWrite(ShadowValue PtrOp, uint64_t Size,
                                        ShadowInstruction *Writer,
                                        ShadowBB *BB) {

  // Occasionally zero-length memset or memcpy instructions get here: these are
  // trivially removed by other means so no need to handle the special case
  // here.
  if (Size == 0)
    return;
...
// Main DSE entry point: check if this instruction has any DSE consequences (is
// itself a reader or a killable writer) commitDisabledHere: this context has
// been analysed, but will not be emitted as specialised code, so any reads will
// happen for real at runtime. disableWrites: only prove writers to be needed,
// don't record any new ones in the DSEMap. enterCalls: recurse into calls, used
// when analysing unbounded loops and recursive functions.
void IntegrationAttempt::DSEAnalyseInstruction(ShadowInstruction *I,
                                               bool commitDisabledHere,
                                               bool disableWrites,
                                               bool enterCalls, bool &bail) {
  ...
  // Even if a memcpy or read fails its test, it will overwrite memory one way
  // or another. Therefore it is OK to register their write operation first,
  // before checking if a runtime check will take place.

  if (inst_is<CallInst>(I)) {

    DenseMap<ShadowInstruction *, ReadFile>::iterator RI =
  ...
  if (inst_is<MemIntrinsic>(I)) {
    ...
    if (inst_is<MemTransferInst>(I)) {
      ...
      if (commitDisabledHere || I->isThreadLocal == TLS_MUSTCHECK ||
          !canSynthMTI(I)) {
    ...
    if (InlineAttempt *IA = getInlineAttempt(I)) {
      ...
      IA->tryKillStores(commitDisabledHere || (!IA->isEnabled()),
                        disableWrites);
      ...
      if (RI != pass->resolvedReadCalls.end()) {
        ...
        // Repeat this, as a way of effectively sparing it from being needed
        // because the read requires a runtime check.
        if (requiresRuntimeCheck(ShadowValue(I), true))
          DSEHandleWrite(I->getCallArgOperand(1), RI->second.readSize, I, BB);

      } else if ((F = getCalledFunction(I)) &&
                 (findit = SpecialFunctionMap.find(F)) !=
        ...
        } else if (findit->second == SF_MALLOC ||
                   findit->second == SF_REALLOC) {
        ...
        // in VFSCallModRef are /worst/ case write assumptions used for
        // clobbering, whereas here we need /conservative/ always-overwrites
        // information.
        ...
        if (FI) {
          ...
          for (uint32_t arg = 0, arglim = I->getNumArgOperands(); arg != arglim;
               ++arg) {
            ...
            if (getImprovedValSetSingle(argval, argivs)) {
              ...
              if (argivs.SetType == ValSetTypeScalar ||
                  argivs.SetType == ValSetTypeFD)
                return;
            }
  ...
  } else if (inst_is<LoadInst>(I) || inst_is<AtomicRMWInst>(I) ||
             inst_is<AtomicCmpXchgInst>(I)) {

    // Conveniently, all three classes read from op0.
    // The two Atomics will necessarily have TLS_MUSTCHECK, skipping that bit.
    // The Atomics also write to memory, but I don't call DSEHandleWrite because
    // they should never be deleted in any case.
    ...
    if (I->isThreadLocal != TLS_MUSTCHECK && mayBeReplaced(I) &&
        !commitDisabledHere) {
    ...
    uint64_t StoreSize =
        GlobalTD->getTypeStoreSize(I->invar->I->getOperand(0)->getType());
    DSEHandleWrite(Pointer, StoreSize, I, BB);
  }
}

// Try to kill all stores in this context. Generally DSE processes an
// instruction at a time, but this recursive-descent path is used when analysing
...
void IntegrationAttempt::tryKillStoresInUnboundedLoop(const ShadowLoopInvar *UL,
                                                      bool commitDisabledHere,
                                                      bool disableWrites) {
  ...
  if (!edgeIsDead(getBBInvar(UL->latchIdx), getBBInvar(UL->headerIdx))) {
    ...
    if (!disableWrites) {
      ...
      tryKillStoresInLoop(BB->invar->naturalScope, commitDisabledHere,
                          /*disableWrites=*/false, /*latchToHeader=*/true);
      BB->dseStore = getBB(UL->latchIdx)->dseStore;
    }
    // This time, mark needed writers but don't record any new writers for
    // elimination, in order to note writers from later in the loop that are
    // needed in the event it iterates, but not offer in-loop writers for
    // elimination solely because they aren't needed outside the loop. Give a
    // DSE store to each exit block this time, not the loop header.
    tryKillStoresInLoop(BB->invar->naturalScope, commitDisabledHere,
                        /*disableWrites=*/true, /*latchToHeader=*/false);
...
void IntegrationAttempt::tryKillStoresInLoop(const ShadowLoopInvar *L,
                                             bool commitDisabledHere,
                                             bool disableWrites,
                                             bool latchToHeader) {
  ...
  for (uint32_t i = startIdx, ilim = nBBs + BBsOffset;
    ...
    if (BB->invar->naturalScope != L) {
      ...
      if ((LPA = getPeelAttempt(BB->invar->naturalScope)) &&
        ...
        LPA->Iterations[0]->BBs[0]->dseStore =
            getBB(NewLInfo->preheaderIdx)->dseStore;
        ...
        for (uint32_t j = 0, jlim = LPA->Iterations.size(); j != jlim; ++j) {
          ...
          LPA->Iterations[j]->tryKillStoresInLoop(
              BB->invar->naturalScope, commitDisabled, disableWrites);
          if (j + 1 != jlim)
            LPA->Iterations[j + 1]->BBs[0]->dseStore =
                LPA->Iterations[j]->getBB(latchIdx)->dseStore;
        }

      } else {

        tryKillStoresInUnboundedLoop(
            BB->invar->naturalScope,
            commitDisabledHere || (LPA && !LPA->isEnabled()), disableWrites);
      }
    ...
    if ((!pass->omitChecks) &&
      ...
      // Reaches a path condition check, where unspecialised code might use this
      // value.
      setAllNeededTop(BB->dseStore);
      BB->dseStore = BB->dseStore->getEmptyMap();
    }
    ...
    for (uint32_t j = 0, jlim = BB->insts.size();
         j != jlim && !brokeOnUnreachableCall; ++j) {

      ShadowInstruction *I = &BB->insts[j];
      DSEAnalyseInstruction(I, commitDisabledHere, disableWrites, true,
                            brokeOnUnreachableCall);
    }
