...
// Bug check: make sure null pointer values haven't been accidentally annotated
// as scalars.
...
// We use an IntervalMap (named Map) to describe how component IVSes are laid
// out. They might describe a whole object, or if Underlying is set describe an
// overlay atop that map.
ImprovedValSetMulti::ImprovedValSetMulti(uint64_t ASize)
    : ImprovedValSet(true), Map(GlobalIHP->IMapAllocator), MapRefCount(1),
      Underlying(0), CoveredBytes(0), AllocSize(ASize) {}

ImprovedValSetMulti::ImprovedValSetMulti(const ImprovedValSetMulti &other)
    : ImprovedValSet(true), Map(GlobalIHP->IMapAllocator), MapRefCount(1),
      Underlying(other.Underlying), CoveredBytes(other.CoveredBytes),
      AllocSize(other.AllocSize) {

  if (Underlying)
    Underlying = Underlying->getReadableCopy();

  for (ImprovedValSetMulti::ConstMapIt it = other.Map.begin(),
                                       itend = other.Map.end();
       it != itend; ++it) {
...
// It still might be possible to flatten the maps to discover they represent the
// same information.
bool llvm::operator==(const ImprovedValSetMulti &PB1,
                      const ImprovedValSetMulti &PB2) {

  ImprovedValSetMulti::ConstMapIt it1 = PB1.Map.begin(), it1end = PB1.Map.end(),
                                  it2 = PB2.Map.begin(), it2end = PB2.Map.end();

  for (; it1 != it1end && it2 != it2end; ++it1, ++it2) {

    if (it1.start() != it2.start() || it1.stop() != it2.stop() ||
        it1.value() != it2.value())
      return false;
  }
...
// Check if Ty is a pointer, or a structure, tuple or similar with a pointer
// member.
static bool containsPointerTypes(Type *Ty) {
  ...
  for (Type::subtype_iterator it = Ty->subtype_begin(), it2 = Ty->subtype_end();
       it != it2; ++it) {
...
bool IntegrationAttempt::tryResolveLoadFromVararg(ShadowInstruction *LoadI,
                                                  ImprovedValSet *&Result) {
  ...
  if (PtrPB.SetType == ValSetTypeVarArg && PtrPB.Values.size() == 1) {
    ...
    if (IV.getVaArgType() != ImprovedVal::va_baseptr) {
      ...
      // LPDEBUG("va_arg " << itcache(IV.V) << " " << IV.Offset << " yielded "
      // << printPB(Result) << "\n");
...
bool IntegrationAttempt::tryResolveLoadFromConstant(
    ShadowInstruction *LoadI, ImprovedVal Ptr, ImprovedValSetSingle &Result) {
  ...
  if (ShadowGV *SGV = Ptr.V.getGV()) {
    ...
    if (GV->isConstant()) {
      ...
      getConstSubVal(ShadowValue(GV->getInitializer()), Ptr.Offset, LoadSize,
                     LoadI->getType(), Result);
      return true;
    }
  }
...
static bool tryMultiload(ShadowInstruction *LI, ImprovedValSet *&NewIV,
                         std::string *report) {

  uint64_t LoadSize = GlobalTD->getTypeStoreSize(LI->getType());

  // We already know that LI's IVSet is made up entirely of nulls and definite
  // pointers.
  ...
  std::unique_ptr<raw_string_ostream> RSO(
      report ? new raw_string_ostream(*report) : 0);

  LI->isThreadLocal = TLS_NEVERCHECK;

  for (uint32_t i = 0, ilim = LIPB.Values.size(); i != ilim && !NewPB->Overdef;
       ++i) {
    ...
    // Permit readValRange to allocate and return a multi if appropriate (i.e.
    // if it finds the desired range includes a non-scalar value)
    readValRange(LIPB.Values[i].V, LIPB.Values[i].Offset, LoadSize, LI->parent,
                 ThisPB, LIPB.Values.size() == 1 ? &ThisMulti : 0,
                 ThisError.get());
    ...
    if (RSO.get()) {
      ...
      if (ThisPB.Overdef) {
        ...
        *RSO << "Load " << itcache(LIPB.Values[i].V, true) << " -> "
             << *ThisError;
...
bool IntegrationAttempt::tryForwardLoadPB(ShadowInstruction *LI,
                                          ImprovedValSet *&NewPB,
                                          bool &loadedVararg) {

  ImprovedValSetSingle ConstResult;
  std::unique_ptr<std::string> error(pass->verboseOverdef ? new std::string()
                                                          : 0);
...
// Get the heap object ID for this ShadowValue. Only applies if this is a
// pointer or special (symbolic) object. Return -1 if this is a non-pointer or
// unknown pointer.
int32_t ShadowValue::getHeapKey() const {
  ...
  case SHADOWVAL_ARG:
    release_assert((u.A->IA->isRootMainCall()) &&
                   "getHeapKey on arg other than root argv?");
...
ShadowValue &llvm::getAllocWithIdx(int32_t idx) {
  ...
  // Warning: need to be sure the allocation still exists, really only for
  // debugging.
  return GlobalIHP->heap[idx].allocValue;
}

// Check this is a pointer and get the frame number it refers to (stack index,
// or -1 if heap allocated or special)
...
// Verbose printout of how many layers of stacked ImprovedValSetMultis are
// involved in IVS.
...
// willWriteSingleObject permits a shortcut in which we allocate space for a
// single object instead of an extent-list as needed for structs etc.
LocStore *ShadowBB::getWritableStoreFor(ShadowValue &V, int64_t Offset,
                                        uint64_t Size,
                                        bool willWriteSingleObject) {
  ...
  // We must return a LocStore for that value that can be updated (i.e. is not
  // shared). We need to write into the block-local store map. COW break it if
  // necessary:
  ...
  if (isNewStore) {
    ...
    // There wasn't an entry in the local map. Make a Single or Multi store
    // depending on whether we're about to cover the whole store or not:
  ...
  if (writeWholeObject && willWriteSingleObject) {
    ...
    // If we're about to overwrite the whole thing with a single, convert a
    // multi to a single.
    ...
    if (ImprovedValSetMulti *M = dyn_cast<ImprovedValSetMulti>(ret->store)) {
      ...
      M->dropReference();
      ret->store = new ImprovedValSetSingle();
      LFV3(errs() << "Free multi " << M << " and replace with single "
                  << ret->store << "\n");
    ...
    // If we're doing a partial overwrite, make sure a multi is writable and
    // promote a single to a multi with that single as base.
    if (!ret->store->isWritableMulti()) {
      ...
      if (isa<ImprovedValSetMulti>(ret->store))
        ...
        LFV3(errs() << "Break single -> multi " << ret->store << " -> "
                    << NewIMap << "\n");
      if (writeWholeObject) {
        ...
        // M's refcount remains unchanged, it's just now referenced as a base
        // rather than being directly used here.
      }
      ret->store = NewIMap;
...
Constant *llvm::extractAggregateMemberAt(Constant *FromC, int64_t Offset,
                                         Type *Target, uint64_t TargetSize,
                                         const DataLayout *TD) {
  ...
  if (Offset == 0 && TargetSize == FromSize) {
    ...
    LLVM_DEBUG(
        dbgs()
        << "Can't use simple element extraction because load implies cast from "
        << (*(FromType)) << " to " << (*Target) << "\n");
    return 0;
  }
  ...
  if (Offset < 0 || Offset + TargetSize > FromSize) {
    ...
    LLVM_DEBUG(dbgs() << "Can't use element extraction because offset "
                      << Offset << " and size " << TargetSize
                      << " are out of bounds for object with size " << FromSize
                      << "\n");
    return 0;
  }
  ...
  if (ConstantArray *CA = dyn_cast<ConstantArray>(FromC)) {
    ...
    if (!SL) {
      LLVM_DEBUG(dbgs() << "Couldn't get struct layout for type "
                        << *(CS->getType()) << "\n");
      return 0;
    }
  ...
  if (mightWork) {
    if (StartE == EndE || (StartE + 1 == EndE && !EndOff)) {
      ...
      return extractAggregateMemberAt(cast<Constant>(FromC->getOperand(StartE)),
                                      StartOff, Target, TargetSize, TD);
    }
    LLVM_DEBUG(dbgs() << "Can't use simple element extraction because load "
                         "spans multiple elements\n");
  } else {
    LLVM_DEBUG(dbgs() << "Can't use simple element extraction because load "
                         "requires sub-field access\n");
  }

  return 0;
}

// Try to read V[Offset:Offset+Size], which has symbolic object 'store', in the
// context of ReadBB. The result is written to Result, or to ResultPV if it is
// necessary to build the result bytewise, e.g. because the read type doesn't
// match the write type or the read is fed by multiple stores. Set
// shouldTryMulti if we note a case where an ImprovedValSetMulti (extent list)
// could accommodate the result but ImprovedValSetSingle cannot. That usually
// happens when reading struct types, vector types and similar that contain
// pointers and so can't be expressed as bytes. In debug builds, give a verbose
// reason why the read failed in 'error'.
void llvm::readValRangeFrom(ShadowValue &V, uint64_t Offset, uint64_t Size,
                            ShadowBB *ReadBB, ImprovedValSet *store,
                            ImprovedValSetSingle &Result, PartialVal *&ResultPV,
                            bool &shouldTryMulti, std::string *error) {
  ...
  if (!IVS) {
    ...
    if (it != IVM->Map.end() && it.start() <= Offset &&
      ...
      Offset -= it.start();
      LFV3(errs() << "Read fully defined by multi subval " << it.start() << "-"
                  << it.stop() << "\n");
    }
  }
  ...
  if (IVS) {
    ...
    if (!ResultPV) {
      ...
      // Try to extract the entire value. Scalar splat values need expanding
      // anyway, so fall through in that case.
      if (IVSSize == Size && Offset == 0 &&
          IVS->SetType != ValSetTypeScalarSplat) {
        Result = *IVS;
      ...
      bool rejectHere =
          IVS->isWhollyUnknown() || (IVS->SetType != ValSetTypeScalar &&
                                     IVS->SetType != ValSetTypeScalarSplat);
      if (rejectHere) {
      ...
      if (IVS->SetType == ValSetTypeScalar) {
        ...
        for (uint32_t i = 0, endi = IVS->Values.size(); i != endi; ++i) {
          ...
          Constant *smallConst =
              extractAggregateMemberAt(bigConst, Offset, 0, Size, GlobalTD);
          if (smallConst) {
    ...
    if (!addIVSToPartialVal(*IVS, Offset, 0, Size, ResultPV, error)) {
      ...
      release_assert(ResultPV->isComplete() &&
                     "Fetch defined by a Single value but not complete?");
  ...
  // If we get to here the value is not wholly covered by this Multi map. Add
  // what we can and defer:
  ...
  for (; it != IVM->Map.end() && it.start() < (Offset + Size); ++it) {
    ...
    if (!addIVSToPartialVal(it.value(), FirstReadByte - it.start(),
                            FirstReadByte - Offset,
                            LastReadByte - FirstReadByte, ResultPV, error)) {
      delete ResultPV;
      ResultPV = 0;
      Result.setOverdef();

      if (it.value().SetType == ValSetTypePB ||
          it.value().SetType == ValSetTypeFD) {
        if (FirstReadByte == it.start() && LastReadByte == it.stop()) {

          // This read would read a whole FD or pointer, but can't because we
          // can't express those as bytes. It's therefore worth trying again
          // with a more expensive multi descriptor.
  ...
  if ((!ResultPV) || !ResultPV->isComplete()) {
    ...
    // If this is an out-of-bounds read it's legitimate to reach the bottom
    // without resolution:
    if (!IVM->Underlying) {
      ...
      readValRangeFrom(V, Offset, Size, ReadBB, IVM->Underlying, Result,
                       ResultPV, shouldTryMulti, error);
    }
  }
}

// Try to read V[Offset:Offset+Size], in the context of ReadBB, storing the
// result in Result or ResultMulti if an extent-list is necessary. In debug
// builds, give a verbose error report as 'error'.
void llvm::readValRange(ShadowValue &V, int64_t Offset, uint64_t Size,
                        ShadowBB *ReadBB, ImprovedValSetSingle &Result,
                        ImprovedValSetMulti **ResultMulti, std::string *error) {

  // Try to make an IVS representing the block-local value of V+Offset -> Size.
  // Limitations for now: because our output is a single IVS, non-scalar types
  // may only be described if they correspond to a whole object.
  ...
  if (!firstStore) {
    if (ReadBB->localStore->allOthersClobbered) {
      ...
      Value *UD = UndefValue::get(
          Type::getIntNTy(ReadBB->invar->BB->getContext(), Size));
  ...
  if(logReadDepth(firstStore->store, 1) >= 10) {
    ...
    errs() << "Deep read for " << itcache(V, true) << " from " <<
  ReadBB->invar->BB->getName() << " / " << ReadBB->IA->SeqNumber << "\n";
  ...
  */
  ...
  readValRangeFrom(V, Offset, Size, ReadBB, firstStore->store, Result, ResultPV,
                   shouldTryMulti, error);
  ...
  if (ResultPV) {
    ...
    // Relabel offsets relative to this read, and check whether we're returning
    // anything worthwhile (i.e. not overdef'd).
    bool anyGoodValues = false;
    for (SmallVector<IVSRange, 4>::iterator it = Results.begin(),
                                            itend = Results.end();
         it != itend; ++it) {
    ...
    if (anyGoodValues) {
      ...
      ImprovedValSetMulti *IVM =
          new ImprovedValSetMulti(ReadBB->getAllocSize(V));
...
bool llvm::allowTotalDefnImplicitPtrToInt(Type *From, Type *To,
                                          const DataLayout *TD) {

  return From->isPointerTy() && To->isIntegerTy() &&
         TD->getTypeSizeInBits(To) >= TD->getTypeSizeInBits(From);
}

// Check if we can coerce this value to Target type.
// TODO: Discover problems without running the whole check twice.
bool ImprovedValSetSingle::canCoerceToType(Type *Target, uint64_t TargetSize,
                                           std::string *error,
                                           bool allowImplicitPtrToInt) {

  ImprovedValSetSingle Copy(*this);
  return Copy.coerceToType(Target, TargetSize, error, allowImplicitPtrToInt);
}

bool ImprovedValSetSingle::coerceToType(Type *Target, uint64_t TargetSize,
                                        std::string *error,
                                        bool allowImplicitPtrToInt) {
  ...
  if (allowTotalDefnImplicitCast(Source, Target))
    return true;
  if (allowImplicitPtrToInt &&
      allowTotalDefnImplicitPtrToInt(Source, Target, GlobalTD))
    return true;

  bool mightWork = (SetType == ValSetTypeScalar ||
                    (SetType == ValSetTypePB && onlyContainsNulls()));
  ...
  for (uint32_t i = 0, iend = Values.size(); i != iend; ++i) {
    ...
    if (containsPointerTypes(Target)) {
      ...
      // If we're trying to synthesise a pointer from raw bytes, only a null
      // pointer is allowed.
    ...
    Values[i].V = ShadowValue(
        constFromBytes((unsigned char *)PV.partialBuf, Target, GlobalTD));
  }
...
#define AddIVSSV(x, y, z)                                                      \
  do {                                                                         \
    Dest.push_back(                                                            \
        IVSR(x + OffsetAbove, x + y + OffsetAbove,                             \
             ImprovedValSetSingle(ImprovedVal(z), ValSetTypeScalar)));         \
  } while (0);

// As above, but turn a constant into a symbolic object first.
#define AddIVSConst(x, y, z)                                                   \
  do {                                                                         \
    std::pair<ValSetType, ImprovedVal> V = getValPB(z);                        \
    Dest.push_back(IVSR(x + OffsetAbove, x + y + OffsetAbove,                  \
                        ImprovedValSetSingle(V.second, V.first)));             \
  } while (0);
...
void llvm::executeStoreInst(ShadowInstruction *StoreSI) {
  ...
  uint64_t PtrSize =
      GlobalTD->getTypeStoreSize(StoreSI->invar->I->getOperand(0)->getType());

  ImprovedValSetSingle PtrSet;
  release_assert(getImprovedValSetSingle(Ptr, PtrSet) &&
                 "Write through uninitialised PB?");
  if (!(PtrSet.isWhollyUnknown() || PtrSet.SetType == ValSetTypePB))
    PtrSet.setOverdef();

  ShadowValue Val = StoreSI->getOperand(0);

  // TODO: do better than marking a value as escaped whenever a pointer to it is
  // stored.
  valueEscaped(Val, StoreSI->parent);

  if (ImprovedValSetMulti *IVM =
          dyn_cast_or_null<ImprovedValSetMulti>(tryGetIVSRef(Val))) {
    ...
    if (PtrSet.Values.size() != 1) {
      ...
      for (ImprovedValSetMulti::MapIt it = IVM->Map.begin(),
        ...
        Vals.push_back(IVSR(it.start() + PtrSet.Values[0].Offset,
                            it.stop() + PtrSet.Values[0].Offset, it.value()));
      }

      writeExtents(Vals, PtrSet.Values[0].V, PtrSet.Values[0].Offset, PtrSize,
                   StoreBB);
...
void llvm::executeMemsetInst(ShadowInstruction *MemsetSI) {
  ...
  ConstantInt *LengthCI = dyn_cast_or_null<ConstantInt>(
      getConstReplacement(MemsetSI->getCallArgOperand(2)));
  ConstantInt *ValCI = dyn_cast_or_null<ConstantInt>(
      getConstReplacement(MemsetSI->getCallArgOperand(1)));
  ...
  executeWriteInst(&Ptr, PtrSet, ValSet,
                   LengthCI ? LengthCI->getLimitedValue() : ULONG_MAX,
                   MemsetSI);
}

bool llvm::executeAtomicRMW(ShadowInstruction *SI, ImprovedValSet *&OldPB,
                            bool &loadedVararg) {
  ...
  // The tryForwardLoad call may set SI->isThreadLocal if the object we're
  // reading may be global.
  ...
  for (uint32_t i = 0, ilim = LoadedPB.Values.size(); i != ilim; ++i) {
    ...
    for (uint32_t j = 0, jlim = ValPB.Values.size(); j != jlim; ++j) {
      ...
      case AtomicRMWInst::UMax:
        newVal = std::max(op1Val, op2Val);
        break;
      case AtomicRMWInst::UMin:
        newVal = std::min(op1Val, op2Val);
        break;
      default:
...
bool llvm::executeCmpXchg(ShadowInstruction *SI, ImprovedValSet *&OldPB,
                          bool &loadedVararg) {
...
// Read Src[Offset:Offset+Size] into Dest, a list of extents, adding OffsetAbove
// to all extent offsets. This usually means a single extent, but might
// disintegrate a composite constant into multiple extents.
void llvm::getIVSSubVals(const ImprovedValSetSingle &Src, uint64_t Offset,
                         uint64_t Size, int64_t OffsetAbove,
                         SmallVector<IVSRange, 4> &Dest) {

  // Subvals only allowed for scalars:

  if (Src.isWhollyUnknown() || Src.Values.size() == 0) {
    Dest.push_back(
        IVSR(OffsetAbove + Offset, OffsetAbove + Offset + Size, Src));
    return;
  }
  ...
  default:
    ...
    Dest.push_back(IVSR(OffsetAbove + Offset, OffsetAbove + Offset + Size,
                        ImprovedValSetSingle(ValSetTypeUnknown, true)));
    return;
  }
  ...
  if (Src.Values.size() == 1) {
    ...
    // Punt on the tricky business of merging potentially misaligned sets of
    // constants for now; only allow subvalues expressible as a single constant.
    ...
    Dest.push_back(
        IVSR(OffsetAbove + Offset, OffsetAbove + Offset + Size, DestSingle));
  }
}

// Try to extract Src[Offset:Offset+Size] into Dest.
void llvm::getIVSSubVal(const ImprovedValSetSingle &Src, uint64_t Offset,
                        uint64_t Size, ImprovedValSetSingle &Dest) {
...
// Makes some effort to coalesce PBs (e.g. using a big ConstantArray rather than
// an extent per element) but could do more. Writes Overdef extents where we
// couldn't read the source constant. OffsetAbove specifies all recorded extents
// should have OffsetAbove added; saves post-processing when making a subquery.
void llvm::getConstSubVals(ShadowValue FromSV, uint64_t Offset,
                           uint64_t TargetSize, int64_t OffsetAbove,
                           SmallVector<IVSRange, 4> &Dest) {
  ...
  if (Offset + TargetSize > FromSize) {
    ...
    Dest.push_back(IVSR(OffsetAbove + FromSize,
                        OffsetAbove + Offset + TargetSize,
                        ImprovedValSetSingle(ValSetTypeUnknown, true)));
    return;
  }
  ...
  // * ConstantDataSequential / ConstantAggregateZero / vectors / primitives: Do
  // byte-wise constant extraction.
  ...
  if (ConstantArray *CA = dyn_cast<ConstantArray>(FromC)) {
    ...
    if (StartOff) {
      ...
      getConstSubVals(ShadowValue(CA->getAggregateElement(StartE)), StartOff,
                      ThisReadSize, OffsetAbove + (ESize * StartE), Dest);
    ...
    if (EndE - StartE == 1) {
      ...
      AddIVSConst(StartE * ESize, ESize * (EndE - StartE),
                  ConstantArray::get(CA->getType(), subArray));
    }

    // Read final subelement
    if (EndOff)
      getConstSubVals(ShadowValue(CA->getAggregateElement(EndE)), 0, EndOff,
                      OffsetAbove + (ESize * EndE), Dest);
    ...
    if (!SL) {
      LLVM_DEBUG(dbgs() << "Couldn't get struct layout for type "
                        << *(CS->getType()) << "\n");
      Dest.push_back(IVSR(Offset, TargetSize,
                          ImprovedValSetSingle(ValSetTypeUnknown, true)));
      return;
    }
    ...
    if (StartOff) {
      ...
      getConstSubVals(ShadowValue(StartC), StartOff, ThisReadSize,
                      OffsetAbove + SL->getElementOffset(StartE), Dest);
    ...
    for (; StartE < EndE; ++StartE) {
      ...
      if (StartE + 1 < CS->getType()->getNumElements()) {
        ...
        if (PaddingBytes) {
          ...
          Type *PaddingType =
              Type::getIntNTy(FromC->getContext(), PaddingBytes * 8);
    ...
    if (EndOff) {
      ...
      getConstSubVals(ShadowValue(E), 0, EndOff,
                      OffsetAbove + SL->getElementOffset(EndE), Dest);
    }
    ...
    // C is a primitive, constant-aggregate-zero, constant-data-array or
    // similar. Attempt bytewise extraction and present as a CDA.
    SmallVector<uint8_t, 16> Buffer(TargetSize);
    if (XXXReadDataFromGlobal(FromC, Offset, Buffer.data(), TargetSize,
                              *GlobalTD)) {
      ...
      if (TargetSize <= 8) {
        ...
        SubC = ConstantDataArray::get(FromC->getContext(),
...
Constant *llvm::valsToConst(SmallVector<IVSRange, 4> &subVals,
                            uint64_t TargetSize, Type *targetType) {

  if (subVals.size() == 0)
    return 0;

  for (SmallVector<IVSRange, 4>::iterator it = subVals.begin(),
                                          itend = subVals.end();
       it != itend; ++it) {
  ...
  for (SmallVector<IVSRange, 4>::iterator it = subVals.begin(),
    ...
    if (!XXXReadDataFromGlobal(getSingleConstant(it->second.Values[0].V), 0,
                               ReadPtr, it->first.second - it->first.first,
                               *GlobalTD))
      return 0;
  }
...
// Try to extract FromSV[Offset:Offset+TargetSize] into Result, which should
// have type TargetType.
void llvm::getConstSubVal(ShadowValue FromSV, uint64_t Offset,
                          uint64_t TargetSize, Type *TargetType,
                          ImprovedValSetSingle &Result) {
...
// Try to extract a sub-constant at FromC[Offset:Offset+TargetSize], usually a
// struct member.
Constant *llvm::getSubConst(Constant *FromC, uint64_t Offset,
                            uint64_t TargetSize, Type *targetType) {

  SmallVector<IVSRange, 4> subVals;
  getConstSubVals(ShadowValue(FromC), Offset, TargetSize, -((int64_t)Offset),
                  subVals);

  return valsToConst(subVals, TargetSize, targetType);
}

// Overwrite Target[Offset:Offset+Size] with NewVal. Target is a symbolic memory
// object; ensuring it is writable and resolving pointers etc has already been
// taken care of here.
void llvm::replaceRangeWithPB(ImprovedValSet *Target,
                              const ImprovedValSetSingle &NewVal,
  ...
  if (ImprovedValSetSingle *S = dyn_cast<ImprovedValSetSingle>(Target)) {
    *S = NewVal;
    ...
    if (Size == ULONG_MAX) {
      ...
      release_assert(NewVal.Overdef &&
                     "Indefinite write with non-clobber value?");
    }
    ...
    if (M->Underlying && M->CoveredBytes == M->AllocSize) {
      ...
      // This Multi now defines the whole object: drop the underlying object as
...
void llvm::clearRange(ImprovedValSetMulti *M, uint64_t Offset, uint64_t Size) {
  ...
  if (found.start() < Offset) {
    ...
    // Back up the stop() value here, are some truncateRight paths will adjust
    // it. Also back up start() as the iterator will be moved.
    ...
    // Some truncate paths don't update the value's stop index (e.g. the
    // setOverdef just above) so ensure it is correct here:
    ...
    if (RHS.isInitialised()) {
      ...
      // Insert the original value with its original value. This will
      // temporarily produce an invalid interval map (e.g. 0-16: [i32 1, i32 2,
      // i32 3, i32 4] where we're replacing 4-8 will have become 0-4: i32 1,
      // (4-8 blanked), 0-16: [i32 1, i32 2, i32 3, i32 4]; therefore
      // truncateLeft
      // -> truncateConstVal must not do any imap searches.
      // The setStartUnchecked command below will make the map well-formed
      // again.
  ...
  while (found != M->Map.end() && found.start() < LastByte &&
         found.stop() <= LastByte) {
...
// Target is a symbolic memory object. Overwrite Target[Offset:Offset+Size] with
// extent-list NewVals.
void llvm::replaceRangeWithPBs(ImprovedValSet *Target,
                               const SmallVector<IVSRange, 4> &NewVals,
...
// 'it' points to a scalar which is part of some symbolic memory object.
// Truncate it to leave byte range off - off+size. Leave firstPtr pointing to
// the start and 'it' pointing to the last newly-inserted range.
void llvm::truncateConstVal(ImprovedValSetMulti::MapIt &it, uint64_t off,
                            uint64_t size,
                            ImprovedValSetMulti::MapIt &firstPtr) {

  const ImprovedValSetSingle &S = it.value();
  firstPtr = it;

  // Dodge problem of taking e.g. { complex_val, other_complex_val } that
  // split into multiple values and then recombining: only allow value splitting
  // for singleton sets.
  if (S.Values.size() == 1) {

    SmallVector<IVSRange, 4> SubVals;
    Constant *OldC = getSingleConstant(S.Values[0].V);
    getConstSubVals(ShadowValue(OldC), off, size,
                    /* reporting offset = */ it.start(), SubVals);
    if (SubVals.size() == 1) {
      ...
      for (SmallVector<IVSRange, 4>::iterator valit = SubVals.begin(),
                                              valend = SubVals.end();
           valit != valend; ++valit) {

        it.insert(valit->first.first, valit->first.second, valit->second);

        // Record an iterator pointing to the start of the replacement sequence
        // if our caller needs it.
...
// 'it' points into a symbolic memory object. Trim it on the right-hand side to
// have length 'n'.
void llvm::truncateRight(ImprovedValSetMulti::MapIt &it, uint64_t n) {
  ...
  if (S.SetType == ValSetTypeScalarSplat) {
    ...
    it.insert(oldStart, oldStop,
              newVal); // Moves iterator to point at new entry
    return;
  }
...
// 'it' points into a symbolic memory object. Trim it on the left-hand side to
// have length 'n'. Leave 'it' pointing at the last and 'firstPtr' pointing at
// the first new entry in the IVSMulti. They can differ e.g. when we break a
// struct value into fields.
void llvm::truncateLeft(ImprovedValSetMulti::MapIt &it, uint64_t n,
  ...
  // On most paths we'll just modify it in place, and so firstPtr will continue
  // to point to the start of the truncated value; however truncateConstVal
  // might break 'it' into several smaller extents and set firstPtr
  // appropriately.
  firstPtr = it;
...
// Is S suitable to be truncated? For example truncating a pointer is
// meaningless.
bool llvm::canTruncate(const ImprovedValSetSingle &S) {

  return S.isWhollyUnknown() || S.SetType == ValSetTypeScalar ||
         S.SetType == ValSetTypeScalarSplat;
}

// 'store' is a symbolic memory object.
// Read store[Offset:Offset+Size] into extent-list 'Results'.
// ASize is the total size of 'store'. If 'ignoreBelowStore' is set and 'store'
// is an IVSMulti
//   (which can be a stack of overlaid extent lists), treat 'ignoreBelowStore'
//   as the bottom of the stack.
void llvm::readValRangeMultiFrom(uint64_t Offset, uint64_t Size,
                                 ImprovedValSet *store,
                                 SmallVector<IVSRange, 4> &Results,
                                 ImprovedValSet *ignoreBelowStore,
                                 uint64_t ASize) {

  if (!store) {

    Value *UD = UndefValue::get(Type::getIntNTy(GInt8->getContext(), Size));
    Results.push_back(IVSR(
        Offset, Offset + Size,
        ImprovedValSetSingle(ImprovedVal(ShadowValue(UD)), ValSetTypeScalar)));
    return;
  }
  ...
  if (ImprovedValSetSingle *IVS = dyn_cast<ImprovedValSetSingle>(store)) {
    ...
    if (IVS->SetType == ValSetTypeDeallocated) {
      ...
      Results.push_back(IVSR(Offset, Offset + Size,
                             ImprovedValSetSingle(ValSetTypeUnknown, true)));
    ...
    if (it != IVM->Map.end() && it.start() < Offset) {
      ...
      LFV3(errs() << "Add val at " << it.start() << "-" << it.stop()
                  << " subval " << SubvalOffset << "-"
                  << (SubvalOffset + SubvalSize) << "\n");
    ...
    while (it != IVM->Map.end() && it.start() < (Offset + Size)) {
      ...
      if (it.start() != Offset) {
        ...
        release_assert(it.start() > Offset &&
                       "Overlapping-on-left should be caught already");
        // Gap -- defer this bit to our parent map. If there is none, the value
        // is undefined here.
        ...
        if (!IVM->Underlying) {
          ...
          Results.push_back(
              IVSR(Offset, Offset + Size,
                   ImprovedValSetSingle(ImprovedVal(UD), ValSetTypeScalar)));

        } else {

          LFV3(errs() << "Defer to underlying map " << IVM->Underlying
                      << " for range " << Offset << "-" << it.start() << "\n");
          readValRangeMultiFrom(Offset, it.start() - Offset, IVM->Underlying,
                                Results, ignoreBelowStore, ASize);
        }
    ...
    if (Size != 0) {
      ...
      if (!IVM->Underlying) {
        ...
        Type *UndefType =
            IntegerType::get(GInt8Ptr->getContext(), UndefSize * 8);
        Value *UD = UndefValue::get(UndefType);
        Results.push_back(
            IVSR(Offset, Offset + Size,
                 ImprovedValSetSingle(ImprovedVal(UD), ValSetTypeScalar)));
        ...
        LFV3(errs() << "Defer to underlying map " << IVM->Underlying
                    << " for range " << Offset << "-" << (Offset + Size)
                    << " (end path)\n");
        readValRangeMultiFrom(Offset, Size, IVM->Underlying, Results,
                              ignoreBelowStore, ASize);
      }
    }
  }
}

void llvm::readValRangeMulti(ShadowValue &V, uint64_t Offset, uint64_t Size,
                             ShadowBB *ReadBB,
                             SmallVector<IVSRange, 4> &Results) {
  ...
  if (ShadowGV *G = V.getGV()) {
    ...
    if (G->G->isConstant()) {
      ...
      getConstSubVals(ShadowValue(G->G->getInitializer()), Offset, Size, 0,
                      Results);
      return;
    }
  }
  ...
  if (!firstStore) {
    if (ReadBB->localStore->allOthersClobbered) {
      ...
      Value *UD = UndefValue::get(
          Type::getIntNTy(ReadBB->invar->BB->getContext(), Size));
      Results.push_back(IVSR(Offset, Offset + Size,
                             ImprovedValSetSingle(ImprovedVal(ShadowValue(UD)),
                                                  ValSetTypeScalar)));
      return;
    }
  } else {
  ...
  readValRangeMultiFrom(Offset, Size, firstStore->store, Results, 0,
                        ReadBB->getAllocSize(V));
}
...
void llvm::executeMemcpyInst(ShadowInstruction *MemcpySI) {
  ...
  ConstantInt *LengthCI = dyn_cast_or_null<ConstantInt>(
      getConstReplacement(MemcpySI->getCallArgOperand(2)));

  ShadowValue SrcPtr = MemcpySI->getCallArgOperand(1);
  ImprovedValSetSingle SrcPtrSet;
  release_assert(getImprovedValSetSingle(SrcPtr, SrcPtrSet) &&
                 "Memcpy from uninitialised PB?");
  if (!(SrcPtrSet.isWhollyUnknown() || SrcPtrSet.SetType == ValSetTypePB))
    SrcPtrSet.setOverdef();

  executeCopyInst(&Ptr, PtrSet, SrcPtrSet,
                  LengthCI ? LengthCI->getLimitedValue() : ULONG_MAX, MemcpySI);
}
...
void llvm::executeAllocInst(ShadowInstruction *SI, AllocData &AD,
                            Type *AllocType, uint64_t AllocSize, int32_t frame,
                            uint32_t idx) {

  // Represent the store by a big undef value at the start, or if !AllocType
  // (implying AllocSize
  ...
  LocStore &localStore = *SI->parent->getWritableStoreFor(
      AllocSV, 0, AllocSize, /* willWriteSingle = */ true);
  ...
  // Note that the new object is unreachable from old objects, thread-local and
  // unescaped.
...
static void markVagueAllocation(ShadowInstruction *SI) {
  ...
  release_assert(SI->i.PB && isa<ImprovedValSetSingle>(SI->i.PB) &&
                 IVS->SetType == ValSetTypePB);
  IVS->Values[0].V.getAllocData(SI->parent->localStore)->allocVague = true;
  // errs() << "Allocation " << itcache(SI) << " in " <<
  // SI->parent->IA->SeqNumber << " vague\n";
}

void llvm::executeAllocaInst(ShadowInstruction *SI) {

  // If the store is already initialised this must represent the general case of
  // an allocation within a loop or recursive call.

  // Don't mark allocas vague unless they occur within a function-local loop:
  // because they are deallocated on function exit, a pointer always refers to
  // the latest version of this instruction. Compare a malloc in a similar
  // situation: if we have while(dyn) { f(); } where f() calls malloc, that
  // object may refer to any f in the unbounded loop, not just the most recent
  // call.
  ...
  if (AI->isArrayAllocation()) {
    ...
    ConstantInt *N =
        cast_or_null<ConstantInt>(getConstReplacement(AI->getArraySize()));
    if (!N)
      allocType = 0;
    else
  ...
  executeAllocInst(SI, AD, allocType,
                   allocType ? GlobalTD->getTypeStoreSize(allocType)
                             : ULONG_MAX,
                   parentIA->stack_depth, allocIdx);
}
...
static void executeMallocInst2(ShadowInstruction *SI, AllocatorFn &param) {
  ...
  if (param.isConstantSize)
    ...
    AllocSize = cast_or_null<ConstantInt>(
        getConstReplacement(SI->getCallArgOperand(param.sizeArg)));
  Type *allocType = 0;
  if (AllocSize)
    allocType = ArrayType::get(Type::getInt8Ty(SI->invar->I->getContext()),
  ...
  executeAllocInst(SI, AD, allocType,
                   AllocSize ? AllocSize->getLimitedValue() : ULONG_MAX, -1,
                   GlobalIHP->heap.size() - 1);
}
...
void llvm::executeFreeInst(ShadowInstruction *SI, Function *FreeF) {
  ...
  ImprovedValSetSingle *FreedIVS =
      dyn_cast_or_null<ImprovedValSetSingle>(FreedPtr->i.PB);
  if (!FreedIVS)
    return;
  ...
  executeWriteInst(0, *FreedIVS, TagIVS,
                   SI->parent->getAllocSize(FreedIVS->Values[0].V), SI);
}
...
void llvm::executeReallocInst(ShadowInstruction *SI, Function *F) {
  ...
  if (!SI->i.PB) {
    ...
    ConstantInt *AllocSize = cast_or_null<ConstantInt>(
        getConstReplacement(SI->getCallArgOperand(Re.sizeArg)));
    Type *allocType = 0;
    if (AllocSize)
      allocType = ArrayType::get(Type::getInt8Ty(SI->invar->I->getContext()),
                                 AllocSize->getLimitedValue());
...
static void checkStore(ImprovedValSet* IV, ShadowValue& V) {
  ...
  if(IV->isMulti) {
    ...
    for(ImprovedValSetMulti::MapIt it = IVM->Map.begin(), itend =
IVM->Map.end(); it != itend; ++it) { release_assert(it.start() >= lastOffset);
      lastOffset = it.stop();
...
// Write extent-list copyValues through to (*Ptr)[Offset:Offset+Size], in the
// context of BB.

void llvm::writeExtents(SmallVector<IVSRange, 4> &copyValues, ShadowValue &Ptr,
                        int64_t Offset, uint64_t Size, ShadowBB *BB) {

  LocStore *Store =
      BB->getWritableStoreFor(Ptr, Offset, Size, copyValues.size() == 1);
...
void llvm::executeCopyInst(ShadowValue *Ptr, ImprovedValSetSingle &PtrSet,
                           ImprovedValSetSingle &SrcPtrSet, uint64_t Size,
                           ShadowInstruction *CopySI) {
  ...
  // No need to check the copy instruction is as expected in any of the coming
  // failure cases.
  CopySI->isThreadLocal = TLS_NEVERCHECK;

  if (Size == ULONG_MAX || PtrSet.isWhollyUnknown() ||
      PtrSet.Values.size() != 1 || PtrSet.Values[0].Offset == LLONG_MAX ||
      SrcPtrSet.isWhollyUnknown() || SrcPtrSet.Values.size() != 1 ||
      SrcPtrSet.Values[0].Offset == LLONG_MAX) {

    // Only support memcpy from single pointer to single pointer for the time
    // being: If we're known to be copying scalars, note that in our overdef to
    // prevent unnecessary clobbers.
    ...
    if (!SrcPtrSet.isWhollyUnknown()) {
      ...
      for (uint32_t i = 0; i < SrcPtrSet.Values.size() && !foundPointers; ++i) {
        ...
        if (G && G->G->isConstant()) {
          ...
          // Pointer to a constant. See if the pointed object itself has
          // pointers:
  ...
  readValRangeMulti(SrcPtrSet.Values[0].V, SrcPtrSet.Values[0].Offset, Size, BB,
                    copyValues);
  ...
  for (SmallVector<IVSRange, 4>::iterator it = copyValues.begin(),
    ...
    // The copied values are labelled according to source offsets; relabel for
    // the destination.
    it->first.first += OffDiff;
    it->first.second += OffDiff;
  }

  // OK now blow a hole in the local map for that value and write this list of
  // extents into the gap:
  writeExtents(copyValues, PtrSet.Values[0].V, PtrSet.Values[0].Offset, Size,
               BB);

  for (SmallVector<IVSRange, 4>::iterator it = copyValues.begin(),
                                          itend = copyValues.end();
       it != itend; ++it) {
...
void llvm::executeVaStartInst(ShadowInstruction *SI) {
  ...
  ImprovedValSetSingle nonFPOffset = ImprovedValSetSingle(
      ImprovedVal(ShadowValue(SI), ImprovedVal::first_nonfp_arg),
      ValSetTypeVarArg);
  vaStartVals.push_back(IVSR(0, 4, nonFPOffset));

  ImprovedValSetSingle FPOffset = ImprovedValSetSingle(
      ImprovedVal(ShadowValue(SI), ImprovedVal::first_fp_arg),
      ValSetTypeVarArg);
  vaStartVals.push_back(IVSR(4, 8, FPOffset));

  ImprovedValSetSingle AnyPtr = ImprovedValSetSingle(
      ImprovedVal(ShadowValue(SI), ImprovedVal::first_any_arg),
      ValSetTypeVarArg);
  vaStartVals.push_back(IVSR(8, 16, AnyPtr));

  ImprovedValSetSingle StackBase = ImprovedValSetSingle(
      ImprovedVal(ShadowValue(SI), ImprovedVal::va_baseptr), ValSetTypeVarArg);
  vaStartVals.push_back(IVSR(16, 24, StackBase));

  LocStore *Store = BB->getWritableStoreFor(PtrSet.Values[0].V,
                                            PtrSet.Values[0].Offset, 24, false);
  release_assert(Store && "Non-writable location in executeVaStartInst?");
  replaceRangeWithPBs(Store->store, vaStartVals,
                      (uint64_t)PtrSet.Values[0].Offset, 24);
  checkStore(Store->store, PtrSet.Values[0].V);
}

void llvm::executeReadInst(ShadowInstruction *ReadSI, std::string &Filename,
  ...
  ImprovedValSetSingle PtrSet;
  release_assert(getImprovedValSetSingle(Ptr, PtrSet) &&
                 "Write through uninitialised PB (read)?");
  if (!(PtrSet.isWhollyUnknown() || PtrSet.SetType == ValSetTypePB))
    PtrSet.setOverdef();
  ...
  if (PtrSet.isWhollyUnknown() || PtrSet.Values.size() != 1) {
    ...
    if (getFileBytes(Filename, FileOffset, Size, constBytes, Context, errors)) {
      ...
      WriteIVS =
          ImprovedValSetSingle(ImprovedVal(ByteArray, 0), ValSetTypeScalar);
    }
  }
...
// Can Ptr alias objects that existed before the specialisation root was
// entered?
...
// A visitor that walks over objects reachable through a particular pointer, set
// of pointers or extent-list containing pointers.
...
static void visitReachableObjects(const ImprovedValSetSingle &Ptr, ShadowBB *BB,
                                  ReachableObjectVisitor &V);

static void visitReachableObjects(const ImprovedValSet *Obj, ShadowBB *BB,
                                  ReachableObjectVisitor &V) {
  ...
  if (const ImprovedValSetSingle *IVS = dyn_cast<ImprovedValSetSingle>(Obj)) {
    ...
    for (ImprovedValSetMulti::ConstMapIt it = IVM->Map.begin(),
                                         itend = IVM->Map.end();
         it != itend; ++it) {
...
static void visitReachableObjects(const ImprovedValSetSingle &Ptr, ShadowBB *BB,
  ...
  for (uint32_t i = 0, ilim = Ptr.Values.size(); i != ilim; ++i) {
    ...
    case SHADOWVAL_GV:
      ...
      break;
    case SHADOWVAL_OTHER:
      release_assert(ThisPtr.isNullPointer() || isa<UndefValue>(ThisPtr.u.V) ||
                     isFunction(ThisPtr.u.V));
      continue;
    default:
      break;
    }
...
// Visitor that determines whether Ptr could point to anything allocated since
// the specialisation root was entered (informally, 'new' objects, cf. 'old'
// objects that were allocated beforehand and are reachable through arguments to
// the root function or globals). If 'ignoreOldObjects' is set, determine
// whether it could refer to any object new or old.
struct ReachesAllPointersVisitor : public ReachableObjectVisitor {

  bool mayReachAll;
  bool ignoreOldObjects;
  ReachesAllPointersVisitor(bool ignoreOld)
      : mayReachAll(false), ignoreOldObjects(ignoreOld) {}
  ...
  virtual bool shouldContinue() { return !mayReachAll; }
};

static bool reachesAllPointers(const ImprovedValSetSingle &Ptr, ShadowBB *BB,
                               bool ignoreOldObjects) {
...
static void setObjectsMayAliasOld(const ImprovedValSetSingle &Ptr,
                                  ShadowBB *BB) {
...
static void setValueMayAliasOld(ShadowValue V, ShadowBB *BB) {
  ...
  if (ImprovedValSetMulti *IVM =
          dyn_cast_or_null<ImprovedValSetMulti>(tryGetIVSRef(V))) {
...
static void setObjectsThreadGlobal(const ImprovedValSetSingle &Ptr,
                                   ShadowBB *BB) {
  ...
  if (const IHPFunctionInfo *FI = GlobalIHP->getMRInfo(F)) {
    ...
    for (unsigned i = 0; Details[i].Location; ++i) {
      ...
      if (Details[i].Location->getLocation) {
        Details[i].Location->getLocation(ShadowValue(SI), ClobberV,
                                         ClobberSize);
      } else {
    ...
    CallInst *CI;
    if ((CI = dyn_cast_inst<CallInst>(SI)) &&
        GlobalIHP->pessimisticLocks.count(CI)) {
      ...
      SmallDenseMap<CallInst *, std::vector<GlobalVariable *>, 4>::iterator
          findit = GlobalIHP->lockDomains.find(CI);
      if (findit != GlobalIHP->lockDomains.end()) {

        ImprovedValSetSingle OD(ValSetTypeUnknown, true);

        for (std::vector<GlobalVariable *>::iterator
                 it = findit->second.begin(),
                 itend = findit->second.end();
             it != itend; ++it) {

          ShadowGV *SGV =
              &GlobalIHP->shadowGlobals[GlobalIHP->getShadowGlobalIndex(*it)];
...
void llvm::executeUnexpandedCall(ShadowInstruction *SI) {
  ...
  if (F) {
    ...
    {
      DenseMap<Function *, specialfunctions>::iterator it =
          SpecialFunctionMap.find(F);
    ...
    {
      SmallDenseMap<Function *, SpecialLocationDescriptor>::iterator it =
          GlobalIHP->specialLocations.find(F);
    ...
    if (SI->parent->IA->tryGetAsDefPathValue(ShadowValue(SI), SI->parent,
                                             PathVal)) {
    ...
    // The function might cause unwinding if it isn't explicitly annotated to
    // the contrary:
    if (!F->doesNotThrow())
  ...
  if (inst_is<CallInst>(SI) &&
      (ASM = dyn_cast_or_null<InlineAsm>(
           cast<CallInst>(SI->invar->I)->getCalledOperand())) &&
  ...
  if (clobbersMemory) {
    ...
    // errs() << "Warning: unhandled call to " << itcache(SI) << " clobbers all
    // locations\n";
    ImprovedValSetSingle OD(ValSetTypeUnknown, true);
  ...
  // Args to an unhandled call escape, may be stored in old object, and may be
...
// This is used when e.g. writing through an unknown pointer, but one which is
// known not to alias objects that predate specialistion, or not to alias
// unescaped thread-local objects, or...
void ShadowBB::clobberAllExcept(DenseSet<ShadowValue> &Save, bool verbose) {
  ...
  for (DenseSet<ShadowValue>::iterator it = Save.begin(), itend = Save.end();
       it != itend; ++it) {
    ...
    SaveVals.push_back(
        std::make_pair(*it, CurrentVal->store->getReadableCopy()));
    if (verbose)
  ...
  for (std::vector<std::pair<ShadowValue, ImprovedValSet *>>::iterator
           it = SaveVals.begin(),
           itend = SaveVals.end();
       it != itend; ++it) {
...
// Is V a 'vague' allocation? This means might instruction instance V run more
// than once, e.g. if it is an allocation instruction within an unexpanded loop?
// (compare the case where the loop is unrolled and each iteration considered
// individually)
...
// Top write-instruction entry point. ValPB is written through PtrSet by
// instruction WriteSI (which may be null if the write does not orginate from an
// instruction)
void llvm::executeWriteInst(ShadowValue *Ptr, ImprovedValSetSingle &PtrSet,
                            ImprovedValSetSingle &ValPB, uint64_t PtrSize,
                            ShadowInstruction *WriteSI) {
  ...
  if (PtrSet.isWhollyUnknown()) {
    ...
    if (PtrSet.isOldValue()) {
      ...
      // Start with a plain local store map giving no locations except unescaped
      // objects that cannot alias this one.
      noteBarrierInst(WriteSI);
      StoreBB->clobberAllExcept(StoreBB->localStore->es.unescapedObjects,
                                false);
      LFV3(errs() << "Write through overdef; local map " << StoreBB->localStore
                  << " clobbered\n");
    }
    ...
    // Best case: store through a single, certain pointer. Overwrite the
    // location with our new PB.

    LocStore *Store = StoreBB->getWritableStoreFor(
        PtrSet.Values[0].V, PtrSet.Values[0].Offset, PtrSize, true);
    if (!Store)
      return;

    replaceRangeWithPB(Store->store, ValPB, (uint64_t)PtrSet.Values[0].Offset,
                       PtrSize);
    checkStore(Store->store, PtrSet.Values[0].V);

  } else {

    for (SmallVector<ImprovedVal, 1>::iterator it = PtrSet.Values.begin(),
                                               it2 = PtrSet.Values.end();
         it != it2; ++it) {
      ...
      if (it->Offset == LLONG_MAX) {
        ...
        if (ValPB.Overdef) {
          ...
          // Overdef merges with any other value to make Overdef, so skip the
          // lookup.
          oldValSet = ValPB;
          ...
          if ((!oldValSet.isWhollyUnknown()) && oldValSet.isInitialised()) {
            ...
            Type *oldType =
                WriteSI->parent->IA->getValueType(oldValSet.Values[0].V);
            Type *newType =
                WriteSI->parent->IA->getValueType(ValPB.Values[0].V);

            if (ValPB.canCoerceToType(oldType, PtrSize, 0, false)) {

              ValPB.coerceToType(oldType, PtrSize, 0, false);
              LFV3(errs() << "Read-modify-write failure coercing to type "
                          << (*oldType) << "\n");

            } else if ((!ValPB.isWhollyUnknown()) && ValPB.Values.size() > 0 &&
                       oldValSet.canCoerceToType(newType, PtrSize, 0, false)) {
        ...
        replaceRangeWithPB(Store->store, oldValSet, (uint64_t)it->Offset,
                           PtrSize);
      }
...
// as of StoreBB. If ValPB is a particular pointer and PtrSet has itself leaked
// / escaped, propagate that status to pointers in ValPB.
void llvm::propagateStoreFlags(ImprovedValSetSingle &PtrSet,
                               ImprovedValSetSingle &ValPB, ShadowBB *StoreBB) {

  // Propagate is-thread-local and may-alias-old-objects information

  bool targetMayAliasOld =
      PtrSet.isWhollyUnknown() || containsOldObjects(PtrSet, StoreBB);
  bool targetIsThreadGlobal =
      PtrSet.isWhollyUnknown() || containsGlobalObjects(PtrSet, StoreBB);
  ...
  if (ValPB.isWhollyUnknown()) {
    ...
    // Apply alias-old or global flags to any objects whose pointers are written
    // in, and any objects recursively reachable from their (current) values.
...
// For IVSSingles a common base is identified by value equality; for IVSMultis
// it is only identified by pointer equality, so Multis that describe the same
// values may go unnoticed.
static bool getCommonAncestor(ImprovedValSet *LHS, ImprovedValSet *RHS,
                              ImprovedValSet *&LHSResult,
                              ImprovedValSet *&RHSResult,
                              SmallPtrSet<ImprovedValSetMulti *, 4> &Seen) {

  LFV3(errs() << "gca " << LHS << " " << RHS << " "
              << isa<ImprovedValSetSingle>(LHS) << " "
              << isa<ImprovedValSetSingle>(RHS) << "\n");
  ...
  if ((!LHSM->Underlying)) {
    ...
    if (isa<ImprovedValSetSingle>(RHS) ||
        (!cast<ImprovedValSetMulti>(RHS)->Underlying))
      return false;
  ...
  return getCommonAncestor(RHS, LHSM->Underlying ? LHSM->Underlying : LHS,
                           RHSResult, LHSResult, Seen);
}
...
static void scalarSplatToScalar(ImprovedValSetSingle &IVS, Type *targetType) {
  ...
  for (uint32_t i = 0, ilim = IVS.Values.size(); i != ilim; ++i) {
    ...
    PartialVal PV(Size);
    uint8_t SplatVal =
        (uint8_t)cast<ConstantInt>(IVS.Values[i].V.getVal())->getLimitedValue();
...
static void mergeValues(ImprovedValSetSingle &consumeVal,
                        ImprovedValSetSingle &otherVal,
                        OrdinaryMerger *Visitor) {

  if (Visitor->useVarargMerge && consumeVal.SetType == ValSetTypeVarArg &&
      otherVal.SetType == ValSetTypeVarArg && consumeVal.Values.size() == 1 &&
    ...
    if (consumeVal.SetType == ValSetTypeScalarSplat &&
        consumeVal.Values.size() && otherVal.SetType == ValSetTypeScalar &&
        otherVal.Values.size()) {

      scalarSplatToScalar(consumeVal, otherVal.Values[0].V.getNonPointerType());
      // Fall through to merge

    } else if (consumeVal.SetType == ValSetTypeScalar &&
               consumeVal.Values.size() &&
               otherVal.SetType == ValSetTypeScalarSplat &&
               otherVal.Values.size()) {
    ...
    // Convert scalar zeroes to null pointers if appropriate.
               ...
               consumeVal.SetType == ValSetTypeScalarSplat) &&
              consumeVal.Values.size() && otherVal.SetType == ValSetTypePB &&
              otherVal.Values.size()) ||

             ((otherVal.SetType == ValSetTypeScalar ||
               otherVal.SetType == ValSetTypeScalarSplat) &&
              otherVal.Values.size() && consumeVal.SetType == ValSetTypePB &&
              consumeVal.Values.size())) {

      // Asymmetry to avoid altering the read-only otherVal (consumeVal is
      // mutable).
      if (consumeVal.onlyContainsZeroes()) {

        Constant *newNull = Constant::getNullValue(
            Visitor->originContext->getValueType(otherVal.Values[0].V));
        ...
        Constant *newNull = Constant::getNullValue(
            Visitor->originContext->getValueType(consumeVal.Values[0].V));
        if (newNull->getType()->isPointerTy())
...
void LocStore::mergeStores(LocStore *mergeFromStore, LocStore *mergeToStore,
                           uint64_t ASize, OrdinaryMerger *Visitor) {

  if (mergeFromStore->store == mergeToStore->store)
    return;

  if (ImprovedValSetSingle *IVS =
          dyn_cast<ImprovedValSetSingle>(mergeToStore->store)) {
    ...
    if (ImprovedValSetSingle *IVS2 =
            dyn_cast<ImprovedValSetSingle>(mergeFromStore->store)) {
  ...
  // Get an IVS list for each side that contains gaps where there is a common
  // ancestor:
  ImprovedValSet *LHSAncestor, *RHSAncestor;
  {
    SmallPtrSet<ImprovedValSetMulti *, 4> Seen;
    if (!getCommonAncestor(mergeToStore->store, mergeFromStore->store,
                           LHSAncestor, RHSAncestor, Seen)) {

      LHSAncestor = 0;
      RHSAncestor = 0;
    }
    LFV3(errs() << "Merging multi stores; use common ancestor " << LHSAncestor
                << "/" << RHSAncestor << "\n");
  }

  {
    ...
    readValRangeMultiFrom(0, ASize, mergeToStore->store, LHSVals, LHSAncestor,
                          ASize);
    readValRangeMultiFrom(0, ASize, mergeFromStore->store, RHSVals, RHSAncestor,
                          ASize);
    ...
    SmallVector<IVSRange, 4>::iterator LHSit = LHSVals.begin(),
                                       RHSit = RHSVals.begin();
    SmallVector<IVSRange, 4>::iterator LHSitend = LHSVals.end(),
                                       RHSitend = RHSVals.end();
    uint64_t LastOffset = 0;
    bool anyGaps = false;
    ...
    while (LHSit != LHSitend || RHSit != RHSitend) {
      ...
      if (LHSit == LHSitend)
        ...
        if (consumeLHS == consumeRHS)
          consumeNext =
              LHSit->first.second <= RHSit->first.second ? &LHSit : &RHSit;
        else
          consumeNext = consumeLHS < consumeRHS ? &LHSit : &RHSit;
      }
      SmallVector<IVSRange, 4>::iterator &consumeit = *consumeNext;
      SmallVector<IVSRange, 4>::iterator &otherit =
          (consumeNext == &LHSit ? RHSit : LHSit);
      SmallVector<IVSRange, 4>::iterator &otherend =
          (consumeNext == &LHSit ? RHSitend : LHSitend);

      LFV3(errs() << "Consume from "
                  << ((consumeNext == &LHSit) ? "LHS" : "RHS") << " val at "
                  << consumeit->first.first << "-" << consumeit->first.second
                  << "\n");

      // consumeit is now the input iterator that
      // (a) is not at the end
      // (b) is defined at LastOffset, in which case otherit is not defined
      // here, (c) or it is defined and otherit is also defined here and otherit
      // remains defined for longer, (d) or else both iterators are not defined
      // here and consumeit becomes defined first. In short we should leave a
      // gap until consumeit becomes defined, or merge the next consumeit object
      // with either the base (if otherit is not defined) or with a partial
      // otherit object.
      ...
      if (LastOffset < consumeit->first.first) {
        ...
        // consumeit entry begins here or earlier but otherit is not defined,
        // case (b). Merge it with base up to this entry's end or otherit
        // becoming defined.
        uint64_t stopAt;
        bool bump;
        if (otherit == otherend ||
            otherit->first.first >= consumeit->first.second) {
        ...
        readValRangeMultiFrom(LastOffset, stopAt - LastOffset, LHSAncestor,
                              baseVals, 0, ASize);

        for (SmallVector<IVSRange, 4>::iterator baseit = baseVals.begin(),
                                                baseend = baseVals.end();
             baseit != baseend; ++baseit) {

          ImprovedValSetSingle subVal;
          getIVSSubVal(consumeit->second,
                       baseit->first.first - consumeit->first.first,
                       baseit->first.second - baseit->first.first, subVal);
          mergeValues(subVal, baseit->second, Visitor);
          MergedVals.push_back(
              IVSR(baseit->first.first, baseit->first.second, subVal));
        }
        ...
        LFV3(errs() << "Merge two vals " << LastOffset << "-"
                    << consumeit->first.second << "\n");

        // Both entries are defined here, case (c), so consumeit finishes equal
        // or sooner.
        ImprovedValSetSingle consumeVal;
        getIVSSubVal(consumeit->second, LastOffset - consumeit->first.first,
                     consumeit->first.second - LastOffset, consumeVal);

        ImprovedValSetSingle otherVal;
        getIVSSubVal(otherit->second, LastOffset - otherit->first.first,
                     consumeit->first.second - LastOffset, otherVal);
        ...
        MergedVals.push_back(
            IVSR(LastOffset, consumeit->first.second, consumeVal));
    ...
    if (mergeToStore->store->isWritableMulti()) {
      ...
      LFV3(errs() << "Drop existing store " << mergeToStore->store
                  << ", allocate new multi " << newStore << "\n");
    }

    newStore->Underlying = newUnderlying;

    ImprovedValSetMulti::MapIt insertit = newStore->Map.end();
    for (SmallVector<IVSRange, 4>::iterator finalit = MergedVals.begin(),
                                            finalitend = MergedVals.end();
         finalit != finalitend; ++finalit) {

      insertit.insert(finalit->first.first, finalit->first.second,
                      finalit->second);
...
// If store merging has left a common base store with only single reference,
// merge down.
...
static bool sizeLT(const DenseSet<ShadowValue> *a,
                   const DenseSet<ShadowValue> *b) {

  return a->size() < b->size();
}

// mergeBegin -> mergeEnd are already listed smallest set first.
void llvm::intersectSets(DenseSet<ShadowValue> *Target,
                         MutableArrayRef<DenseSet<ShadowValue> *> Merge) {

  SmallVector<ShadowValue, 8> toKeep;

  MutableArrayRef<DenseSet<ShadowValue> *>::iterator mergeBegin = Merge.begin(),
                                                     mergeEnd = Merge.end();
  std::sort(mergeBegin, mergeEnd, sizeLT);

  if (Target->size() < (*mergeBegin)->size()) {

    for (DenseSet<ShadowValue>::iterator it = Target->begin(),
                                         itend = Target->end();
         it != itend; ++it) {

      bool keepThis = true;
      for (MutableArrayRef<DenseSet<ShadowValue> *>::iterator findit =
               mergeBegin;
           findit != mergeEnd && keepThis; ++findit) {
...
static void dumpSets(OrdinaryLocalStore *Map) {
  ...
  errs() << "Not-old:\n";
  for (DenseSet<ShadowValue>::iterator it = Map->es.noAliasOldObjects.begin(),
                                       itend = Map->es.noAliasOldObjects.end();
       it != itend; ++it) {
  ...
  errs() << "Thread-local:\n";
  for (DenseSet<ShadowValue>::iterator it = Map->es.threadLocalObjects.begin(),
                                       itend = Map->es.threadLocalObjects.end();
       it != itend; ++it) {
  ...
  errs() << "Unescaped:\n";
  for (DenseSet<ShadowValue>::iterator it = Map->es.unescapedObjects.begin(),
                                       itend = Map->es.unescapedObjects.end();
       it != itend; ++it) {
...
void OrdinaryStoreExtraState::dump(OrdinaryLocalStore *Map) { dumpSets(Map); }

// Merge the sets of objects known not to escape from this thread, not to be
// written to global memory, and so on. In all current cases an object only has
// the desired property if it has it in all predecessor blocks.

void OrdinaryStoreExtraState::doMerge(
    OrdinaryLocalStore *toMap,
    SmallVector<OrdinaryLocalStore *, 4>::iterator fromBegin,
    SmallVector<OrdinaryLocalStore *, 4>::iterator fromEnd, bool verbose) {
...

  {
    ...
    for (SmallVector<OrdinaryLocalStore *, 4>::iterator it = fromBegin;
         it != fromEnd; ++it)
...
// Return false if this block turns out to have no live predecessors at the
// moment. This is possible in the unusual case that a per-iteration loop
// exploration has created the block to find invariants but it isn't yet
// reachable according to the fixed point analyser -- e.g. this block only
// becomes reachable on iteration 2.
// TODO: invariants like that have been removed, so could probably drop the
// return value and tests on same.
bool llvm::doBlockStoreMerge(ShadowBB *BB) {

  // We're entering BB; one or more live predecessor blocks exist and we must
  // produce an appropriate localStore from them.

  LFV3(errs() << "Start block store merge\n");

  // This BB is a merge of all that has gone before; merge to values' base
  // stores rather than a local map.
...
// Is IA a child of this context, down to the nearest function scope
// (InlineAttempt)?
bool InlineAttempt::ctxContains(IntegrationAttempt *IA) { return this == IA; }
...
AllocData *IntegrationAttempt::getAllocData(ShadowValue V) {
  ...
  if (V.u.PtrOrFd.frame == -1)
    ...
    return &getFunctionRoot()
                ->getStackFrameCtx(V.u.PtrOrFd.frame)
                ->localAllocas[V.u.PtrOrFd.idx];
}
