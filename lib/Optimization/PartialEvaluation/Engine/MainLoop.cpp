...
// Check scope (see if we need to enter a loop, ignore because out of scope,
// etc) checkBlock it. If it hasn't been created then it has no live
// predecessors; skip. If it's the top of a loop:
//   Open up the loop, investigate within the loop according to the same rules.
//   If we didn't establish that it terminates analyse for invariants (in parent
//   scope).
// Otherwise, for each instruction:
//   If it's a store, memcpy, memset or other memory-writing instruction
//   (including read from file), modify the local store. Elif it's a load
//   instruction, try to read from the block-local store Elif it's a VFS call,
//   try to forward it. Elif it's an expandable call (and e.g. certainty /
//   recursion doesn't forbid it), expand it and recurse. Else, (or if it's a
//   load not yet resolved), try to constant fold it.
//
// For the topo walk, use reverse postorder DFS, where loop headers are entered
// in the ordering implying that we should at that point enter the loop rather
// than listing all blocks in some order.
...
// Analyse this function call instance. SI is the call instruction;
// inLoopAnalyser indicates that we're investigating the general case of a loop
// body or mutually recursive function; inAnyLoop indicates we're within a loop
// at some level of the call graph even if we're investigating a specific loop
// iteration; parent_stack_depth is the stack frame depth used by the parent
// call.
bool InlineAttempt::analyseWithArgs(ShadowInstruction *SI, bool inLoopAnalyser,
                                    bool inAnyLoop,
...
// As above, but exclude trying to pull argument values from our parent. Called
// directly for the entry point of the specialisation root, since its arguments
// are given by user directives, not its caller.
bool InlineAttempt::analyseNoArgs(bool inLoopAnalyser, bool inAnyLoop,
                                  uint32_t parent_stack_depth) {

  uint32_t new_stack_depth = (invarInfo->frameSize == -1)
                                 ? parent_stack_depth
                                 : parent_stack_depth + 1;
...
// Essentially this just imports globals if possible and notes that things that
// predate specialisation have a special "old" aliasing class compared to some
// objects allocated during specialisation.
static void initialiseStore(ShadowBB *BB) {
  ...
  for (uint32_t i = 0, ilim = GlobalIHP->heap.size(); i != ilim; ++i) {
    ...
    LocStore *LS =
        BB->getWritableStoreFor(AD.allocValue, 0, AD.storeSize, true);
...
// Get the initial store for this loop iteration, either from the preheader or
// the latch block.
void PeelIteration::getInitialStore(bool inLoopAnalyser) {

  if (iterationCount == 0) {

    BBs[0]->takeStoresFrom(parent->getBB(parentPA->L->preheaderIdx),
                           inLoopAnalyser);

  } else {

    // Take the previous latch's store
    BBs[0]->takeStoresFrom(
        parentPA->Iterations[iterationCount - 1]->getBB(parentPA->L->latchIdx),
        inLoopAnalyser);
  }
}

// Root analysis function for this context. Analyse each basic block in
// top-sorted order; recurse into any child contexts as they are encountered.
// Parameter meanings are as for InlineAttempt::analyseWithArgs.
bool IntegrationAttempt::analyse(bool inLoopAnalyser, bool inAnyLoop,
                                 uint32_t new_stack_depth) {
...
// Root analysis entry point, called after parsing user directives such as
// assertions about initial state when entering analysis.
void IntegrationAttempt::analyse() { analyse(false, false, 0); }

// Analyse a loop in the particular case where we consider each iteration
// individually. parent_stack_depth is the parent function's stack frame index;
// readsTentativeData gets set if the loop ever makes a read that needs checking
// for correctness (e.g. to guard against concurrent alteration by another
// thread); containsCheckedReads gets set if any file-reading operations require
// a runtime check.
bool PeelAttempt::analyse(uint32_t parent_stack_depth, bool &readsTentativeData,
  ...
  for (PeelIteration *PI = Iterations[0]; PI;
       PI = PI->getOrCreateNextIteration()) {
...
// Set the block-local store for BBI to S. 'kind' is used to implement
// half-assed polymorphism, since TLLocalStore and DSELocalStore don't have a
// common base class at the moment.
void PeelIteration::setExitingStore(void *S, ShadowBBInvar *BBI,
                                    const ShadowLoopInvar *exitLoop,
                                    StoreKind kind) {
  ...
  if (BBI->naturalScope != L &&
    ...
    // in the presence of user options allowing loop exploration to continue
    // regardless.
  ...
  // For each live edge leaving the loop, replace the exiting block's store with
  // S.
  ...
  for (uint32_t i = 0, ilim = BBI->succIdxs.size(); i != ilim; ++i) {
    ...
    if (ExitingBB->succsAlive[i] &&
        ((!ExitedBBI->naturalScope) ||
         !exitLoop->contains(ExitedBBI->naturalScope))) {
...
void PeelIteration::setExitingStores(void *S, StoreKind SK) {
  ...
  for (std::vector<uint32_t>::const_iterator
           it = parentPA->L->exitingBlocks.begin(),
           itend = parentPA->L->exitingBlocks.end();
       it != itend; ++it) {
...
// Our parent want to take NewCBs, NewFCBs and NewFs from us. These are
// parentless basic blocks, failed-path basic blocks and functions generated by
// this context and its children.
void PeelIteration::inheritCommitBlocksAndFunctions(
    std::vector<BasicBlock *> &NewCBs, std::vector<BasicBlock *> &NewFCBs,
    std::vector<Function *> &NewFs) {

  parentPA->CommitBlocks.insert(parentPA->CommitBlocks.end(), NewCBs.begin(),
                                NewCBs.end());
  parentPA->CommitFailedBlocks.insert(parentPA->CommitFailedBlocks.end(),
                                      NewFCBs.begin(), NewFCBs.end());
  parentPA->CommitFunctions.insert(parentPA->CommitFunctions.end(),
                                   NewFs.begin(), NewFs.end());
...
// As above.
void InlineAttempt::inheritCommitBlocksAndFunctions(
    std::vector<BasicBlock *> &NewCBs, std::vector<BasicBlock *> &NewFCBs,
    std::vector<Function *> &NewFs) {

  CommitBlocks.insert(CommitBlocks.end(), NewCBs.begin(), NewCBs.end());
  CommitFailedBlocks.insert(CommitFailedBlocks.end(), NewFCBs.begin(),
                            NewFCBs.end());
...
// Analyse / interpret each instruction in block BBs[blockIdx]. inLoopAnalyser
// and inAnyLoop have the same meanings as for InlineAttempt::analyseWithArgs
// above. skipStoreMerge means we shouldn't try to pull and merge block-local
// stores from our predecessor blocks, usually because there is a special case
// here like a function entry block that doesn't have local predecessors.
bool IntegrationAttempt::analyseBlock(uint32_t &blockIdx, bool inLoopAnalyser,
                                      bool inAnyLoop, bool skipStoreMerge,
                                      const ShadowLoopInvar *MyL) {
  ...
  if (BBL != MyL) {
    ...
    if ((!inLoopAnalyser) && (LPA = getOrCreatePeelAttempt(BBL))) {
      ...
      // to calculate a general version of the loop body if it doesn't
      // terminate.
      ...
      LPA->analyse(stack_depth, loopReadsTentativeData,
                   loopContainsCheckedReads);
      readsTentativeData |= loopReadsTentativeData;
      containsCheckedReads |= loopContainsCheckedReads;

      // We're certainly not in the loop analyser, so pick whether to keep a
      // terminated version of the loop now.
      ...
      if (LPA->isTerminated() && LPA->isEnabled()) {
        ...
        inheritCommitBlocksAndFunctions(
            LPA->CommitBlocks, LPA->CommitFailedBlocks, LPA->CommitFunctions);
    ...
    if ((!LPA) || !LPA->isTerminated()) {
      ...
      if (!inLoopAnalyser) {
        ...
        findTentativeLoadsInUnboundedLoop(
            BBL, /* commit disabled here = */ false, /* second pass = */ false);
        tryKillStoresInUnboundedLoop(BBL, /* commit disabled here = */ false,
                                     /* disable writes = */ false);
      }

    } else {

      // The loop preheader's local store was copied by the loop analysis
      // assuming we'd need it to analyse the loop body, but we've found the
      // loop terminates; drop the extra ref.

      // For the common case where the loop has a single known exit point,
      // perform store simplifications. These apply because the store was forked
      // anticipating failure to establish an iteration count.
      ...
      // Copy edges found always dead to local scope, to accelerate edgeIsDead
      // queries without checking every iteration every time.
    ...
    // Advance the main loop past this loop. Loop blocks are always contiguous
    // in the topo ordering.
    while (blockIdx < invarInfo->BBs.size() &&
           BBL->contains(getBBInvar(blockIdx)->naturalScope))
      ++blockIdx;
    --blockIdx;
  ...
  if (!skipStoreMerge) {
    ...
    // Check if the block becomes a certainty (only applicable when not in a
    // loop!)
  ...
  // they are cleared each time around because the flag might not make sense
  // anymore if the instruction's operands have degraded to the point that the
  // instruction will no longer be resolved. The noteAsExpected function here
  // only tags those which are mentioned in path conditions.
  ...
  LFV3(errs() << nestingIndent() << "Start block " << BB->invar->BB->getName()
              << " store " << BB->localStore << " refcount "
  ...
  LFV3(errs() << nestingIndent() << "End block " << BB->invar->BB->getName()
              << " store " << BB->localStore << " refcount "
              << BB->localStore->refCount << "\n");
...
// Try to analyse a call/invoke instruction SI by creating or reusing a
// specialisation context representing the call. Set 'changed' if anything about
// the analysis changed (e.g. arguments or instructions had different values to
// any previous analysis). inLoopAnalyser, inAnyLoop have the same meanings as
// for InlineAttempt::analyseWithArgs. Return true if we ended up with an
// InlineAttempt available for this call.
bool IntegrationAttempt::analyseExpandableCall(ShadowInstruction *SI,
                                               bool &changed,
                                               bool inLoopAnalyser,
                                               bool inAnyLoop) {
  ...
  if (IA) {
    ...
    if (needsAnalyse) {
      ...
      // Setting active = true prevents incomplete dependency information from
      // being used to justify sharing the function node.
      IA->active = true;

      changed |=
          IA->analyseWithArgs(SI, inLoopAnalyser, inAnyLoop, stack_depth);
      ...
      if (changed && IA->hasFailedReturnPath()) {
        ...
        // Must create a copy of this block for failure paths, starting at the
        // call successor. Invoke instructions fail directly to their
        // non-exception successors; call instructions introduce a break in
        // their basic block.
        if (inst_is<CallInst>(SI))
          getFunctionRoot()->markBlockAndSuccsReachableUnspecialised(
              SI->parent->invar->idx, SI->invar->idx + 1);
        else
          getFunctionRoot()->markBlockAndSuccsReachableUnspecialised(
...
// Analyse instruction SI in this context. inLoopAnalyser and anyLoop have the
// same meanings as in InlineAttempt::analyseWithArgs above. loadedVarargsHere
// indicates this instruction read a vararg. bail indicates that this path ended
// in an unreachable instruction.
bool IntegrationAttempt::analyseInstruction(ShadowInstruction *SI,
                                            bool inLoopAnalyser, bool inAnyLoop,
                                            bool &loadedVarargsHere,
                                            bool &bail) {

  ShadowInstructionInvar *SII = SI->invar;
  Instruction *I = SII->I;

  if (SI->invar->I->isTerminator() && !inst_is<InvokeInst>(SI)) {
  ...
  // All fence logic is implemented in the tentative loads passlet; nothing to
  // do here.
  ...
  case Instruction::Invoke: {
    ...
    if (Function *F = getCalledFunction(SI)) {
      if (canConstantFoldCallTo(cast<CallBase>(I), F))
        break;
    }
    ...
    bool isExpanded =
        analyseExpandableCall(SI, changed, inLoopAnalyser, inAnyLoop);
    if (isExpanded) {
...
bool IntegrationAttempt::analyseBlockInstructions(ShadowBB *BB,
                                                  bool inLoopAnalyser,
                                                  bool inAnyLoop) {
  ...
  for (uint32_t i = 0, ilim = BB->insts.size(); i != ilim; ++i) {
    ...
    bool bail = false;
    anyChange |= analyseInstruction(SI, inLoopAnalyser, inAnyLoop,
                                    loadedVarargsHere, bail);
    if (bail)
      return anyChange;

    if (!inLoopAnalyser) {

      // In the loop analysis case we reach a fixed point before running other
      // passes.
      ...
      TLAnalyseInstruction(*SI, /* commit disabled = */ false,
                           /* second pass = */ false,
                           /* in loop analyser = */ false);
...
// Loop latch edges retain a reference on their block-local stores used when
// finding fixed-point solutions concerning general cases of loop bodies,
// recursive functions and their children. This releases those references when
// we get back to top level.
void InlineAttempt::releaseCallLatchStores() { releaseLatchStores(0); }
...
void IntegrationAttempt::releaseLatchStores(const ShadowLoopInvar *L) {
  ...
  for (uint32_t i = startBlock; i < (BBsOffset + nBBs); ++i) {
    ...
    if (BBL != L) {
      ...
      // Skip past the loop:
      while (i < (BBsOffset + nBBs) &&
             BBL->contains(getBBInvar(i)->naturalScope))
        ++i;
      --i;
...
// Analyse a loop's general case (i.e. trying to find a fixed-point solution
// regarding the body, rather than analysing each individual iteration; for that
// see PeelAttempt::analyse) nestedLoop indicates we're being analysed in the
// context of a loop further out, either in our call or a parent call.
bool IntegrationAttempt::analyseLoop(const ShadowLoopInvar *L,
                                     bool nestedLoop) {
  ...
  LFV3(errs() << "Loop " << L->getHeader()->getName()
              << " refcount at entry: " << PHBB->localStore->refCount << "\n");

  // Stop iterating if we show that the latch edge died!
  while (anyChange &&
         (firstIter || !edgeIsDead(getBBInvar(L->latchIdx), HBB->invar))) {

    ++iters;

    // Give the preheader store an extra reference to ensure it is never
    // modified. This ref corresponds to ph retaining its reference (h has
    // already been given one by ph's successor code).
    PHBB->refStores();

    {
      ...
      if (!firstIter) {
        ...
        for (std::vector<std::pair<uint32_t, uint32_t>>::const_iterator
                 it = L->exitEdges.begin(),
                 itend = L->exitEdges.end();
             it != itend; ++it) {
          ...
          if (BB && (!edgeIsDead(BB->invar, getBBInvar(it->second))) &&
              (!edgeBranchesToUnspecialisedCode(BB->invar,
                                                getBBInvar(it->second)))) {

            LFV3(errs() << "Drop exit edge " << BB->invar->BB->getName()
                        << " -> " << getBBInvar(it->second)->BB->getName()
                        << " with store " << BB->localStore << "\n");
            BB->derefStores();
      ...
      if (pass->latchStoresRetained.count(std::make_pair(this, L))) {
        ...
        // We've analysed this loop before -- we must be under analysis as a
        // nested loop. If our latch edge lives we should use its store ref,
        // which was saved last time around.
        firstIter = false;
      }
  ...
  if (edgeIsDead(getBBInvar(L->latchIdx), HBB->invar))
    release_assert(
        iters == 1 &&
        "Loop analysis found the latch dead but not first time around?");

  // Release the preheader store that was held for merging in each iteration:
  PHBB->derefStores();

  // If this is a nested loop, hang onto the reference given from latch to
  // header for use in the next iteration of analysing this loop.
  ...
  // Similarly the latch block will have given the header a reference towards
  // determining successor block certainty. Drop it.
  if (thisLatchAlive)
    --pendingEdges;

  LFV3(errs() << "Loop " << L->getHeader()->getName()
              << " refcount at exit: " << PHBB->localStore->refCount << "\n");

  return everChanged;
}

// Run this function instance through the 'interpreter' without changing
// findings about any of the instructions etc. Used when a function instance is
// being shared as we have detected that all relevant state matches a
// previously- analysed version.
...
void IntegrationAttempt::executeLoop(const ShadowLoopInvar *ThisL) {
  ...
  if (pass->latchStoresRetained.erase(std::make_pair(this, ThisL))) {
    ...
    release_assert(LBB &&
                   "Executed loop with retained latch, but not available?");
    LBB->derefStores();
  }
  ...
  for (uint32_t i = ThisL->headerIdx; i < (BBsOffset + nBBs); ++i) {
    ...
    if (!ThisL->contains(BBI->naturalScope))
      ...
      while (i < (BBsOffset + nBBs) &&
             BBI->naturalScope->contains(getBBInvar(i)->naturalScope))
        ++i;
      --i;
...
// Similar to above.
// Like analyse(), but used from sharing pathways when we're sure none of the
// functions need re-evaluating. We really only want to recreate its effects on
// the store.
  ...
  for (uint32_t i = 0; i < nBBs; ++i) {
    ...
    if (BBI->naturalScope != L) {
      ...
      if (LPA && LPA->isTerminated()) {
        ...
        for (std::vector<PeelIteration *>::iterator
                 it = LPA->Iterations.begin(),
                 itend = LPA->Iterations.end();
             it != itend; ++it) {
      ...
      while (i < nBBs && BBI->naturalScope->contains(
...
void IntegrationAttempt::executeBlock(ShadowBB *BB) {
  ...
  // Frame push happens in getInitialStore; pop would usually happen in
