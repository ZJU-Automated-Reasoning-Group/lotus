...
// This file contains the pass' master runOnModule entry point, and
// specialisation context and pass initialisation functions.
...
InlineAttempt::InlineAttempt(LLPEAnalysisPass *Pass, Function &F,
                             ShadowInstruction *_CI, int depth, bool pathCond)
...
PeelIteration::PeelIteration(LLPEAnalysisPass *Pass, IntegrationAttempt *P,
                             PeelAttempt *PP, Function &F, int iter, int depth)
    : IntegrationAttempt(Pass, F, PP->L, depth, 0), iterationCount(iter),
      parentPA(PP), parent(P), iterStatus(IterationStatusUnknown) {
  SeqNumber = Pass->IAs.size();
  Pass->IAs.push_back(this);
  prepareShadows();
}

PeelAttempt::PeelAttempt(LLPEAnalysisPass *Pass, IntegrationAttempt *P,
                         Function &_F, const ShadowLoopInvar *_L, int depth)
    : pass(Pass), parent(P), F(_F), residualInstructions(-1),
      nesting_depth(depth), stack_depth(0), enabled(true), L(_L),
      totalIntegrationGoodness(0), integrationGoodnessValid(false) {
...
// General destructor. Free all instruction instances etc relating to this
// context.
...
// Caller SI will no longer target this function instance. Used when function
// sharing has been used, but is no longer appropriate due to diverging
// circumstances.
...
// Free all memory belonging to the pass. The specialisation contexts'
// destructors will take care of the real work.
...
// Run a simple possible-allocation-sites exploration of V, used in finding out
// whether root function arguments can be simply shown not to alias one another.
// seenValues is used to break loops. If a complete list of possible allocation
// sites can be found return true; otherwise return false and sites is
// undefined.
static bool getAllocSitesFrom(Value *V, std::vector<Value *> &sites,
  ...
  if (isa<GlobalVariable>(V)) {
    ...
    for (Use &U : F->uses()) {
      ...
      if (Instruction *I = dyn_cast<Instruction>(U.getUser())) {
        ...
        if (CallBase *CB = dyn_cast<CallBase>(I)) {
          if (!getAllocSitesFrom(CB->getArgOperand(A->getArgNo()), sites,
                                 seenValues))
            return false;
        }
      }
    ...
    case Instruction::Invoke: {
      if (CallBase *CB = dyn_cast<CallBase>(I)) {
        if (CB->paramHasAttr(0, Attribute::NoAlias)) {
          sites.push_back(V);
          return true;
        }
      }
...
// Initialise symbolic values concerning the arguments to the specialisation
// root function. Pesimistically these could point to anything, but we might be
// able to show they can't alias one another (a la C99 restrict pointers).
void LLPEAnalysisPass::createPointerArguments(InlineAttempt *IA) {
  ...
  for (uint32_t i = 0; AI != AE; ++i, ++AI) {
    ...
    if (A->getType()->isPointerTy()) {
      ...
      ImprovedValSetSingle *IVS =
          cast<ImprovedValSetSingle>(IA->argShadows[i].i.PB);
      if (IVS->SetType == ValSetTypeOldOverdef) {
        ...
        if (!allocs.empty()) {
          ...
          // Create a new heap location for this argument if it has any
          // non-global constituents. Just note any globals in the alias list.
          bool anyNonGlobals = false;
          for (std::vector<Value *>::iterator it = allocs.begin(),
                                              itend = allocs.end();
               it != itend; ++it) {
  ...
  // give it an initial pointer set corresponding to each other arg it may
  // alias.
    ...
    if (!allocs.empty()) {
      ...
      for (uint32_t j = 0, jlim = IA->F.arg_size(); j != jlim; ++j) {
        ...
        if (!argAllocSites[j].empty()) {
          ...
          for (std::vector<Value *>::iterator it = otherallocs.begin(),
                                              itend = otherallocs.end();
               it != itend; ++it) {
            ...
            if (std::find(allocs.begin(), allocs.end(), *it) != allocs.end()) {
              ...
              // i and j share a non-global allocation site, so arg i may alias
              // arg j.

              IVS->Values.push_back(
                  ImprovedVal(ShadowValue(&IA->argShadows[j]), 0));
              break;
            }
          }
        }
...
// Set up special heap objects that correspond to return values from particular
// functions. The function has to return the same object every time (i.e. all
// calls must-alias other calls).

void LLPEAnalysisPass::createSpecialLocations() {

  for (SmallDenseMap<Function *, SpecialLocationDescriptor>::iterator
           it = specialLocations.begin(),
           itend = specialLocations.end();
       it != itend; ++it) {
...
bool LLPEAnalysisPass::runOnModule(Module &M) {
  ...
  GlobalAA = nullptr;
  GlobalTLI = nullptr; // Set below after we have the root function
  GlobalIHP = this;
  ...
  Function &F = *FoundF;
  GlobalTLI = &getAnalysis<TargetLibraryInfoWrapperPass>().getTLI(F);

  // XXX: commented by JN while porting to llvm 5.0
  ...
  // Last parameter: reserve extra GV slots for the constants that path
  // condition parsing will produce.
  ...
  if (argvIdx != 0xffffffff) {
    ...
    NewIVS->set(ImprovedVal(ShadowValue(&IA->argShadows[argvIdx]), 0),
                ValSetTypePB);
...
void LLPEAnalysisPass::getAnalysisUsage(AnalysisUsage &AU) const {
  ...
  const PassInfo *BAAInfo = PR.getPassInfo(StringRef("basicaa"));
  if (!BAAInfo) {
