...
static void readWholeFile(std::string &path, std::string &out,
                          bool addnewline) {
...
GlobalVariable *llvm::getStringArray(std::string &bytes, Module &M,
  ...
  Constant *EnvInit =
      ConstantDataArray::getString(M.getContext(), bytes, addNull);
  return new GlobalVariable(M, EnvInit->getType(), true,
                            GlobalValue::PrivateLinkage, EnvInit,
                            "spec_env_str");
}

// Create an array of pointers into a constant string, as seen in C's argv and
// env pointers.
static Constant *getStringPtrArray(std::string &bytes,
                                   std::vector<size_t> &lineStarts, Module &M) {
  ...
  for (unsigned i = 0; i < lineStarts.size(); ++i) {
    ...
    lineStartConsts.push_back(ConstantExpr::getGetElementPtr(
        cast<PointerType>(EnvInitG->getType())->getPointerElementType(),
        EnvInitG, gepArgs, 2));
  }

  // Conclude with 2 nulls to signal we don't supply an ELF header at spec time.
  lineStartConsts.push_back(
      Constant::getNullValue(Type::getInt8PtrTy(M.getContext())));
  lineStartConsts.push_back(
      Constant::getNullValue(Type::getInt8PtrTy(M.getContext())));

  ArrayType *PtrArrT =
      ArrayType::get(lineStartConsts[0]->getType(), lineStartConsts.size());
  Constant *PtrArray = ConstantArray::get(PtrArrT, lineStartConsts);
  GlobalVariable *EnvPtrsG = new GlobalVariable(M, PtrArray->getType(), true,
                                                GlobalValue::PrivateLinkage,
                                                PtrArray, "spec_env_ptrs");
  Constant *gepArgs[] = {Zero, Zero};
  Constant *EnvPtrsPtr = ConstantExpr::getGetElementPtr(
      cast<PointerType>(EnvPtrsG->getType())->getPointerElementType(), EnvPtrsG,
      gepArgs, 2);

  return EnvPtrsPtr;
}

// Fetch a newline-delimited command-line (saves escaping spaces etc) and
// provide a char** argv replacement.
void LLPEAnalysisPass::loadArgv(Function *F, std::string &path,
                                unsigned argvIdx, unsigned &argc) {
  ...
  for (unsigned i = 0; i < argc; ++i) {
    ...
    if (lineStarts[i] != -1) {
      ...
      Constant *gepArgs[] = {ConstantInt::get(Int64, 0),
                             ConstantInt::get(Int64, lineStarts[i])};
      Constant *stringPtr = ConstantExpr::getGetElementPtr(
          cast<PointerType>(ArgvConsts->getType())->getPointerElementType(),
          ArgvConsts, gepArgs, 2);

      // Get a pointer into the real argv:
      Constant *gepArg = ConstantInt::get(Int64, i);
      assert(Arg->getType()->isPointerTy());

      Instruction *argvPtr = GetElementPtrInst::Create(
          cast<PointerType>(Arg->getType())->getPointerElementType(), Arg,
          gepArg, "argv_ptr", InsertBefore);
  ...
  assert(Arg->getType()->isPointerTy());
  Instruction *argvEndPtr = GetElementPtrInst::Create(
      cast<PointerType>(Arg->getType())->getPointerElementType(), Arg, gepArg,
      "argv_end_ptr", InsertBefore);
  Constant *nullPtr = Constant::getNullValue(BytePtr);
  new StoreInst(nullPtr, argvEndPtr, InsertBefore);

  // JN: I couldn't find it in llvm 5.0
  // F->setDoesNotAlias(argvIdx+1);
}

// Fetch an environment (newline-delimited key=value settings) from path and
// provide a constant suitable for replacing the char** environ pointer.
  ...
  for (size_t findidx = useenv.find('\n'); findidx != std::string::npos;
    ...
    if (!foundequals) {
      ...
      if (foundalpha) {
        ...
        errs() << "Warning: discarded junk "
               << useenv.substr(startidx, findidx - startidx) << "\n";
      }
