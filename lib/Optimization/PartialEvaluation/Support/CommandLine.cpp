...
static cl::opt<std::string> GraphOutputDirectory("llpe-graphs-dir",
                                                 cl::init(""));
...
static cl::list<std::string> AlwaysInlineFunctions("llpe-always-inline",
                                                   cl::ZeroOrMore);
static cl::list<std::string> OptimisticLoops("llpe-optimistic-loop",
                                             cl::ZeroOrMore);
static cl::list<std::string> AlwaysIterLoops("llpe-always-iterate",
                                             cl::ZeroOrMore);
...
static cl::list<std::string>
    IgnoreLoopsWithChildren("llpe-ignore-loop-children", cl::ZeroOrMore);
static cl::list<std::string> AlwaysExploreFunctions("llpe-always-explore",
                                                    cl::ZeroOrMore);
...
static cl::list<std::string> PathConditionsInt("llpe-path-condition-int",
                                               cl::ZeroOrMore);
static cl::list<std::string> PathConditionsFptr("llpe-path-condition-fptr",
                                                cl::ZeroOrMore);
static cl::list<std::string> PathConditionsString("llpe-path-condition-str",
                                                  cl::ZeroOrMore);
static cl::list<std::string> PathConditionsIntmem("llpe-path-condition-intmem",
                                                  cl::ZeroOrMore);
static cl::list<std::string>
    PathConditionsFptrmem("llpe-path-condition-fptrmem", cl::ZeroOrMore);
static cl::list<std::string> PathConditionsFunc("llpe-path-condition-func",
                                                cl::ZeroOrMore);
static cl::list<std::string> PathConditionsStream("llpe-path-condition-stream",
                                                  cl::ZeroOrMore);
static cl::list<std::string>
    PathConditionsGlobalInit("llpe-path-condition-global-unmodified",
                             cl::ZeroOrMore);
...
static cl::list<std::string> SpecialLocations("llpe-special-location",
                                              cl::ZeroOrMore);
static cl::list<std::string> ModelFunctions("llpe-model-function",
                                            cl::ZeroOrMore);
static cl::list<std::string> YieldFunctions("llpe-yield-function",
                                            cl::ZeroOrMore);
static cl::list<std::string> TargetStack("llpe-target-stack", cl::ZeroOrMore);
static cl::list<std::string> SimpleVolatiles("llpe-simple-volatile-load",
                                             cl::ZeroOrMore);
static cl::list<std::string> LockDomains("llpe-lock-domain", cl::ZeroOrMore);
static cl::list<std::string> PessimisticLocks("llpe-pessimistic-lock",
                                              cl::ZeroOrMore);
static cl::opt<bool> DumpDSE("llpe-dump-dse");
static cl::opt<bool> DumpTL("llpe-dump-tl");
static cl::list<std::string> ForceNoAliasArgs("llpe-force-noalias-arg",
                                              cl::ZeroOrMore);
static cl::list<std::string> VarAllocators("llpe-allocator-fn", cl::ZeroOrMore);
static cl::list<std::string> ConstAllocators("llpe-allocator-fn-const",
                                             cl::ZeroOrMore);
...
// JN: commented so that porting to llvm 5.0 it's easier.
...
static void dieArgvUsage() {
  ...
  errs()
      << "--spec-argv must have form M,N,filename where M and N are integers\n";
  exit(1);
}
...
static bool parseIntCommaString(const std::string &str, long &idx,
                                std::string &rest) {

  size_t splitIdx = str.find(',');

  if (splitIdx == std::string::npos || splitIdx == 0 ||
      splitIdx == EnvFileAndIdx.size() - 1) {
    return false;
  }
...
static void parseFB(const char *paramName, const std::string &arg, Module &M,
                    Function *&F, BasicBlock *&BB1) {
...
static void parseFBB(const char *paramName, const std::string &arg, Module &M,
                     Function *&F, BasicBlock *&BB1, BasicBlock *&BB2) {
...
static void parseFBI(const char *paramName, const std::string &arg, Module &M,
                     Function *&F, BasicBlock *&BB, uint64_t &IOut) {
...
void LLPEAnalysisPass::setParam(InlineAttempt *IA, long Idx, Constant *Val) {
  ...
  if (Val->getType() != Target) {
    ...
    errs() << "Type mismatch: constant " << *Val
           << " supplied for parameter of type " << *Target << "\n";
    exit(1);
  }
...
#define CHECK_ARG(i, c)                                                        \
  if (((uint32_t)i) >= c.size()) {                                             \
    errs() << "Function " << F.getName()                                       \
           << " has does not have a (zero-based) argument #" << i << "\n";     \
    exit(1);                                                                   \
  }
...
static Type *getIntTypeAtOffset(Type *Ty, uint64_t Offset) {
  ...
  Type *ElTy = Ptr->getPointerElementType();
  ...
  if (StructType *ST = dyn_cast<StructType>(ElTy)) {
    ...
    release_assert(SL->getElementOffset(FieldNo) == Offset &&
                   "Bad path condition alignment");
...
BasicBlock *LLPEAnalysisPass::parsePCBlock(Function *fStack,
                                           std::string &bbName) {
...
int64_t LLPEAnalysisPass::parsePCInst(BasicBlock *bb, Module *M,
...
void LLPEAnalysisPass::parsePathConditions(cl::list<std::string> &L,
                                           PathConditionTypes Ty,
                                           InlineAttempt *IA) {

  uint32_t newGVIndex = 0;
  if (Ty == PathConditionTypeString)
    newGVIndex = std::distance(IA->F.getParent()->global_begin(),
                               IA->F.getParent()->global_end());

  for (cl::list<std::string>::iterator it = L.begin(), itend = L.end();
       it != itend; ++it) {
    ...
    if (fStackIdxStr.empty() || bbName.empty() || instIndexStr.empty() ||
        assumeStr.empty() || assumeStackIdxStr.empty() || assumeBlock.empty()) {
    ...
    if (fStackIdx != UINT_MAX) {
      ...
      if (assumeStackIdxStr != fStackIdxStr) {
        ...
        errs() << "Non-stack path conditions must not make assumptions that "
    ...
    if (Ty == PathConditionTypeInt && assumeStackIdx != fStackIdx) {
      ...
      errs() << "SSA assumptions cannot cross function boundaries (assume "
                "about an argument instead)\n";
      exit(1);
    }
    ...
    case PathConditionTypeIntmem: {
      ...
      if (bb) {
        ...
        GlobalVariable *GV =
            IA->F.getParent()->getGlobalVariable(instIndexStr, true);
      ...
      if (Ty == PathConditionTypeInt)
        ...
        release_assert(ConstType &&
                       "Failed to find assigned type for path condition");
      }
      release_assert(
          (ConstType->isIntegerTy() ||
           (ConstType->isPointerTy() && !assumeInt)) &&
          "Instructions with an integer assumption must be integer typed");
    ...
    case PathConditionTypeFptrmem: {
      ...
      if (Offset != 0) {
        ...
        assert(assumeC->getType()->isPointerTy());
        assumeC = ConstantExpr::getGetElementPtr(
            cast<PointerType>(assumeC->getType())->getPointerElementType(),
            assumeC, ConstantInt::get(GInt64, Offset));
      }
    ...
    case PathConditionTypeString: {
      GlobalVariable *NewGV =
          getStringArray(assumeStr, *IA->F.getParent(), /*addNull=*/true);
      assumeC = NewGV;
    ...
    case PathConditionTypeGlobalInit: {
      release_assert((!bb) &&
                     "Unmodified global assumption relating to a non-global?");
    ...
    PathCondition newCond((uint32_t)fStackIdx, bb, (uint32_t)instIndex,
                          assumeStackIdx, assumeBB, assumeC, offset);

    if (fStackIdx == UINT_MAX) {

      // Path condition applies to all instances of some function -- attach it
      // to the invarInfo for that function.
...
void LLPEAnalysisPass::parseArgs(Function &F,
                                 std::vector<Constant *> &argConstants,
                                 uint32_t &argvIdxOut) {
  ...
  if (ArgvFileAndIdxs != "") {
    ...
    argConstants[argcIdx] =
        ConstantInt::get(Type::getInt32Ty(F.getContext()), argc);
    argvIdxOut = argvIdx;
  }

  for (cl::list<std::string>::const_iterator ArgI = SpecialiseParams.begin(),
                                             ArgE = SpecialiseParams.end();
       ArgI != ArgE; ++ArgI) {
    ...
    if (ArgTy->isIntegerTy()) {
      ...
      Type *ElemTy = ArgTyP->getPointerElementType();
      ...
      if (ArgTyP == StrTy) {
        ...
        Constant *GStr =
            new GlobalVariable(*(F.getParent()), Str->getType(), true,
                               GlobalValue::InternalLinkage, Str, "specstr");
        Constant *Zero = ConstantInt::get(Type::getInt64Ty(F.getContext()), 0);
        Constant *GEPArgs[] = {Zero, Zero};
        Constant *StrPtr =
            ConstantExpr::getGetElementPtr(Str->getType(), GStr, GEPArgs, 2);
        CHECK_ARG(idx, argConstants);
  ...
  for (cl::list<std::string>::const_iterator
           ArgI = AlwaysInlineFunctions.begin(),
           ArgE = AlwaysInlineFunctions.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator
           ArgI = AlwaysExploreFunctions.begin(),
           ArgE = AlwaysExploreFunctions.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator ArgI = OptimisticLoops.begin(),
                                             ArgE = OptimisticLoops.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator ArgI = AlwaysIterLoops.begin(),
                                             ArgE = AlwaysIterLoops.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator ArgI = AssumeEdges.begin(),
                                             ArgE = AssumeEdges.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator ArgI = IgnoreLoops.begin(),
                                             ArgE = IgnoreLoops.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator
           ArgI = IgnoreLoopsWithChildren.begin(),
           ArgE = IgnoreLoopsWithChildren.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator ArgI = LoopMaxIters.begin(),
                                             ArgE = LoopMaxIters.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator ArgI = SpecialLocations.begin(),
                                             ArgE = SpecialLocations.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::const_iterator ArgI = ModelFunctions.begin(),
                                             ArgE = ModelFunctions.end();
       ArgI != ArgE; ++ArgI) {
    ...
    if ((!realF) || !modelF) {
      ...
      errs() << "-llpe-model-function: no such function " << realFName << " or "
             << modelFName << "\n";
      exit(1);
    }
  ...
  for (cl::list<std::string>::const_iterator ArgI = YieldFunctions.begin(),
                                             ArgE = YieldFunctions.end();
       ArgI != ArgE; ++ArgI) {
  ...
  for (cl::list<std::string>::iterator it = TargetStack.begin(),
    ...
    if (fName.empty() || bbName.empty() || instIdxStr.empty()) {
      errs()
          << "llpe-target-stack must have form functionName,blockName,index\n";
      exit(1);
    }
    ...
    uint32_t instIdx =
        (uint32_t)getInteger(instIdxStr, "llpe-target-stack instruction index");
  ...
  for (cl::list<std::string>::iterator it = SimpleVolatiles.begin(),
    ...
    parseFBI("llpe-simple-volatile-load", *it, *(F.getParent()), LoadF, BB,
             Offset);

    BasicBlock::iterator BI = BB->begin();
    std::advance(BI, Offset);

    if (!(isa<LoadInst>(BI) || isa<AtomicRMWInst>(BI) ||
          isa<AtomicCmpXchgInst>(BI))) {
      errs() << "llpe-simple-volatile-load: " << *it
             << " does not denote an atomic op\n";
      exit(1);
    }
  ...
  for (cl::list<std::string>::iterator it = LockDomains.begin(),
    ...
    // TODO here: add a better specification of lock domain than just a list of
    // globals!
    ...
    for (uint32_t i = 0; i < 3; ++i) {
      ...
      if (pos == std::string::npos) {
        errs() << "llpe-lock-domain: usage: "
                  "lockf,lockblock,lockoffset,global1,...,globaln\n";
        exit(1);
      }
      ++pos;
    }
  ...
  for (cl::list<std::string>::iterator it = ForceNoAliasArgs.begin(),
                                       itend = ForceNoAliasArgs.end();
       it != itend; ++it) {

    uint32_t argIdx =
        (uint32_t)getInteger(*it, "llpe-force-noalias-arg parameter");
    forceNoAliasArgs.insert(argIdx);
  }

  for (cl::list<std::string>::iterator it = VarAllocators.begin(),
                                       itend = VarAllocators.end();
       it != itend; ++it) {
    ...
    if (!freeName.empty()) {
      ...
      uint32_t releaseArg =
          getInteger(freeIdxStr, "llpe-allocator-fn fourth param");
  ...
  for (cl::list<std::string>::iterator it = ConstAllocators.begin(),
                                       itend = ConstAllocators.end();
       it != itend; ++it) {

    std::string fName, sizeStr, freeName, freeIdxStr, reallocName,
        reallocPtrIdxStr, reallocSizeIdxStr;
    ...
    allocatorFunctions[allocF] =
        AllocatorFn::getConstantSize(ConstantInt::get(I32, size));
    SpecialFunctionMap[allocF] = SF_MALLOC;
    ...
    if (!reallocName.empty()) {
      ...
      uint32_t reallocPtrIdx =
          getInteger(reallocPtrIdxStr, "llpe-allocator-fn sixth param");
      uint32_t reallocSizeIdx =
          getInteger(reallocPtrIdxStr, "llpe-allocator-fn seventh param");
      reallocatorFunctions[reallocF] =
          ReallocatorFn(reallocPtrIdx, reallocSizeIdx);
      SpecialFunctionMap[reallocF] = SF_REALLOC;
    }
  }

  for (cl::list<std::string>::iterator it = NeverInline.begin(),
                                       itend = NeverInline.end();
       it != itend; ++it) {
  ...
  for (cl::list<std::string>::iterator it = SplitFunctions.begin(),
                                       itend = SplitFunctions.end();
       it != itend; ++it) {
  ...
  //   DIB.createCompileUnit(dwarf::DW_LANG_C89, "llpe.file", "/nonesuch",
  //   "LLPE", true, "", 0); DIBasicType retType =
  //   DIB.createBasicType("fakechar", 8, 0, dwarf::DW_ATE_signed); DITypeArray
  //   functionParamTypes =
  //   DIB.getOrCreateTypeArray(ArrayRef<Metadata*>((Metadata*)retType));
...
void LLPEAnalysisPass::parseArgsPostCreation(InlineAttempt *IA) {
  ...
  for (cl::list<std::string>::iterator it = IgnoreBlocks.begin(),
                                       itend = IgnoreBlocks.end();
       it != itend; ++it) {
    ...
    if (fName != IA->F.getName()) {
      ...
      errs() << "llpe-ignore-block currently only supported in the root "
                "function\n";
      exit(1);
    }
  ...
  parsePathConditions(PathConditionsGlobalInit, PathConditionTypeGlobalInit,
                      IA);

  for (cl::list<std::string>::iterator it = PathConditionsFunc.begin(),
                                       itend = PathConditionsFunc.end();
       it != itend; ++it) {
    ...
    if (fStackIdxStr.empty() || bbName.empty() || calledName.empty() ||
        verifyName.empty()) {

      errs() << "-llpe-path-condition-func usage: "
                "context-function,context-block,path-function,verify-function"
             << "\n";
      exit(1);
    }
    ...
    PC->FuncPathConditions.push_back(
        PathFunc(fStackIdx, assumeBlock, CallF, VerifyF));
    ...
    while (!istr.eof()) {
      ...
      if (fStackIdx == UINT_MAX) {
        ...
        argStackIdx =
            getInteger(argStackIdxStr, "Path function argument stack index");
    ...
    release_assert(FType->getNumParams() == newFunc.args.size() &&
                   "Path function with wrong arg count");
  }
}

// Cheeky export for TopLevel.cpp's consumption:
namespace llvm {
size_t getStringPathConditionCount() { return PathConditionsString.size(); }
} // namespace llvm
