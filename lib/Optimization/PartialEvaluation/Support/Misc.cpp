...
// LLVM 14: isIdentifiedObject was in AliasAnalysis and is not in public API.
// Reimplement for PE: true for globals, allocas, and functions.
bool llvm::isIdentifiedObject(const Value *V) {
  if (!V || !V->getType()->isPointerTy())
    return false;
  V = V->stripPointerCasts();
  return isa<GlobalValue>(V) || isa<AllocaInst>(V);
}

// A context's parent is usually unique, except when function instance sharing
// is enabled.

IntegrationAttempt *InlineAttempt::getUniqueParent() { return uniqueParent; }

IntegrationAttempt *PeelIteration::getUniqueParent() { return parent; }

Module &IntegrationAttempt::getModule() { return *(F.getParent()); }
...
bool InlineAttempt::isRootMainCall() { return this == pass->getRoot(); }

// Fetch vararg idx if possible.
void InlineAttempt::getVarArg(int64_t idx, ImprovedValSet *&Result) {

  release_assert(idx >= ImprovedVal::first_any_arg &&
                 idx < ImprovedVal::max_arg);
...
// GDB callable:
void IntegrationAttempt::dump() const { describe(outs()); }
...
std::string IntegrationAttempt::getFunctionName() { return F.getName().str(); }

// Write a full dump of this context and its children:
void PeelAttempt::print(raw_ostream &OS) const {

  OS << nestingIndent() << "Loop " << getLName()
     << (Iterations.back()->iterStatus == IterationStatusFinal
             ? "(terminated)"
             : "(not terminated)")
     << "\n";

  for (std::vector<PeelIteration *>::const_iterator it = Iterations.begin(),
                                                    it2 = Iterations.end();
       it != it2; ++it) {
...
void PeelAttempt::dumpMemoryUsage(int indent) {
  ...
  errs() << ind(indent) << "Loop " << getLName() << " (" << Iterations.size()
         << " iterations)\n";
...
Constant *llvm::constFromBytes(unsigned char *Bytes, Type *Ty,
                               const DataLayout *TD) {
  ...
  if (Ty->isVectorTy() || Ty->isFloatingPointTy() || Ty->isIntegerTy()) {
    ...
    // I *think* arrays are always dense, i.e. it's for the child type to
    // specify padding.
    ...
    LLVM_DEBUG(dbgs() << "Can't build a constant containing unhandled type "
                      << (*Ty) << "\n");
    return 0;
  }
}

Module *llvm::getGlobalModule() { return GlobalIHP->RootIA->F.getParent(); }

std::string PeelAttempt::getLName() const { return Iterations[0]->getLName(); }

std::string PeelIteration::getLName() const {
  return getBBInvar(L->headerIdx)->BB->getName().str();
}
