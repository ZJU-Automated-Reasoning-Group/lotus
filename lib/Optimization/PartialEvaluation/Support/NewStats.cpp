...
void IntegrationAttempt::preCommitStats(bool enabledHere) {
  ...
  for (uint32_t i = 0; i < nBBs; ++i) {
    ...
    GlobalIHP->stats.condChecks +=
        GlobalIHP->countPathConditionsAtBlockStart(BBs[i]->invar, this);

    bool hasUniqueSucc = false;
    for (uint32_t j = 0, jlim = BBs[i]->invar->succIdxs.size(); j != jlim;
         ++j) {
    ...
    for (uint32_t j = 0, jlim = BBs[i]->insts.size(); j != jlim; ++j) {
      ...
      if (j + 1 == jlim) {
        ...
        BranchInst *BI;
        if (hasUniqueSucc &&
            ((!(BI = dyn_cast_inst<BranchInst>(&SI))) || BI->isConditional()))
        ...
        if (IVS && IVS->Values.size() == 1) {
          ...
          // Count instructions successfully resolved to a constant or known
          // symbolic pointer.
          ...
          // Count instructions with some concrete information but nontheless
          // residualised. These are assigned a set of possible values during
          // specialisation.
          ++GlobalIHP->stats.setInstructions;

        } else {

          // Count instructions about which we have no information during
          // specialisation.
          ++GlobalIHP->stats.unknownInstructions;
        }

        // Count instructions proved un-needed during specialisation, e.g.
        // stores whose corresponding loads have been resolved.
        if (SI.dieStatus)
          ++GlobalIHP->stats.deadInstructions;

        // Count memory-reading instructions whose results must be checked
        // against interference by concurrent threads.
        if (SI.readsMemoryDirectly()) {
          if (SI.isThreadLocal == TLS_MUSTCHECK)
            ++GlobalIHP->stats.threadChecks;
        }

        // Count instructions that need I/O-related checks, either bytewise
        // (memcmp) or using the file-watcher daemon.
        if (SI.needsRuntimeCheck == RUNTIME_CHECK_READ_MEMCMP ||
        ...
        if (SI.needsRuntimeCheck == RUNTIME_CHECK_AS_EXPECTED) {
          ...
          if (inst_is<ICmpInst>(&SI) && (isHeapPointer(SI.getOperand(0)) ||
                                         isHeapPointer(SI.getOperand(1))))
  ...
  for (DenseMap<const ShadowLoopInvar *, PeelAttempt *>::iterator
    ...
    bool enabled =
        enabledHere && it->second->isTerminated() && it->second->isEnabled();

    // If the loop wasn't known-terminated we would have analysed and committed
    // a general iteration, which is part of this context not a child.
