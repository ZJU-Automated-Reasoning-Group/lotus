...
namespace SLOT {
LLVMNode::LLVMNode(bool t_shiftToMultiply, context &t_scx,
                   LLVMFunction &t_function, Value *t_contents)
    : shiftToMultiply(t_shiftToMultiply), scx(t_scx), function(t_function),
      contents(t_contents) {}

std::unique_ptr<LLVMNode> LLVMNode::MakeLLVMNode(bool shiftToMultiply,
                                                 context &scx,
                                                 LLVMFunction &function,
                                                 Value *contents) {
  if (isa<Argument>(contents)) {
    return std::make_unique<LLVMArgument>(shiftToMultiply, scx, function,
                                          contents);
  } else if (isa<Constant>(contents)) {
    return std::make_unique<LLVMConstant>(shiftToMultiply, scx, function,
                                          contents);
  } else if (isa<Instruction>(contents)) {
    Instruction *uv = ((Instruction *)contents);
    if (uv->getOpcode() == Instruction::Call) {
      return std::make_unique<LLVMIntrinsicCall>(shiftToMultiply, scx, function,
                                                 contents);
    } else if (uv->getOpcode() == Instruction::ICmp) {
      return std::make_unique<LLVMIcmp>(shiftToMultiply, scx, function,
                                        contents);
    } else if (uv->getOpcode() == Instruction::FCmp) {
      return std::make_unique<LLVMFcmp>(shiftToMultiply, scx, function,
                                        contents);
    } else {
      return std::make_unique<LLVMExpression>(shiftToMultiply, scx, function,
                                              contents);
    }
  } else {
    throw UnsupportedLLVMOpException(
        "node construction on non-instruction, argument, or constant",
        contents);
  }
}
...
LLVMArgument::LLVMArgument(bool t_shiftToMultiply, context &t_scx,
                           LLVMFunction &t_function, Value *t_contents)
    : LLVMNode(t_shiftToMultiply, t_scx, t_function, t_contents) {
...
LLVMConstant::LLVMConstant(bool t_shiftToMultiply, context &t_scx,
                           LLVMFunction &t_function, Value *t_contents)
    : LLVMNode(t_shiftToMultiply, t_scx, t_function, t_contents) {
...
expr LLVMConstant::ToSMT() {
  ...
  } else // FPA case
  {
    ...
    ((ConstantFP *)contents)
        ->getValue()
        .bitcastToAPInt()
        .toString(str, 10, false);
    return scx.bv_val(str.c_str(), Width()).mk_from_ieee_bv(SMTSort());
  }
}

//============================LLVMIntrinsicCall==================================
LLVMIntrinsicCall::LLVMIntrinsicCall(bool t_shiftToMultiply, context &t_scx,
                                     LLVMFunction &t_function,
                                     Value *t_contents)
    : LLVMExpression(t_shiftToMultiply, t_scx, t_function, t_contents) {
...
expr LLVMIntrinsicCall::FPClassCheck(context &scx, expr val, int64_t bits) {
  ...
  default:
    throw UnsupportedSMTOpException(
        "unsupported floating point class check flag bitmask " +
            std::to_string(bits),
        val);
  }
}
...
expr LLVMIntrinsicCall::AsRoundingMode(unsigned n) {
  ...
  if (rso.str() == "!\"round.tonearest\"") {
    ...
    throw UnsupportedLLVMOpException("unsupported constrained rounding mode",
                                     contents);
  }
}
...
expr LLVMIntrinsicCall::ToSMT() {
  Intrinsic::ID id =
      ((CallInst *)contents)->getCalledFunction()->getIntrinsicID();
  ...
  case Intrinsic::fma:
    return z3::fma(Child(0)->ToSMT(), Child(1)->ToSMT(), Child(2)->ToSMT(),
                   scx.fpa_rounding_mode());
  case Intrinsic::sqrt:
  ...
  case Intrinsic::fshl:
    ...
    return shl(concat(Child(0)->ToSMT(), Child(1)->ToSMT()),
               zext(urem(Child(2)->ToSMT(), scx.bv_val(Width(), Width())),
                    Width()))
        .extract((Width() * 2) - 1, Width());
  case Intrinsic::fshr:
    // Funnel shift amount is interpreted modulo element bit width.
    return lshr(concat(Child(0)->ToSMT(), Child(1)->ToSMT()),
                zext(urem(Child(2)->ToSMT(), scx.bv_val(Width(), Width())),
                     Width()))
        .extract(Width() - 1, 0);
  ...
  case Intrinsic::roundeven:
    return expr(scx,
                Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rne(scx)),
                                            Child(0)->ToSMT()));
  case Intrinsic::lround:
    return expr(scx,
                Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rna(scx)),
                                            Child(0)->ToSMT()));
  case Intrinsic::ceil:
    return expr(scx,
                Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rtp(scx)),
                                            Child(0)->ToSMT()));
  case Intrinsic::floor:
    return expr(scx,
                Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rtn(scx)),
                                            Child(0)->ToSMT()));
  case Intrinsic::trunc:
    return expr(scx,
                Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rtz(scx)),
                                            Child(0)->ToSMT()));
  case Intrinsic::experimental_constrained_uitofp:
    return expr(scx, Z3_mk_fpa_to_fp_unsigned(scx, AsRoundingMode(1),
                                              Child(0)->ToSMT(), SMTSort()));
  case Intrinsic::experimental_constrained_sitofp:
    return expr(scx, Z3_mk_fpa_to_fp_signed(scx, AsRoundingMode(1),
                                            Child(0)->ToSMT(), SMTSort()));
  case Intrinsic::experimental_constrained_fpext:
  case Intrinsic::experimental_constrained_fptrunc:
    return expr(scx, Z3_mk_fpa_to_fp_float(scx, AsRoundingMode(1),
                                           Child(0)->ToSMT(), SMTSort()));
  case Intrinsic::experimental_constrained_fadd:
    return expr(scx, Z3_mk_fpa_add(scx, AsRoundingMode(2), Child(0)->ToSMT(),
                                   Child(1)->ToSMT()));
  case Intrinsic::experimental_constrained_fsub:
    return expr(scx, Z3_mk_fpa_sub(scx, AsRoundingMode(2), Child(0)->ToSMT(),
                                   Child(1)->ToSMT()));
  case Intrinsic::experimental_constrained_fmul:
    return expr(scx, Z3_mk_fpa_mul(scx, AsRoundingMode(2), Child(0)->ToSMT(),
                                   Child(1)->ToSMT()));
  case Intrinsic::experimental_constrained_fdiv:
    return expr(scx, Z3_mk_fpa_div(scx, AsRoundingMode(2), Child(0)->ToSMT(),
                                   Child(1)->ToSMT()));
  case Intrinsic::experimental_constrained_fma:
    return expr(scx, Z3_mk_fpa_fma(scx, AsRoundingMode(3), Child(0)->ToSMT(),
                                   Child(1)->ToSMT(), Child(2)->ToSMT()));
  case Intrinsic::experimental_constrained_sqrt:
    return expr(scx, Z3_mk_fpa_sqrt(scx, AsRoundingMode(1), Child(0)->ToSMT()));
  default:
    throw UnsupportedLLVMOpException("unsupported intrinsic function",
                                     contents);
  }
}

//============================LLVMIcmp==================================
LLVMIcmp::LLVMIcmp(bool t_shiftToMultiply, context &t_scx,
                   LLVMFunction &t_function, Value *t_contents)
    : LLVMExpression(t_shiftToMultiply, t_scx, t_function, t_contents) {
...
expr LLVMIcmp::ToSMT() {
  ...
  if (Predicate() == CmpInst::ICMP_EQ) {
    // This arose from a floating point = comparison. Important since there is
    // no floating point to bitvector bitcast in SMT
    if (isa<BitCastInst>(vi->getOperand(0)) &&
        isa<BitCastInst>(vi->getOperand(1))) {
      return std::unique_ptr<LLVMExpression>{
                 static_cast<LLVMExpression *>(Child(0).release())}
                 ->Child(0)
                 ->ToSMT() ==
             std::unique_ptr<LLVMExpression>{
                 static_cast<LLVMExpression *>(Child(1).release())}
                 ->Child(0)
                 ->ToSMT();
    } else if (isa<BitCastInst>(vi->getOperand(0))) {
      expr left = std::unique_ptr<LLVMExpression>{static_cast<LLVMExpression *>(
                                                      Child(0).release())}
                      ->Child(0)
                      ->ToSMT();
    ...
    // This arose from a floating point distinct comparison. Important since
    // there is no floating point to bitvector bitcast in SMT
    if (isa<BitCastInst>(vi->getOperand(0)) &&
        isa<BitCastInst>(vi->getOperand(1))) {
      return std::unique_ptr<LLVMExpression>{
                 static_cast<LLVMExpression *>(Child(0).release())}
...
//============================LLVMFcmp==================================
LLVMFcmp::LLVMFcmp(bool t_shiftToMultiply, context &t_scx,
                   LLVMFunction &t_function, Value *t_contents)
    : LLVMExpression(t_shiftToMultiply, t_scx, t_function, t_contents) {
...
LLVMExpression::LLVMExpression(bool t_shiftToMultiply, context &t_scx,
                               LLVMFunction &t_function, Value *t_contents)
    : LLVMNode(t_shiftToMultiply, t_scx, t_function, t_contents) {
...
expr LLVMExpression::ToSMT() {
  ...
  case Instruction::ExtractValue:
    ...
    if (isa<CallInst>(child) &&
        ((CallInst *)child)->getCalledFunction()->getIntrinsicID() ==
      ...
      expr right =
          std::unique_ptr<LLVMExpression>{
              ...
              ->ToSMT();
      expr zero =
          scx.bv_val(0, left.get_sort().bv_size()); // Not the same as Zero()
      return ite(((left == zero) || (right == zero)), scx.bool_val(false),
                 !(udiv((left * right), left) == right));
    } else {
      throw UnsupportedLLVMOpException(
          "extract value without umul.with.overflow", contents);
    }
  ...
  case Instruction::FPToUI:
    ...
    if (isa<CallInst>(child)) {
      ...
      case Intrinsic::roundeven:
        return expr(scx, Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rne(scx)),
                                          Child(0)->ToSMT(), Width()));
      case Intrinsic::lround:
        return expr(scx, Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rna(scx)),
                                          Child(0)->ToSMT(), Width()));
      case Intrinsic::ceil:
        return expr(scx, Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rtp(scx)),
                                          Child(0)->ToSMT(), Width()));
      case Intrinsic::floor:
        return expr(scx, Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rtn(scx)),
                                          Child(0)->ToSMT(), Width()));
      default:
        break;
      }
    ...
    return expr(scx, Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rtz(scx)),
                                      Child(0)->ToSMT(), Width()));
  case Instruction::FPToSI:
    ...
    if (isa<CallInst>(child)) {
      switch (((CallInst *)child)->getCalledFunction()->getIntrinsicID()) {
      case Intrinsic::roundeven:
        return expr(scx, Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rne(scx)),
                                          Child(0)->ToSMT(), Width()));
      case Intrinsic::lround:
        return expr(scx, Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rna(scx)),
                                          Child(0)->ToSMT(), Width()));
      case Intrinsic::ceil:
        return expr(scx, Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rtp(scx)),
                                          Child(0)->ToSMT(), Width()));
      case Intrinsic::floor:
        return expr(scx, Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rtn(scx)),
                                          Child(0)->ToSMT(), Width()));
      default:
        break;
      }
    ...
    return expr(scx, Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rtz(scx)),
                                      Child(0)->ToSMT(), Width()));
  ...
  case Instruction::BitCast:
    ...
    if (SMTSort().is_fpa()) {
      ...
      // There is no floating to bitvector bitcast equivalent in SMT since NaN
      // has multiple representations Add a new variable and constrain it equal
  ...
  case Instruction::Freeze:
    // Frontend guarantees no undefined behavior, so freeze always returns its
    // argument
    return Child(0)->ToSMT();
  ...
  case Instruction::Shl:
    left = Child(0)->ToSMT();
    right = Child(1)->ToSMT();
    // Check for shift left by a constant; this can be replaced with
    // multiplication
    if (shiftToMultiply && right.is_const() &&
        ((rr = std::atoi(right.to_string().c_str())) > 0)) {
