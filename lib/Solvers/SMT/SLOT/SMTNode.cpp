...
inline FloatingNode SMTNode::FloatingChild(expr cont) {
  return FloatingNode(lcx, lmodule, builder, variables, value_cache, cont);
}
inline FloatingNode SMTNode::FloatingChild(int index) {
  return FloatingNode(lcx, lmodule, builder, variables, value_cache,
                      contents.arg(index));
}
inline BitvectorNode SMTNode::BitvectorChild(expr cont) {
  return BitvectorNode(lcx, lmodule, builder, variables, value_cache, cont);
}
inline BitvectorNode SMTNode::BitvectorChild(int index) {
  return BitvectorNode(lcx, lmodule, builder, variables, value_cache,
                       contents.arg(index));
}
inline BooleanNode SMTNode::BooleanChild(expr cont) {
  return BooleanNode(lcx, lmodule, builder, variables, value_cache, cont);
}
inline BooleanNode SMTNode::BooleanChild(int index) {
  return BooleanNode(lcx, lmodule, builder, variables, value_cache,
                     contents.arg(index));
}

Value *ConcatTwoBV(Value *arg0, unsigned arg0Width, Value *arg1,
                   unsigned arg1Width, IRBuilder<> &builder, LLVMContext &lcx) {
  ...
  auto *sel2 =
      builder.CreateShl(sel0, ConstantInt::get(totalWidthType, arg1Width));
  return builder.CreateOr(sel1, sel2);
}

SMTNode::SMTNode(LLVMContext &t_lcx, Module *t_lmodule, IRBuilder<> &t_builder,
                 const LLMAPPING &t_variables, SMTValueCache *t_value_cache,
                 expr t_contents)
    : lcx(t_lcx), lmodule(t_lmodule), builder(t_builder),
      variables(t_variables), value_cache(t_value_cache), contents(t_contents) {
...
BooleanNode::BooleanNode(LLVMContext &t_lcx, Module *t_lmodule,
                         IRBuilder<> &t_builder, const LLMAPPING &t_variables,
                         SMTValueCache *t_value_cache, expr t_contents)
    : SMTNode(t_lcx, t_lmodule, t_builder, t_variables, t_value_cache,
              t_contents) {
...
Value *BooleanNode::ToLLVMInternal() {
  ...
  } else // Expression case
  {
    ...
    case Z3_OP_IMPLIES:
      ...
      return builder.CreateOr(builder.CreateNot(BooleanChild(0).ToLLVM()),
                              BooleanChild(1).ToLLVM());
    case Z3_OP_ITE:
      assert(contents.num_args() == 3);
      return builder.CreateSelect(BooleanChild(0).ToLLVM(),
                                  BooleanChild(1).ToLLVM(),
                                  BooleanChild(2).ToLLVM());
    // Bitvector comparisons
    case Z3_OP_SLEQ:
      assert(contents.num_args() == 2);
      return builder.CreateICmpSLE(BitvectorChild(0).ToLLVM(),
                                   BitvectorChild(1).ToLLVM());
    case Z3_OP_SGEQ:
      assert(contents.num_args() == 2);
      return builder.CreateICmpSGE(BitvectorChild(0).ToLLVM(),
                                   BitvectorChild(1).ToLLVM());
    case Z3_OP_SLT:
      assert(contents.num_args() == 2);
      return builder.CreateICmpSLT(BitvectorChild(0).ToLLVM(),
                                   BitvectorChild(1).ToLLVM());
    case Z3_OP_SGT:
      assert(contents.num_args() == 2);
      return builder.CreateICmpSGT(BitvectorChild(0).ToLLVM(),
                                   BitvectorChild(1).ToLLVM());
    case Z3_OP_ULEQ:
      assert(contents.num_args() == 2);
      return builder.CreateICmpULE(BitvectorChild(0).ToLLVM(),
                                   BitvectorChild(1).ToLLVM());
    case Z3_OP_UGEQ:
      assert(contents.num_args() == 2);
      return builder.CreateICmpUGE(BitvectorChild(0).ToLLVM(),
                                   BitvectorChild(1).ToLLVM());
    case Z3_OP_ULT:
      assert(contents.num_args() == 2);
      return builder.CreateICmpULT(BitvectorChild(0).ToLLVM(),
                                   BitvectorChild(1).ToLLVM());
    case Z3_OP_UGT:
      assert(contents.num_args() == 2);
      return builder.CreateICmpUGT(BitvectorChild(0).ToLLVM(),
                                   BitvectorChild(1).ToLLVM());
    // Floating comparisons. All comparisons are ordered
    //(SMT LIB says comparison is false if either argument is NAN)
    case Z3_OP_FPA_EQ:
      assert(contents.num_args() == 2);
      return builder.CreateFCmpOEQ(FloatingChild(0).ToLLVM(),
                                   FloatingChild(1).ToLLVM());
    case Z3_OP_FPA_LT:
      assert(contents.num_args() == 2);
      return builder.CreateFCmpOLT(FloatingChild(0).ToLLVM(),
                                   FloatingChild(1).ToLLVM());
    case Z3_OP_FPA_GT:
      assert(contents.num_args() == 2);
      return builder.CreateFCmpOGT(FloatingChild(0).ToLLVM(),
                                   FloatingChild(1).ToLLVM());
    case Z3_OP_FPA_LE:
      assert(contents.num_args() == 2);
      return builder.CreateFCmpOLE(FloatingChild(0).ToLLVM(),
                                   FloatingChild(1).ToLLVM());
    case Z3_OP_FPA_GE:
      assert(contents.num_args() == 2);
      return builder.CreateFCmpOGE(FloatingChild(0).ToLLVM(),
                                   FloatingChild(1).ToLLVM());
    ...
    case Z3_OP_EQ:
      ...
      if (contents.arg(0).is_bool()) {
        return builder.CreateICmpEQ(BooleanChild(0).ToLLVM(),
                                    BooleanChild(1).ToLLVM());
      } else if (contents.arg(0).is_bv()) {
        return builder.CreateICmpEQ(BitvectorChild(0).ToLLVM(),
                                    BitvectorChild(1).ToLLVM());
    ...
    case Z3_OP_DISTINCT:
      if (contents.num_args() < 2) {
        ...
        if (contents.arg(0).is_bool()) {
          return builder.CreateICmpNE(BooleanChild(0).ToLLVM(),
                                      BooleanChild(1).ToLLVM());
        } else if (contents.arg(0).is_bv()) {
          return builder.CreateICmpNE(BitvectorChild(0).ToLLVM(),
                                      BitvectorChild(1).ToLLVM());
        }
        ...
        for (unsigned int i = 0; i < contents.num_args(); i++) {
          for (unsigned int j = i + 1; j < contents.num_args(); j++) {
            if (contents.arg(0).is_bool()) {
              ...
              IntegerType *iType =
                  IntegerType::get(lcx, FloatingChild(i).Width());
              Value *lb = builder.CreateBitCast(childValues[i], iType);
              Value *rb = builder.CreateBitCast(childValues[j], iType);
              Function *fpclassFun = Intrinsic::getDeclaration(
                  lmodule, Function::lookupIntrinsicID("llvm.is.fpclass"),
                  childValues[i]->getType());
              Value *nanMask = ConstantInt::get(
                  IntegerType::get(lcx, 32),
                  FloatingNode::class_flags.at(Z3_OP_FPA_IS_NAN));
              Value *lhsNan =
                  builder.CreateCall(fpclassFun, {childValues[i], nanMask});
              Value *rhsNan =
                  builder.CreateCall(fpclassFun, {childValues[j], nanMask});
              v = builder.CreateAnd(
                  builder.CreateNot(builder.CreateAnd(lhsNan, rhsNan)),
                  builder.CreateICmpNE(lb, rb));
            } else {
              throw UnsupportedSMTOpException(X_DISTINCT_TYPE, contents);
            }
...
Value *BitvectorNode::LlURem(IRBuilder<> &builder, Value *left, Value *right) {
  return builder.CreateSelect(
      builder.CreateICmpEQ(right, ConstantInt::get(right->getType(), 0)), left,
      builder.CreateURem(left, right));
}

BitvectorNode::BitvectorNode(LLVMContext &t_lcx, Module *t_lmodule,
                             IRBuilder<> &t_builder,
                             const LLMAPPING &t_variables,
                             SMTValueCache *t_value_cache, expr t_contents)
    : SMTNode(t_lcx, t_lmodule, t_builder, t_variables, t_value_cache,
              t_contents) {
  // Sanity check for translation from Z3 expressions
...
Value *BitvectorNode::ToLLVMInternal() {
  if (IsVariable()) {
    ...
    assert(contents.to_string()[0] == '#' &&
           (contents.to_string()[1] == 'b' || contents.to_string()[1] == 'x'));
    bool isBinary = contents.to_string()[1] == 'b';
    return ConstantInt::get(IntegerType::get(lcx, Width()),
                            contents.to_string().substr(2), isBinary ? 2 : 16);
  } else // Expression case
  {
    Value *one = ConstantInt::get(IntegerType::get(lcx, Width()), 1);
    Value *mone = builder.CreateNeg(
        one); // ConstantInt::get(IntegerType::get(lcx, Width()), -1);
    Function *fun;
    ...
    case Z3_OP_FPA_TO_UBV:
      ...
      switch (RoundingMode()) {
      case Z3_OP_FPA_RM_NEAREST_TIES_TO_EVEN:
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::roundeven,
            FloatingNode::ToFloatingType(lcx, contents.to_string(),
                                         FloatingChild(1).Width()));
        break;
      case Z3_OP_FPA_RM_NEAREST_TIES_TO_AWAY:
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::lround,
            FloatingNode::ToFloatingType(lcx, contents.to_string(),
                                         FloatingChild(1).Width()));
        break;
      case Z3_OP_FPA_RM_TOWARD_POSITIVE:
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::ceil,
            FloatingNode::ToFloatingType(lcx, contents.to_string(),
                                         FloatingChild(1).Width()));
        break;
      case Z3_OP_FPA_RM_TOWARD_NEGATIVE:
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::floor,
            FloatingNode::ToFloatingType(lcx, contents.to_string(),
                                         FloatingChild(1).Width()));
        break;
      case Z3_OP_FPA_RM_TOWARD_ZERO:
        // Default behavior of llvm fptoui
        return builder.CreateFPToUI(FloatingChild(1).ToLLVM(),
                                    IntegerType::get(lcx, Width()));
      default:
        throw UnsupportedSMTOpException(X_RM_VAR, contents);
      }
      return builder.CreateFPToUI(builder.CreateCall(fun, args),
                                  IntegerType::get(lcx, Width()));
    case Z3_OP_FPA_TO_SBV:
      ...
      case Z3_OP_FPA_RM_TOWARD_ZERO:
        ...
        return builder.CreateFPToSI(FloatingChild(1).ToLLVM(),
                                    IntegerType::get(lcx, Width()));
      default:
        throw UnsupportedSMTOpException(X_RM_VAR, contents);
      }
      return builder.CreateFPToSI(builder.CreateCall(fun, args),
                                  IntegerType::get(lcx, Width()));
    case Z3_OP_BNEG:
      assert(contents.num_args() == 1);
    ...
    case Z3_OP_BADD:
      assert(contents.num_args() == 2);
      return builder.CreateAdd(BitvectorChild(0).ToLLVM(),
                               BitvectorChild(1).ToLLVM());
    case Z3_OP_BSUB:
      assert(contents.num_args() == 2);
      return builder.CreateSub(BitvectorChild(0).ToLLVM(),
                               BitvectorChild(1).ToLLVM());
    case Z3_OP_BMUL:
      assert(contents.num_args() == 2);
      return builder.CreateMul(BitvectorChild(0).ToLLVM(),
                               BitvectorChild(1).ToLLVM());
    case Z3_OP_BSDIV:
      assert(contents.num_args() == 2);
      return builder.CreateSelect(
          BitvectorChild(1).IsZero(),
          builder.CreateSelect(BitvectorChild(0).IsNegative(), one, mone),
          builder.CreateSDiv(BitvectorChild(0).ToLLVM(),
                             BitvectorChild(1).ToLLVM()));
    case Z3_OP_BSDIV_I:
      assert(contents.num_args() == 2);
      // [z3's docs] It has the same semantics as Z3_OP_BSDIV, but created in a
      // context where the second operand can be assumed to be non-zero.
      return builder.CreateSDiv(BitvectorChild(0).ToLLVM(),
                                BitvectorChild(1).ToLLVM());
    case Z3_OP_BUDIV:
      assert(contents.num_args() == 2);
      {
        ...
        Value *rightIsZero = builder.CreateICmpEQ(
            rightVal, ConstantInt::get(rightVal->getType(), 0));
        return builder.CreateSelect(rightIsZero, mone,
    ...
    case Z3_OP_BSREM:
      assert(contents.num_args() == 2);
      {
        ...
        Value *rightIsZero = builder.CreateICmpEQ(
            rightVal, ConstantInt::get(rightVal->getType(), 0));
        return builder.CreateSelect(rightIsZero, leftVal,
                                    builder.CreateSRem(leftVal, rightVal));
      }
    case Z3_OP_BSREM_I:
      assert(contents.num_args() == 2);
      // [z3's docs] It has the same semantics as Z3_OP_BSREM, but created in a
      // context where the second operand can be assumed to be non-zero.
      return builder.CreateSRem(BitvectorChild(0).ToLLVM(),
                                BitvectorChild(1).ToLLVM());
    case Z3_OP_BUREM:
      assert(contents.num_args() == 2);
      {
    ...
    case Z3_OP_BSMOD:
      assert(contents.num_args() == 2);
      {
        ...
        Value *absLeft = builder.CreateSelect(
            leftIsNegative, builder.CreateSub(Zero(), leftVal), leftVal);
        Value *absRight = builder.CreateSelect(
            rightIsNegative, builder.CreateSub(Zero(), rightVal), rightVal);
        u = BitvectorNode::LlURem(builder, absLeft, absRight);
        sel0 = builder.CreateSelect(
            builder.CreateAnd(leftIsPositive, rightIsNegative),
            builder.CreateAdd(u, rightVal), builder.CreateSub(Zero(), u));
        sel1 = builder.CreateSelect(
            builder.CreateAnd(leftIsNegative, rightIsPositive),
            builder.CreateAdd(builder.CreateSub(Zero(), u), rightVal), sel0);
        sel2 = builder.CreateSelect(
            builder.CreateAnd(leftIsPositive, rightIsPositive), u, sel1);
    ...
    case Z3_OP_BNAND:
      assert(contents.num_args() == 2);
      return builder.CreateNot(builder.CreateAnd(BitvectorChild(0).ToLLVM(),
                                                 BitvectorChild(1).ToLLVM()));
    case Z3_OP_BNOR:
      assert(contents.num_args() == 2);
      return builder.CreateNot(builder.CreateOr(BitvectorChild(0).ToLLVM(),
                                                BitvectorChild(1).ToLLVM()));
    case Z3_OP_BXNOR:
      assert(contents.num_args() == 2);
      return builder.CreateNot(builder.CreateXor(BitvectorChild(0).ToLLVM(),
                                                 BitvectorChild(1).ToLLVM()));
    case Z3_OP_CONCAT: {
      // assert(contents.num_args()==2);
      // sel0 = builder.CreateZExt(BitvectorChild(0).ToLLVM(),
      // IntegerType::get(lcx, Width())); sel1 =
      // builder.CreateZExt(BitvectorChild(1).ToLLVM(), IntegerType::get(lcx,
      // Width())); sel2 = builder.CreateShl(sel0,
      // ConstantInt::get(IntegerType::get(lcx, Width()),
      // //BitvectorChild(1).Width())); return builder.CreateOr(sel1, sel2);
      ...
      for (unsigned i = 1; i < numArgs; ++i) {
        ...
        concatenatedResult = ConcatTwoBV(concatenatedResult, totalWidth, nextBV,
                                         nextWidth, builder, lcx);
    ...
    case Z3_OP_SIGN_EXT:
      assert(contents.num_args() == 1);
      return builder.CreateSExt(BitvectorChild(0).ToLLVM(),
                                IntegerType::get(lcx, Width()));
    case Z3_OP_ZERO_EXT:
      assert(contents.num_args() == 1);
      return builder.CreateZExt(BitvectorChild(0).ToLLVM(),
                                IntegerType::get(lcx, Width()));
    case Z3_OP_EXTRACT:
      assert(contents.num_args() == 1);
      ...
      return builder.CreateTrunc(
          builder.CreateLShr(BitvectorChild(0).ToLLVM(),
                             ConstantInt::get(oldTp, contents.lo())),
          newTp);
    case Z3_OP_REPEAT:
      assert(contents.num_args() == 1);
      {
        ...
        if (times == 1) {
          ...
          Value *zextChild =
              builder.CreateZExt(u, IntegerType::get(lcx, Width()));
          temp = zextChild;
          for (int i = 1; i < times; i++) {
            temp = builder.CreateOr(
                temp,
                builder.CreateShl(
                    zextChild, ConstantInt::get(IntegerType::get(lcx, Width()),
                                                i * oldWidth)));
          }
          return temp;
        }
    ...
    case Z3_OP_BSHL:
      ...
      return builder.CreateSelect(
          builder.CreateICmpUGE(BitvectorChild(1).ToLLVM(),
                                ConstantInt::get(newTp, Width())),
          ConstantInt::get(newTp, 0),
          builder.CreateShl(BitvectorChild(0).ToLLVM(),
                            BitvectorChild(1).ToLLVM()));
    case Z3_OP_BLSHR:
      assert(contents.num_args() == 2);
      newTp = IntegerType::get(lcx, BitvectorChild(0).Width());
    ...
    case Z3_OP_BASHR:
      assert(contents.num_args() == 2);
      newTp = IntegerType::get(lcx, BitvectorChild(0).Width());
      temp = builder.CreateSelect(BitvectorChild(0).IsNegative(), mone,
                                  ConstantInt::get(newTp, 0));
      return builder.CreateSelect(
          builder.CreateICmpUGE(BitvectorChild(1).ToLLVM(),
                                ConstantInt::get(newTp, Width())),
          temp,
          builder.CreateAShr(BitvectorChild(0).ToLLVM(),
                             BitvectorChild(1).ToLLVM()));
    case Z3_OP_ROTATE_LEFT:
      ...
      args.push_back(ConstantInt::get(
          ity, Z3_get_decl_int_parameter(contents.ctx(), contents.decl(), 0) %
                   Width()));
...
Type *FloatingNode::ToFloatingType(LLVMContext &lcx, std::string name,
                                   unsigned width) {
  switch (width) {
  case 16:
...
const std::map<Z3_decl_kind, int> FloatingNode::class_flags = {
    ...
    {Z3_OP_FPA_IS_INF,
     516}, // 1000000100 = negative infinity, positive infinity
    ...
    {Z3_OP_FPA_IS_SUBNORMAL,
     144}, // 0010010000 = negative subnormal, positive subnormal
    {Z3_OP_FPA_IS_NEGATIVE,
     60}, // 0000111100 = neg infinity, neg normal, neg subnormal, neg zero
    {Z3_OP_FPA_IS_POSITIVE,
     960} // 1111000000 = pos infinity, pos normal, pos subnormal, pos zero
};
...
Value *FloatingNode::LLVMClassCheck(Z3_decl_kind op) {
  ...
  args.push_back(
      ConstantInt::get(IntegerType::get(lcx, 32), class_flags.at(op)));
  // Get intrinsic function for class check
  Function *fun = Intrinsic::getDeclaration(
      lmodule, Function::lookupIntrinsicID("llvm.is.fpclass"), val->getType());
  return builder.CreateCall(fun, args);
}
...
Value *FloatingNode::LLVMNE(FloatingNode other) {
  ...
  Function *fpclassFun = Intrinsic::getDeclaration(
      lmodule, Function::lookupIntrinsicID("llvm.is.fpclass"), lhs->getType());
  Value *nanMask = ConstantInt::get(IntegerType::get(lcx, 32),
                                    class_flags.at(Z3_OP_FPA_IS_NAN));
  ...
  return builder.CreateAnd(builder.CreateNot(builder.CreateAnd(lhsNan, rhsNan)),
                           builder.CreateICmpNE(lb, rb));
}
...
Value *FloatingNode::LLVMEq(FloatingNode other) {
  ...
  return builder.CreateOr(builder.CreateAnd(lhsNan, rhsNan),
                          builder.CreateICmpEQ(lb, rb));
}

FloatingNode::FloatingNode(LLVMContext &t_lcx, Module *t_lmodule,
                           IRBuilder<> &t_builder, const LLMAPPING &t_variables,
                           SMTValueCache *t_value_cache, expr t_contents)
    : SMTNode(t_lcx, t_lmodule, t_builder, t_variables, t_value_cache,
              t_contents) {
  ...
  else // Expression case
  {
    ...
    case Z3_OP_FPA_TO_FP_UNSIGNED:
      ...
      if (IsRNE()) {
        ...
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::experimental_constrained_uitofp, types);
        return builder.CreateCall(fun, args);
      }
    case Z3_OP_FPA_TO_FP:
      if (contents.num_args() == 1) {
        assert(contents.num_args() == 1);
        return builder.CreateBitCast(BitvectorChild(0).ToLLVM(),
                                     FloatingType());
      } else if (contents.arg(1).is_bv()) {
        assert(contents.num_args() == 2); // has rounding mode

        if (IsRNE()) {
          return builder.CreateSIToFP(BitvectorChild(1).ToLLVM(),
                                      FloatingType());
        } else {
          args.push_back(BitvectorChild(1).ToLLVM());
          args.push_back(LLVMRoundingMode());
          ...
          fun = Intrinsic::getDeclaration(
              lmodule, Intrinsic::experimental_constrained_sitofp, types);
          return builder.CreateCall(fun, args);
        }
        ...
        if (IsRNE()) {
          if (Width() > FloatingChild(1).Width()) {
            return builder.CreateFPExt(FloatingChild(1).ToLLVM(),
                                       FloatingType());
          } else {
            return builder.CreateFPTrunc(FloatingChild(1).ToLLVM(),
                                         FloatingType());
          }
        } else {
          args.push_back(FloatingChild(1).ToLLVM());
          ...
          if (Width() > FloatingChild(1).Width()) {
            fun = Intrinsic::getDeclaration(
                lmodule, Intrinsic::experimental_constrained_fpext,
                FloatingType());
          } else {
            fun = Intrinsic::getDeclaration(
                lmodule, Intrinsic::experimental_constrained_fptrunc,
                FloatingType());
          }
          return builder.CreateCall(fun, args);
        }
      } else {
        throw UnsupportedSMTOpException(X_FP_CONVERT, contents);
      }
    case Z3_OP_FPA_FP:
      assert(contents.num_args() == 3);
      sign = builder.CreateShl(
          builder.CreateZExt(BitvectorChild(0).ToLLVM(),
                             IntegerType::get(lcx, Width())),
          ConstantInt::get(IntegerType::get(lcx, Width()), Width() - 1));
      exp = builder.CreateShl(
          builder.CreateZExt(BitvectorChild(1).ToLLVM(),
                             IntegerType::get(lcx, Width())),
          ConstantInt::get(IntegerType::get(lcx, Width()), SBits() - 1));
      sig = builder.CreateZExt(BitvectorChild(2).ToLLVM(),
                               IntegerType::get(lcx, Width()));

      return builder.CreateBitCast(
          builder.CreateOr(sign, builder.CreateOr(exp, sig)), FloatingType());
    case Z3_OP_FPA_ROUND_TO_INTEGRAL:
      ...
      switch (RoundingMode()) {
      case Z3_OP_FPA_RM_NEAREST_TIES_TO_EVEN:
        fun = Intrinsic::getDeclaration(lmodule, Intrinsic::roundeven,
                                        FloatingType());
        break;
      case Z3_OP_FPA_RM_NEAREST_TIES_TO_AWAY:
        fun = Intrinsic::getDeclaration(lmodule, Intrinsic::lround,
                                        FloatingType());
        break;
      case Z3_OP_FPA_RM_TOWARD_POSITIVE:
        fun =
            Intrinsic::getDeclaration(lmodule, Intrinsic::ceil, FloatingType());
        break;
      case Z3_OP_FPA_RM_TOWARD_NEGATIVE:
        fun = Intrinsic::getDeclaration(lmodule, Intrinsic::floor,
                                        FloatingType());
        break;
      case Z3_OP_FPA_RM_TOWARD_ZERO:
        fun = Intrinsic::getDeclaration(lmodule, Intrinsic::trunc,
                                        FloatingType());
        break;
      default:
        throw UnsupportedSMTOpException(X_RM_VAR, contents);
    ...
    case Z3_OP_FPA_ADD:
      ...
      if (IsRNE()) {
        return builder.CreateFAdd(FloatingChild(1).ToLLVM(),
                                  FloatingChild(2).ToLLVM());
      } else {
        args.push_back(FloatingChild(1).ToLLVM());
        ...
        args.push_back(LLVMException());
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::experimental_constrained_fadd, FloatingType());
        return builder.CreateCall(fun, args);
      }
    case Z3_OP_FPA_SUB:
      assert(contents.num_args() == 3); // has rounding mode
      if (IsRNE()) {
        return builder.CreateFSub(FloatingChild(1).ToLLVM(),
                                  FloatingChild(2).ToLLVM());
      } else {
        args.push_back(FloatingChild(1).ToLLVM());
        args.push_back(FloatingChild(2).ToLLVM());
        ...
        args.push_back(LLVMException());
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::experimental_constrained_fsub, FloatingType());
        return builder.CreateCall(fun, args);
      }
    case Z3_OP_FPA_MUL:
      assert(contents.num_args() == 3); // has rounding mode
      if (IsRNE()) {
        return builder.CreateFMul(FloatingChild(1).ToLLVM(),
                                  FloatingChild(2).ToLLVM());
      } else {
        args.push_back(FloatingChild(1).ToLLVM());
        args.push_back(FloatingChild(2).ToLLVM());
        ...
        args.push_back(LLVMException());
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::experimental_constrained_fmul, FloatingType());
        return builder.CreateCall(fun, args);
      }
    case Z3_OP_FPA_DIV:
      assert(contents.num_args() == 3); // has rounding mode
      if (IsRNE()) {
        return builder.CreateFDiv(FloatingChild(1).ToLLVM(),
                                  FloatingChild(2).ToLLVM());
      } else {
        args.push_back(FloatingChild(1).ToLLVM());
        args.push_back(FloatingChild(2).ToLLVM());
        ...
        args.push_back(LLVMException());
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::experimental_constrained_fdiv, FloatingType());
        return builder.CreateCall(fun, args);
      }
    case Z3_OP_FPA_REM:
      assert(contents.num_args() == 2);
      return builder.CreateFRem(FloatingChild(0).ToLLVM(),
                                FloatingChild(1).ToLLVM());
    case Z3_OP_FPA_FMA:
      ...
      if (IsRNE()) {
        fun =
            Intrinsic::getDeclaration(lmodule, Intrinsic::fma, FloatingType());
      } else {
        args.push_back(LLVMRoundingMode());
        args.push_back(LLVMException());
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::experimental_constrained_fma, FloatingType());
      }
      return builder.CreateCall(fun, args);
    case Z3_OP_FPA_SQRT:
      assert(contents.num_args() == 2); // has rounding mode
      ...
      if (IsRNE()) {
        fun =
            Intrinsic::getDeclaration(lmodule, Intrinsic::sqrt, FloatingType());
      } else {
        args.push_back(LLVMRoundingMode());
        args.push_back(LLVMException());
        fun = Intrinsic::getDeclaration(
            lmodule, Intrinsic::experimental_constrained_sqrt, FloatingType());
      }
      return builder.CreateCall(fun, args);
    case Z3_OP_FPA_MIN:
      assert(contents.num_args() == 2);
      ...
      fun =
          Intrinsic::getDeclaration(lmodule, Intrinsic::minnum, FloatingType());
      return builder.CreateCall(fun, args);
    case Z3_OP_FPA_MAX:
      assert(contents.num_args() == 2);
      args.push_back(FloatingChild(0).ToLLVM());
      ...
      fun =
          Intrinsic::getDeclaration(lmodule, Intrinsic::maxnum, FloatingType());
      return builder.CreateCall(fun, args);
    default:
