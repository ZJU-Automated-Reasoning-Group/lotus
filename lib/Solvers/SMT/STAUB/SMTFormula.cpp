...
namespace STAUB {
SMTFormula::SMTFormula(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
                       IRBuilder<> &t_builder, std::string t_string)
    : scx(t_scx), lcx(t_lcx), lmodule(t_lmodule), builder(t_builder),
      string(t_string), contents(t_scx) {
  // Regular expression matching to get variables
  std::string s = string;
  std::smatch m;
  std::regex e(
      R"(\((declare-fun\s(\|.*\||[\~\!\@\$\%\^\&\*_\-\+\=\<\>\.\?\/A-Za-z0-9]+)\s*\(\s*\)\s*(\(\s*_\s*FloatingPoint\s*(\d+)\s*(\d+)\s*\)|Float16|Float32|Float64|Float128|FPN|Real|Int|Bool|\(\s*_\s*BitVec\s*(\d+)\s*\))\s*|declare-const\s(\|.*\||[\~\!\@\$\%\^\&\*_\-\+\=\<\>\.\?\/A-Za-z0-9]+)\s*(\(\s*_\s*FloatingPoint\s*(\d+)\s*(\d+)\s*\)|Float16|Float32|Float64|Float128|FPN|Real|Int|Bool|\(\s*_\s*BitVec\s*(\d+)\s*\))\s*)\))");
  ...
  function = Function::Create(fnty, Function::ExternalLinkage,
                              LLVM_FUNCTION_NAME, lmodule);
  ...
  for (expr e : contents) {
    ...
    assertions.push_back(BooleanNode(scx, lcx, lmodule, builder,
                                     PLACEHOLDER_WIDTH, variables, e));
  }
  // exit(0);
...
void SMTFormula::ToSMT(unsigned width, solver *sol) {
  ...
  for (auto s : variables) {
    ...
    if (variables[s.first]->getType()->getIntegerBitWidth() == 1) {
      ...
      svariables.insert(
          make_pair(s.first, scx.bv_const(s.first.c_str(), width)));
    }
  }
...
void SMTFormula::ToSMTFloat(unsigned ebits, unsigned sbits, solver *sol) {
  ...
  for (auto s : variables) {
    ...
    if (variables[s.first]->getType()->getIntegerBitWidth() == 1) {
      ...
      svariables.insert(
          make_pair(s.first, scx.fpa_const(s.first.c_str(), ebits, sbits)));
    }
  }
...
bool SMTFormula::CheckAssignment(model m) {
  ...
  /*for (unsigned i = 0; i < newModel.size(); i++)
  {
      ...
      std::cout << v.to_string() << "   " <<
  newModel.get_const_interp(v).simplify().to_string() << "\n"; std::cout <<
  m[i].to_string() << "   " << m.get_const_interp(m[i]).simplify().to_string()
  << "\n";
  }*/
