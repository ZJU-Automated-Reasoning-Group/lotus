...
inline IntegerNode SMTNode::IntegerChild(expr cont) {
  return IntegerNode(scx, lcx, lmodule, builder, integer_width, variables,
                     cont);
}
inline IntegerNode SMTNode::IntegerChild(int index) {
  return IntegerNode(scx, lcx, lmodule, builder, integer_width, variables,
                     contents.arg(index));
}
inline FloatingNode SMTNode::FloatingChild(expr cont) {
  return FloatingNode(scx, lcx, lmodule, builder, integer_width, variables,
                      cont);
}
...
inline BitvectorNode SMTNode::BitvectorChild(expr cont) {
  return BitvectorNode(scx, lcx, lmodule, builder, integer_width, variables,
                       cont);
}
...
inline BooleanNode SMTNode::BooleanChild(expr cont) {
  return BooleanNode(scx, lcx, lmodule, builder, integer_width, variables,
                     cont);
}
...
inline RealNode SMTNode::RealChild(expr cont) {
  return RealNode(scx, lcx, lmodule, builder, integer_width, variables, cont);
}
inline RealNode SMTNode::RealChild(int index) {
  return RealNode(scx, lcx, lmodule, builder, integer_width, variables,
                  contents.arg(index));
}

inline func_decl SMTNode::Z3_bvnego(int width) {
  return function("bvnego", scx.bv_sort(width), scx.bool_sort());
}
inline func_decl SMTNode::Z3_bvsaddo(int width) {
  return function("bvsaddo", scx.bv_sort(width), scx.bv_sort(width),
                  scx.bool_sort());
}
inline func_decl SMTNode::Z3_bvssubo(int width) {
  return function("bvssubo", scx.bv_sort(width), scx.bv_sort(width),
                  scx.bool_sort());
}
inline func_decl SMTNode::Z3_bvsmulo(int width) {
  return function("bvsmulo", scx.bv_sort(width), scx.bv_sort(width),
                  scx.bool_sort());
}
inline func_decl SMTNode::Z3_bvsdivo(int width) {
  return function("bvsdivo", scx.bv_sort(width), scx.bv_sort(width),
                  scx.bool_sort());
}
...
SMTNode::SMTNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
                 IRBuilder<> &t_builder, unsigned t_integer_width,
                 const std::map<std::string, Value *> &t_variables,
                 expr t_contents)
    : scx(t_scx), lcx(t_lcx), lmodule(t_lmodule), builder(t_builder),
      integer_width(t_integer_width), variables(t_variables),
      contents(t_contents) {}
//============================RealNode=====================================
...
bool RealNode::IsComparison(expr expression) // static
{
  ...
  bool direct =
      (k == Z3_OP_LE || k == Z3_OP_LT || k == Z3_OP_GE || k == Z3_OP_GT);
  return expression.num_args() >= 1 && expression.arg(0).is_real() &&
         (expression.is_eq() || expression.is_distinct() || direct);
}

RealNode::RealNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
                   IRBuilder<> &t_builder, unsigned t_integer_width,
                   const std::map<std::string, Value *> &t_variables,
                   expr t_contents)
    : SMTNode(t_scx, t_lcx, t_lmodule, t_builder, t_integer_width, t_variables,
              t_contents) {
...
expr RealNode::ToSMT(unsigned width, std::map<std::string, expr> svariables,
                     solver *sol) {
...
PAIR RealNode::LargestPreciseConstant() {
  ...
  {
    return PairMax(RealChild(0).LargestPreciseConstant(),
                   RealChild(1).LargestPreciseConstant());
  }
}
...
PAIR RealNode::AbstractFloat(PAIR assumption) {
  ...
  } else // Expression
  {
    ...
    case Z3_OP_ITE:
      ...
      return PairMax(RealChild(1).AbstractFloat(assumption),
                     RealChild(2).AbstractFloat(assumption));
    case Z3_OP_UMINUS:
      // Only absolute value
    ...
    case Z3_OP_ADD:
      ...
      return PairPlus(RealChild(0).AbstractFloat(assumption),
                      RealChild(1).AbstractFloat(assumption));
    case Z3_OP_MUL:
      return PairMult(RealChild(0).AbstractFloat(assumption),
                      RealChild(1).AbstractFloat(assumption));
    case Z3_OP_DIV:
      // Can't add extra bits
      return PairDiv(RealChild(0).AbstractFloat(assumption),
                     RealChild(1).AbstractFloat(assumption));
    default:
...
expr RealNode::ToSMTFloat(z3::sort type,
  ...
  if (IsVariable()) {
    ...
    return expr(scx, Z3_mk_fpa_to_fp_real(scx, scx.fpa_rounding_mode(),
                                          scx.real_val(str.c_str()), type))
        .simplify();
  } else // Expression
  {
    ...
    switch (Op()) {
    case Z3_OP_ITE:
      return ite(BooleanChild(0).ToSMTFloat(type, svariables),
                 RealChild(1).ToSMTFloat(type, svariables),
                 RealChild(2).ToSMTFloat(type, svariables));
    case Z3_OP_UMINUS:
...
bool IntegerNode::IsComparison(expr expression) // static
{
  ...
  bool direct =
      (k == Z3_OP_LE || k == Z3_OP_LT || k == Z3_OP_GE || k == Z3_OP_GT);
  return expression.num_args() >= 1 && expression.arg(0).is_int() &&
         (expression.is_eq() || expression.is_distinct() || direct);
}

IntegerNode::IntegerNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
                         IRBuilder<> &t_builder, unsigned t_integer_width,
                         const std::map<std::string, Value *> &t_variables,
                         expr t_contents)
    : SMTNode(t_scx, t_lcx, t_lmodule, t_builder, t_integer_width, t_variables,
  ...
  } else // Expression
  {
    ...
    case Z3_OP_ITE:
      return builder.CreateSelect(BooleanChild(0).ToLLVM(),
                                  IntegerChild(1).ToLLVM(),
                                  IntegerChild(2).ToLLVM());
    case Z3_OP_UMINUS:
      return builder.CreateSub(
          ConstantInt::get(IntegerType::get(lcx, Width()), 0),
          IntegerChild(0).ToLLVM());
    case Z3_OP_SUB:
      return builder.CreateSub(IntegerChild(0).ToLLVM(),
                               IntegerChild(1).ToLLVM());
    case Z3_OP_ADD:
      return builder.CreateAdd(IntegerChild(0).ToLLVM(),
                               IntegerChild(1).ToLLVM());
    case Z3_OP_MUL:
      return builder.CreateMul(IntegerChild(0).ToLLVM(),
                               IntegerChild(1).ToLLVM());
    case Z3_OP_IDIV:
      return builder.CreateSDiv(IntegerChild(0).ToLLVM(),
                                IntegerChild(1).ToLLVM());
    case Z3_OP_MOD:
      return builder.CreateSRem(IntegerChild(0).ToLLVM(),
                                IntegerChild(1).ToLLVM());
    default:
      throw UnsupportedSMTOpException("integer operation", contents);
    }
...
APInt IntegerNode::LargestIntegerConstant() {
  ...
  {
    return APMax(IntegerChild(0).LargestIntegerConstant(),
                 IntegerChild(1).LargestIntegerConstant());
  }
}
APInt IntegerNode::AbstractSingle(APInt assumption) {
  ...
  } else // Expression
  {
    ...
    case Z3_OP_ITE:
      // Take whichever branch is bigger
      return APMax(IntegerChild(1).AbstractSingle(assumption),
                   IntegerChild(2).AbstractSingle(assumption));
    case Z3_OP_UMINUS:
    ...
    case Z3_OP_ADD:
      // Subtraction and addition can both grow (absolute value)
      return APPlus(IntegerChild(0).AbstractSingle(assumption),
                    IntegerChild(1).AbstractSingle(assumption));
    case Z3_OP_MUL:
      return APMult(IntegerChild(0).AbstractSingle(assumption),
                    IntegerChild(1).AbstractSingle(assumption));
    case Z3_OP_IDIV:
      // Can't add extra bits
      return APDiv(IntegerChild(0).AbstractSingle(assumption),
                   IntegerChild(1).AbstractSingle(assumption));
    case Z3_OP_MOD:
...
expr IntegerNode::ToSMT(unsigned width, std::map<std::string, expr> svariables,
  ...
  if (IsVariable()) {
    ...
    if (isNegative) {
      if (!forSlot) {
        // func_decl g = function("bvnego", scx.bv_sort(width),
        // scx.bool_sort());
        ...
        /*expr conjecture1 = implies(g(g(x) - g(y)) != g(z) && x + z <= y && y
        <= x, z < 0);
  ...
  } else // Expression
  {
    ...
    switch (Op()) {
    case Z3_OP_ITE:
      return ite(BooleanChild(0).ToSMT(width, svariables, sol),
                 IntegerChild(1).ToSMT(width, svariables, sol),
                 IntegerChild(2).ToSMT(width, svariables, sol));
    case Z3_OP_UMINUS:
    ...
    case Z3_OP_IDIV:
      ...
      // return left / right;
    case Z3_OP_MOD:
      return mod(IntegerChild(0).ToSMT(width, svariables, sol),
                 IntegerChild(1).ToSMT(width, svariables, sol));
    default:
      throw UnsupportedSMTOpException("integer operation", contents);
    }
...
BooleanNode::BooleanNode(context &t_scx, LLVMContext &t_lcx, Module *t_lmodule,
                         IRBuilder<> &t_builder, unsigned t_integer_width,
                         const std::map<std::string, Value *> &t_variables,
                         expr t_contents)
    : SMTNode(t_scx, t_lcx, t_lmodule, t_builder, t_integer_width, t_variables,
...
Value *BooleanNode::ToLLVM() {
  ...
  } else // Expression case
  {
    ...
    // Integer comparisons
    case Z3_OP_LE:
      return builder.CreateICmpSLE(IntegerChild(0).ToLLVM(),
                                   IntegerChild(1).ToLLVM());
    case Z3_OP_LT:
      return builder.CreateICmpSLT(IntegerChild(0).ToLLVM(),
                                   IntegerChild(1).ToLLVM());
    case Z3_OP_GE:
      return builder.CreateICmpSGE(IntegerChild(0).ToLLVM(),
                                   IntegerChild(1).ToLLVM());
    case Z3_OP_GT:
      return builder.CreateICmpSGT(IntegerChild(0).ToLLVM(),
                                   IntegerChild(1).ToLLVM());
    ...
    case Z3_OP_EQ:
      ...
      if (contents.arg(0).is_bool()) {
        ...
        return builder.CreateICmpEQ(IntegerChild(0).ToLLVM(),
                                    IntegerChild(1).ToLLVM());
      }
      // Floating point comparison
        ...
        UnsupportedSMTOpException("= comparison with unsupported child type",
                                  contents);
      }
    case Z3_OP_DISTINCT:
      if (contents.num_args() < 2) {
        throw UnsupportedSMTOpException(
            "distinct comparison with less than 2 children", contents);
        ...
        if (contents.arg(0).is_bool()) {
          ...
          return builder.CreateICmpNE(IntegerChild(0).ToLLVM(),
                                      IntegerChild(1).ToLLVM());
        }
        // Floating point comparison
        else if (contents.arg(0).is_fpa()) {
          ...
          UnsupportedSMTOpException(
              "distinct comparison with unsupported child type", contents);
        }
      } else {
...
APInt BooleanNode::LargestIntegerConstant() {
  ...
  // Expressions (TODO: assumes only integer constraints in children)
  ...
  } else if (Op() == Z3_OP_AND || Op() == Z3_OP_OR || Op() == Z3_OP_XOR ||
             Op() == Z3_OP_ITE ||
             (Op() == Z3_OP_DISTINCT && contents.arg(0).is_bool())) {
    ...
    return val;
             ...
             (Op() == Z3_OP_EQ && contents.arg(0).is_bool())) {
    return APMax(BooleanChild(0).LargestIntegerConstant(),
                 BooleanChild(1).LargestIntegerConstant());
  } else if (Op() == Z3_OP_LE || Op() == Z3_OP_LT || Op() == Z3_OP_GE ||
             Op() == Z3_OP_GT ||
             (Op() == Z3_OP_EQ && contents.arg(0).is_int())) {
    return APMax(IntegerChild(0).LargestIntegerConstant(),
                 IntegerChild(1).LargestIntegerConstant());
    ...
    throw UnsupportedSMTOpException(
        "largest constant computation with non-integer elements", contents);
  }
}
...
APInt BooleanNode::AbstractSingle(APInt assumption) {
  ...
  // Expressions (TODO: assumes only integer constraints in children)
    ...
    return val;
             ...
             (Op() == Z3_OP_EQ && contents.arg(0).is_bool())) {
    return APMax(BooleanChild(0).AbstractSingle(assumption),
                 BooleanChild(1).AbstractSingle(assumption));
  } else if (Op() == Z3_OP_LE || Op() == Z3_OP_LT || Op() == Z3_OP_GE ||
             Op() == Z3_OP_GT ||
             (Op() == Z3_OP_EQ && contents.arg(0).is_int())) {
    return APMax(IntegerChild(0).AbstractSingle(assumption),
                 IntegerChild(1).AbstractSingle(assumption));
  } else if (Op() == Z3_OP_DISTINCT && contents.arg(0).is_int()) {
...
expr BooleanNode::ToSMT(unsigned width, std::map<std::string, expr> svariables,
  ...
  } else // Expression case
  {
    ...
    case Z3_OP_XOR:
      ...
      return temp;
    case Z3_OP_IMPLIES:
      return implies(BooleanChild(0).ToSMT(width, svariables, sol),
                     BooleanChild(1).ToSMT(width, svariables, sol));
    case Z3_OP_ITE:
      return ite(BooleanChild(0).ToSMT(width, svariables, sol),
                 BooleanChild(1).ToSMT(width, svariables, sol),
                 BooleanChild(2).ToSMT(width, svariables, sol));
    // Integer comparisons
    case Z3_OP_LE:
      return IntegerChild(0).ToSMT(width, svariables, sol) <=
             IntegerChild(1).ToSMT(width, svariables, sol);
    case Z3_OP_LT:
      return IntegerChild(0).ToSMT(width, svariables, sol) <
             IntegerChild(1).ToSMT(width, svariables, sol);
    ...
    case Z3_OP_DISTINCT:
      if (contents.num_args() < 2) {
        ...
        for (unsigned int i = 0; i < contents.num_args(); i++) {
          assert((contents.arg(i).is_bool() || contents.arg(i).is_int()) &&
                 contents.arg(i).is_bool() == tt);
          if (tt) {
...
PAIR BooleanNode::LargestPreciseConstant() {
  ...
  // Expressions (TODO: assumes only integer constraints in children)
    ...
    return val;
             ...
             (Op() == Z3_OP_EQ && contents.arg(0).is_bool())) {
    return PairMax(BooleanChild(0).LargestPreciseConstant(),
                   BooleanChild(1).LargestPreciseConstant());
  } else if (Op() == Z3_OP_LE || Op() == Z3_OP_LT || Op() == Z3_OP_GE ||
             Op() == Z3_OP_GT ||
             (Op() == Z3_OP_EQ && contents.arg(0).is_real())) {
    return PairMax(RealChild(0).LargestPreciseConstant(),
                   RealChild(1).LargestPreciseConstant());
    ...
    throw UnsupportedSMTOpException(
        "largest precision compution with non-floating point elements",
        contents);
  }
}
...
PAIR BooleanNode::AbstractFloat(PAIR assumption) {
  ...
  // Expressions (TODO: assumes only integer constraints in children)
    ...
    return val;
             ...
             (Op() == Z3_OP_EQ && contents.arg(0).is_bool())) {
    return PairMax(BooleanChild(0).AbstractFloat(assumption),
                   BooleanChild(1).AbstractFloat(assumption));
  } else if (Op() == Z3_OP_LE || Op() == Z3_OP_LT || Op() == Z3_OP_GE ||
             Op() == Z3_OP_GT ||
             (Op() == Z3_OP_EQ && contents.arg(0).is_real())) {
    return PairMax(RealChild(0).AbstractFloat(assumption),
                   RealChild(1).AbstractFloat(assumption));
  } else if (Op() == Z3_OP_DISTINCT && contents.arg(0).is_real()) {
...
expr BooleanNode::ToSMTFloat(z3::sort type,
                             std::map<std::string, expr> svariables) {
  if (IsVariable()) {
    return svariables.at(contents.to_string());
  } else if (IsConstant()) {
  ...
  } else // Expression case
  {
    ...
    case Z3_OP_XOR:
      ...
      return temp;
    case Z3_OP_IMPLIES:
      return implies(BooleanChild(0).ToSMTFloat(type, svariables),
                     BooleanChild(1).ToSMTFloat(type, svariables));
    case Z3_OP_ITE:
      return ite(BooleanChild(0).ToSMTFloat(type, svariables),
                 BooleanChild(1).ToSMTFloat(type, svariables),
                 BooleanChild(2).ToSMTFloat(type, svariables));
    // Integer comparisons
    case Z3_OP_LE:
      return RealChild(0).ToSMTFloat(type, svariables) <=
             RealChild(1).ToSMTFloat(type, svariables);
    ...
    case Z3_OP_DISTINCT:
      if (contents.num_args() < 2) {
        ...
        for (int i = 0; i < contents.num_args(); i++) {
          assert((contents.arg(i).is_bool() || contents.arg(i).is_real()) &&
                 contents.arg(i).is_bool() == tt);
          if (tt) {
...
BitvectorNode::BitvectorNode(context &t_scx, LLVMContext &t_lcx,
                             Module *t_lmodule, IRBuilder<> &t_builder,
                             unsigned t_integer_width,
                             const std::map<std::string, Value *> &t_variables,
                             expr t_contents)
    : SMTNode(t_scx, t_lcx, t_lmodule, t_builder, t_integer_width, t_variables,
              t_contents) {
...
APInt BitvectorNode::LargestIntegerConstant() {
  throw UnsupportedSMTOpException(
      "largest constant computation on bitvector value", contents);
}
APInt BitvectorNode::AbstractSingle(APInt assumption) {
  throw UnsupportedSMTOpException("abstract interpretation on bitvector value",
                                  contents);
}
expr BitvectorNode::ToSMT(unsigned width,
                          std::map<std::string, expr> svariables, solver *sol) {
  throw UnsupportedSMTOpException("converstion to smt for bitvector value",
                                  contents);
}
...
Type *FloatingNode::ToFloatingType(LLVMContext &lcx, std::string name,
  ...
  default:
    throw UnsupportedSMTTypeException(
        "Floating point type with width " + std::to_string(width), name);
  }
}
...
Value *FloatingNode::LLVMNE(FloatingNode other) {
  ...
  return builder.CreateOr(builder.CreateNot(builder.CreateAnd(
                              LLVMClassCheck(Z3_OP_FPA_IS_NAN),
                              other.LLVMClassCheck(Z3_OP_FPA_IS_NAN))),
...
Value *FloatingNode::LLVMEq(FloatingNode other) {
  ...
  return builder.CreateOr(
      builder.CreateAnd(LLVMClassCheck(Z3_OP_FPA_IS_NAN),
                        other.LLVMClassCheck(Z3_OP_FPA_IS_NAN)),
      builder.CreateICmpEQ(lb, rb));
}

FloatingNode::FloatingNode(context &t_scx, LLVMContext &t_lcx,
                           Module *t_lmodule, IRBuilder<> &t_builder,
                           unsigned t_integer_width,
                           const std::map<std::string, Value *> &t_variables,
                           expr t_contents)
...
Value *FloatingNode::ToLLVM() {
  return ConstantFP::getNaN(
      FloatingNode::ToFloatingType(lcx, contents.to_string(), Width()));
}

APInt FloatingNode::LargestIntegerConstant() {
  throw UnsupportedSMTOpException(
      "largest constant computation on floating value", contents);
}
APInt FloatingNode::AbstractSingle(APInt assumption) {
  throw UnsupportedSMTOpException("abstract interpretation on floating value",
                                  contents);
}
expr FloatingNode::ToSMT(unsigned width, std::map<std::string, expr> svariables,
                         solver *sol) {
  throw UnsupportedSMTOpException("converstion to smt for floating value",
                                  contents);
}
