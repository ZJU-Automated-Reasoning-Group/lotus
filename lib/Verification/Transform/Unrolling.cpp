#include "Verification/Transform/Unrolling.h"
...
static cl::opt<unsigned> UnrollCount("lotus-loop-unroll-count",
...
static BasicBlock *createTerminatingBlock(Function *F, Instruction *I) {
  auto *M = F->getParent();
...
cloneLoopBody(Function *F, const std::vector<BasicBlock *> Blocks) {
  ...
  for (auto *Block : Blocks) {
    NewBlocks.push_back(CloneBasicBlock(Block, VMap, "", F));
...
char UnrollingPass::ID = 0;

bool UnrollingPass::runOnLoop(Loop *L, LPPassManager & /*LPM*/) {
...
static RegisterPass<lotus::verification::transform::UnrollingPass>
    X("lotus-loop-unroll", "Unroll loops in the program", false, false);
