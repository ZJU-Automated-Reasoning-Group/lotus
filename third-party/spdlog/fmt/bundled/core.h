...
public:
    ...
    FMT_CONSTEXPR basic_string_view(const std::basic_string<Char, Alloc>& s) FMT_NOEXCEPT : data_(s.data()),
                                                                                            size_(s.size()) {}
...
template <typename V>
struct can_output_to_llvm_ostream_impl {
    template <typename T = V, FMT_ENABLE_IF(!is_string<T>::value && !is_char<T>::value)>  // template parameters here to enable SFINAE
    static auto test(T&& t, llvm::raw_ostream& os) -> decltype(os << t, void(), std::true_type{});

    static auto test(...) -> std::false_type;
    using type = decltype(test(std::declval<V>(), std::declval<llvm::raw_ostream&>()));
};

template <typename V>
struct can_output_to_llvm_ostream : can_output_to_llvm_ostream_impl<V>::type {};

// template <typename V>
// constexpr bool can_output_to_llvm_ostream_v = can_output_to_llvm_ostream<V>::value;

// Formats an object of type T that has an overloaded ostream operator<<.
template <typename T, typename Char>
struct fallback_formatter<T, Char, enable_if_t<can_output_to_llvm_ostream<T>::value>>
    : formatter<basic_string_view<Char>, Char> {

    fallback_formatter() = default;
    ...
    auto format(const T& value, Context& ctx) -> decltype(ctx.out()) {
        ...
        os << value;
        basic_string_view<Char> str(os.str());
...
private:
    ...
    static void format_custom_arg(const void* arg, basic_parse_context<char_type>& parse_ctx, Context& ctx) {
        Formatter f;
...
struct arg_mapper {
    ...
    FMT_CONSTEXPR int map(const T*) {
        ...
        // forbids formatting of "[const] volatile char *" which is printed as
        // bool by iostreams.
...
public:
    ...
    format_arg get(int index) const {
        ...
        if (arg.type_ == internal::named_arg_type) arg = arg.value_.named_arg->template deserialize<Context>();
        return arg;
    }
