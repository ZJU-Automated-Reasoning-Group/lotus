...
#define BUILDER_SCOPED_TIMER_(name, active) BUILDER_SCOPED_TIMER_##active(name)
#define BUILDER_SCOPED_TIMER_0(name)
#define BUILDER_SCOPED_TIMER_1(name) crab::ScopedCrabStats __st__(name, false);
 ...
 *
 *    select(x s>= 0, select(y s>= 0, x u< y, true), select(y s>= 0, false, x u<
 *y)
 *
...
static void unsignedCmpIntoSignOnes(var_t &res, lin_exp_t &op0, lin_exp_t &op1,
  ...
  lin_cst_t falseC =
      !isNegated ? lin_cst_t::get_false() : lin_cst_t::get_true();
  ...
  if (cst_op0.is_tautology()) {   // if the first operand is >= 0
    ...
    if (cst_op1.is_tautology()) { // if the second operand is >= 0
      ...
      /**
       *   select(x s>= 0, select(y s>= 0, x u< y, true), select(y s>= 0, false,
       *x u< y)
       **/
...
static void cmpInstToCrabBool(CmpInst &I, crabLitFactory &lfac,
  ...
  case CmpInst::ICMP_ULT: {
    if (removeUnsignedCmp) {
      unsignedCmpIntoSignOnes(lhs, op0, op1, lfac, bb, false /*isNegated*/,
                              false /* isNonStrict*/);
    } else {
      CLAM_WARNING("Crab does not support unsigned constraints. Use "
                   "--crab-lower-unsigned-icmp option");
    }
    break;
  }
...
static std::optional<lin_cst_t> cmpInstToCrabInt(CmpInst &I, crabLitFactory &lfac,
...
static std::optional<var_t> unsignedCmpInstToCrabInt(CmpInst &I,
  ...
  default:
    CLAM_ERROR("non unsigned comparisons should be handled by "
               "unsignedCmpInstToCrabInt");
  }
  return std::nullopt;
}
          ...
          if (crab_lit_ref_t phi_val_lit = m_lfac.getLit(v)) {
            ...
            if (phi_val_lit->isBool()) {
              ...
              if (phi_val_lit->isVar()) {
                ...
                insertCrabIRWithEmitter::gep_ref(
                    *phi, m_propertyEmitters, m_bb, lhs,
                    m_lfac.mkRegionVar(rgn_phi_val), phi_val_lit->getVar(),
    ...
    for (; isa<PHINode>(curr); ++curr) {
      ...
      if (it != old_val_map.end()) {
        ...
        if (isBool(phi)) {
          ...
          insertCrabIRWithEmitter::gep_ref(
              *(const_cast<PHINode *>(&phi)), m_propertyEmitters, m_bb, lhs,
              m_lfac.mkRegionVar(rgn_phi), it->second,
        ...
        if (crab_lit_ref_t phi_val_lit = m_lfac.getLit(v)) {
          if (phi_val_lit->isBool()) {
            ...
            if (phi_val_lit->isVar()) {
              ...
              insertCrabIRWithEmitter::gep_ref(
                  *(const_cast<PHINode *>(&phi)), m_propertyEmitters, m_bb, lhs,
                  m_lfac.mkRegionVar(rgn_phi), phi_val_lit->getVar(),
...
public:
  VerifierCallOptimizer(const CrabBuilderParams &params, crabLitFactory &lfac,
                        uint32_t &dbg_id)
  ...
  bool skipTranslation(Instruction &I) {
    ...
    if (ZExtInst *ZEInst = dyn_cast<ZExtInst>(&I)) {
      ...
      if (ZEInst->getSrcTy()->isIntegerTy(1)) {
        ...
        if (AllUsesAreVerifierCalls(I, false /*goThroughIntegerCasts*/,
        ...
        if (ZEInst->hasOneUse()) {
          ...
          if (SelectInst *SI =
                  dyn_cast<SelectInst>(ZEInst->use_begin()->getUser())) {
            ...
            if (isBool(*selectCond)) {
              ...
              if (canSkipSelect) {
                if (AllUsesAreVerifierCalls(
                        *SI, false /*goThroughIntegerCasts*/,
                        false /*nonBoolCond*/, verifierCalls,
                        true /*onlyAssume*/)) {
                  m_skipped.insert(SI);
                  for (CallInst *veriCall : verifierCalls) {
                    m_pending_bool_vericall.insert({veriCall, I.getOperand(0)});
                    m_pending_bool_assume.insert(
                        {veriCall, {selectCond, negatedSelectCond}});
                  }
                  return true;
                }
              }
  ...
  bool doTranslation(const CallInst &I, const Function &callee,
                     basic_block_t &bb) {
...
class CrabIntraBlockBuilder : public InstVisitor<CrabIntraBlockBuilder> {
  ...
  unsigned getPointerSizeInBits() const { return m_dl->getPointerSizeInBits(); }
...
public:
  CrabIntraBlockBuilder(
      crabLitFactory &lfac, tag_manager &as_man, HeapAbstraction &mem,
      const DataLayout *dl, const TargetLibraryInfo *tli, basic_block_t &bb,
      basic_block_t &entry_bb, const CrabBuilderParams &params,
      ...
      VerifierCallOptimizer &vericall_opt, uint32_t &assertion_id,
      CrabIREmitterVec &propertyEmitters);
...
CrabIntraBlockBuilder::CrabIntraBlockBuilder(
    crabLitFactory &lfac, tag_manager &as_man, HeapAbstraction &mem,
    const DataLayout *dl, const TargetLibraryInfo *tli, basic_block_t &bb,
    basic_block_t &entry_bb /*unused*/, const CrabBuilderParams &params,
    ...
    : m_lfac(lfac), m_as_man(as_man), m_mem(mem), m_dl(dl), m_tli(tli),
      m_bb(bb), m_params(params), m_func_globals(func_globals),
      m_ret_insts(ret_insts), m_man(man), m_has_seahorn_fail(has_seahorn_fail),
...
// instrumentations.
bool skipAssertionIfUntypedOrCyclic(CallInst &I, Value *cond,
                                    HeapAbstraction &mem,
                                    RegionSet &func_regions,
                                    const CrabBuilderParams &params) {

  if (!params.check_only_typed_regions &&
      !params.check_only_noncyclic_regions) {
    return false;
  }
  ...
  Value *Ptr = nullptr;
  std::string assertKind =
      getAssertKindFromMetadata(I.getMetadata("clam-assertion"));
...
void CrabIntraBlockBuilder::doVerifierCall(CallInst &I) {
  ...
  auto translateToBoolVerifierCall = [this, &I](crab_lit_ref_t cond,
  ...
  auto translateToIntVerifierCall = [this, &I](crab_lit_ref_t cond,
                                               const Function *callee) {
    assert(cond->isVar());
    assert(cond->isInt());
    var_t v = cond->getVar();
  ...
  // -- free errors.
  if (isAssertFn(*callee) && skipAssertionIfUntypedOrCyclic(
                                 I, cond, m_mem, m_func_regions, m_params)) {
    return;
  }
  ...
  if (ConstantInt *CI = dyn_cast<ConstantInt>(cond)) {
    ...
    bool isTooLarge;
    z_number cond_val =
        getIntConstant(CI, m_params, true /*interpretAsSigned*/, isTooLarge);
    if (!isTooLarge) {
      if (cond_val > 0) {
...
void CrabIntraBlockBuilder::visitCmpInst(CmpInst &I) {
  ...
  if (isBool(v0) && isBool(v1)) {
    ...
    if (AllUsesAreBrOrIntSelectCondInst(I, m_params, [](SelectInst *I) {
    ...
    if (isa<PtrToIntInst>(I)) {
      ...
      if (src->isRef()) {
        if (m_lfac.isRefNull(src)) {
          ...
          insertCrabIRWithEmitter::gep_ref(
              I, m_propertyEmitters, m_bb, dst->getVar(),
              m_lfac.mkRegionVar(rgn_dst), src->getVar(),
              m_lfac.mkRegionVar(rgn_src));
          return;
        }
      }
      CLAM_WARNING("translation skipped " << I << " at line " << __LINE__);
...
void CrabIntraBlockBuilder::visitSelectInst(SelectInst &I) {
  ...
  if (isReference(I, m_params)) {
    ...
    if (ConstantInt *ci = dyn_cast<ConstantInt>(&condV)) {
      if (ci->isOne()) {
        if (op1->isVar()) {
          insertCrabIRWithEmitter::gep_ref(I, m_propertyEmitters, m_bb,
                                           lhs->getVar(), lhs_rgn,
                                           op1->getVar(), op1_rgn);
        } else {
...
void CrabIntraBlockBuilder::doAllocFn(CallInst &I) {
  ...
  auto addMakeRefFromMalloc = [this, &I](crab_lit_ref_t retRef, Region rgn,
                                         const Value &size) {
    crab_lit_ref_t litS = m_lfac.getLit(size);
    if (litS->isInt()) {
      var_or_cst_t size =
          (litS->isVar() ? var_or_cst_t(litS->getVar())
                         : var_or_cst_t(m_lfac.getIntCst(litS),
                                        crab::variable_type(
                                            INT_TYPE, getPointerSizeInBits())));
      insertCrabIRWithEmitter::make_ref(
          I, m_propertyEmitters, m_bb, *m_tli, retRef->getVar(),
          m_lfac.mkRegionVar(rgn), size, m_as_man.mk_tag());
    } else {
      CLAM_ERROR("doAllocFn expected " << size << " to be an integer in " << I);
    }
  };

  auto addMakeRefFromCalloc = [this, &I](crab_lit_ref_t retRef, Region rgn,
                                         const Value &nElts,
                                         const Value &szElt) {
    ...
    if (lit_nElts->isInt() && lit_szElt->isInt()) {
      if (!lit_nElts->isVar() && !lit_szElt->isVar()) {
        var_or_cst_t size(
            m_lfac.getIntCst(lit_nElts) * m_lfac.getIntCst(lit_szElt),
      ...
      CLAM_ERROR("doAllocFn expected " << nElts << " and " << szElt
                                       << " to be integers in " << I);
    }
  };
  ...
  // 		    retRef->getVar(), m_lfac.mkRegionVar(rgn), size,
  // m_as_man.mk_tag());
  // };
  ...
  if (!I.getType()->isVoidTy()) {
    ...
    if (isReference(I, m_params)) {
      ...
      if (isMallocLikeFn(&I, m_tli)) {
        ...
        addMakeRefFromCalloc(retRef, rgn, *(I.getOperand(0)),
                             *(I.getOperand(1)));
      } else if (isReallocLikeFn(&I, m_tli)) {
        // ptr' := realloc(ptr, new_size)
        // TODO(TRANSLATION): we ignore that the contents of the new allocated
        // memory
        addMakeRefFromMalloc(retRef, rgn, *(I.getOperand(1)));
      } /*else if (isStrdupLikeFn(&I, m_tli)) {
        // After LLVM14, MemoryBuiltins doesn't tell you if a call is strdup
        like
        // str' := strdup(str,...)
      ...
      // -- havoc return value
      havoc(retRef->getVar(), valueToStr(I), m_bb,
            m_params.include_useless_havoc);
    }
  }
}
...
void CrabIntraBlockBuilder::visitAllocaInst(AllocaInst &I) {
  ...
  // note that it has side-effects (it might add a multiplication in the basic
  // block)
  ...
  if (isReference(I, m_params)) {
    ...
    insertCrabIRWithEmitter::make_ref(I, m_propertyEmitters, m_bb, *m_tli,
                                      lhs->getVar(), m_lfac.mkRegionVar(rgn),
                                      size, m_as_man.mk_tag());
...
void CrabIntraBlockBuilder::doFreeFn(CallInst &I) {
  ...
  if (m_params.trackMemory()) {
    ...
    if (Ptr->isVar() && Ptr->isRef()) {
      ...
      insertCrabIRWithEmitter::remove_ref(I, m_propertyEmitters, m_bb, *m_tli,
                                          m_lfac.mkRegionVar(RgnPtr),
                                          Ptr->getVar());
    }
  }
}
  ...
  if (m_params.trackMemory()) {
    ...
    auto translateLength = [this, &I](Value &len) -> var_or_cst_t {
      ...
      if (lit->isInt()) {
        ...
        CLAM_ERROR("doMemInstrinsics expected "
                   << len << " to be an integer in " << I);
      }
    };
    auto translatePtr = [this, &I](Value &ptr) -> std::pair<var_t, var_t> {
      ...
      var_t rgnVar = m_lfac.mkRegionVar(
    ...
    if (MemSetInst *MSI = dyn_cast<MemSetInst>(&I)) {
      ...
      var_or_cst_t lenV = translateLength(*len);
      CrabMemTransferOps s(psrcV.first, psrcV.second, pdestV.first,
                           pdestV.second, lenV, m_bb);
      for (unsigned i = 0, sz = m_propertyEmitters.size(); i < sz; ++i) {
...
void CrabIntraBlockBuilder::doGep(GetElementPtrInst &I, var_t lhs,
                                  std::optional<var_t> base) {
  ...
  if (I.accumulateConstantOffset(*m_dl, offset)) {
    bool isTooBig = false;
    z_number o(
        toZNumber(offset, m_params, true /*interpretAsSigned*/, isTooBig));
    if (isTooBig) {
      ...
      if (lhs.get_type().is_reference()) {
        ...
        insertCrabIRWithEmitter::gep_ref(I, m_propertyEmitters, m_bb, lhs,
                                         crab_rgn, base.value(),
                                         crab_base_rgn, lin_exp_t(o));
        ...
        if (base.has_value()) {
          ...
          CRAB_LOG("cfg-gep", crab::outs() << lhs << ":" << lhs.get_type()
                                           << ":=" << base.value() << "+"
                                           << o << "\n");
        } else {
    ...
    if (const StructType *st = GTI.getStructTypeOrNull()) {
      if (const ConstantInt *ci =
        ...
        if (lhs.get_type().is_reference()) {
          ...
          var_t crab_base_rgn = m_lfac.mkRegionVar(base_rgn);
          insertCrabIRWithEmitter::gep_ref(
              I, m_propertyEmitters, m_bb, lhs, crab_rgn,
              (!already_assigned) ? base.value() : lhs, crab_base_rgn,
              offset);
          CRAB_LOG(
              "cfg-gep",
              if (!already_assigned) {
      ...
      if (Iidx->isVar()) {
        ...
        if (w < getPointerSizeInBits()) {
          ...
          CRAB_LOG("cfg-gep", crab::outs() << "sext(" << Iidx->getVar() << ","
                                           << sext_idx << ")\n";);
      ...
      if (lhs.get_type().is_reference()) {
        ...
        insertCrabIRWithEmitter::gep_ref(
            I, m_propertyEmitters, m_bb, lhs, crab_rgn,
            (!already_assigned) ? base.value() : lhs, crab_base_rgn, offset);
        CRAB_LOG(
            "cfg-gep",
            if (!already_assigned) {
...
void CrabIntraBlockBuilder::visitGetElementPtrInst(GetElementPtrInst &I) {
  ...
  if (m_params.trackOnlySingletonMemory() &&
      evalOffset(I, I.getContext()).has_value()) {
  ...
  if (m_params.trackMemory()) {
    ...
    if (isSingletonMemory) {
      std::optional<var_t> baseAddress =
          std::nullopt; /* i.e., zero base address*/
...
void CrabIntraBlockBuilder::visitStoreInst(StoreInst &I) {
  ...
  if (m_params.trackOnlySingletonMemory()) {
    ...
    if (rgn.isUnknown()) {
      ...
      const statement_t *crab_stmt = nullptr;
      if (val->isVar()) {
        crab_stmt = insertCrabIRWithEmitter::store_to_ref(
            I, m_propertyEmitters, m_bb, ptr->getVar(), m_lfac.mkRegionVar(rgn),
            val->getVar());
      } else {
        crab_stmt = insertCrabIRWithEmitter::store_to_ref(
            I, m_propertyEmitters, m_bb, ptr->getVar(), m_lfac.mkRegionVar(rgn),
      ...
      if (val->isVar()) {
        ...
        if (rgn.getRegionInfo().getType().second != val_bitwidth) {
          ...
          ext_or_trunc_val =
              m_lfac.mkIntVar(rgn.getRegionInfo().getType().second);
        }
        ...
        auto crab_stmt = insertCrabIRWithEmitter::store_to_ref(
            I, m_propertyEmitters, m_bb, ptr->getVar(), m_lfac.mkRegionVar(rgn),
            ext_or_trunc_val);
        insertRevMap(crab_stmt, I);
...
void CrabIntraBlockBuilder::visitLoadInst(LoadInst &I) {
  ...
  if (m_params.trackOnlySingletonMemory()) {
    ...
    if (rgn.isUnknown()) {
      /* untyped region */
      auto crab_stmt = insertCrabIRWithEmitter::load_from_ref(
          I, m_propertyEmitters, m_bb, lhs->getVar(), ptr->getVar(),
      ...
      auto crab_stmt = insertCrabIRWithEmitter::load_from_ref(
          I, m_propertyEmitters, m_bb, lhs_v, ptr->getVar(),
          m_lfac.mkRegionVar(rgn));
      insertRevMap(crab_stmt, I);
...
void CrabIntraBlockBuilder::doArithmeticWithOverflowIntrinsic(
    WithOverflowInst &I, var_t res) {
...
void CrabIntraBlockBuilder::visitExtractValueInst(ExtractValueInst &I) {
  ...
  if (m_params.lower_arithmetic_with_overflow_intrinsics) {
    if (WithOverflowInst *II =
            dyn_cast<WithOverflowInst>(I.getAggregateOperand())) {
...
HeapAbstraction &CfgBuilderImpl::getHeapAbstraction() { return m_mem; }
...
std::optional<var_t> CfgBuilderImpl::getCrabVariable(const llvm::Value &v) {
  ...
  if (lit == nullptr || !lit->isVar()) {
    // getLit only supports integers and pointers. For instance, if v is the lhs
    // of this instruction:
...
std::optional<varset_t> CfgBuilderImpl::getLiveSymbols(const BasicBlock *B) const {
  if (!m_ls) {
    return std::nullopt;
  } else {
...
std::optional<var_t>
CfgBuilderImpl::getCrabRegionVariable(const Function &f, const llvm::Value &v) {
    ...
    if (m_params.allocateGlobals()) {
      ...
      if (getSingletonValue(rgn, m_params.lower_singleton_aliases)) {
        ...
        if (IsCString(gv)) {
          ...
          if (tSize.isScalable()) {
            var_t unknownSizeVar =
                m_lfac.mkIntVar(m_dl->getPointerSizeInBits());
            ...
            // Revisit: we do not call insertCrabIRWithEmitter
            entry.make_ref(
                gv_lit->getVar(), m_lfac.mkRegionVar(rgn),
    ...
    if (m_params.allocateGlobals()) {
      for (auto &F : M) {
        if (F.hasAddressTaken()) {
          ...
          // Revisit: we do not call insertCrabIRWithEmitter
          entry.make_ref(
              funptr->getVar(), m_lfac.mkRegionVar(rgn),
              var_or_cst_t(
                  m_dl->getPointerSizeInBits() / 8,
                  crab::variable_type(INT_TYPE, m_dl->getPointerSizeInBits())),
              m_as_man.mk_tag());
...
void CfgBuilderImpl::initializeRegions() {
  ...
  if (m_cfg->has_func_decl()) {
    ...
    // and return.
    std::set_difference(outputs.begin(), outputs.end(), inputs.begin(),
                        inputs.end(),
    ...
    for (auto it = (*bit).begin(), et = (*bit).end(); it != et; ++it) {
      if ((*it).is_callsite()) {
        ...
        std::copy_if(s->get_lhs().begin(), s->get_lhs().end(),
                     ...
                     [](const var_t &v) { return v.get_type().is_region(); });
        std::set_difference(
            outputs.begin(), outputs.end(), inputs.begin(), inputs.end(),
        ...
        std::set<var_t> inputs, outputs;
        for (const typename basic_block_t::variable_or_constant_t &v :
             s->get_args()) {
  ...
  CRAB_LOG("cfg-mem", llvm::errs() << "Region variables initialized by "
                                   << m_func.getName() << "{";);
  for (auto it = mayInitVars.rbegin(), et = mayInitVars.rend(); it != et;
       ++it) {
      ...
      if (const ConstantInt *ci = dyn_cast<const ConstantInt>(&c)) {
        ...
        if (CmpInst *CI = dyn_cast<CmpInst>(&const_cast<Value &>(c))) {
          ...
          if (!lower_cond_as_bool) {
            ...
            lower_cond_as_bool = !AllUsesAreBrOrIntSelectCondInst(
                *CI, m_params,
                [](SelectInst *I) { return !AnyUseIsVerifierCall(*I); });
          }
        } else {
...
void CfgBuilderImpl::buildCfg() {
  ...
  CRAB_VERBOSE_IF(1, crab::get_msg_stream() << "Starting CFG construction for "
                                            << m_func.getName().str() << "\n";);
  ...
  for (const BasicBlock *BB : bbs) {
    ...
    CrabIntraBlockBuilder v(m_lfac, m_as_man, m_mem, m_dl, tli, *bb, *entry_bb,
                            m_params, m_globals, m_ret_insts, m_man,
                            has_seahorn_fail, m_func_regions, m_rev_map,
                            regions_with_store, gep_map, vericall_opt, m_dbg_id,
                            m_propertyEmitters);
    ...
    for (const BasicBlock *dst : succs_vector) {
      ...
      std::unique_ptr<basic_block_t> tmp_mid_bb =
          (mid_bb ? makeTempBlock() : nullptr);
  ...
  CRAB_VERBOSE_IF(1, crab::get_msg_stream() << "Finished CFG construction for "
                                            << m_func.getName().str() << "\n";);
...
void CfgBuilderImpl::addFunctionDeclaration() {
  ...
  //                          }
  //
  auto translateInputValueAsScalar = [this, &inputs](const Value &inputVal,
                                                     basic_block_t &bb) {
    ...
    if (inputVar.get_type().is_bool()) {
      ...
      Region inputRgn =
          getRegion(m_mem, m_func_regions, m_params, m_func, inputVal);
      if (!getSingletonValue(inputRgn, m_params.lower_singleton_aliases)) {
        var_t inputPrime = m_lfac.mkRefVar();
        // Revisit: we do not call insertCrabIRWithEmitter
        bb.gep_ref(inputVar, m_lfac.mkRegionVar(inputRgn), inputPrime,
                   m_lfac.mkRegionVar(inputRgn));
        inputs.push_back(inputPrime);
      }
    } else if (inputVar.get_type().is_array()) {
      CLAM_ERROR(
          "translateScalarInputVar should not be called on array variable");
    } else if (inputVar.get_type().is_region()) {
      CLAM_ERROR(
          "translateScalarInputVar should not be called on region variable");
    } else {
      CLAM_ERROR("translateScalarInputVar called on unexpected variable");
    }
  };
  auto translateInputRegionAsScalar = [this, &inputs](const Region &inputRgn,
                                                      basic_block_t &bb) {
    ...
    if (inputVar.get_type().is_bool()) {
      ...
      CLAM_ERROR("translateInputRegionAsScalar supported only for boolean or "
                 "integer scalars");
    }
  };
  auto translateInputRegionAsArray = [this, &inputs](const Region &inputRgn,
                                                     basic_block_t &bb) {
    var_t inputPrime = m_lfac.mkArrayVar(inputRgn.getRegionInfo());
    bb.array_assign(m_lfac.mkArrayVar(inputRgn), inputPrime);
    inputs.push_back(inputPrime);
  };
  auto translateInputRegionAsRegion = [this, &inputs](const Region &inputRgn,
                                                      basic_block_t &bb) {
      ...
      if (it != m_globals.end()) {
        for (auto gv : it->second) {
          ...
          if (m_params.lower_singleton_aliases) {
            Region gvRgn =
                getRegion(m_mem, m_func_regions, m_params, m_func, *gv);
...
void CrabBuilderParams::write(raw_ostream &o) const {
  ...
  o << "\tavoid some boolean CrabIR statements (e.g., bool_assume, "
       "bool_assert): "
    << avoid_boolean << "\n";
  ...
  o << "\tadd CrabIR for checking buffer bounds errors: " << add_bounds_checks
    << "\n";
  o << "\tadd CrabIR for checking sea_is_dereferenceable intrinsics: "
    << add_is_deref << "\n";
}
...
std::optional<var_t> CfgBuilder::getCrabRegionVariable(const llvm::Function &f,
                                                        const llvm::Value &v) {
...
void CfgBuilder::computeLiveSymbols() { m_impl->computeLiveSymbols(); }

const liveness_t *CfgBuilder::getLiveSymbols() const {
  return m_impl->getLiveSymbols();
}

std::optional<varset_t> CfgBuilder::getLiveSymbols(const BasicBlock *B) const {
...
private:
  ...
  llvm::DenseMap<const llvm::Function *, std::unique_ptr<CfgBuilder>>
      m_cfg_builder_map;
  bool m_initialization_done = false;
...
CrabBuilderManagerImpl::CrabBuilderManagerImpl(
    ...
    std::unique_ptr<HeapAbstraction> mem, CrabIREmitterVec &&propertyEmitters)
    : m_params(params), m_tli(tli), m_lfac(m_vfac, m_params),
  ...
  if (params.add_null_checks) {
    m_property_emitters.emplace_back(
        std::make_unique<EmitNullDerefChecks>(m_params, m_lfac, m_dbg_id));
  }
  if (params.add_uaf_checks) {
    m_property_emitters.emplace_back(
        std::make_unique<EmitUafChecks>(m_params, m_lfac, m_dbg_id));
  }
...
void CrabIntraBlockBuilder::doCallInst(CallInst &I) {
  ...
  if (true /*crab::CrabSanityCheckFlag*/) {
    ...
    auto hasCompatibleTypes = [](const typename var_t::type_t &t1,
      ...
      return ((t1 == t2) ||
              (t1.is_unknown_region() &&
               (t2.is_region() && !t2.is_unknown_region())) ||
              (t2.is_unknown_region() &&
               (t1.is_region() && !t1.is_unknown_region())));
    };
    ...
    if (calleeF_decl) {
      ...
      if (calleeF_decl->get_outputs().size() != outputs.size()) {
        ...
        CLAM_ERROR(
            "Mismatch of number of outputs between callsite and function "
            "declaration");
      }

      for (unsigned i = 0, num_args = inputs.size(); i < num_args; ++i) {
        if (!hasCompatibleTypes(calleeF_decl->get_input_type(i),
                                inputs[i].get_type())) {
      ...
      for (unsigned i = 0, num_args = outputs.size(); i < num_args; ++i) {
        if (!hasCompatibleTypes(calleeF_decl->get_output_type(i),
                                outputs[i].get_type())) {
          llvm::errs() << "Callsite:" << I << "\n";
          llvm::errs() << "Caller  :" << I.getParent()->getParent()->getName()
                       << "\n";
    ...
    auto unifyRgnType =
        [this](var_t &cparam, const var_t &fparam,
          ...
          if (cparam.get_type() == fparam.get_type()) {
            ...
            cparam =
                std::get<1>(pendingRgnCasts.back()); // reference to aux_rgn
          } else {
            CLAM_ERROR(
  ...
  if (m_params.addPointerAssumptions()) {
    ...
    if (I.getType()->isPointerTy() && calleeF->getParamDereferenceableBytes(0) >
    0) { crab_lit_ref_t ret = m_lfac.getLit(I); error_if_null(ret, I);
...
bool CrabIntraBlockBuilder::isSpecialCrabIntrinsic(
    const Function &calleeF) const {
  if (!isCrabIntrinsic(calleeF)) {
    return false;
  }
  std::string name = getCrabIntrinsicName(calleeF);
...
void CrabIntraBlockBuilder::doCrabSpecialIntrinsic(CallInst &I) {
  ...
  if (name == IS_UNFREED_OR_NULL || name == IS_DEREFERENCEABLE) {
    ...
    if (is_unfreed_or_null) {
      if (CB.arg_size() != 1) {
        CLAM_ERROR("unexpected number of parameters in special intrinsic "
                   << I);
      }
    } else {
      if (CB.arg_size() != 2) {
    ...
    if (!I.getType()->isIntegerTy()) {
      CLAM_ERROR("unexpected non-integer output parameter in special intrinsic "
                 << I);
    }
    if (!Ptr->getType()->isPointerTy()) {
      CLAM_ERROR("unexpected non-pointer input parameter in special intrinsic "
                 << I);
    }
    ...
    if (getSingletonValue(rgn, m_params.lower_singleton_aliases)) {
      ...
      if (!is_unfreed_or_null) {
        ...
        if (!szBytesLit->isInt()) {
          CLAM_ERROR(
              "unexpected non-integer input parameter in special intrinsic "
              << I);
        }
        var_or_cst_t szBytes =
            (szBytesLit->isVar()
                 ? var_or_cst_t(szBytesLit->getVar())
                 : var_or_cst_t(
                       m_lfac.getIntCst(szBytesLit),
                       crab::variable_type(INT_TYPE, getPointerSizeInBits())));
      ...
      if (is_unfreed_or_null) {
        ...
        CrabIsDerefOps s(outputs[0], inputs[0].get_variable(),
                         inputs[1].get_variable(), inputs[2], m_bb);
        for (unsigned i = 0, sz = m_propertyEmitters.size(); i < sz; ++i) {
...
CfgBuilderImpl::CfgBuilderImpl(const Function &func,
    ...
    : m_is_cfg_built(false),
      ...
      m_func(const_cast<Function &>(func)), m_lfac(man.getCrabLitFactory()),
      m_as_man(man.getAllocSiteMan()), m_dbg_id(man.m_dbg_id),
      m_mem(man.getHeapAbstraction()), m_cfg(nullptr), m_id(0),
      m_ret_insts(nullptr), m_dl(&(func.getParent()->getDataLayout())),
      m_tli(&(man.getTLIWrapper())), m_params(man.getCfgBuilderParams()),
      m_globals(man.m_globals), m_man(man),
      m_propertyEmitters(m_man.getPropertyEmitters()) {
  m_cfg =
...
CrabBuilderManager::CrabBuilderManager(CrabBuilderParams params,
                                       llvm::TargetLibraryInfoWrapperPass &tli,
                                       std::unique_ptr<HeapAbstraction> mem)
    : m_impl(std::make_unique<CrabBuilderManagerImpl>(
          params, tli, std::move(mem), CrabIREmitterVec())) {}
...
CrabBuilderManager::CrabBuilderManager(CrabBuilderParams params,
                                       ...
                                       CrabIREmitterVec &&propEmitters)
    : m_impl(std::make_unique<CrabBuilderManagerImpl>(
          params, tli, std::move(mem), std::move(propEmitters))) {}
