...
static void getLoopExitBlocks(Loop *L,
...
llvm::cl::opt<bool> UnfoldAllLoops(
    "unfold-all-loops",
...
void UnfoldLoopForDsa::RemapInstruction(Instruction *I,
...
bool UnfoldLoopForDsa::isSafeToUnfold(Loop *L) {
  ...
  if (!BI || BI->isUnconditional()) {
    LOG("unfold-loop-for-dsa", errs() << "  Can't unroll; loop not terminated "
...
bool UnfoldLoopForDsa::OneUnfoldLoop(Loop *L) {
  ...
  if (!UnfoldedHeader || !UnfoldedLatchBlock) {
    ...
    //      properly above.
    LOG("unfold-loop-for-dsa", errs() << "  Won't unroll loop: header not "
                                         "found during reverse postorder.\n"
                                      << *L);
    return false;
  }
  ...
  BranchInst *UnfoldedBI =
      dyn_cast<BranchInst>(UnfoldedLatchBlock->getTerminator());
...
namespace seahorn {
Pass *createUnfoldLoopForDsaPass() { return new UnfoldLoopForDsa(); }

} // end namespace seahorn

static llvm::RegisterPass<UnfoldLoopForDsa>
    X("unfold-loop-dsa", "Unfold a loop iteration if useful for DSA", false,
      false);
