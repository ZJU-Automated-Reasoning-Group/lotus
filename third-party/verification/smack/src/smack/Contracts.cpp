...
Slice *ContractsExtractor::extractSlice(Value *V) {
  ...
  if (Decl *D = (Decl *)S->getBoogieDecl(naming, rep))
    rep->getProgram()->getDeclarations().push_back(D);
  S->remove();
  return S;
}
...
void ContractsExtractor::visitCallInst(CallInst &ci) {
  ...
  if (name == "forall") {
    ...
    proc->getRequires().push_back(extractSlice(V)->getBoogieExpression(naming, rep));
    ci.eraseFromParent();
    ...
    Value *V = ci.getArgOperand(0);
    ci.setArgOperand(0, ConstantInt::getTrue(ci.getContext()));
    proc->getEnsures().push_back(extractSlice(V)->getBoogieExpression(naming, rep));
    ci.eraseFromParent();
