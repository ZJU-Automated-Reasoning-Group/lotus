...
std::string IntegerOverflowChecker::getMax(unsigned bits, bool isSigned) {
  if (isSigned)
    return toString(APInt::getSignedMaxValue(bits), 10, true);
  else
    return toString(APInt::getMaxValue(bits), 10, false);
}

std::string IntegerOverflowChecker::getMin(unsigned bits, bool isSigned) {
  if (isSigned)
    return toString(APInt::getSignedMinValue(bits), 10, true);
  else
    return toString(APInt::getMinValue(bits), 10, false);
}
