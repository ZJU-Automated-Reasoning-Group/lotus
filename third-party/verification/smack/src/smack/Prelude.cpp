...
const Expr *Prelude::mapSelExpr(unsigned idx) {
  const auto *ptrVar = makePtrVarExpr(0);
  const auto *idxExpr = idx ? Expr::fn(indexedName("$add", {Naming::PTR_TYPE}), ptrVar,
...
const Expr *Prelude::mapUpdExpr(unsigned idx, const Expr *val,
                                const Expr *map) {
  const auto *ptrVar = makePtrVarExpr(0);
  const auto *mapExpr = map ? map : makeMapVarExpr(0);
  const auto *idxExpr = idx ? Expr::fn(indexedName("$add", {Naming::PTR_TYPE}), ptrVar,
                                rep.pointerLit((unsigned long long)idx))
                     : ptrVar;
...
FuncDecl *Prelude::safeStore(Binding elemBinding) {
  ...
  const auto *elemExpr = Expr::id(elemBinding.first);
  return inlinedOp(
      "$store", {elemType},
...
struct IntOpGen::IntArithOp : public IntOp {
  ...
  static const Expr *tosExpr(unsigned size) {
    const auto *i = makeIntVarExpr(0);
    const auto *limitMinusOne = Expr::lit(getIntLimit(size - 1), 0);
    const auto *c = Expr::and_(
  ...
  static const Expr *touExpr(unsigned size) {
    const auto *i = makeIntVarExpr(0);
    const auto *limit = Expr::lit(getIntLimit(size), 0);
    const auto *c = Expr::and_(new BinExpr(BinExpr::Gte, i, Expr::lit(0ULL)),
...
void IntOpGen::generateArithOps(std::stringstream &s) const {
  ...
  auto *const bvBuiltinOp = new BuiltinOp<IntOp::attrT>(IntOp::bvAttrFunc);
  auto *const intBuiltinOp = new BuiltinOp<IntOp::attrT>(IntOp::intAttrFunc);
  auto *const uninterpretedOp = new UninterpretedOp();
...
void IntOpGen::generatePreds(std::stringstream &s) const {
  ...
  auto *const bvBuiltinOp = new BuiltinOp<IntOp::attrT>(IntOp::bvAttrFunc);
  auto *const uleInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Lte, false>);
  auto *const ultInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Lt, false>);
  auto *const ugeInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Gte, false>);
  auto *const ugtInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Gt, false>);
  auto *const sleInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Lte, true>);
  auto *const sltInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Lt, true>);
  auto *const sgeInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Gte, true>);
  auto *const sgtInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Gt, true>);
  // normalize operands into 2's complement
  auto *const eqInlinedOp = new InlinedOp<IntOp::exprT>(
      IntOpGen::IntPred::intPredExpr<BinExpr::Eq, false>);
  auto *const neInlinedOp = new InlinedOp<IntOp::exprT>(
...
void IntOpGen::generateConvOps(std::stringstream &s) const {
  ...
  auto *const bvTrunc = new InlinedOp<IntConv::truncExprT>(IntConv::bvTruncExpr);
  auto *const builtinSext =
      new BuiltinOp<IntConv::attrT>(IntConv::extAttr<true>);
  auto *const builtinZext =
...
void IntOpGen::generateMemOps(std::stringstream &s) const {
  ...
  for (auto size : INTEGER_SIZES) {
    if (!SmackOptions::BitPrecise) {
      ...
      const auto *valExpr = makeIntVarExpr(0);
      ...
      if (size < 8) {
        ...
        const auto *loadBody = prelude.mapSelExpr(0);
        const auto *storeBody = prelude.mapUpdExpr(0, Expr::bvExtract(valExpr, 8, 0));
...
void PtrOpGen::generateMemOps(std::stringstream &s) const {
  ...
  if (SmackOptions::BitPrecise) {
    ...
    const auto *indexExpr = makePtrVarExpr(0);
    s << prelude.unsafeStore(
             binding,
...
struct FpOpGen::FpArithOp : public FpOp {
  ...
  FuncDecl *getModeledFpFunc(unsigned bitWidth) const override {
    ...
    const auto *builtinAttr = ((FpOp::attrT)bop->func)(opName);
...
struct FpOpGen::FpPred : public FpOp {
  ...
  FuncDecl *getModeledFpFunc(unsigned bitWidth) const override {
    ...
    if (auto *bop = dyn_cast<BuiltinOp<FpOp::attrT>>(op)) {
      ...
      const auto *a1 = makeFpVarExpr(1);
      const auto *a2 = makeFpVarExpr(2);
  ...
  static const Expr *fpPredExpr(std::string opName, const Expr *a1,
    ...
    const auto *ret = fpPredExprTable.at(opName)(a1, a2, bitWidth);
    return ret;
  }
};
...
void FpOpGen::generatePreds(std::stringstream &s) const {
  ...
  auto *const fpBuiltinOp = new BuiltinOp<FpOp::attrT>(FpOp::fpAttrFunc);
  auto *const fpInlinedPred = new InlinedOp<FpPred::exprT>(FpPred::fpPredExpr);
