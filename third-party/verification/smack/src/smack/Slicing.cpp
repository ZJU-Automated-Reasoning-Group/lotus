...
bool contains(EdgeList &backedges, const BasicBlock *src,
...
bool hasIncomingBackEdge(PHINode *Phi, EdgeList &backedges) {
  for (PHINode::block_iterator B = Phi->block_begin(); B != Phi->block_end();
       ++B) {
          ...
          if (CE->getOpcode() == Instruction::GetElementPtr)
            if (GlobalValue *G = dyn_cast<GlobalValue>(CE->getOperand(0)))
              if (ConstantDataSequential *S =
                      dyn_cast<ConstantDataSequential>(G->getOperand(0)))
                return S;
  return 0;
}
...
Slice::Slice(Instruction &I, Slices &S, string name)
  ...
  while (!workList.empty()) {
    ...
    if (PHINode *Phi = dyn_cast<PHINode>(I)) {
      for (PHINode::block_iterator B = Phi->block_begin();
           B != Phi->block_end(); ++B) {
...
string Slice::getName() { return name; }
...
const Expr *Slice::getCode(Naming &naming, SmackRep &rep) {
  ...
  for (Function::iterator B = function.begin(), E = function.end(); B != E;
       ++B) {
    if (!values.count(B))
      continue;
...
const Decl *Slice::getBoogieDecl(Naming &naming, SmackRep &rep) {
  ...
  for (unordered_set<Value *>::iterator V = inputs.begin(), E = inputs.end();
       V != E; ++V)
    params.push_back(getParameter(*V, localNaming, rep));
  Decl *D =
      Decl::function(getName(), params, "bool", getCode(localNaming, rep));
