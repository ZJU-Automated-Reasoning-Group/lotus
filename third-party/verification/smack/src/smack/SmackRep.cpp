...
const Stmt *SmackRep::valueAnnotation(const CallInst &CI) {
  ...
  if (CI.arg_size() == 1) {
    ...
    } else if (const auto *LI = dyn_cast<const LoadInst>(V)) {
      const auto *GEP = dyn_cast<const GetElementPtrInst>(LI->getPointerOperand());
      assert(GEP && "Expected GEP argument to load instruction.");
      const auto *A = dyn_cast<const Argument>(GEP->getPointerOperand());
    ...
    if ((A = dyn_cast<const Argument>(V))) {
      const auto *PT = dyn_cast<const PointerType>(A->getType());
      assert(PT && "Expected pointer argument.");
      T = PT->getElementType();
      addr = expr(A);

    } else if (const auto *GEP = dyn_cast<const GetElementPtrInst>(V)) {
  ...
  if (const ConstantInt *ci = llvm::dyn_cast<const ConstantInt>(v)) {
    ...
    bool neg = width > 1 &&
               (isUnsigned ? (isUnsignedInst ? false : API.getSExtValue() == -1)
                           : ci->isNegative());
    std::string str = toString(neg ? API.abs() : API, 10, false);
    const Expr *e =
    ...
    if (SmackOptions::FloatEnabled) {
      ...
      std::string hexSig = toString(sig, 16, false).substr(1);
      hexSig.insert(leftSize / 4, ".");

      return Expr::lit(API.isNegative(), hexSig, toString(finalExp, 10, true),
                       sigSize, expSize);
    } else {
      const APFloat APF = CFP->getValueAPF();
      std::string str;
...
const Expr *SmackRep::expr(const llvm::Value *v, bool isConstIntUnsigned,
  ...
  if (isa<GlobalValue>(v)) {
    ...
    } else if (const auto *cv = dyn_cast<const ConstantDataVector>(constant)) {
      return VectorOperations(this).constant(cv);

    } else if (const auto *cd = dyn_cast<const ConstantAggregateZero>(constant)) {
...
void SmackRep::addAllocSizeAttr(const llvm::GlobalVariable *G,
  ...
  const auto *T = dyn_cast<const PointerType>(G->getType());
