...
Value *SplitAggregateValue::splitAggregateLoad(Type *T, Value *P,
  ...
  for (auto &e : info) {
    ...
    Value *GEP = irb.CreateGEP(PointeeTy, P, ArrayRef<Value *>(getFirsts(idxs)));
    V = irb.CreateInsertValue(
        V, irb.CreateLoad(cast<GetElementPtrInst>(GEP)->getResultElementType(), GEP),
...
void SplitAggregateValue::splitAggregateStore(Value *P, Value *V,
  ...
  for (auto &e : info) {
    ...
    auto *PtrTy = cast<PointerType>(P->getType());
    Type *PointeeTy = PtrTy->getPointerElementType();
    Value *GEP = irb.CreateGEP(PointeeTy, P, ArrayRef<Value *>(vidxs));
    if (c)
