...
static void simplifyGEP(GetElementPtrInst *GEP) {
  ...
  if (GEPOperator *Src = dyn_cast<GEPOperator>(PtrOp)) {
    ...
    if (!Indices.empty()){
      Type *PointeeTy = Src->getSourceElementType();
      GetElementPtrInst *GEPNew =  (GEP->isInBounds() && Src->isInBounds()) ?
        GetElementPtrInst::CreateInBounds(PointeeTy, Src->getOperand(0), Indices,
                                          GEP->getName(), GEP) :
        GetElementPtrInst::Create(PointeeTy, Src->getOperand(0), Indices,
                                  GEP->getName(), GEP);
      numMerged++;
