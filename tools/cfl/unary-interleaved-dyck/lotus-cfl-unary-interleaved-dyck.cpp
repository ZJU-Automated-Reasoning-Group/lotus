...
namespace unary = lotus::cfl::unary_interleaved_dyck;
...
void usage(std::ostream &output) {
  output
      << "usage: lotus-cfl-unary-interleaved-dyck [options] <graph.dot>\n"
         "\n"
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "lotus-cfl-unary-interleaved-dyck: " << error.what() << '\n';
    return 1;
  }
}
