...
void Help() {
  ...
  std::cout << "   -o <file>      : The output file. If not provided, output "
               "is sent to stdout\n";
  std::cout << "   -lu <file>     : Output intermediate LLVM IR before "
               "optimization (optional)\n";
  std::cout << "   -lo <file>     : Output intermediate LLVM IR after "
               "optimization (optional)\n";
  std::cout
      << "   -m             : Convert constant shifts to multiplication\n";
  std::cout << "   -t <file>      : Output statistics file. If not provided, "
               "output is sent to stdout\n";
  std::cout << "   -pall          : Run all relevant passes (roughly "
               "equivalent to -O3 in LLVM). By default, no passes are run\n";
  ...
  std::cout << "   -sccp          : Run sparse conditional constant "
               "propagation (SCCP) pass\n";
  std::cout << "   -dce           : Run dead code elimination (DCE) pass\n";
  std::cout << "   -adce          : Run aggressive dead code elimination "
               "(ADCE) pass\n";
  std::cout << "   -instsimplify  : Run instsimplify pass\n";
  std::cout << "   -gvn           : Run global value numbering (GVN) pass\n";
}

// Not safe: assumes HasFlag returned true and that there is an argument after
...
unsigned short RunPasses(unsigned short flags, Function &fun) {
  // instcombine and aggressive instcombine are run twice, according to the -O3
  // optimization pass sequence
...
int main(int argc, char *argv[]) {
  ...
  SMTFormula a =
      SMTFormula(lcx, &lmodule, builder, smt_str, LLVM_FUNCTION_NAME);
  a.ToLLVM();
  ...
  if (HasFlag(argc, argv, "-o") &&
      (outputFilename = GetFlag(argc, argv, "-o"))) {
  ...
  // Print statistics
  char *statsFilename;
  if (HasFlag(argc, argv, "-t") &&
      (statsFilename = GetFlag(argc, argv, "-t"))) {
    std::ofstream out;
    out.open(statsFilename, std::ios_base::app);
    out << inputFilename << "," << (shiftToMultiply ? "true" : "false") << ","
        << PrintPasses(parsedPasses) << "," << frontTime.count() << ","
        << optTime.count() << "," << backTime.count() << ","
        << PrintPasses(usedPasses) << "\n";
  } else {
    std::cout << inputFilename << "," << (shiftToMultiply ? "true" : "false")
              << "," << PrintPasses(parsedPasses) << "," << frontTime.count()
              << "," << optTime.count() << "," << backTime.count() << ","
              << PrintPasses(usedPasses) << "\n";
  }
