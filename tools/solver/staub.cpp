...
int main(int argc, char *argv[]) {
  ...
  if (HasFlag(argc, argv, "-h")) {
    ...
    std::cout << "-r <ebits,sbits|aix|aix4>    Real numbers: if integer "
                 ...
                 "16/32/64/128 width domain.\n";
    std::cout
        << "-i <N|aix|aix2>              Integers: if integer N is given uses "
           "this fixed withd; if aix, uses abstract interpretation, if aix2, "
           "uses abstract interpretation with domain of powers of 2.\n";
    std::cout << "Either real number of integer must be specified.\n";
    return 0;
  }

  if ((!HasFlag(argc, argv, "-r") && !HasFlag(argc, argv, "-i")) ||
      (HasFlag(argc, argv, "-r") && HasFlag(argc, argv, "-i"))) {
