...
int main(int argc, char **argv) {
  ...
  if (!Bmc && !KInduction && !BoogieOutput)
  ...
  if (!AsmOutputFilename.empty()) {
    if (!KeepShadows) {
      ...
      if (Bmc || KInduction || BoogieOutput || HoudiniInv || PredAbs || Solve) {
  ...
  if (Bmc || KInduction) {
    ...
    if (KInduction) {
#ifdef HAVE_CLAM
      pass_manager.add(seahorn::createKInductionPass(out));
#else
      ERR << "k-induction requires CLAM (PathBMC); rebuild with HAVE_CLAM";
      std::exit(1);
#endif
    } else {
      switch (BmcEngine) {
